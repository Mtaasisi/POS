...
in the LATS Complete Customer Contacts Excel file.
"""
...
def update_complete_excel_whatsapp():
    """Copy phone numbers to WhatsApp column in Complete Excel file where WhatsApp is empty"""
    
    # Read the Excel file
    print("Reading LATS_Complete_Customer_Contacts.xlsx...")
    df = pd.read_excel('LATS_Complete_Customer_Contacts.xlsx')
    ...
    # Create a backup
    backup_filename = 'LATS_Complete_Customer_Contacts_backup.xlsx'
    ...
    df.to_excel('LATS_Complete_Customer_Contacts.xlsx', index=False)
    print("Updated LATS_Complete_Customer_Contacts.xlsx saved successfully!")
...
if __name__ == "__main__":
    try:
        updated_df = update_complete_excel_whatsapp()
        print("\n✅ Complete Excel Phone to WhatsApp copy operation completed successfully!")
    except Exception as e:
        print(f"❌ Error: {e}")
        print("Please check if the Excel file exists and is accessible.")
