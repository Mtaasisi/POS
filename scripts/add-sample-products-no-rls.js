...
const addSampleProductsNoRLS = async () => {
  try {
    console.log('📦 Adding sample products (temporarily disabling RLS)...');
    
    // Temporarily disable RLS
    console.log('🔓 Temporarily disabling RLS...');
    await supabase.rpc('exec_sql', { sql: 'ALTER TABLE lats_categories DISABLE ROW LEVEL SECURITY;' });
    await supabase.rpc('exec_sql', { sql: 'ALTER TABLE lats_products DISABLE ROW LEVEL SECURITY;' });
    await supabase.rpc('exec_sql', { sql: 'ALTER TABLE lats_product_variants DISABLE ROW LEVEL SECURITY;' });
    ...
    await supabase.rpc('exec_sql', { sql: 'ALTER TABLE lats_categories ENABLE ROW LEVEL SECURITY;' });
    await supabase.rpc('exec_sql', { sql: 'ALTER TABLE lats_products ENABLE ROW LEVEL SECURITY;' });
    await supabase.rpc('exec_sql', { sql: 'ALTER TABLE lats_product_variants ENABLE ROW LEVEL SECURITY;' });
