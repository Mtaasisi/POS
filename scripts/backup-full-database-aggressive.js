...
async function getAllRecordsFromTableAggressive(tableName) {
  const allRecords = [];
  let from = 0;
  const pageSize = 1000; // Use smaller page size to avoid limits
  let retryCount = 0;
  const maxRetries = 5;
  let consecutiveEmptyPages = 0;
  const maxEmptyPages = 3;
  
  console.log(`   🔄 Starting AGGRESSIVE backup for '${tableName}'...`);
  
  while (true) {
    try {
      // Try multiple strategies to get records
      let data = null;
      let error = null;
      
      // Strategy 1: Normal pagination
      try {
      ...
      // Strategy 2: If normal fails, try with order by id
      if (error || !data) {
        try {
          const result = await supabase
            .from(tableName)
      ...
      // Strategy 3: If still fails, try without range
      if (error || !data) {
        try {
          const result = await supabase
            .from(tableName)
      ...
      if (!data || data.length === 0) {
        ...
        console.log(`   ⚠️  Empty page ${consecutiveEmptyPages} for '${tableName}' (offset: ${from})`);
        
        if (consecutiveEmptyPages >= maxEmptyPages) {
          console.log(`   ✅ Completed '${tableName}' - ${maxEmptyPages} consecutive empty pages`);
          break;
        }
      ...
      console.log(`   📥 Fetched ${data.length} records from '${tableName}' (offset: ${from}, total: ${allRecords.length})`);
...
async function backupFullDatabaseAggressive() {
  console.log('💾 Creating AGGRESSIVE Database Backup - ALL Tables and ALL Data (Bypass All Limits)\n');
  
  const backupDir = path.join(process.cwd(), 'backups');
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const backupFile = path.join(backupDir, `full-database-backup-aggressive-${timestamp}.json`);
  ...
  const backup = {
    ...
    databaseInfo: {
      ...
      backupType: 'AGGRESSIVE - BYPASS ALL LIMITS'
    },
    tables: {},
    summary: {
      totalTables: 0,
  ...
  console.log(`📋 Starting AGGRESSIVE backup of ${ALL_TABLES.length} tables...\n`);
  console.log('🚀 This backup will use multiple strategies to bypass ALL limits!\n');
  ...
  for (const tableName of ALL_TABLES) {
    try {
      ...
      // Get ALL data from the table using aggressive strategies
      const data = await getAllRecordsFromTableAggressive(tableName);
  ...
  try {
    ...
    console.log(`💾 AGGRESSIVE backup saved to: ${backupFile}`);
  ...
  // Summary
  console.log('\n🎉 AGGRESSIVE Database Backup Summary:');
  ...
  console.log('\n🔒 AGGRESSIVE database backup completed successfully!');
  console.log('🚀 You now have a complete backup of your entire database with ALL records - BYPASSING ALL LIMITS!');
  console.log('💡 This backup can be used to restore your database if needed.');

  return backup;
}

// Run the aggressive backup
backupFullDatabaseAggressive().catch(console.error);
