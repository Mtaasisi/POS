...
async function applyNotificationsSchema() {
  try {
    console.log('🔧 Applying notifications schema to database...');

    // Step 1: Create tables
    console.log('📋 Creating notification tables...');
    const { error: tablesError } = await supabase.rpc('exec_sql', {
      sql: notificationsSchema
    });
    ...
    // Step 2: Create indexes and policies
    console.log('🔐 Creating indexes and RLS policies...');
    const { error: policiesError } = await supabase.rpc('exec_sql', {
      sql: indexesAndPolicies
    });

    if (policiesError) {
      console.error('❌ Error creating indexes and policies:', policiesError);
      return;
    }

    console.log('✅ Indexes and RLS policies created successfully');

    // Step 3: Create triggers and templates
    console.log('⚡ Creating triggers and default templates...');
    const { error: triggersError } = await supabase.rpc('exec_sql', {
      sql: triggersAndTemplates
    });

    if (triggersError) {
      console.error('❌ Error creating triggers and templates:', triggersError);
      return;
    }

    console.log('✅ Triggers and default templates created successfully');

    console.log('🎉 Notifications schema applied successfully!');
    console.log('');
    console.log('📝 Next steps:');
    console.log('1. Run: node scripts/add-sample-notifications.js');
    console.log('2. Add notification routes to your app');
