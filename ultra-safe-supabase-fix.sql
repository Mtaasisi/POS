-- ULTRA SAFE SUPABASE FIX
-- This script addresses both 406 and 400 errors comprehensively
-- Completely avoids duplicate policy errors by checking first
...
-- 5. Check existing policies and only create if they don't exist
DO $$
DECLARE
    policy_exists BOOLEAN;
BEGIN
    -- Check if policy exists for auth_users
    SELECT EXISTS (
    ...
    IF NOT policy_exists THEN
        ...
        RAISE NOTICE '✅ Created policy for auth_users';
    ELSE
        RAISE NOTICE '✅ Policy already exists for auth_users';
    END IF;
END $$;

-- 6. Check and create policies for lats_sales
DO $$
DECLARE
    policy_exists BOOLEAN;
BEGIN
    -- Check if policy exists for lats_sales
    SELECT EXISTS (
        SELECT 1 FROM pg_policies 
    ...
    IF NOT policy_exists THEN
        ...
        FOR ALL USING (true) WITH CHECK (true);
        RAISE NOTICE '✅ Created policy for lats_sales';
    ELSE
        RAISE NOTICE '✅ Policy already exists for lats_sales';
    END IF;
END $$;

-- 7. Check and create policies for lats_sale_items
DO $$
DECLARE
    policy_exists BOOLEAN;
BEGIN
    -- Check if policy exists for lats_sale_items
    SELECT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'lats_sale_items' 
        AND policyname = 'Enable all operations for all users'
    ) INTO policy_exists;
    ...
    IF NOT policy_exists THEN
        ...
        FOR ALL USING (true) WITH CHECK (true);
        RAISE NOTICE '✅ Created policy for lats_sale_items';
    ELSE
        RAISE NOTICE '✅ Policy already exists for lats_sale_items';
    END IF;
END $$;
...
BEGIN
    ...
    ELSE
        RAISE NOTICE '✅ Test sale already exists';
    END IF;
END $$;

-- 12. Final verification
SELECT 
    '🎉 ULTRA SAFE FIX COMPLETED!' as status,
