...
async function testBalanceCheck() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (balanceMatch) {
        console.log(`✅ Balance check successful. Current balance: ${balanceMatch[1]} credits`);
        return true;
      }
      ...
      if (!isNaN(Number(trimmedResponse))) {
        console.log(`✅ Balance check successful. Current balance: ${trimmedResponse} credits`);
        return true;
      }
    }
