-- Fix add_quality_checked_items_to_inventory function parameter mismatch
-- Handle multiple function versions by dropping all variants first

-- Drop all existing function variants with different parameter lists
DROP FUNCTION IF EXISTS add_quality_checked_items_to_inventory(UUID, UUID, UUID, DECIMAL, VARCHAR);
DROP FUNCTION IF EXISTS add_quality_checked_items_to_inventory(UUID, UUID, UUID, DECIMAL, VARCHAR, DECIMAL);
DROP FUNCTION IF EXISTS add_quality_checked_items_to_inventory(UUID, UUID, UUID);
...
SELECT 'Fixed add_quality_checked_items_to_inventory function parameter mismatch!' as status;
