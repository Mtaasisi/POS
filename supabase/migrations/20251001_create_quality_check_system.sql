...
DROP FUNCTION IF EXISTS create_quality_check_from_template(UUID, UUID);
...
BEGIN
    ...
    -- Update the purchase order status to completed if quality check passed
...
-- Drop existing function if it exists (with all possible signatures)
DROP FUNCTION IF EXISTS receive_quality_checked_items(UUID, UUID, UUID);
