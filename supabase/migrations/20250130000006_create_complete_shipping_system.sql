-- Complete Shipping System Migration
-- This migration creates all missing shipping-related tables and views
...
CREATE INDEX IF NOT EXISTS idx_shipping_agents_business_id ON lats_shipping_agents(business_id);
...
CREATE INDEX IF NOT EXISTS idx_shipping_settings_user_business ON lats_shipping_settings(user_id, business_id);
...
DROP POLICY IF EXISTS "Users can read own business agents" ON lats_shipping_agents;
DROP POLICY IF EXISTS "Users can manage own business agents" ON lats_shipping_agents;
...
DROP POLICY IF EXISTS "Users can read own business agent offices" ON lats_shipping_agent_offices;
DROP POLICY IF EXISTS "Users can manage own business agent offices" ON lats_shipping_agent_offices;
CREATE POLICY "Users can read own business agent offices" ON lats_shipping_agent_offices FOR SELECT USING (
    agent_id IN (
        SELECT id FROM lats_shipping_agents 
        WHERE business_id IN (SELECT id FROM lats_businesses WHERE user_id = auth.uid())
    )
);
CREATE POLICY "Users can manage own business agent offices" ON lats_shipping_agent_offices FOR ALL USING (
    agent_id IN (
        SELECT id FROM lats_shipping_agents 
        WHERE business_id IN (SELECT id FROM lats_businesses WHERE user_id = auth.uid())
    )
