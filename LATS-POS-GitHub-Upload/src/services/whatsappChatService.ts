    ...
    try {
      ...
      const result = await retryWithBackoff(async () => {
        ...
        if (error) {
          ...
          if (error.message?.includes('relation "whatsapp_messages" does not exist') || 
              error.message?.includes('table "whatsapp_messages" does not exist') ||
              error.code === 'PGRST116' || 
              error.code === '42P01') {
            console.warn('⚠️ whatsapp_messages table does not exist. Please run database setup.');
            return [];
          }
          throw error;
        }
    ...
    try {
      ...
      const result = await retryWithBackoff(async () => {
        ...
        if (error) {
          ...
          // If table doesn't exist, skip saving instead of throwing
          if (error.message?.includes('relation "whatsapp_messages" does not exist') || 
              error.message?.includes('table "whatsapp_messages" does not exist') ||
              error.code === 'PGRST116' || 
              error.code === '42P01') {
            console.warn('⚠️ whatsapp_messages table does not exist. Skipping message save.');
            return null;
          }
          throw error;
        }
    ...
    try {
      ...
      await retryWithBackoff(async () => {
        ...
        if (error) {
          ...
          // If table doesn't exist, skip update instead of throwing
          if (error.message?.includes('relation "whatsapp_messages" does not exist') || 
              error.message?.includes('table "whatsapp_messages" does not exist') ||
              error.code === 'PGRST116' || 
              error.code === '42P01') {
            console.warn('⚠️ whatsapp_messages table does not exist. Skipping status update.');
            return;
          }
          throw error;
        }
    ...
    try {
      ...
      const result = await retryWithBackoff(async () => {
        ...
        if (error) {
          ...
          if (error.message?.includes('relation "whatsapp_hub_settings" does not exist')) {
            console.warn('⚠️ whatsapp_hub_settings table does not exist. Please run database setup.');
            return null;
          }
          throw error;
        }
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message?.includes('relation "whatsapp_messages" does not exist') || 
            ...
            error.code === 'PGRST116' || 
            error.code === '42P01') {
          console.warn('⚠️ whatsapp_messages table does not exist. Returning empty search results.');
          return [];
        }
        throw error;
      }
