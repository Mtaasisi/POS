...
import { PurchaseOrder, ShippingInfo } from '../types/inventory';
...
const ShippingManagementPage: React.FC = () => {
  ...
  const loadShippingData = async () => {
    ...
    try {
      const filters: any = {};
      if (statusFilter !== 'all') {
        filters.status = statusFilter === 'shipped' ? 'in_transit' : statusFilter;
      }
      
      const data = await shippingDataService.getAllShippingInfo(filters);
  ...
  // Reload shipping data when filters change
  useEffect(() => {
    loadShippingData();
  }, [statusFilter]);

  // Filter and sort shipping data
  const filteredShipments = useMemo(() => {
    let filtered = shippingData.filter(shipping => {
      // Search filter
      if (searchQuery.trim()) {
        const query = searchQuery.toLowerCase();
        const matchesSearch = 
          shipping.trackingNumber?.toLowerCase().includes(query) ||
          shipping.carrier?.toLowerCase().includes(query) ||
          shipping.agent?.name?.toLowerCase().includes(query);
        
        if (!matchesSearch) return false;
      }

      // Carrier filter
      if (carrierFilter !== 'all' && shipping.carrier !== carrierFilter) {
        return false;
      }

      // Status filter
      if (statusFilter === 'shipped' && shipping.status !== 'in_transit') return false;
      if (statusFilter === 'delivered' && shipping.status !== 'delivered') return false;
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        case 'shippedDate':
          valueA = new Date(a.shippedDate || 0).getTime();
          valueB = new Date(b.shippedDate || 0).getTime();
          break;
        case 'trackingNumber':
          valueA = a.trackingNumber || '';
          valueB = b.trackingNumber || '';
          break;
        case 'estimatedDelivery':
          valueA = new Date(a.estimatedDelivery || 0).getTime();
          valueB = new Date(b.estimatedDelivery || 0).getTime();
          break;
        case 'carrier':
          valueA = a.carrier || '';
          valueB = b.carrier || '';
    ...
    return filtered;
  }, [shippingData, searchQuery, carrierFilter, statusFilter, sortBy, sortOrder]);

  // Get unique carriers for filter
  const carriers = useMemo(() => {
    const carrierSet = new Set(shippingData.map(shipping => shipping.carrier).filter(Boolean));
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex gap-2">
            <GlassButton
              onClick={() => navigate('/lats/shipping/agents')}
              icon={<Users size={18} />}
              variant="outline"
            ...
            <GlassButton
              onClick={() => navigate('/lats/shipping/settings')}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <span className="text-gray-600">
                    Showing {filteredShipments.length} of {shippingData.length} shipments
                  </span>
                </div>
                <div>
                  <span className="text-gray-600">
