  ...
  useEffect(() => {
    const fetchShippingAgents = async () => {
      ...
      try {
        ...
        console.log('🚢 Shipping agents response:', response);
        
        if (response.ok && response.data) {
          setShippingAgents(response.data);
          console.log('✅ Shipping agents loaded:', response.data.length, 'agents');
        } else {
          setAgentsError('Failed to load shipping agents');
        }
      } catch (error) {
        console.error('❌ Error fetching shipping agents:', error);
        setAgentsError('Error loading shipping agents');
      } finally {
