...
class ShippingDataServiceImpl implements ShippingDataService {
  ...
  async getShippingInfo(purchaseOrderId: string): Promise<ShippingInfo | null> {
    try {
      ...
      // Try simple query first (more compatible)
      let { data: shippingInfoData, error } = await supabase
        .from('lats_shipping_info')
        .select('*')
        .eq('purchase_order_id', purchaseOrderId);

      // If simple query fails, try foreign key approach
      if (error && error.code !== 'PGRST116') {
        console.log('🔄 [ShippingDataService] Simple query failed, trying foreign key query...', error);
        ...
        if (complexError) {
          console.error('❌ [ShippingDataService] Both queries failed:', complexError);
          throw complexError;
        }
        
        shippingInfoData = complexData;
        error = null;
      } else if (error) {
      ...
      const cargoItems = await this.getShippingCargoItems(shippingRecord.id);

      // Transform to ShippingInfo format
      const shippingInfo: ShippingInfo = {
        id: shippingRecord.id,
        carrier: shippingRecord.carrier?.name || shippingRecord.carrier_name || 'Unknown Carrier',
        ...
        agent: shippingRecord.agent ? {
          id: shippingRecord.agent.id,
          name: shippingRecord.agent.name,
          company: shippingRecord.agent.company,
          phone: shippingRecord.agent.phone,
          email: shippingRecord.agent.email,
          isActive: shippingRecord.agent.is_active
        ...
        manager: shippingRecord.manager ? {
          id: shippingRecord.manager.id,
          name: shippingRecord.manager.name,
          department: shippingRecord.manager.department,
          phone: shippingRecord.manager.phone,
          email: shippingRecord.manager.email
  ...
  async getAllShippingInfo(filters?: {
    ...
    try {
      ...
      // First try the foreign key approach (newer schema)
      let query = supabase
        .from('lats_shipping_info')
        .select(`
          *,
      ...
      // If foreign key approach fails, try simpler query (older schema)
      if (error && (error.message?.includes('406') || error.message?.includes('Not Acceptable') || error.status === 406 || error.code === 'PGRST301')) {
        console.log('🔄 [ShippingDataService] Foreign key query failed, trying simple query...', error);
        ...
        if (filters?.status) {
          simpleQuery = simpleQuery.eq('shipping_status', filters.status);
        }
        if (filters?.dateFrom) {
        ...
        if (simpleError) {
          console.error('❌ [ShippingDataService] Error fetching all shipping info:', simpleError);
          throw simpleError;
        }
      ...
      const shippingInfoList: ShippingInfo[] = data.map(item => ({
        id: item.id,
        carrier: item.carrier?.name || item.carrier_name || 'Unknown Carrier',
