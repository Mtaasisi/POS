-- Fix: Create customer_communications table
...
-- Create RLS policies (simplified since customers table doesn't have user_id)
CREATE POLICY "Users can view communications" ON customer_communications
    FOR SELECT USING (
        auth.uid() = sent_by OR
...
CREATE POLICY "Users can update communications" ON customer_communications
    FOR UPDATE USING (
        auth.uid() = sent_by OR
        auth.uid() IS NOT NULL
    );
...
-- Test the table
SELECT 'customer_communications table created successfully!' as status;
