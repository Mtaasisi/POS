-- Safe Database Fix - Handles Existing Tables and Policies
-- This script fixes the 403 and 400 errors safely
...
-- 10. Drop existing policies safely (ignore errors if they don't exist)
DO $$
BEGIN
    ...
    -- Drop installment_payments policies
    ...
    -- Drop product_variants policies
    ...
    -- Drop inventory_categories policies
    DROP POLICY IF EXISTS "Allow authenticated users to view inventory categories" ON inventory_categories;
    DROP POLICY IF EXISTS "Allow authenticated users to insert inventory categories" ON inventory_categories;
    DROP POLICY IF EXISTS "Allow authenticated users to update inventory categories" ON inventory_categories;
...
-- 11. Create new permissive policies
...
-- 13. Insert basic sample data safely
