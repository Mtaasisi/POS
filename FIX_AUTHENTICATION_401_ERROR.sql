...
-- 1. First, let's check the current state of the lats_sales table
-- and ensure it has the correct structure
...
-- 6. Create a function to test authentication
CREATE OR REPLACE FUNCTION test_auth_for_sales()
RETURNS TABLE (
...
-- 7. Create a function to get current user info for sales
CREATE OR REPLACE FUNCTION get_current_user_for_sales()
RETURNS TABLE (
    user_id UUID,
    user_email TEXT,
    user_role TEXT
...
-- 8. Test the policies by checking if we can access the tables
DO $$
DECLARE
    sales_count INTEGER;
    items_count INTEGER;
...
-- 9. Create a view for easy access to sales data with user info
CREATE OR REPLACE VIEW sales_with_user_info AS
SELECT 
    s.*,
    COALESCE(s.created_by, 'System') as cashier_name,
    CASE 
