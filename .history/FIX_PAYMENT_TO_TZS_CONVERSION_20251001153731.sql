-- FIX: Convert all foreign currency payments to TZS
-- This script updates the payment function to convert any foreign currency to TZS before processing
...
BEGIN
    ...
    -- Check if account has enough balance (in TZS)
    -- If account is in foreign currency, convert its balance to TZS for comparison
    DECLARE
...
SELECT 'Payment function updated! All foreign currency payments will now be converted to TZS.' as status;
