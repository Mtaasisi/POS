#!/usr/bin/env python3
"""
Generate Full Call Log SQL
This script processes the complete call log CSV and generates SQL to update all customers
"""
...
def generate_sql(call_data):
    ...
    sql_parts.append("-- Update ALL Customers from Call Log")
    sql_parts.append("-- This script processes the complete call log and updates all customers")
    sql_parts.append("")
    sql_parts.append("-- Step 1: Create temporary table with processed call log data")
    ...
    sql_parts.append("-- Step 2: Show analysis of call log data")
    sql_parts.append("SELECT 'Call Log Analysis:' as info, COUNT(*) as unique_phone_numbers FROM temp_call_log_processed;")
    sql_parts.append("")
    
    sql_parts.append("-- Step 3: Show customers that will be updated")
    sql_parts.append("SELECT 'Customers that will be updated:' as info, COUNT(*) as count FROM customers c JOIN temp_call_log_processed t ON c.phone = t.phone;")
    sql_parts.append("")
    
    # Add update query
    sql_parts.append("-- Step 4: Update existing customers with call log data")
    sql_parts.append("UPDATE customers SET")
    sql_parts.append("    name = CASE WHEN customers.name = '__' OR customers.name IS NULL OR (t.best_name IS NOT NULL AND t.best_name != 'Unknown' AND LENGTH(t.best_name) > LENGTH(customers.name)) THEN COALESCE(t.best_name, customers.name) ELSE customers.name END,")
    ...
    sql_parts.append("-- Step 5: Show updated customers count")
    sql_parts.append("SELECT 'Updated customers:' as info, COUNT(*) as count FROM customers WHERE updated_at > NOW() - INTERVAL '1 minute';")
    sql_parts.append("")
    
    sql_parts.append("-- Step 6: Show loyalty level distribution")
    sql_parts.append("SELECT 'Loyalty level distribution:' as info, call_loyalty_level, COUNT(*) as count FROM customers WHERE call_loyalty_level IS NOT NULL GROUP BY call_loyalty_level ORDER BY count DESC;")
    
    return "\n".join(sql_parts)

def main():
    csv_file = "/Users/mtaasisi/Downloads/Call_Log_With_Names.csv"
    output_file = "/Users/mtaasisi/Desktop/LATS CHANCE copy/update-all-customers-complete.sql"
