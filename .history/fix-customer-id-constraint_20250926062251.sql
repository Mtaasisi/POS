...
WHERE tc.table_name = 'lats_sales' 
  AND tc.constraint_name LIKE '%customer_id%';
...
BEGIN
    ...
    IF walk_in_customer_id IS NULL THEN
        -- Create walk-in customer with required fields
        INSERT INTO customers (name, phone, email, loyalty_level, color_tag, gender)
        VALUES ('Walk-in Customer', '+255000000000', 'walkin@lats.com', 'bronze', 'new', 'other')
