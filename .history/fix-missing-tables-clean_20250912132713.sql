-- Fix Missing Tables Migration - Clean Version
...
-- 3. Create uuid_diagnostic_log table
...
-- 4. Create whatsapp_instances table
...
('Welcome Message', 'greeting', 'Welcome to LATS CHANCE! Hi {{name}}, Thank you for choosing our services.', '["name"]', 'both', true),
('Order Update', 'service', 'Order Update: Hi {{name}}, Your order #{{orderId}} has been {{status}}!', '["name", "orderId", "status"]', 'both', true),
('Appointment Reminder', 'service', 'Appointment Reminder: Hi {{name}}, Your appointment is scheduled for {{date}} at {{time}}.', '["name", "date", "time"]', 'both', true)
...
DECLARE
    ...
    whatsapp_instances_exists BOOLEAN;
...
BEGIN
    ...
    ) INTO whatsapp_instances_exists;
    ...
    IF stock_movements_exists AND sms_logs_exists AND uuid_diagnostic_log_exists AND whatsapp_instances_exists AND whatsapp_templates_exists AND whatsapp_view_exists THEN
        ...
        RAISE NOTICE '✅ whatsapp_instances: %', whatsapp_instances_exists;
