...
BEGIN
    ...
    -- Count failed items
    SELECT COUNT(*) INTO failed_items
    FROM purchase_order_quality_check_items
    WHERE quality_check_id = NEW.quality_check_id AND result = 'fail';
    
    -- Determine overall status and result
    IF failed_items = 0 AND passed_items = total_items THEN
        overall_status := 'passed';
        overall_result := 'pass';
    ELSIF failed_items > 0 AND passed_items > 0 THEN
        overall_status := 'partial';
        overall_result := 'conditional';
    ELSIF failed_items > 0 THEN
