...
async function moveProductLocation(fromPosition, toPosition) {
  try {
    ...
    const { data: fromShelfData, error: fromShelfError } = await supabase
    ...
    if (!fromShelfData || fromShelfData.length === 0) {
    ...
    if (fromShelfData.length > 1) {
      console.error(`❌ Multiple shelves found with code "${fromPosition}":`);
      fromShelfData.forEach((shelf, index) => {
        console.log(`   ${index + 1}. ${shelf.name} (ID: ${shelf.id})`);
      });
      console.log(`Please specify which shelf to use by providing more details.`);
      return;
    }

    const fromShelf = fromShelfData[0];
