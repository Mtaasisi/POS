-- Finance Tables Setup - Fixed Version (Copy and paste this into your Supabase SQL Editor)
...
-- Simple RLS Policies (without role references)
-- Finance Accounts
CREATE POLICY "Users can view their own finance accounts" ON finance_accounts
    FOR SELECT USING (auth.uid() = created_by);

CREATE POLICY "Users can insert their own finance accounts" ON finance_accounts
    FOR INSERT WITH CHECK (auth.uid() = created_by);

CREATE POLICY "Users can update their own finance accounts" ON finance_accounts
    FOR UPDATE USING (auth.uid() = created_by);

CREATE POLICY "Users can delete their own finance accounts" ON finance_accounts
    FOR DELETE USING (auth.uid() = created_by);

-- Finance Expense Categories (allow all authenticated users to view)
CREATE POLICY "Users can view expense categories" ON finance_expense_categories
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Users can insert expense categories" ON finance_expense_categories
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Users can update expense categories" ON finance_expense_categories
    FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY "Users can delete expense categories" ON finance_expense_categories
    FOR DELETE USING (auth.role() = 'authenticated');

-- Finance Expenses
CREATE POLICY "Users can view their own expenses" ON finance_expenses
    FOR SELECT USING (auth.uid() = created_by);

CREATE POLICY "Users can insert their own expenses" ON finance_expenses
    FOR INSERT WITH CHECK (auth.uid() = created_by);

CREATE POLICY "Users can update their own expenses" ON finance_expenses
    FOR UPDATE USING (auth.uid() = created_by);

CREATE POLICY "Users can delete their own expenses" ON finance_expenses
    FOR DELETE USING (auth.uid() = created_by);

-- Finance Transfers
CREATE POLICY "Users can view their own transfers" ON finance_transfers
    FOR SELECT USING (auth.uid() = created_by);

CREATE POLICY "Users can insert their own transfers" ON finance_transfers
    FOR INSERT WITH CHECK (auth.uid() = created_by);

CREATE POLICY "Users can update their own transfers" ON finance_transfers
    FOR UPDATE USING (auth.uid() = created_by);

CREATE POLICY "Users can delete their own transfers" ON finance_transfers
    FOR DELETE USING (auth.uid() = created_by);
