...
SELECT 
  ...
  COUNT(CASE WHEN ABS(p.total_value - COALESCE(SUM(pv.cost_price * pv.quantity), 0)) > 1 THEN 1 END) as products_with_discrepancies,
  SUM(ABS(p.total_value - COALESCE(SUM(pv.cost_price * pv.quantity), 0))) as total_discrepancy_value
FROM lats_products p
LEFT JOIN lats_product_variants pv ON p.id = pv.product_id
WHERE p.is_active = true
