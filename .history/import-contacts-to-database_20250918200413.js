...
async function importContacts() {
  ...
  try {
    ...
    // Process each row (limit to first 10000 for testing)
    console.log('\n🔄 Processing contacts (first 10000 only for testing)...\n');
    
    const maxRows = Math.min(10001, lines.length); // Process first 10000 contacts (plus header)
    for (let i = 1; i < maxRows; i++) {
      ...
      try {
        ...
        if (existingCustomer) {
          results.duplicates++;
          console.log(`⚠️  Row ${i + 1}: ${formattedCustomer.name} - Phone ${formattedCustomer.phone} already exists (${existingCustomer.name})`);
          continue;
        }
