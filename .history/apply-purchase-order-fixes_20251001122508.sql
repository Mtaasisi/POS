...
-- Drop all existing versions of the function (there might be multiple with different signatures)
DO $$
BEGIN
    -- Drop any existing versions of the function
    DROP FUNCTION IF EXISTS log_purchase_order_audit(UUID, TEXT, TEXT, UUID) CASCADE;
    DROP FUNCTION IF EXISTS log_purchase_order_audit(UUID, TEXT, TEXT) CASCADE;
    DROP FUNCTION IF EXISTS log_purchase_order_audit(UUID, TEXT) CASCADE;
    DROP FUNCTION IF EXISTS log_purchase_order_audit() CASCADE;
EXCEPTION
    WHEN OTHERS THEN
