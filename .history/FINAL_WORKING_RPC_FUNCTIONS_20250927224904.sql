-- FINAL WORKING RPC FUNCTIONS - CORRECTED FOR YOUR DATABASE STRUCTURE
-- This script will fix the "structure of query does not match function result type" errors
...
-- This function matches your actual table structure exactly
...
SELECT 'Testing functions after creation...' as message;

-- Test with a real purchase order ID
SELECT 
    'get_purchase_order_items_with_products' as function_name,
    COUNT(*) as result_count
FROM get_purchase_order_items_with_products(
    (SELECT id FROM lats_purchase_orders LIMIT 1)
);

SELECT 
    'get_po_inventory_stats' as function_name,
    COUNT(*) as result_count
FROM get_po_inventory_stats(
    (SELECT id FROM lats_purchase_orders LIMIT 1)
);

SELECT 
    'get_received_items_for_po' as function_name,
    COUNT(*) as result_count
FROM get_received_items_for_po(
    (SELECT id FROM lats_purchase_orders LIMIT 1)
);

-- Success message
SELECT '🎉 All RPC functions have been created successfully! The 400 Bad Request errors should now be resolved.' as message;
