...
-- Fix any missing device relationships (if devices table exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'devices') THEN
        ...
        RAISE NOTICE 'Updated device-customer relationships';
    ELSE
        RAISE NOTICE 'devices table not found, skipping device relationship updates';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'customer_payments') THEN
        ...
        RAISE NOTICE 'Cleaned up orphaned customer_payments records';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'devices') THEN
        DELETE FROM devices 
        WHERE customer_id NOT IN (SELECT id FROM customers);
        RAISE NOTICE 'Cleaned up orphaned devices records';
    END IF;
END $$;
