-- ALTERNATIVE FIX FOR AUTH_USERS 400 ERROR (No table modifications)
-- This version doesn't require adding unique constraints
...
-- 5. Create test data if needed (without ON CONFLICT)
-- Check if users already exist before inserting
DO $$
BEGIN
    -- Insert 'care' user if not exists
    ...
    IF NOT EXISTS (SELECT 1 FROM auth_users WHERE name = 'admin') THEN
        INSERT INTO auth_users (name, email, username, role, is_active)
        VALUES ('admin', 'admin@example.com', 'admin', 'admin', true);
    END IF;
    
    -- Insert 'technician' user if not exists
    IF NOT EXISTS (SELECT 1 FROM auth_users WHERE name = 'technician') THEN
        INSERT INTO auth_users (name, email, username, role, is_active)
        VALUES ('technician', 'tech@example.com', 'technician', 'technician', true);
    END IF;
END $$;
