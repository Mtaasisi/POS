...
-- Add missing columns to existing lats_inventory_adjustments table
DO $$
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added reference_type column to lats_inventory_adjustments';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added reference_id column to lats_inventory_adjustments';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE lats_inventory_adjustments 
        ADD COLUMN notes TEXT;
        RAISE NOTICE 'Added notes column to lats_inventory_adjustments';
    END IF;
END $$;
...
-- Only create reference index if both columns exist
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Created reference index on lats_inventory_adjustments';
    END IF;
END $$;
