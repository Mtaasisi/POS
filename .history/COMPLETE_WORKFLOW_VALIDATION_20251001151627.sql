...
WITH po_workflow AS (
    ...
    LEFT JOIN lats_purchase_order_payments popm ON po.id = popm.purchase_order_id
...
WITH recent_events AS (
    ...
    SELECT 
        ...
        'PAY' as category
    FROM lats_purchase_order_payments popm
...
SELECT 
    ...
    (SELECT COUNT(*) FROM lats_purchase_order_payments) as total_payments,
    (SELECT COUNT(*) 
     FROM inventory_items ii
     JOIN lats_purchase_order_items poi ON ii.product_id = poi.product_id
     WHERE (ii.metadata->>'purchase_order_id' IS NULL OR ii.metadata->>'purchase_order_id' = '')
