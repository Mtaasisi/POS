...
-- Create permissive policies for authenticated users (only if they don't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- Create permissive policies for all tables (only if they don't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'device_remarks' 
        AND policyname = 'Enable all access for authenticated users'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'device_transitions' 
        AND policyname = 'Enable all access for authenticated users'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'device_ratings' 
        AND policyname = 'Enable all access for authenticated users'
    ) THEN
