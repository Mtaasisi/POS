-- Simple fix for diagnostic_checks table foreign key constraint
-- Since the table is empty, we can safely add the constraint

-- Ensure the diagnostic_checks table exists with proper structure
...
-- Add the foreign key constraint (safe since table is empty)
DO $$
BEGIN
...
SELECT 
    ...
    'Foreign key constraint added to diagnostic_checks table' as message;
