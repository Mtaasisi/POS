-- =====================================================
-- SIMPLE QUALITY CHECK SYSTEM FIX - NO CONFLICTS
-- This script creates the quality check system without ON CONFLICT clauses
-- =====================================================
...
DO $$
BEGIN
    -- Insert electronics template
    IF NOT EXISTS (SELECT 1 FROM quality_check_templates WHERE name = 'Electronics Quality Check') THEN
    ...
    -- Insert accessories template
    IF NOT EXISTS (SELECT 1 FROM quality_check_templates WHERE name = 'Accessories Quality Check') THEN
        INSERT INTO quality_check_templates (name, description, category) VALUES
        ('Accessories Quality Check', 'Standard quality check for accessories', 'accessories');
    END IF;
    
    -- Insert general template
    IF NOT EXISTS (SELECT 1 FROM quality_check_templates WHERE name = 'General Quality Check') THEN
        INSERT INTO quality_check_templates (name, description, category) VALUES
...
BEGIN
    ...
    IF v_electronics_template_id IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_electronics_template_id AND name = 'Physical Condition') THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_electronics_template_id AND name = 'Functionality Test') THEN
            INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) VALUES
            (v_electronics_template_id, 'Functionality Test', 'Test basic functionality and features', true, 2);
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_electronics_template_id AND name = 'Accessories Check') THEN
            INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) VALUES
            (v_electronics_template_id, 'Accessories Check', 'Verify all accessories are included', true, 3);
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_electronics_template_id AND name = 'Packaging Condition') THEN
            INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) VALUES
    ...
    IF v_accessories_template_id IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_accessories_template_id AND name = 'Physical Condition') THEN
            INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) VALUES
            (v_accessories_template_id, 'Physical Condition', 'Check for damage or defects', true, 1);
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_accessories_template_id AND name = 'Compatibility Check') THEN
            INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) VALUES
            (v_accessories_template_id, 'Compatibility Check', 'Verify compatibility with intended device', true, 2);
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_accessories_template_id AND name = 'Packaging Check') THEN
            INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) VALUES
    ...
    IF v_general_template_id IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_general_template_id AND name = 'Physical Condition') THEN
            INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) VALUES
            (v_general_template_id, 'Physical Condition', 'Check for damage or defects', true, 1);
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_general_template_id AND name = 'Quantity Verification') THEN
            INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) VALUES
            (v_general_template_id, 'Quantity Verification', 'Verify correct quantity received', true, 2);
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_general_template_id AND name = 'Packaging Check') THEN
            INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) VALUES
