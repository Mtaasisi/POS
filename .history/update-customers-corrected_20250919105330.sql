...
-- Show customers that will be updated (matching by phone number format conversion)
SELECT 
...
FROM customers c
INNER JOIN temp_customer_data t ON (
    -- Match by converting database phone (+255712612596) to local format (712612596)
    -- and comparing with CSV phone (712612596)
    RIGHT(c.phone, 9) = RIGHT(t.phone, 9)
    OR 
    -- Also try matching the full local number
    SUBSTRING(c.phone, 4) = t.phone
)
WHERE t.rn = 1  -- Only take the first record for each phone number
ORDER BY c.name;

-- Update existing customers with new names and locations
-- Handle phone number format conversion: +255712612596 -> 712612596
UPDATE customers 
SET 
...
WHERE (
    -- Match by converting database phone (+255712612596) to local format (712612596)
    RIGHT(customers.phone, 9) = RIGHT(t.phone, 9)
    OR 
    -- Also try matching the full local number
    SUBSTRING(customers.phone, 4) = t.phone
)
