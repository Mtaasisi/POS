...
async function applyUnifiedPaymentMethods() {
  try {
    ...
    // Check if payment_methods_accounts table exists
    ...
    if (accountsError) {
      console.error('❌ Error checking payment_methods_accounts table:', accountsError.message);
    } else {
      console.log('✅ payment_methods_accounts table is accessible');
    }
