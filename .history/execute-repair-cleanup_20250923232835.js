...
async function executeRepairCleanupDirect() {
    ...
    try {
        ...
        const { data: repairPayments, error: checkError } = await supabase
            ...
            .or(`
                ...
                notes.ilike.%screen%,
                notes.ilike.%battery%,
                notes.ilike.%charging%,
                notes.ilike.%water damage%,
                notes.ilike.%software%,
                notes.ilike.%update%,
                notes.ilike.%flash%,
                notes.ilike.%unlock%,
                notes.ilike.%root%,
                notes.ilike.%jailbreak%,
                method.eq.repair,
                method.eq.maintenance,
                method.eq.fix,
                method.eq.service,
                method.eq.technician,
                method.eq.workshop,
                payment_type.eq.repair,
                payment_type.eq.maintenance,
                payment_type.eq.fix,
                payment_type.eq.service,
                payment_type.eq.technician,
                payment_type.eq.workshop
            `);
        ...
        console.log(`📊 Found ${repairPayments?.length || 0} repair payments to remove`);

        if (repairPayments && repairPayments.length > 0) {
            console.log('\n📋 Repair payments found:');
            repairPayments.forEach((payment, index) => {
            ...
            const repairPaymentIds = repairPayments.map(p => p.id);
