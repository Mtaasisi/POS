...
/**
 * Upload file to Hostinger storage with comprehensive fallback
 */
async function uploadToHostinger(filePath, remotePath) {
  try {
    if (!config.hostinger.apiToken) {
      console.log('⚠️  Hostinger API token not configured, using local backup only');
      return { success: false, error: 'API token not configured', fallback: true };
    }

    // Alternative Hostinger API endpoints to try
    const apiEndpoints = [
    ...
    for (const endpoint of apiEndpoints) {
      try {
        console.log(`🔧 Trying Hostinger API endpoint: ${endpoint}`);
        ...
        if (response.ok) {
          ...
          console.log(`✅ Uploaded to Hostinger: ${remotePath} (via ${endpoint})`);
        ...
        lastError = `DNS/Network error with ${endpoint}: ${error.message}`;
        console.log(`❌ DNS/Network error with ${endpoint}: ${error.message}`);
        
        // Count DNS errors
        if (error.code === 'ENOTFOUND' || error.message.includes('DNS') || error.message.includes('getaddrinfo')) {
          dnsErrorCount++;
        }
      }
    }

    // If we had multiple DNS errors, provide specific guidance
    if (dnsErrorCount >= 3) {
      console.log('🌐 DNS Resolution Issues Detected');
      console.log('💡 DNS Issue Solutions:');
      console.log('   1. Change DNS servers to:');
      console.log('      - Google DNS: 8.8.8.8, 8.8.4.4');
      console.log('      - Cloudflare DNS: 1.1.1.1, 1.0.0.1');
      console.log('   2. Check if api.hostinger.com is blocked');
      console.log('   3. Try using a VPN');
      console.log('   4. Verify internet connection');
      console.log('');
      console.log('✅ Local backup completed successfully');
      console.log('📁 Backup files saved locally in ./backups/');
      console.log('🔄 Will retry Hostinger upload on next backup');
      
      return { 
        success: false, 
        error: `DNS resolution failed for all endpoints. Local backup completed.`,
        fallback: true,
        localBackup: true
      };
    }

    // If it's not a DNS issue, try a simple connection test
    console.log('🔍 Testing basic Hostinger API connectivity...');
    try {
      const testResponse = await fetch('https://api.hostinger.com/v1/domains', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${config.hostinger.apiToken}`
        },
      ...
      if (testResponse.ok) {
        console.log('✅ Hostinger API is reachable, but file upload failed');
        console.log('💡 This might be an API endpoint or authentication issue');
      } else {
        console.log('❌ Hostinger API is not reachable');
      }
    } catch (testError) {
      console.log('❌ Hostinger API connectivity test failed:', testError.message);
    }
    ...
    return { 
      success: false, 
      error: `All Hostinger API endpoints failed. Local backup completed.`,
      fallback: true,
      localBackup: true
    };
  } catch (error) {
...
async function performBackup() {
  ...
  try {
    ...
    const uploadResult = await uploadToHostinger(completeBackupPath, `${remotePath}/complete_backup.json`);
    
    if (uploadResult.success) {
      console.log('✅ Upload to Hostinger successful');
      ...
      console.log('⚠️  Hostinger upload failed, but local backup completed successfully');
      console.log('📁 Backup files saved locally in ./backups/');
      console.log('🔄 Will retry Hostinger upload on next backup');
    } else {
      console.log('❌ Upload to Hostinger failed:', uploadResult.error);
    }
...
async function testBackup() {
  ...
  try {
    ...
    if (config.hostinger.apiToken) {
      console.log('🔍 Testing Hostinger API connectivity...');
      ...
      for (const endpoint of apiEndpoints) {
        try {
          ...
          if (response.ok) {
            console.log(`✅ Hostinger API connection: OK (via ${endpoint})`);
            hostingerConnected = true;
            break;
          } else {
          ...
          // If it's a DNS error, try the next endpoint
      ...
      if (!hostingerConnected) {
        console.log('❌ All Hostinger API endpoints failed - DNS issue detected');
        ...
        console.log('   4. Verify your Hostinger API token is correct');
      }
    } else {
