-- =====================================================
-- EXECUTE PO PAYMENT FIXES
-- =====================================================
-- This script fixes the payment issues for the specific PO
...
-- Step 3: Fix currency and amount (convert USD to TZS)
UPDATE purchase_order_payments 
SET 
    currency = 'TZS',
    amount = CASE 
        WHEN currency = 'USD' THEN amount * 2500 -- Replace with actual exchange rate
        ELSE amount
    END,
    payment_date = NOW(),
...
SELECT 
    ...
    'system',
    'Fixed payment currency from USD to TZS, updated exchange rate to 2500',
    NOW()
FROM lats_purchase_orders 
WHERE order_number LIKE '%1759046163%';
