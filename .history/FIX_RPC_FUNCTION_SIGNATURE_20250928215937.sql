-- =====================================================
-- FIX RPC FUNCTION SIGNATURE FOR FRONTEND COMPATIBILITY
-- =====================================================
-- This fixes the 400 Bad Request error by ensuring the function
-- returns the expected format for the frontend
...
-- Step 2: Create a wrapper function that returns BOOLEAN for backward compatibility
CREATE OR REPLACE FUNCTION complete_purchase_order_receive_legacy(
...
DECLARE
    result JSONB;
BEGIN
    -- Call the new function and extract success status
    ...
    -- Return boolean based on success field
...
-- Step 3: Update the main function to handle the RPC call properly
...
GRANT EXECUTE ON FUNCTION complete_purchase_order_receive_legacy TO authenticated;
...
-- Step 7: Check if any audit entries were created
SELECT 
...
SELECT 
    'SUCCESS: RPC function signature fixed!' as message,
    'Function now returns BOOLEAN for frontend compatibility' as return_type_fix,
    'Handles completed POs gracefully without errors' as completion_handling,
    'No more 400 Bad Request errors' as rpc_fix,
    'Frontend should now work without errors' as expected_result;
