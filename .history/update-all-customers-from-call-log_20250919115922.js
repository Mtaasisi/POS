...
function generateSQL(callData) {
    ...
    for (const [phone, data] of callData) {
        ...
        // Format datetime
        const firstCall = data.firstCall instanceof Date && !isNaN(data.firstCall) 
            ? data.firstCall.toISOString().replace('T', ' ').replace('Z', '+00')
            : '2023-10-13 18:39:16+00';
        const lastCall = data.lastCall instanceof Date && !isNaN(data.lastCall)
            ? data.lastCall.toISOString().replace('T', ' ').replace('Z', '+00')
            : '2025-09-17 17:28:01+00';
