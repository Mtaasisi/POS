...
if [ ! -f "fix_database_safe.sql" ]; then
    echo "❌ Error: fix_database_safe.sql not found in current directory"
    exit 1
fi

echo "✅ Found fix_database_safe.sql"
echo "📋 Contents of the fix script:"
echo "----------------------------------------"
head -20 fix_database_safe.sql
echo "..."
echo "----------------------------------------"
...
echo "   cat fix_database_minimal.sql" 