  ...
  async createRepairPayment(data: CreateRepairPaymentData, userId: string): Promise<RepairPayment> {
    try {
      ...
      if (refreshError) {
        ...
        // Don't throw error, continue with current session
      } else {
      ...
      const paymentData = {
        ...
        created_by: user.id // Use authenticated user's ID, not the passed userId
        // REMOVED: updated_at - let the database trigger handle this
      };
