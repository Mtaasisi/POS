...
BEGIN
  ...
  WITH updated_products AS (
    ...
    SET 
      total_value = (
        SELECT COALESCE(SUM(cost_price * quantity), 0)
        FROM lats_product_variants 
        WHERE product_id = lats_products.id
      ),
      total_quantity = (
        SELECT COALESCE(SUM(quantity), 0)
        FROM lats_product_variants 
  ...
  SELECT 
    up.id,
    up.name,
    p.total_value as old_total_value,
    up.total_value as new_total_value,
    p.total_quantity as old_total_quantity,
    up.total_quantity as new_total_quantity,
    (up.total_value - p.total_value) as difference
  FROM updated_products up
  JOIN lats_products p ON up.id = p.id
  WHERE ABS(up.total_value - p.total_value) > 1; -- Only show significant differences
END;
$$ LANGUAGE plpgsql;
