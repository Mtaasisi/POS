...
async function executeRepairCleanupDirect() {
    ...
    try {
        ...
        // Final verification
        console.log('\n🔍 Final verification...');
        const { data: finalCheck1, error: finalError1 } = await supabase
            .from('customer_payments')
            .select('id')
            .or('notes.ilike.%repair%,notes.ilike.%device repair%,notes.ilike.%repair payment%,notes.ilike.%fix%,notes.ilike.%maintenance%,notes.ilike.%xiaomi%,notes.ilike.%redmi%')
            .limit(1);

        const { data: finalCheck2, error: finalError2 } = await supabase
            .from('customer_payments')
            .select('id')
            .or('method.eq.repair,method.eq.maintenance,method.eq.fix,payment_type.eq.repair,payment_type.eq.maintenance,payment_type.eq.fix')
            .limit(1);

        const finalError = finalError1 || finalError2;
        const finalCheck = (finalCheck1 || []).concat(finalCheck2 || []);
