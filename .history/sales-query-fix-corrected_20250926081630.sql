-- CORRECTED SALES QUERY FIX
-- This version uses the actual table structure (without p.price column)
...
-- NOTE: Removed p.price, p.cost, p.stock_quantity, p.min_stock_level as these columns don't exist
SELECT 
    p.id,
    p.name,
    p.description,
...
-- NOTE: Removed pv.price, pv.cost, pv.stock_quantity, pv.min_stock_level as these columns don't exist
SELECT 
    pv.id,
    pv.product_id,
    pv.name,
