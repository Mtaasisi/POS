-- Safe SMS Database Setup
-- This script safely creates SMS tables and handles existing tables

-- Enable required extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 1. Create or modify settings table
...
BEGIN
    IF NOT EXISTS (
        ...
        AND conrelid = 'sms_templates'::regclass
    ) THEN
        ALTER TABLE sms_templates ADD CONSTRAINT sms_templates_name_key UNIQUE (name);
    END IF;
END $$;

-- 4. Create sms_triggers table
...
-- 5. Create sms_trigger_logs table
...
-- Insert SMS settings using safe upsert
...
UPDATE settings SET value = 'test_api_key_123' WHERE key = 'sms_provider_api_key';
UPDATE settings SET value = 'https://httpbin.org/post' WHERE key = 'sms_api_url';
UPDATE settings SET value = '15' WHERE key = 'sms_price';

-- Insert default SMS templates using safe insert
INSERT INTO sms_templates (name, content, variables, is_active) 
SELECT 'Device Received', 
       '✅ Tumepokea Kimepokelewa!\n\nHellow Mtaasisi {name},\n\nHabari njema! {device_brand} {device_model} yako imepokelewa na sasa iko katika foleni ya ukarabati wa Inauzwa.\n\n📋 Namba ya Kumbukumbu: #{device_id}\n📅 Tarehe ya Kupokea: {date}\n🔧 Tatizo: {issue}\n\nSubiri ujumbe kupitia SMS kikiwa tayari!\n\nAsante kwa kumtumaini Inauzwa 🚀', 
       '["name", "device_brand", "device_model", "device_id", "date", "issue"]'::jsonb, 
       true
WHERE NOT EXISTS (SELECT 1 FROM sms_templates WHERE name = 'Device Received');
...
SELECT 'Device Ready', 
       ...
       '["name", "device_brand", "device_model", "device_id", "date"]'::jsonb, 
       true
WHERE NOT EXISTS (SELECT 1 FROM sms_templates WHERE name = 'Device Ready');
