    ...
    if (numberError) {
      ...
      if (itemsError) {
        ...
        items.forEach((item, index) => {
          console.log(`   ${index + 1}. ${item.lats_products?.name || item.lats_product_variants?.name || 'Unknown Product'}`);
          console.log(`      SKU: ${item.lats_products?.sku || item.lats_product_variants?.sku || 'N/A'}`);
