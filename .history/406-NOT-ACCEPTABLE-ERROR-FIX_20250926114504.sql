...
DROP POLICY IF EXISTS "Users can delete their own sales" ON lats_sales;

-- Create comprehensive RLS policies
CREATE POLICY "Users can view their own sales" ON lats_sales
    FOR SELECT
    USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own sales" ON lats_sales
    FOR INSERT
    WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own sales" ON lats_sales
    FOR UPDATE
    USING (auth.uid() = user_id)
    WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete their own sales" ON lats_sales
    FOR DELETE
    USING (auth.uid() = user_id);
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE lats_sales ADD COLUMN user_id UUID REFERENCES auth.users(id);
    END IF;
    
    -- Set default value for user_id
    ALTER TABLE lats_sales ALTER COLUMN user_id SET DEFAULT auth.uid();
    
    -- Make user_id NOT NULL
    ALTER TABLE lats_sales ALTER COLUMN user_id SET NOT NULL;
END $$;
