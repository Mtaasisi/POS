...
-- Insert default goals for existing users (only if they don't exist)
INSERT INTO user_daily_goals (user_id, goal_type, goal_value, is_active) 
SELECT id, 'new_customers', 5, true 
FROM auth_users 
WHERE role = 'technician'
AND NOT EXISTS (
    SELECT 1 FROM user_daily_goals 
    WHERE user_daily_goals.user_id = auth_users.id 
    AND goal_type = 'new_customers'
);

INSERT INTO user_daily_goals (user_id, goal_type, goal_value, is_active) 
SELECT id, 'checkins', 10, true 
FROM auth_users 
WHERE role = 'technician'
AND NOT EXISTS (
    SELECT 1 FROM user_daily_goals 
...
INSERT INTO user_daily_goals (user_id, goal_type, goal_value, is_active) 
SELECT id, 'devices_processed', 8, true 
FROM auth_users 
WHERE role = 'technician'
AND NOT EXISTS (
    SELECT 1 FROM user_daily_goals 
    WHERE user_daily_goals.user_id = auth_users.id 
    AND goal_type = 'devices_processed'
);

INSERT INTO user_daily_goals (user_id, goal_type, goal_value, is_active) 
SELECT id, 'repairs_completed', 3, true 
FROM auth_users 
WHERE role = 'technician'
AND NOT EXISTS (
    SELECT 1 FROM user_daily_goals 
    WHERE user_daily_goals.user_id = auth_users.id 
    AND goal_type = 'repairs_completed'
);
