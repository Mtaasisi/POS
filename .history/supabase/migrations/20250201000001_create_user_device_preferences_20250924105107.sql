...
-- Create RLS policies
CREATE POLICY IF NOT EXISTS "Users can view their own device preferences" ON user_device_preferences
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own device preferences" ON user_device_preferences
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own device preferences" ON user_device_preferences
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own device preferences" ON user_device_preferences
