...
BEGIN
    ...
    IF (OLD.repair_price IS DISTINCT FROM NEW.repair_price) OR 
       (OLD.price IS DISTINCT FROM NEW.price) OR
       (OLD.cost IS DISTINCT FROM NEW.cost) THEN
        
        -- Determine which price field changed and get the values
        DECLARE
            old_price_value NUMERIC := 0;
            new_price_value NUMERIC := 0;
        BEGIN
            -- Try to get price from different possible field names
            IF OLD.repair_price IS DISTINCT FROM NEW.repair_price THEN
                old_price_value := COALESCE(OLD.repair_price, 0);
                new_price_value := COALESCE(NEW.repair_price, 0);
            ELSIF OLD.price IS DISTINCT FROM NEW.price THEN
                old_price_value := COALESCE(OLD.price, 0);
                new_price_value := COALESCE(NEW.price, 0);
            ELSIF OLD.cost IS DISTINCT FROM NEW.cost THEN
                old_price_value := COALESCE(OLD.cost, 0);
                new_price_value := COALESCE(NEW.cost, 0);
            END IF;
            ...
            INSERT INTO device_price_history (
                ...
                auth.uid(),
                jsonb_build_object(
                    'device_name', COALESCE(NEW.name, 'Unknown Device'),
...
-- Note: This trigger will only work if the devices table has one of these price fields:
-- repair_price, price, or cost
