...
-- Add NOT NULL constraint to lats_sales table (if customer_id exists)
...
-- Add NOT NULL constraint to payment_transactions table (if customer_id exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Add check constraint to lats_sales table (if customer_id exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'lats_sales' AND column_name = 'customer_id') THEN
...
-- Add check constraint to payment_transactions table (if customer_id exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'payment_transactions' AND column_name = 'customer_id') THEN
