    ...
    if (strpos($apiUrl, 'mshastra.com') !== false) {
        ...
        return [
            ...
            'body' => http_build_query([
                'username' => $apiKey,
                'password' => $apiKey, // Mobishastra uses same value for both
                'to' => $phone,
                'message' => $message,
                'from' => $senderId
            ])
...
function parseSMSResponse($apiUrl, $response, $httpCode) {
    // Detect provider based on URL
    if (strpos($apiUrl, 'mshastra.com') !== false) {
        // Mobishastra typically returns simple text responses
        if ($httpCode >= 200 && $httpCode < 300) {
            ...
            return [
                'success' => false,
                'data' => null,
                'error' => 'SMS sending failed: ' . $response
            ];
        }
    } else {
