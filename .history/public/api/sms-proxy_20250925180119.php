...
try {
    ...
    error_log("🔍 DEBUG: Raw input length: " . strlen($input));
    
    $request = json_decode($input, true);
    error_log("🔍 DEBUG: Decoded request: " . json_encode($request));
    
    if (!$request) {
        error_log("❌ Invalid JSON data received");
        error_log("🔍 DEBUG: JSON decode error: " . json_last_error_msg());
        http_response_code(400);
        echo json_encode(['error' => 'Invalid JSON data', 'raw_input' => $input, 'json_error' => json_last_error_msg()]);
        exit();
    }
    ...
    error_log("🔍 DEBUG: Extracted parameters:");
    error_log("   Phone: " . $phone);
    error_log("   API URL: " . $apiUrl);
    error_log("   API Key: " . $apiKey);
    error_log("   API Password: " . ($apiPassword ? '[SET]' : '[NOT SET]'));
    ...
    $providerData = prepareSMSRequest($apiUrl, $phone, $message, $apiKey, $apiPassword, $senderId);
 ...
 */
function prepareSMSRequest($apiUrl, $phone, $message, $apiKey, $apiPassword, $senderId) {
    ...
    if (strpos($apiUrl, 'mshastra.com') !== false) {
        ...
        $queryParams = [
            'user' => $apiKey,
            'pwd' => $apiPassword, // Use the actual password
            'senderid' => $senderId,
            'mobileno' => $phone,
            'msgtext' => $message,
            'priority' => 'High',
