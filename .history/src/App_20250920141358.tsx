...
const createSafeLazyComponent = (importFunction: () => Promise<any>) => {
  return lazy(() => {
    return importFunction().catch((error) => {
      ...
      // If it's a primitive conversion error, try to reload the module
      if (error.message && error.message.includes('Cannot convert object to primitive value')) {
        console.warn('Primitive conversion error detected, attempting module reload...');
        // Clear any cached modules and retry
        if (typeof window !== 'undefined' && 'caches' in window) {
          caches.keys().then(names => {
            names.forEach(name => {
              if (name.includes('vite') || name.includes('app')) {
                caches.delete(name);
              }
            });
          });
        ...
        return new Promise((resolve) => {
          setTimeout(() => {
            importFunction().then(resolve).catch(() => {
              // If retry fails, return a fallback component wrapped in a Promise
              resolve({
                default: () => (
      ...
      // For other errors, return a fallback component wrapped in a Promise
