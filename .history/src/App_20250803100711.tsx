...
const AppContent: React.FC<{ isOnline: boolean; isSyncing: boolean }> = ({ isOnline, isSyncing }) => {
  // Handle offline sync without using context hooks here
  useEffect(() => {
    async function syncPending() {
      ...
      for (const action of actions) {
        if (action.type === 'submitData') {
          ...
          // Handle customer creation directly with Supabase
          const { data, error } = await supabase
            ...
            .single();
          if (error) {
            console.error('Error creating customer from offline action:', error);
          }
          ...
          // Handle technician assignment directly with Supabase
          await supabase
            .from('devices')
            .update({ assignedTo: selectedTechId })
            .eq('id', deviceId);
        } else if (action.type === 'markDeviceFailed') {
          const { deviceId, remark } = action.payload;
          // Handle device status update directly with Supabase
          await supabase
            .from('devices')
            .update({ status: 'failed', remarks: remark || '' })
            .eq('id', deviceId);
        }
      }
      if (actions.length > 0) {
