  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
        try {
          ...
          // Get current session first without clearing state
          const { data: { session }, error: sessionError } = await supabase.auth.getSession();
          
          if (sessionError) {
            logError('AuthProvider', 'Session error:', sessionError);
            
            // Only clear auth state for specific errors, not all session errors
          ...
          // If no session, just set user to null without clearing state
          if (!session?.user) {
            logInfo('AuthProvider', 'No active session found');
            setCurrentUser(null);
            setLoading(false);
            initializedRef.current = true;
            return;
