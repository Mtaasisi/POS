...
export function validateRepairStart(parts: RepairPart[]): ValidationResult {
  ...
  // Check if all required parts are received or installed
  const requiredParts = parts.filter(part => part.status !== 'installed');
  ...
  // Check if all parts are ready (received or installed)
...
export function hasNeededParts(parts: RepairPart[]): boolean {
  ...
  // Check if any parts are not yet received or installed
...
export function allPartsReady(parts: RepairPart[]): boolean {
  ...
  // All parts must be received or installed
...
/**
 * Gets parts that are ready for repair (received or installed)
 */
