...
export function validateRepairProgress(parts: RepairPart[], currentStatus: string, targetStatus: string): ValidationResult {
  ...
  if (!parts || parts.length === 0) {
    console.log('🔍 [DEBUG] No parts - allowing transition');
    return {
      valid: true,
      message: 'No parts required for this repair'
    };
  ...
  console.log('🔍 [DEBUG] Checking transition:', transition, 'in required transitions:', partsRequiredTransitions.includes(transition));
  ...
  if (partsRequiredTransitions.includes(transition)) {
    ...
    if (!allPartsReady) {
      ...
      console.log('🔍 [DEBUG] Blocking transition - pending parts:', {
        pendingParts: pendingParts.map(p => ({ name: p.name, status: p.status })),
        pendingStatuses
      });
      
      return {
        valid: false,
        message: `Cannot continue repair: ${pendingParts.length} part(s) still pending (${pendingStatuses.join(', ')}). All parts must be received before continuing.`
      };
    }
  }

  console.log('🔍 [DEBUG] Allowing transition - no parts validation required');
  return {
    valid: true,
