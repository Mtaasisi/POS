...
const AppLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'transparent' }}>
      ...
      <header className="sticky top-0 z-20 backdrop-blur-xl bg-white/60 border-b border-white/30 shadow-lg p-4 md:hidden">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <span className="font-bold text-gray-900 truncate">Repair Shop</span>
          </div>
      ...
      <div className="hidden md:block fixed top-0 bottom-0 left-0 z-50 w-72 bg-white/80 backdrop-blur-xl border-r border-white/30 shadow-xl transition-all duration-500">
        <div className="flex flex-col h-full">
              ...
              {navItems.map(item => (
                <li key={item.path}>
                  <Link
                    ...
                    `}
                  >
                    <span className={location.pathname === item.path ? 'text-blue-600' : 'text-blue-500'}>
                      {item.icon}
                    </span>
                    <span className="flex-1">{item.label}</span>
                    
                    {item.count && item.count > 0 && (
                      <ActivityCounter count={item.count} />
                    )}
                    
                    {location.pathname === item.path && (
                      <ChevronRightIcon size={16} className="ml-auto text-blue-500" />
                    )}
                  </Link>
                </li>
              ))}
          ...
          <div className="p-4 border-t border-white/20">
            <div className="flex items-center gap-3 p-2 rounded-lg bg-gradient-to-br from-gray-100 to-gray-50 backdrop-blur-sm mb-4">
      ...
      {/* Main Content */}
      <main className={`transition-all duration-500 min-h-screen relative z-10 pt-4 md:pt-16 pb-8 md:ml-72`}>
        <Outlet />
