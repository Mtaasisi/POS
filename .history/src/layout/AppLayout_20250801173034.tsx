...
import SimpleBackgroundSelector from '../components/ui/SimpleBackgroundSelector';
import AddCustomerModal from '../components/forms/AddCustomerModal';
import AddDevicePage from '../pages/NewDevicePage';
...
const AppLayout: React.FC = () => {
  ...
  const handleBackgroundChange = (backgroundClass: string) => {
    ...
    // Remove all background classes from body
    document.body.classList.remove(
      's-curve-red-blue', 'warm-gradient', 'cool-gradient', 'sunset',
      'ocean', 'forest', 'animated-gradient', 'glass-overlay',
      'dark-mode', 'pos-wallpaper', 'database-wallpaper', 'admin-wallpaper'
    );
    ...
    localStorage.setItem('selectedBackground', backgroundClass);
  };
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'transparent' }}>
      ...
      <main className={`transition-all duration-500 min-h-screen relative z-10 pt-4 md:pt-16 pb-8 ${isNavCollapsed ? 'md:ml-[5.5rem]' : 'md:ml-72'}`}>
        <Outlet />
        {/* Background button for all users */}
        <BackgroundButton
