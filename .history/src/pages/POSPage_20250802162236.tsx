...
import SeparatedProductSearch from '../components/pos/SeparatedProductSearch';
...
const POSPage: React.FC = () => {
  ...
  const [searchParts, setSearchParts] = useState<Product[]>([]);
  ...
  const loadProducts = async () => {
    try {
      ...
      const { products, parts } = await searchProductsForPOS('');
      console.log('Loaded initial products - Products:', products.length, 'Parts:', parts.length);
      setSearchResults([...products, ...parts].slice(0, 10)); // Show first 10 products
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      const { products, parts } = await searchProductsForPOS(query);
      console.log('Search results - Products:', products.length, 'Parts:', parts.length);
      setSearchResults([...products, ...parts]);
      setSearchProducts(products);
      setSearchParts(parts);
    } catch (error) {
