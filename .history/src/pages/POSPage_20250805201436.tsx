            ...
            <GlassCard className="bg-white/90 flex-1 flex flex-col min-h-0">
              ...
              {cart.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-2 flex-1 overflow-y-auto">
                  {cart.map((item, idx) => (
                    <div key={item.id} className={`bg-white/80 backdrop-blur-sm rounded-lg p-2 border border-gray-200/50 hover:border-blue-300/50 transition-all duration-200 ${editingCartIndex === idx ? 'ring-2 ring-blue-400 shadow-md' : 'hover:shadow-sm'}`}
                      tabIndex={0}
                      ...
                      <div className="flex flex-col h-full">
                        ...
                        <div className="flex items-start gap-2 mb-2">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {item.variant && (
                              <p className="text-xs text-gray-600 font-medium line-clamp-1">{item.variant}</p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-auto">
                          <div className="text-left">
                            <p className="text-xs text-gray-500">Tsh{item.unitPrice.toLocaleString()}</p>
                            <p className="font-semibold text-gray-900 text-xs">Tsh{item.total.toLocaleString()}</p>
                          </div>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
                              <button
                                className="w-6 h-6 rounded-md bg-white text-gray-600 hover:bg-gray-200 transition-colors flex items-center justify-center text-xs font-bold"
                                onClick={(e) => {
                                  e.stopPropagation();
                              ...
                              {editingCartIndex === idx ? (
                                <input
                                  type="number"
                                  className="w-8 text-center text-xs font-medium border-0 bg-transparent focus:ring-0 focus:outline-none"
                                  value={item.quantity}
                                  autoFocus
                                  min={1}
                                ...
                                <span
                                  className="w-8 text-center text-xs font-medium cursor-pointer hover:bg-blue-50 rounded px-1 py-1"
                                  tabIndex={0}
                                  onClick={() => setEditingCartIndex(idx)}
                                  onKeyDown={e => {
                            ...
                            {/* Remove Button */}
                            <button
                              className="w-6 h-6 rounded-md bg-red-50 text-red-600 hover:bg-red-100 transition-colors flex items-center justify-center"
                              onClick={(e) => {
                                e.stopPropagation();
