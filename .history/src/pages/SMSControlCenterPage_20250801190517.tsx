...
const SMSControlCenterPage: React.FC = () => {
  ...
  const [triggerForm, setTriggerForm] = useState({ name: '', status: '', template_id: '' });
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2 rounded-lg bg-white/20 backdrop-blur-md border border-white/30 hover:bg-white/30 transition-colors"
          >
            <ArrowLeft size={20} />
          </button>
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">SMS Center</h1>
            <p className="text-gray-600">Customer care SMS management</p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <GlassButton icon={<RefreshCw size={16} />} variant="secondary" onClick={() => window.location.reload()}>
            Refresh
          </GlassButton>
        </div>
      </div>
