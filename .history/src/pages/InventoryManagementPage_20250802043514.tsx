...
const InventoryManagementPage: React.FC = () => {
  ...
  // Categories state
  const [categories, setCategories] = useState<inventoryApi.InventoryCategory[]>([]);
  const [showNewCategory, setShowNewCategory] = useState(false);
  const [editingCategory, setEditingCategory] = useState<inventoryApi.InventoryCategory | null>(null);
  ...
  // Suppliers state
  const [suppliers, setSuppliers] = useState<inventoryApi.Supplier[]>([]);
  const [showNewSupplier, setShowNewSupplier] = useState(false);
  const [editingSupplier, setEditingSupplier] = useState<inventoryApi.Supplier | null>(null);
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      const category = await inventoryApi.createInventoryCategory(newCategory);
  ...
  const handleUpdateCategory = async () => {
    ...
    try {
      const updatedCategory = await inventoryApi.updateInventoryCategory(editingCategory.id, {
  ...
  const handleDeleteCategory = async (id: string) => {
    ...
    try {
      await inventoryApi.deleteInventoryCategory(id);
