  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      ...
      {lowStockParts.length > 0 && (
        <GlassCard className="bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-l-red-500 shadow-md">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-red-700">{lowStockParts.length} items need attention</p>
              </div>
            </div>
            <div className="px-2 py-1 bg-red-100 rounded-full">
              <span className="text-red-800 font-semibold text-xs">URGENT</span>
            </div>
          </div>
          
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 mb-3">
            {lowStockParts.slice(0, 8).map((part, index) => (
              <div key={part.id} className="group bg-white/90 p-3 rounded-lg border border-red-200 hover:border-red-300 transition-all duration-200 hover:shadow-sm">
                <div className="flex items-center gap-2 mb-2">
                  <div className="p-1 bg-red-50 rounded">
                    {getCategoryIcon(part.category)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-medium text-gray-900 text-sm truncate group-hover:text-red-700">
                      {part.name}
                    </h4>
          ...
          {lowStockParts.length > 8 && (
            <div className="flex items-center justify-between pt-2 border-t border-red-200">
              <span className="text-xs text-red-700">+{lowStockParts.length - 8} more items</span>
              <GlassButton
                onClick={() => setStockFilter('low')}
                variant="primary"
                className="bg-red-600 hover:bg-red-700 text-white px-4 py-1.5 text-sm rounded-md transition-all duration-200"
              >
                View All
              </GlassButton>
            </div>
      ...
      {outOfStockParts.length > 0 && (
        <GlassCard className="bg-gradient-to-r from-orange-50 to-red-50 border-l-4 border-l-orange-500 shadow-md">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-lg font-bold text-orange-900">Out of Stock Alert</h3>
                <p className="text-sm text-orange-700">{outOfStockParts.length} items completely out</p>
              </div>
            </div>
            <div className="px-2 py-1 bg-orange-100 rounded-full">
              <span className="text-orange-800 font-semibold text-xs">CRITICAL</span>
            </div>
          </div>
          
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 mb-3">
            {outOfStockParts.slice(0, 8).map((part, index) => (
              <div key={part.id} className="group bg-white/90 p-3 rounded-lg border border-orange-200 hover:border-orange-300 transition-all duration-200 hover:shadow-sm">
                <div className="flex items-center gap-2 mb-2">
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className="text-gray-500">Stock:</span>
                  <span className="font-bold text-orange-600">{part.stock_quantity}</span>
                </div>
                
                <div className="mt-1">
                  <div className="w-full bg-gray-200 rounded-full h-1.5">
                    <div className="bg-gradient-to-r from-orange-500 to-red-500 h-1.5 rounded-full" style={{ width: '0%' }}></div>
                  </div>
                </div>
              </div>
            ))}
          ...
          {outOfStockParts.length > 8 && (
            <div className="flex items-center justify-between pt-2 border-t border-orange-200">
              <span className="text-xs text-orange-700">+{outOfStockParts.length - 8} more items</span>
              <GlassButton
                onClick={() => setStockFilter('out')}
                variant="primary"
                className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-1.5 text-sm rounded-md transition-all duration-200"
              >
                View All
              </GlassButton>
            </div>
