    ...
    <div className={`min-h-screen ${isDarkMode ? 'dark' : ''}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 pb-24">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6">
            ...
            <div className="min-h-[calc(100vh-200px)]">
              {activeTab === 'products' && (
                <div>
                  ...
                  {/* Products Grid */}
                  {!productsLoading && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 h-full overflow-y-auto">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        {paymentAccounts.length > 0 ? (
                          paymentAccounts.map((account) => (
                            <button
                              key={account.id}
                              onClick={() => handlePaymentSelect(account.id, amountPaid)}
          ...
          <div className="lg:col-span-4 sticky top-6 h-[calc(100vh-120px)] flex flex-col">
            {/* Order Summary - Fixed Above Cart */}
            <div className="p-2 lg:p-3 rounded-xl bg-gradient-to-br from-emerald-50 to-green-50 border border-emerald-200/60 hover:border-emerald-300/70 hover:shadow-lg transition-all duration-300 relative overflow-hidden mb-3 lg:mb-4 flex-shrink-0">
              {/* Background Pattern */}
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-2 lg:mb-3">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 lg:w-8 lg:h-8 bg-gradient-to-r from-emerald-500 to-green-500 rounded-full flex items-center justify-center shadow-sm">
                      <DollarSign size={14} className="lg:w-4 lg:h-4 text-white" />
                    </div>
                    <span className="text-xs lg:text-sm font-semibold text-gray-700">Total Amount</span>
                  </div>
                  <span className="text-lg lg:text-xl font-bold bg-gradient-to-r from-emerald-600 to-green-600 bg-clip-text text-transparent">
                ...
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-1 text-gray-600">
                    <Package size={10} className="lg:w-3 lg:h-3 text-gray-500" />
                    <span className="font-medium">{cart.length} items</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600">
                    <Receipt size={10} className="lg:w-3 lg:h-3 text-gray-500" />
            ...
            {/* Cart - Scrollable Below Total */}
            <GlassCard className="bg-white/90 flex-1 flex flex-col min-h-0">
              <div className="flex items-center justify-between mb-3 lg:mb-4 p-3 lg:p-4">
                <h2 className="text-lg lg:text-xl font-semibold text-gray-900 flex items-center gap-2 lg:gap-3">
                  <ShoppingCart size={18} className="lg:w-5 lg:h-5" />
                  Cart ({cart.length})
                </h2>
                <button 
                  className="p-2 lg:p-3 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  onClick={clearCart}
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-2 flex-1 overflow-y-auto">
                  {cart.map((item, idx) => (
                    <div key={item.id} className={`bg-white/80 backdrop-blur-sm rounded-lg p-2 lg:p-3 border border-gray-200/50 hover:border-blue-300/50 transition-all duration-200 ${editingCartIndex === idx ? 'ring-2 ring-blue-400 shadow-md' : 'hover:shadow-sm'}`}
                      tabIndex={0}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start gap-2 lg:gap-3">
                            <div className="w-8 h-8 lg:w-10 lg:h-10 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0 mt-1">
                              <Package size={14} className="lg:w-4 lg:h-4 text-blue-600" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <h3 className="font-bold text-gray-900 text-sm lg:text-lg mb-1 line-clamp-2">{item.name}</h3>
                              {item.variant && (
                                <p className="text-xs lg:text-sm text-gray-600 font-medium mb-1 lg:mb-2 line-clamp-1">{item.variant}</p>
                              )}
                            </div>
                          </div>
                        </div>
                        
                        <div className="flex items-center gap-2 lg:gap-3 ml-2 lg:ml-3">
                          {/* Quantity Controls */}
                          <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
                            <button
                              className="w-8 h-8 lg:w-10 lg:h-10 rounded-md bg-white text-gray-600 hover:bg-gray-200 transition-colors flex items-center justify-center text-sm lg:text-lg font-bold"
                              onClick={(e) => {
                                e.stopPropagation();
                            ...
                            {editingCartIndex === idx ? (
                              <input
                                type="number"
                                className="w-10 lg:w-12 text-center text-sm lg:text-base font-medium border-0 bg-transparent focus:ring-0 focus:outline-none"
                                value={item.quantity}
                                autoFocus
                                min={1}
                              ...
                              <span
                                className="w-10 lg:w-12 text-center text-sm lg:text-base font-medium cursor-pointer hover:bg-blue-50 rounded px-1 lg:px-2 py-1"
                                tabIndex={0}
                                onClick={() => setEditingCartIndex(idx)}
                                onKeyDown={e => {
                          ...
                          <div className="text-right min-w-0">
                            <p className="text-xs lg:text-sm text-gray-500">Tsh{item.unitPrice.toLocaleString()}</p>
                            <p className="font-semibold text-gray-900 text-sm lg:text-base">Tsh{item.total.toLocaleString()}</p>
                          </div>
                          
                          {/* Remove Button */}
                          <button
                            className="w-8 h-8 lg:w-10 lg:h-10 rounded-md bg-red-50 text-red-600 hover:bg-red-100 transition-colors flex items-center justify-center"
                            onClick={(e) => {
                              e.stopPropagation();
