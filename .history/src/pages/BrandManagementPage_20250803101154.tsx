...
import { ArrowLeft, Plus, Edit, Trash2, Search, Filter, X, Save, RotateCcw, Tag, Smartphone, Laptop, Monitor, Headphones, Camera, Gaming, Printer, Watch, Speaker, Keyboard, Mouse, Router, Server, Storage, Other } from 'lucide-react';
import { 
  InventoryBrand, 
...
const BrandManagementPage: React.FC = () => {
  ...
  const categories = [
    ...
    { value: 'gaming', label: 'Gaming', icon: <Gaming size={16} /> },
    ...
    { value: 'storage', label: 'Storage', icon: <Storage size={16} /> },
    { value: 'other', label: 'Other', icon: <Other size={16} /> }
  ];
  ...
  const handleSearch = async (query: string) => {
    ...
    if (query.length > 2) {
      try {
        const results = await searchInventoryBrands(query);
  ...
  const handleAddBrand = async (e: React.FormEvent) => {
    ...
    try {
      setError('');
      await createInventoryBrand(formData);
  ...
  const handleUpdateBrand = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('');
      await updateInventoryBrand(editingBrand.id, formData);
  ...
  const handleDeleteBrand = async (brand: InventoryBrand) => {
    if (!confirm(`Are you sure you want to delete "${brand.name}"?`)) return;

    try {
      setError('');
      await deleteInventoryBrand(brand.id);
  ...
  const handleRestoreBrand = async (brand: InventoryBrand) => {
    if (!confirm(`Are you sure you want to restore "${brand.name}"?`)) return;

    try {
      setError('');
      await restoreInventoryBrand(brand.id);
  ...
  const startEdit = (brand: InventoryBrand) => {
