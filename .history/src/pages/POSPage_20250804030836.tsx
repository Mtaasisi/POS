...
const POSPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <GlassButton variant="outline" size="sm" onClick={exitPOS}>
                <ArrowLeft size={16} />
            ...
            <div className="flex items-center space-x-8">
              {[
                ...
                const isCompleted = 
                  ...
                  (step.key === 'payment' && paymentAccount) ||
            ...
            <GlassCard className="h-full">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <GlassButton variant="outline" size="sm" onClick={loadProducts} disabled={productsLoading}>
                    <RotateCcw size={14} className={productsLoading ? 'animate-spin' : ''} />
                  </GlassButton>
                  <GlassButton variant="outline" size="sm" onClick={() => setShowSmartSearch(true)}>
                    <Search size={14} />
                  </GlassButton>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {paymentAccounts.slice(0, 4).map((account) => (
                        <button
                          key={account.id}
      ...
      {/* All existing modals */}
      <POSSettingsModal isOpen={showSettings} onClose={() => setShowSettings(false)} />
      <AddCustomerModal isOpen={showAddCustomer} onClose={() => setShowAddCustomer(false)} />
      <MiniSalesDashboard isOpen={showDashboard} onClose={() => setShowDashboard(false)} />
      <SmartProductSearch isOpen={showSmartSearch} onProductSelect={handleProductSelect} onClose={() => setShowSmartSearch(false)} />
      <SmartNotifications notifications={notifications} onDismiss={dismissNotification} />
      <LocationSelector isOpen={showLocationSelector} onClose={() => setShowLocationSelector(false)} onLocationSelect={handleLocationSelect} currentLocation={currentLocation} />
      <AdvancedInventory isOpen={showAdvancedInventory} onClose={() => setShowAdvancedInventory(false)} onItemSelect={handleInventoryItemSelect} />
      <LoyaltyProgram isOpen={showLoyaltyProgram} onClose={() => setShowLoyaltyProgram(false)} onCustomerSelect={handleLoyaltyCustomerSelect} />
      <GiftCardManager isOpen={showGiftCardManager} onClose={() => setShowGiftCardManager(false)} onGiftCardRedeem={handleGiftCardRedeem} onGiftCardPurchase={handleGiftCardPurchase} />
      <FinanceAccountsModal isOpen={showPaymentsAccounts} onClose={() => setShowPaymentsAccounts(false)} />
      <PaymentSelectionModal isOpen={showPaymentSelection} onClose={() => setShowPaymentSelection(false)} onPaymentSelect={handlePaymentSelect} totalAmount={calculateTotals().total} />
      <DeliveryOptionsModal isOpen={showDeliveryOptions} onClose={() => setShowDeliveryOptions(false)} onConfirm={handleDeliveryOptionsConfirm} currentDeliveryData={{ method: deliveryMethod, address: deliveryAddress, city: deliveryCity, notes: deliveryNotes }} />
    </div>
  );
};
