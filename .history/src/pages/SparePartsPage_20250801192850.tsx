...
const SparePartsPage: React.FC = () => {
  ...
  useEffect(() => {
    console.log('🔍 SparePartsPage mounted');
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      console.log('🔄 Starting to load spare parts data...');
      ...
      if (directError) {
        ...
        console.log('✅ Direct API call successful:', directData?.length, 'parts');
      }
      
      // Test individual API calls with error handling
      console.log('📡 Testing getSpareParts...');
      let partsData;
      try {
        partsData = await getSpareParts();
        console.log('✅ getSpareParts successful:', partsData.length, 'parts');
      } catch (error) {
        console.error('❌ getSpareParts failed:', error);
        throw error;
      }
      
      console.log('📡 Testing getSparePartStats...');
      let statsData;
      try {
        statsData = await getSparePartStats();
        console.log('✅ getSparePartStats successful:', statsData);
      } catch (error) {
        console.error('❌ getSparePartStats failed:', error);
        throw error;
      }
      
      console.log('📡 Testing getSparePartCategories...');
      let categoriesData;
      try {
        categoriesData = await getSparePartCategories();
        console.log('✅ getSparePartCategories successful:', categoriesData);
      } catch (error) {
        console.error('❌ getSparePartCategories failed:', error);
        throw error;
      }
      
      console.log('📡 Testing getSparePartBrands...');
      let brandsData;
      try {
        brandsData = await getSparePartBrands();
        console.log('✅ getSparePartBrands successful:', brandsData);
      } catch (error) {
        console.error('❌ getSparePartBrands failed:', error);
        throw error;
      }
      
      console.log('📡 Testing getLowStockSpareParts...');
      let lowStockData;
      try {
        lowStockData = await getLowStockSpareParts();
        console.log('✅ getLowStockSpareParts successful:', lowStockData.length, 'parts');
      } catch (error) {
        console.error('❌ getLowStockSpareParts failed:', error);
        throw error;
      }
      
      console.log('📡 Testing getOutOfStockSpareParts...');
      let outOfStockData;
      try {
        outOfStockData = await getOutOfStockSpareParts();
        console.log('✅ getOutOfStockSpareParts successful:', outOfStockData.length, 'parts');
      } catch (error) {
        console.error('❌ getOutOfStockSpareParts failed:', error);
        throw error;
      }
        ...
        <GlassCard className="bg-red-50 border-red-200">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-red-700">{lowStockParts.length} parts need restocking</p>
            </div>
          </div>
