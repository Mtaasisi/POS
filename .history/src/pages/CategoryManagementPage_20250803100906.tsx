...
import { ArrowLeft, Plus, Edit, Trash2, X, Search, Filter } from 'lucide-react';
import { 
...
const CategoryManagementPage: React.FC = () => {
  ...
  const handleUpdateCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const categoryData: InventoryCategory = {
        name: formData.name.trim(),
        description: formData.description.trim(),
        color: formData.color
      };
      
      const updatedCategory = await updateInventoryCategory(editingCategory.id, categoryData);
  ...
  const handleRestoreCategory = async (category: InventoryCategory) => {
    try {
      setError('');
      
      // The restoreInventoryCategory function is not defined in the new imports,
      // so this part of the logic needs to be re-evaluated or removed if not available.
      // For now, we'll just remove the restore functionality as it's not in the new imports.
      // If restore functionality is needed, it must be added to the inventoryApi.
      // setCategories(prev => prev.map(cat => 
      //   cat.id === category.id 
      ...
      // setSuccess('Category restored successfully'); // This line is removed as per the new imports
    } catch (error: any) {
