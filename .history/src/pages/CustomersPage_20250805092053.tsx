...
const CustomersPage = () => {
  ...
  const stats = useMemo(() => {
    ...
    // Calculate breakdown of revenue sources
    const deviceRevenue = customers.reduce((sum, customer) => {
      const devicePayments = customer.payments?.filter(p => p.source === 'device_payment' && p.status === 'completed') || [];
      return sum + devicePayments.reduce((customerSum, p) => customerSum + (p.amount || 0), 0);
    }, 0);
    
    const posRevenue = customers.reduce((sum, customer) => {
      const posSales = customer.payments?.filter(p => p.source === 'pos_sale' && p.status === 'completed') || [];
      return sum + posSales.reduce((customerSum, p) => customerSum + (p.amount || 0), 0);
    }, 0);
                            ...
                            return (
                              <div className="text-xs text-gray-500 mt-1">
                                ...
                                <span className="inline-flex items-center gap-1">
                                  ...
                                  <span>Sales: {formatCurrency(posTotal)}</span>
                                </span>
                              </div>
                            );
                          }
          ...
          {filteredCustomers.map(customer => {
            ...
            return (
              ...
              >
                ...
                {(() => {
                  const customer = customers.find(c => c.id === customer.id);
