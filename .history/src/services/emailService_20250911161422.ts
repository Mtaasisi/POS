...
class EmailService {
  ...
  private async getTargetCustomers(audience: string): Promise<any[]> {
    let query = supabase.from('customers').select('id, name, email, phone, gender, city, birth_month, birth_day, total_returns, profile_image, created_at, updated_at, whatsapp, notes, is_active, loyalty_level, color_tag, referred_by, total_spent, points, last_visit, created_by, referral_source, initial_notes, referrals, customer_tag');
    ...
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          ...
          console.log('📧 email_templates table not found - run the SQL in Supabase dashboard');
          return [];
        }
        throw error;
      }
  ...
  async getCampaigns(): Promise<EmailCampaign[]> {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          // Table doesn't exist yet, this is expected
          console.log('📧 email_campaigns table not found - run the SQL in Supabase dashboard');
          return [];
        }
        throw error;
      }
