...
export class SMSService {
  private API_BASE_URL = 'https://mshastra.com/sendurl.aspx';
  private USER = 'Inauzwa';
  private PASSWORD = '@Masika10';
  private SENDER_ID = 'INAUZWA';
  ...
  updateConfig(config: {
    ...
    if (config.username) this.USER = config.username;
    if (config.password) this.PASSWORD = config.password;
    if (config.sender_id) this.SENDER_ID = config.sender_id;
    if (config.api_url) this.API_BASE_URL = config.api_url;
    
    console.log('SMS Service configuration updated:', {
      username: this.USER,
