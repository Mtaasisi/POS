...
const DiagnosticChecklist: React.FC<DiagnosticChecklistProps> = ({
  ...
  const handleConvertToRepair = async () => {
    ...
    try {
      ...
      // Convert diagnostic device to repair device
      const repairData = {
        diagnostic_checklist: {
          items: diagnosticItems,
          notes: notes,
        ...
        status: 'assigned', // Start repair workflow
      ...
      console.log('[DiagnosticChecklist] Saving repair conversion data:', repairData);

      // Update device to start repair workflow
      const { data, error } = await supabase
        .from('devices')
        .update(repairData)
        .eq('id', device.id)
      ...
      if (error) {
        console.error('[DiagnosticChecklist] ❌ Repair conversion failed:', error);
        console.error('[DiagnosticChecklist] Error details:', {
          message: error.message,
          details: error.details,
          hint: error.hint,
      ...
      console.log('[DiagnosticChecklist] ✅ Repair conversion successful:', data);

      toast.success('Device converted to repair workflow! You can now start the repair process.');
