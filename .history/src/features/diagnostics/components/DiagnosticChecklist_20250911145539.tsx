...
import { getDiagnosticTemplateForDevice } from '../../../lib/diagnosticsApi';
...
const DiagnosticChecklist: React.FC<DiagnosticChecklistProps> = ({
    ...
    const initializeDiagnosticChecklist = async () => {
      if (isOpen && device) {
        ...
        if (device.diagnostic_checklist && device.diagnostic_checklist.items) {
          console.log('[DiagnosticChecklist] Loading existing diagnostic checklist:', device.diagnostic_checklist);
          ...
          // Fetch diagnostic template from database
          ...
          try {
            ...
            if (template && template.checklist_items) {
              ...
              const newDiagnostics = convertTemplateToDiagnosticItems(template.checklist_items);
              console.log('[DiagnosticChecklist] Converted template to diagnostic items:', newDiagnostics);
              ...
              console.log('[DiagnosticChecklist] Generated fallback diagnostic items:', newDiagnostics);
              setDiagnosticItems(newDiagnostics);
            }
          } catch (error) {
  ...
  const updateDiagnosticStatus = (itemId: string, status: 'pass' | 'fail') => {
    ...
    setDiagnosticItems(prev => {
      ...
      console.log('[DiagnosticChecklist] Updated diagnostic items:', updated);
      return updated;
    });
