...
const DeviceRepairDetailModal: React.FC<DeviceRepairDetailModalProps> = ({
  ...
  const loadDeviceData = async () => {
    setLoading(true);
    try {
      console.log('[DeviceRepairDetailModal] Loading device data for:', deviceId);
      const deviceData = await getDeviceById(deviceId);
      if (deviceData) {
        console.log('[DeviceRepairDetailModal] Device data loaded:', deviceData);
        setDevice(deviceData);
        await loadUserNames();
        await loadRepairPayments(deviceData.id);
      } else {
        console.error('[DeviceRepairDetailModal] No device data found for:', deviceId);
  ...
  const handleStatusUpdate = async (deviceIdOrStatus: string | DeviceStatus, newStatus?: DeviceStatus, notes?: string) => {
    ...
    console.log('[DeviceRepairDetailModal] handleStatusUpdate called with:', { deviceIdOrStatus, newStatus, notes });
    ...
    if (typeof deviceIdOrStatus === 'string' && newStatus) {
      ...
      status = newStatus;
      signature = notes || '';
      console.log('[DeviceRepairDetailModal] Using RepairStatusGrid signature:', { deviceId, status, signature });
    } else {
      // Called from other components: (status, fingerprint)
      deviceId = device.id;
      status = deviceIdOrStatus as DeviceStatus;
      signature = newStatus || '';
      console.log('[DeviceRepairDetailModal] Using standard signature:', { deviceId, status, signature });
    }
    ...
    try {
      ...
      // Force refresh device data from database
      console.log('[DeviceRepairDetailModal] Forcing device data refresh...');
      await loadDeviceData(); // Refresh data
      
      // Also force a small delay to ensure database is updated
      setTimeout(async () => {
        console.log('[DeviceRepairDetailModal] Secondary refresh after delay...');
