...
import { 
  ...
  Shield, Search, AlertCircle, ExternalLink, TestTube, UserCheck
} from 'lucide-react';
...
const DeviceRepairDetailModal: React.FC<DeviceRepairDetailModalProps> = ({
  ...
  // Smart button logic - determines which actions to show based on device status and context
  const getAvailableActions = () => {
    ...
    const isAssignedTechnician = device.assignedTo === currentUser.id;
    const canManageDevice = userRole === 'admin' || userRole === 'customer-care' || isAssignedTechnician;

    if (!canManageDevice) return [];

    // Get spare parts status
    const hasRequestedParts = repairParts.length > 0;
    const hasNeededParts = repairParts.some(part => part.status === 'needed');
    const hasOrderedParts = repairParts.some(part => part.status === 'ordered');
    const hasReceivedParts = repairParts.some(part => part.status === 'received');
    const allPartsReceived = hasRequestedParts && repairParts.every(part => part.status === 'received' || part.status === 'used');

    // Get payment status
    const needsPayment = device.status === 'repair-complete' && financialInfo && deviceCosts && 
      (financialInfo.totalPending > 0 || (deviceCosts.repairCost > 0 && financialInfo.totalPaid < deviceCosts.repairCost));
    ...
    switch (device.status) {
      case 'assigned':
        actions.push({
          ...
          priority: 'primary',
          action: () => handleStatusUpdate(device.id, 'diagnosis-started')
        });
        break;
      ...
      case 'diagnosis-started':
        actions.push({
          ...
          priority: 'primary',
          action: () => setShowSparePartsSelector(true),
          badge: hasRequestedParts ? `${repairParts.length} requested` : null
        });
        
        if (hasRequestedParts) {
          actions.push({
            id: 'awaiting-parts',
            ...
            priority: 'secondary',
            action: () => handleStatusUpdate(device.id, 'awaiting-parts', 'Parts requested, waiting for delivery')
          });
        }
        ...
        actions.push({
          ...
          priority: hasRequestedParts ? 'secondary' : 'primary',
          action: () => handleStatusUpdate(device.id, 'in-repair')
        });
        break;

      case 'awaiting-parts':
        if (hasReceivedParts || allPartsReceived) {
          actions.push({
            id: 'parts-arrived',
            label: 'Parts Arrived',
            ...
            priority: 'primary',
            action: () => handleStatusUpdate(device.id, 'parts-arrived', 'All requested parts have arrived')
          });
        }
        
        actions.push({
          id: 'request-more-parts',
          label: 'Request More Parts',
          icon: <Package className="w-4 h-4" />,
          color: 'bg-orange-600 hover:bg-orange-700',
          priority: 'secondary',
          action: () => setShowSparePartsSelector(true),
          badge: hasRequestedParts ? `${repairParts.length} total` : null
        });
        break;
      ...
      case 'in-repair':
        actions.push({
          ...
          label: 'Reassembled & Testing',
          icon: <TestTube className="w-4 h-4" />,
          color: 'bg-blue-600 hover:bg-blue-700',
          priority: 'primary',
          action: () => handleStatusUpdate(device.id, 'reassembled-testing')
        });
        break;
      ...
      case 'reassembled-testing':
        actions.push({
          ...
          color: 'bg-green-600 hover:bg-green-700',
          priority: 'primary',
          action: () => handleStatusUpdate(device.id, 'repair-complete')
        });
        break;

      case 'repair-complete':
        if (needsPayment && (userRole === 'admin' || userRole === 'customer-care')) {
          actions.push({
            id: 'process-payment',
        ...
        actions.push({
          ...
          label: 'Return to Customer Care',
          icon: <UserCheck className="w-4 h-4" />,
          color: 'bg-blue-600 hover:bg-blue-700',
          priority: needsPayment ? 'secondary' : 'primary',
          action: () => handleStatusUpdate(device.id, 'returned-to-customer-care')
        });
        break;
      ...
      case 'returned-to-customer-care':
        if (userRole === 'admin' || userRole === 'customer-care') {
          actions.push({
            ...
            color: 'bg-green-600 hover:bg-green-700',
            priority: 'primary',
            action: () => handleStatusUpdate(device.id, 'done')
          });
        }
        break;
    }

    // Always available actions (context-dependent)
    if (userRole === 'customer-care') {
      actions.push({
        ...
        priority: 'tertiary',
        action: () => toast.success('SMS functionality coming soon')
      });
    }
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-4">
                    ...
                    <div className="space-y-2">
                      {getAvailableActions().map((action, index) => (
                        <div key={action.id} className="relative">
                          <button
                            ...
                            className={`w-full flex items-center justify-center gap-2 px-4 py-3 text-white rounded-lg transition-colors font-medium text-sm ${action.color} ${
                              action.priority === 'primary' ? 'shadow-md hover:shadow-lg' : 
                              action.priority === 'secondary' ? 'opacity-90 hover:opacity-100' : 
                      ...
                      {/* Repair Checklist - Always available for technicians */}
                      {(isAssignedTechnician || currentUser?.role === 'admin' || currentUser?.role === 'customer-care') && (
                        <button
                          ...
                          className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors font-medium text-sm opacity-75 hover:opacity-100"
                        >
