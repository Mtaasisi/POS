...
const DiagnosticChecklistModal: React.FC<DiagnosticChecklistModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      console.log('🔍 DiagnosticChecklistModal opened, loading templates...');
  ...
  const loadProblemTemplates = async () => {
    try {
      setLoading(true);
      console.log('📋 Loading problem templates from database...');
      ...
      if (error) {
        console.warn('⚠️ Database error, using fallback templates:', error);
        // Use fallback templates if database fails
        const fallbackTemplates = [
          {
            id: 'audio-problems',
            problem_name: 'Audio Problems',
            problem_description: 'Speaker, microphone, or audio output issues',
            checklist_items: [
              { id: 'audio-1', title: 'Test Speaker Output', description: 'Check if speaker produces sound', required: true },
              { id: 'audio-2', title: 'Test Microphone', description: 'Check if microphone records audio', required: true },
              { id: 'audio-3', title: 'Test Headphone Jack', description: 'Check if headphone jack works', required: false },
              { id: 'audio-4', title: 'Test Bluetooth Audio', description: 'Check if Bluetooth audio works', required: false },
              { id: 'audio-5', title: 'Test Volume Controls', description: 'Check if volume buttons work', required: true },
              { id: 'audio-6', title: 'Test Audio Quality', description: 'Check for distortion or crackling', required: true }
            ]
          },
          {
            id: 'camera-issues',
            problem_name: 'Camera Issues',
            problem_description: 'Front or rear camera problems',
            checklist_items: [
              { id: 'camera-1', title: 'Test Rear Camera', description: 'Check if rear camera takes photos', required: true },
              { id: 'camera-2', title: 'Test Front Camera', description: 'Check if front camera works', required: true },
              { id: 'camera-3', title: 'Test Camera Focus', description: 'Check if camera focuses properly', required: true },
              { id: 'camera-4', title: 'Test Flash', description: 'Check if camera flash works', required: false },
              { id: 'camera-5', title: 'Test Video Recording', description: 'Check if video recording works', required: true },
              { id: 'camera-6', title: 'Test Camera Quality', description: 'Check for blurry or distorted images', required: true }
            ]
          },
          {
            id: 'network-issues',
            problem_name: 'Network Issues',
            problem_description: 'WiFi, cellular, or connectivity problems',
            checklist_items: [
              { id: 'network-1', title: 'Test WiFi Connection', description: 'Check if WiFi connects and works', required: true },
              { id: 'network-2', title: 'Test Cellular Data', description: 'Check if cellular data works', required: true },
              { id: 'network-3', title: 'Test Bluetooth', description: 'Check if Bluetooth connects', required: false },
              { id: 'network-4', title: 'Test GPS', description: 'Check if GPS location works', required: false },
              { id: 'network-5', title: 'Test Internet Speed', description: 'Check internet connection speed', required: true },
              { id: 'network-6', title: 'Test Network Stability', description: 'Check for connection drops', required: true }
            ]
          },
          {
            id: 'phone-no-power',
            problem_name: 'Phone No Power',
            problem_description: 'Device does not turn on or charge',
            checklist_items: [
              { id: 'power-1', title: 'Test Power Button', description: 'Check if power button responds', required: true },
              { id: 'power-2', title: 'Test Charging Port', description: 'Check if charging port works', required: true },
              { id: 'power-3', title: 'Test Battery', description: 'Check battery health and charging', required: true },
              { id: 'power-4', title: 'Test Charging Cable', description: 'Check if charging cable works', required: true },
              { id: 'power-5', title: 'Test Power Adapter', description: 'Check if power adapter works', required: true },
              { id: 'power-6', title: 'Test Boot Process', description: 'Check if device boots properly', required: true },
              { id: 'power-7', title: 'Test Power Indicators', description: 'Check if power indicators work', required: false }
            ]
          },
          {
            id: 'problem-name',
            ...
            checklist_items: [
              { id: 'custom-1', title: 'Custom Diagnostic', description: 'Perform custom diagnostic test', required: true }
            ]
          },
          {
            id: 'screen-issues',
            problem_name: 'Screen Issues',
            problem_description: 'Display problems, touch issues, or screen damage',
            checklist_items: [
              { id: 'screen-1', title: 'Test Display', description: 'Check if screen displays properly', required: true },
              { id: 'screen-2', title: 'Test Touch Response', description: 'Check if touch screen responds', required: true },
              { id: 'screen-3', title: 'Test Brightness', description: 'Check if brightness controls work', required: true },
              { id: 'screen-4', title: 'Test Screen Damage', description: 'Check for cracks or damage', required: true },
              { id: 'screen-5', title: 'Test Screen Calibration', description: 'Check if screen is properly calibrated', required: false },
              { id: 'screen-6', title: 'Test Screen Colors', description: 'Check for color distortion or dead pixels', required: true }
            ]
          }
        ];
        setProblemTemplates(fallbackTemplates);
        return;
      }
      
      console.log('✅ Problem templates loaded:', data);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div 
        ...
        style={{
          ...
          boxShadow: 'rgba(0, 0, 0, 0.18) 0px 12px 48px 0px',
          width: '100%',
          maxWidth: '780px',
          maxHeight: '90vh',
          borderRadius: '15px',
