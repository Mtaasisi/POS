...
const DeviceRepairDetailModal: React.FC<DeviceRepairDetailModalProps> = ({
  ...
  const handleDiagnosisComplete = async (deviceId: string, status: DeviceStatus, notes?: string) => {
    try {
      // Update device status to diagnosis-started after diagnosis completion
      const success = await updateDeviceStatus(deviceId, status, notes);
      
      if (success) {
        toast.success('Diagnosis completed and status updated successfully!');
        setShowDiagnosisModal(false);
        // Refresh diagnostic data to show the new results
        await loadDiagnosticData(deviceId);
      } else {
        toast.error('Failed to update device status after diagnosis completion');
      }
    } catch (error) {
      console.error('Error updating status after diagnosis completion:', error);
      toast.error('Failed to update device status after diagnosis completion');
    }
  };
  ...
  }, [showDiagnosisModal, device?.id, device?.status]);
  ...
  const handleStatusUpdate = async (deviceId: string, newStatus: DeviceStatus, notes?: string) => {
    ...
    try {
      ...
      if (newStatus === 'diagnosis-started') {
        ...
        return; // Don't update status yet, wait for diagnosis completion
      }
    ...
    <div className="fixed inset-0 flex items-center justify-center p-2 sm:p-4 overflow-y-auto" style={{ zIndex: 99999 }}>
      ...
      {device && (
        <DiagnosisModal
          ...
          onStatusUpdate={handleDiagnosisComplete}
        />
      )}
