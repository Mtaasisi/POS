                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div className="text-center p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                            <div className="text-lg font-bold text-yellow-600">
                              {formatCurrency(financialInfo?.totalPending || 0)}
                            </div>
                            <div className="text-xs text-yellow-700 uppercase tracking-wide">Pending</div>
                          </div>
                        </div>

                        {/* Form-Based Costs - Only show if they exist */}
                        {deviceCosts && (deviceCosts.repairCost > 0 || deviceCosts.depositAmount > 0) && (
                          <div className="border-t border-gray-100 pt-3">
                            <div className="text-xs text-gray-500 uppercase tracking-wide mb-2">Form Costs</div>
                            <div className="grid grid-cols-2 gap-2">
                              {deviceCosts.repairCost > 0 && (
                                <div className="text-center p-2 bg-blue-50 rounded border border-blue-200">
                                  <div className="text-sm font-bold text-blue-600">{formatCurrency(deviceCosts.repairCost)}</div>
                                  <div className="text-xs text-blue-700">Repair Cost</div>
                                </div>
                              )}
                              {deviceCosts.depositAmount > 0 && (
                                <div className="text-center p-2 bg-green-50 rounded border border-green-200">
                                  <div className="text-sm font-bold text-green-600">{formatCurrency(deviceCosts.depositAmount)}</div>
                                  <div className="text-xs text-green-700">Deposit</div>
                                </div>
                              )}
                            </div>
                          </div>
