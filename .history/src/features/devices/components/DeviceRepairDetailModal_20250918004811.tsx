...
const DeviceRepairDetailModal: React.FC<DeviceRepairDetailModalProps> = ({
  ...
  const processPayment = async (paymentData: any[], totalPaid?: number) => {
    ...
    try {
      ...
      if (selectedPendingPayment.status === 'completed') {
        console.log('⚠️ Payment is already completed, skipping update');
      ...
      const results = await Promise.all(
        paymentData.map(async (payment) => {
          ...
          if (payment.paymentMethod && payment.paymentMethod.trim()) {
            ...
            const mappedMethod = methodMapping[payment.paymentMethod.trim()] || 'cash';
            updateData.method = mappedMethod;
          }
