    ...
    try {
      ...
      const results = await Promise.all(
        paymentData.map(async (payment) => {
          ...
          // Only add foreign key fields if they are valid
          if (payment.paymentMethodId && payment.paymentMethodId !== 'null' && payment.paymentMethodId !== 'undefined') {
            updateData.payment_method_id = payment.paymentMethodId;
          }
          
          if (payment.paymentAccountId && payment.paymentAccountId !== 'null' && payment.paymentAccountId !== 'undefined') {
            updateData.payment_account_id = payment.paymentAccountId;
          }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          {activeTab === 'payments' && device && (
            <div className="space-y-6">
              ...
              {/* Pending Payments */}
              {financialInfo && financialInfo.payments && financialInfo.payments.filter(p => p.status === 'pending').length > 0 && (
                <div className="bg-white border border-gray-200 rounded-xl p-6">
                  <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="space-y-3">
                    {financialInfo.payments.filter(p => p.status === 'pending').map((payment) => (
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6">
                ...
                ) : financialInfo && financialInfo.payments && financialInfo.payments.length > 0 ? (
                  <div className="space-y-3">
                    {financialInfo.payments.map((payment) => (
                      <div key={payment.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-600">
                              {payment.method} • {new Date(payment.payment_date || payment.created_at).toLocaleDateString()}
                            </div>
                          </div>
                        </div>
                        <div className="text-right">
                          ...
                          <div className="text-xs text-gray-500">
                            {payment.payment_account_name || payment.payment_account_id || 'Unknown Account'}
                          </div>
                        </div>
                      </div>
                    ))}
      ...
      {showPaymentModal && selectedPendingPayment && (
        <PaymentsPopupModal
          ...
          description={`Process pending ${selectedPendingPayment.payment_type} - ${device?.deviceName}`}
          onPaymentComplete={processPayment}
          title="Process Pending Payment"
        />
      )}
