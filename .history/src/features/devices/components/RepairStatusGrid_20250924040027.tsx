...
const RepairStatusGrid: React.FC<RepairStatusGridProps> = ({
  ...
  const getAvailableActions = () => {
    ...
    if (['diagnosis-started', 'awaiting-parts', 'parts-arrived', 'in-repair', 'reassembled-testing'].includes(device.status)) {
      // TEMPORARILY REMOVED ROLE RESTRICTION FOR TESTING
      if (true) { // isAssignedTechnician || isAdmin
        actions.push({
          id: 'request-parts',
          title: 'Request Parts',
          ...
          color: 'bg-orange-500',
          action: async () => {
            // Open the spare parts selector modal
            console.log('Request Parts button clicked, opening modal...');
