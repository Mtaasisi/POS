...
import { 
  ...
  Plus, Edit, Trash2, ShoppingCart, Truck, User, Activity, Info, CreditCard
} from 'lucide-react';
...
import { RepairPart, canStartRepair } from '../../repair/services/repairPartsApi';
...
const RepairStatusGrid: React.FC<RepairStatusGridProps> = ({
  ...
  // Track device status changes for debugging
  useEffect(() => {
    // Only log when status actually changes, not on every render
    console.log('[RepairStatusGrid] Device status changed to:', device.status);
  }, [device.status]);
  ...
  const loadRepairParts = async () => {
    ...
    try {
      const { getRepairParts } = await import('../../repair/services/repairPartsApi');
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'needed': return <AlertTriangle className="w-4 h-4" />;
  ...
  const getAvailableActions = async () => {
    ...
    if (device.status === 'diagnosis-started') {
      ...
      const repairCheck = await canStartRepair(device.id);
      
      if (repairCheck.canStart) {
        actions.push({
          id: 'start-repair',
          title: 'Start Repair',
          description: repairCheck.message,
      ...
      } else if (repairCheck.reason === 'no_parts') {
        actions.push({
          id: 'add-parts',
          title: 'Add Spare Parts',
          description: 'Add spare parts before starting repair',
      ...
      } else if (repairCheck.reason === 'parts_needed' || repairCheck.reason === 'parts_ordered' || repairCheck.reason === 'parts_accepted') {
        actions.push({
          id: 'awaiting-parts',
    ...
    if (device.status === 'awaiting-parts') {
      ...
      const allPartsReady = parts.length > 0 && parts.every(part => 
    ...
    if (device.status === 'repair-complete') {
      // Only admin and customer-care can process payments
      if (currentUser.role === 'admin' || currentUser.role === 'customer-care') {
        actions.push({
          ...
          color: 'bg-blue-500',
          action: async () => {
            setUpdatingAction('process-payments');
            try {
              await onStatusUpdate(device.id, 'process-payments');
            } finally {
              setUpdatingAction(null);
            }
          }
    ...
    if (device.status === 'process-payments') {
      // Only admin and customer-care can give device to customer
      if (currentUser.role === 'admin' || currentUser.role === 'customer-care') {
        actions.push({
  ...
  // Load available actions when device status or parts change
  useEffect(() => {
