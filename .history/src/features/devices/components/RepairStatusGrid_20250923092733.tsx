...
import { 
  ...
  Plus, Edit, Trash2, ShoppingCart, Truck, User, Activity, Info, CheckSquare, UserCheck, FileText
} from 'lucide-react';
...
const RepairStatusGrid: React.FC<RepairStatusGridProps> = ({
  ...
  const handleTemplateSelected = (template: any) => {
    // Start diagnostic with selected template
    setShowDiagnosticTemplateManager(false);
    setShowDiagnosisModal(true);
    toast.success(`Starting diagnostic with template: ${template.problem_name}`);
  };
  ...
  const getAvailableActions = () => {
    ...
    if (device.status === 'assigned') {
      actions.push({
        id: 'choose-diagnostic-template',
        title: 'Choose Diagnostic Template',
        description: 'Select a diagnostic template to guide your repair process',
        icon: <FileText className="w-6 h-6" />,
        color: 'bg-blue-500',
        action: async () => {
          setShowDiagnosticTemplateManager(true);
        }
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <DiagnosticTemplateManager
        device={device}
        isOpen={showDiagnosticTemplateManager}
        onClose={() => setShowDiagnosticTemplateManager(false)}
