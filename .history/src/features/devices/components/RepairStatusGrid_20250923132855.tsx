...
import { 
  ...
  Plus, Edit, Trash2, ShoppingCart, Truck, User, Activity, Info, CheckSquare, UserCheck, CreditCard
} from 'lucide-react';
...
const RepairStatusGrid: React.FC<RepairStatusGridProps> = ({
  ...
  const getAvailableActions = () => {
    ...
    if (device.status === 'repair-complete') {
      // Add payment processing step before customer care
      actions.push({
        id: 'process-payments',
        ...
        action: async () => {
          ...
          try {
            // Get payment info to calculate amount due
            const paymentInfo = await getDevicePaymentInfo(device.id);
            const totalCost = paymentInfo.repairCost + paymentInfo.sparePartsCost;
            const amountDue = totalCost - paymentInfo.totalPaid;
