...
const DeviceRepairDetailModal: React.FC<DeviceRepairDetailModalProps> = ({
  ...
  // Handle diagnostic completion (using DiagnosisModal)
  const handleDiagnosticComplete = async (deviceId: string, status: DeviceStatus, notes?: string) => {
    try {
      // Update device status to diagnosis-started after diagnostic completion
      const success = await updateDeviceStatus(deviceId, 'diagnosis-started', notes || 'Diagnostic completed');
      
      if (success) {
        toast.success('Diagnostic completed and status updated successfully!');
  ...
  const handleStatusUpdate = async (deviceId: string, newStatus: DeviceStatus, notes?: string) => {
    ...
    try {
      ...
      if (newStatus === 'diagnosis-started') {
        ...
        return; // Don't update status yet, wait for diagnostic completion
      }
  ...
  return createPortal(
    <div className="fixed inset-0 flex items-center justify-center p-2 sm:p-4 overflow-y-auto" style={{ zIndex: 99999 }}>
      ...
      {/* Diagnostic Modal - Comprehensive Diagnostic Workflow */}
      {device && (
        <DiagnosisModal
          device={device}
          ...
          onStatusUpdate={handleDiagnosticComplete}
        />
      )}
