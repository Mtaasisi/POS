...
// import DiagnosticChecklistModal from './DiagnosticChecklistModal'; // REMOVED: handled by RepairStatusGrid
...
const DeviceRepairDetailModal: React.FC<DeviceRepairDetailModalProps> = ({
  ...
  // Diagnostic checklist state - Modal handled by RepairStatusGrid
  // const [showDiagnosticChecklist, setShowDiagnosticChecklist] = useState(false); // REMOVED: handled by RepairStatusGrid
  ...
  */
  ...
  // Handle diagnostic checklist completion - REMOVED: handled by RepairStatusGrid
  ...
  // Track diagnostic checklist modal state changes - REMOVED: handled by RepairStatusGrid
  // useEffect(() => {
  ...
  const handleStatusUpdate = async (deviceId: string, newStatus: DeviceStatus, notes?: string) => {
    ...
    try {
      ...
      // Special handling for starting diagnostics - REMOVED: handled by RepairStatusGrid
      ...
      if (newStatus === 'repair-complete' && device.customerId) {
        try {
          ...
          toast.error('Failed to create pending payments - please check payment setup');
    ...
    <div className="fixed inset-0 flex items-center justify-center p-2 sm:p-4 overflow-y-auto" style={{ zIndex: 99999 }}>
      ...
      >
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-3 sm:p-4 space-y-3">
                    ...
                    {repairPartsLoading ? (
                      ...
                      <div className="space-y-2">
                        {/* Customer Care Actions - Only show reject button for customer care users */}
                            ...
                            return (
                              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 space-y-3">
                                <div className="flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-2">
                                    {/* Only show reject button for customer care - accept button removed */}
                                    <button
                    ...
                    <div className="text-center py-6">
                      ...
                      <div className="space-y-2">
                        <button
                          ...
                          disabled
                        >
                          Start Diagnostics → (Use Repair Status Section)
                        </button>
                        <button
                          onClick={() => {
                            if (device?.id) {
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100 bg-gray-50 px-4 py-3">
          <div className="flex items-center gap-2">
            {/* Only show SMS button if diagnostics have started */}
      ...
      {/* Diagnostic Checklist Modal - REMOVED: handled by RepairStatusGrid */}
      {/* {device && (
      ...
      <div className="xl:hidden bg-white border-t border-gray-200 p-2 sm:p-3">
        <div className="flex gap-1.5">
          ...
          {currentUser?.role === 'customer-care' && customer && device.status !== 'assigned' && (
            <button
              type="button"
              onClick={(e) => {
                e.preventDefault();
