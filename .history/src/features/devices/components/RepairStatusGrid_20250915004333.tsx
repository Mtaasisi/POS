...
import { 
  ...
  Plus, Edit, Trash2, ShoppingCart, Truck, User
} from 'lucide-react';
...
const RepairStatusGrid: React.FC<RepairStatusGridProps> = ({
  ...
  // Add debugging and force re-render when device status changes
  useEffect(() => {
  ...
  const [parts, setParts] = useState<RepairPart[]>([
    // Mock data - in real app, this would come from the database
    {
      ...
      description: 'LCD Screen with digitizer for iPhone 12',
      quantity: 1,
      cost: 45000,
      status: 'ordered',
      supplier: 'TechParts Ltd',
      estimatedArrival: '2024-01-15',
      notes: 'Original quality required'
    },
    {
      id: '2',
      name: 'Battery',
      description: 'Li-ion battery 2815mAh',
      quantity: 1,
      cost: 25000,
      status: 'shipped',
      supplier: 'BatteryPro',
  ...
  const handleStartRepair = async () => {
    ...
    try {
      ...
      toast.success('Repair status updated to In Repair');
    } catch (error) {
  ...
  console.log('[RepairStatusGrid] Rendering with device status:', device.status);
  
  if (device.status === 'awaiting-parts') {
    console.log('[RepairStatusGrid] Rendering awaiting-parts view');
    return (
  ...
  const getAvailableActions = () => {
    console.log('[RepairStatusGrid] getAvailableActions called for status:', device.status);
    const actions = [];
    ...
    if (device.status === 'assigned') {
      actions.push({
        ...
        color: 'bg-yellow-500',
        action: () => onStatusUpdate(device.id, 'diagnosis-started')
      });
    }
      ...
      actions.push(
        {
          ...
          color: 'bg-orange-500',
          action: () => onStatusUpdate(device.id, 'awaiting-parts')
        },
        {
          id: 'start-repair',
          title: 'Start Repair',
          ...
          color: 'bg-purple-500',
          action: () => onStatusUpdate(device.id, 'in-repair')
        }
      );
    }
    ...
    if (device.status === 'in-repair') {
      actions.push({
        ...
        color: 'bg-indigo-500',
        action: () => onStatusUpdate(device.id, 'reassembled-testing')
      });
    }
    ...
    if (device.status === 'reassembled-testing') {
      actions.push({
        ...
        icon: <CheckCircle className="w-6 h-6" />,
        color: 'bg-green-500',
        action: () => onStatusUpdate(device.id, 'repair-complete')
      });
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {availableActions.map((action) => (
          <GlassCard key={action.id} className="p-6 hover:shadow-lg transition-all duration-300">
      ...
      <div className="bg-gray-50 rounded-xl p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-gray-700 capitalize">{device.status.replace('-', ' ')}</span>
        </div>
        {device.remarks && (
