...
const RepairStatusGrid: React.FC<RepairStatusGridProps> = ({
  ...
  useEffect(() => {
    console.log('[RepairStatusGrid] Diagnosis modal state changed:', showDiagnosisModal);
  }, [showDiagnosisModal]);
  ...
  const getAvailableActions = () => {
    ...
    if (device.status === 'assigned') {
      actions.push({
        ...
        color: 'bg-yellow-500',
        action: async () => {
          console.log('[RepairStatusGrid] Opening diagnosis modal for device:', device.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="mt-4 p-4 bg-red-100 border border-red-300 rounded-lg">
        <p className="text-sm text-red-700 mb-2">Debug: Test Diagnosis Modal</p>
        <button
          onClick={() => {
            console.log('[RepairStatusGrid] Test button clicked');
            setShowDiagnosisModal(true);
          }}
          className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700"
        >
          Test Open Diagnosis Modal
        </button>
        <p className="text-xs text-red-600 mt-1">Modal state: {showDiagnosisModal ? 'OPEN' : 'CLOSED'}</p>
      </div>
