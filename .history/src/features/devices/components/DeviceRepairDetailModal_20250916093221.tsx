  ...
  const loadRepairParts = async (deviceId: string) => {
    ...
    try {
      ...
      if (response.ok && response.data) {
        console.log('🔍 [DEBUG] Raw repair parts data:', response.data);
        
        // Debug each part to see what's missing
        response.data.forEach((part, index) => {
          console.log(`🔍 [DEBUG] Part ${index + 1}:`, {
            id: part.id,
            spare_part_id: part.spare_part_id,
            spare_part: part.spare_part,
            hasSparePartData: !!part.spare_part,
            sparePartName: part.spare_part?.name,
            sparePartNumber: part.spare_part?.part_number
          });
        });
