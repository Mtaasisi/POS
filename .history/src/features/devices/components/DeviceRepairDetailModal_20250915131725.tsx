...
const DeviceRepairDetailModal: React.FC<DeviceRepairDetailModalProps> = ({
  ...
  const loadDeviceData = async (forceRefresh = false) => {
    ...
    try {
      ...
      // If forceRefresh is true, always fetch from database to get latest data
      // This is important after status updates to avoid stale context data
      if (forceRefresh) {
        console.log('[DeviceRepairDetailModal] Force refreshing device data from database...');
      } else {
      ...
      // If context method fails or forceRefresh is true, fetch directly from database
      if (!deviceData || forceRefresh) {
  ...
  const handleStatusUpdate = async (deviceIdOrStatus: string | DeviceStatus, newStatus?: DeviceStatus, notes?: string) => {
    ...
    try {
      ...
      // Force refresh to get latest data from database after status update
      // This ensures RepairStatusUpdater receives the updated status
                  ...
                  {device.status === 'repair-complete' && customer && (
                    <RepairPaymentButton
                      ...
                      onPaymentComplete={(paymentData) => {
                        ...
                        loadDeviceData(true); // Force refresh to get latest data
