...
const DiagnosticChecklistModal: React.FC<DiagnosticChecklistModalProps> = ({
  ...
  const loadProblemTemplates = async () => {
    try {
      ...
      if (templates.length === 0) {
        toast.error('No diagnostic templates available. Please contact administrator.');
      }
  ...
  const saveChecklistResults = async () => {
    ...
    try {
      ...
      if (deviceError) {
        ...
        toast.warning('Diagnostic results saved, but device update failed');
      }
  ...
  const resetChecklist = () => {
    try {
      if (!checklistItems || checklistItems.length === 0) {
        toast.info('No checklist items to reset');
        return;
      }
  ...
  const goToNextItem = () => {
    if (!checklistItems || checklistItems.length === 0) {
      toast.info('No checklist items available');
      return;
    }
    ...
    if (currentItemIndex < checklistItems.length - 1) {
      ...
      toast.info('Already at the last item');
    }
  };
