  ...
  const loadDeviceData = async () => {
    ...
    try {
      ...
      if (deviceData) {
        ...
        await loadFinancialInfo(deviceData.id);
        await loadDeviceCosts(deviceData.id);
        await loadPendingPayments(deviceData.id);
      } else {
  ...
  const createPendingPaymentsFromForm = async (deviceId: string) => {
    try {
      ...
      if (deviceData.repair_cost && deviceData.repair_cost > 0) {
        const hasRepairCostPayment = existingPayments?.some(p => p.notes?.includes('Repair Cost'));
        if (!hasRepairCostPayment) {
          paymentsToCreate.push({
            ...
            payment_type: 'repair_quote',
            status: 'pending',
            payment_date: new Date().toISOString(),
            created_at: new Date().toISOString(),
            notes: 'Repair Cost - Pending Payment'
          });
        }
      }
      ...
      if (deviceData.deposit_amount && deviceData.deposit_amount > 0) {
        const hasDepositPayment = existingPayments?.some(p => p.notes?.includes('Deposit Amount'));
        if (!hasDepositPayment) {
          paymentsToCreate.push({
            ...
            payment_date: new Date().toISOString(),
            created_at: new Date().toISOString(),
            notes: 'Deposit Amount - Pending Payment'
          });
        }
      }
                    ...
                    {financialLoading ? (
                      ...
                      <div className="space-y-4">
                        ...
                        {financialInfo && (
                          <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1">
                              ...
                              <p className="text-sm font-medium text-green-600">{formatCurrency(financialInfo.totalPaid)}</p>
                            </div>
                            <div className="space-y-1">
                              <span className="text-xs text-gray-500 uppercase tracking-wide">Pending</span>
                              <p className="text-sm font-medium text-yellow-600">{formatCurrency(financialInfo.totalPending)}</p>
                            </div>
                            <div className="space-y-1">
                              <span className="text-xs text-gray-500 uppercase tracking-wide">Failed</span>
                              <p className="text-sm font-medium text-red-600">{formatCurrency(financialInfo.totalFailed)}</p>
                            </div>
                            <div className="space-y-1">
                              <span className="text-xs text-gray-500 uppercase tracking-wide">Payments</span>
                              <p className="text-sm font-medium text-gray-900">{financialInfo.paymentCount}</p>
                            </div>
                          </div>
                        )}

                        {/* Form-Based Costs */}
                        {deviceCosts && (deviceCosts.repairCost > 0 || deviceCosts.depositAmount > 0) && (
                          <div className="border-t border-gray-100 pt-3">
                            <div className="space-y-2">
                              <span className="text-xs text-gray-500 uppercase tracking-wide">Form-Based Costs</span>
                              <div className="grid grid-cols-2 gap-3">
                                {deviceCosts.repairCost > 0 && (
                                  <div className="space-y-1">
                                    <span className="text-xs text-gray-500 uppercase tracking-wide">Repair Cost (Form)</span>
                                    <p className="text-sm font-medium text-blue-600">{formatCurrency(deviceCosts.repairCost)}</p>
                                  </div>
                                )}
                                {deviceCosts.depositAmount > 0 && (
                                  <div className="space-y-1">
                                    <span className="text-xs text-gray-500 uppercase tracking-wide">Deposit Amount (Form)</span>
                                    <p className="text-sm font-medium text-green-600">{formatCurrency(deviceCosts.depositAmount)}</p>
                                  </div>
                                )}
                              </div>
                            </div>
                        ...
                        {deviceCosts && (
                          <div className="border-t border-gray-100 pt-3">
                            <div className="space-y-2">
                              <span className="text-xs text-gray-500 uppercase tracking-wide">Additional Cost Information</span>
                              <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1">
                                  <span className="text-xs text-gray-500 uppercase tracking-wide">Repair Price (DB)</span>
                                  <p className="text-sm font-medium text-gray-900">{formatCurrency(deviceCosts.repairPrice)}</p>
                                </div>
                                <div className="space-y-1">
                                  <span className="text-xs text-gray-500 uppercase tracking-wide">Est. Hours</span>
                                  <p className="text-sm font-medium text-gray-900">{deviceCosts.estimatedHours}h</p>
                                </div>
                                {deviceCosts.hourlyRate > 0 && (
                                  <div className="space-y-1">
                                    <span className="text-xs text-gray-500 uppercase tracking-wide">Hourly Rate</span>
                                    <p className="text-sm font-medium text-gray-900">{formatCurrency(deviceCosts.hourlyRate)}/h</p>
                                  </div>
                                )}
                                <div className="space-y-1">
                                  <span className="text-xs text-gray-500 uppercase tracking-wide">Repair Count</span>
                                  <p className="text-sm font-medium text-gray-900">{deviceCosts.repairCount}</p>
                                </div>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="border-t border-gray-100 pt-3">
                          <div className="grid grid-cols-2 gap-3">
                            {device.repairCost && (
                              <div className="space-y-1">
                                <span className="text-xs text-gray-500 uppercase tracking-wide">Legacy Repair Cost</span>
                        ...
                        {pendingPayments.length > 0 && (
                          <div className="border-t border-gray-100 pt-3">
                            <div className="space-y-2">
                              <span className="text-xs text-gray-500 uppercase tracking-wide">Pending Payments</span>
                              {pendingPayments.map((payment) => (
                                <div key={payment.id} className="flex items-center justify-between p-2 bg-yellow-50 rounded-lg border border-yellow-200">
                                  <div className="flex-1">
                                    <div className="flex items-center gap-2">
                                      ...
                                      {payment.payment_type === 'repair_quote' && (
                        ...
                        {financialInfo?.lastPayment && (
                          <div className="border-t border-gray-100 pt-3">
                            <div className="space-y-1">
                              <span className="text-xs text-gray-500 uppercase tracking-wide">Last Payment</span>
                              <div className="flex items-center justify-between">
                                <p className="text-sm font-medium text-gray-900">
                                  {formatCurrency(financialInfo.lastPayment.amount)} ({financialInfo.lastPayment.method})
                                </p>
                                <span className={`text-xs px-2 py-1 rounded-full ${
                                  financialInfo.lastPayment.status === 'completed' ? 'bg-green-100 text-green-700' :
                                  financialInfo.lastPayment.status === 'pending' ? 'bg-yellow-100 text-yellow-700' :
                                  'bg-red-100 text-red-700'
                                }`}>
                                  {financialInfo.lastPayment.status}
                                </span>
                              </div>
                              <p className="text-xs text-gray-500">
                                {new Date(financialInfo.lastPayment.payment_date).toLocaleDateString()}
                              </p>
                            </div>
                          </div>
                        )}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="text-center p-4 bg-green-50 rounded-lg border border-green-200">
                        <div className="text-2xl font-bold text-green-600">
                          {financialLoading ? (
                            <div className="animate-pulse bg-green-200 h-8 w-24 rounded mx-auto"></div>
                          ) : (
                            formatCurrency(financialInfo?.totalPaid || 0)
                          )}
                        </div>
                ...
                {financialLoading ? (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="text-sm text-gray-600 mb-1">Repair Price</div>
              ...
              <div className="space-y-4">
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    {selectedPendingPayment.payment_type === 'repair_quote' && (
                      <span className="text-xs px-2 py-1 bg-blue-100 text-blue-700 rounded-full">
                        Repair Cost
                      </span>
                    )}
