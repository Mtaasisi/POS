...
const StatusUpdateForm: React.FC<StatusUpdateFormProps> = ({
  ...
  const getAvailableStatusTransitions = () => {
    ...
    switch (status) {
      ...
      case 'repair-complete':
        if (role === 'admin' || role === 'customer-care') {
          return [
            { status: 'process-payments' as DeviceStatus, label: 'Process Payments', icon: <CreditCard size={18} /> }
          ];
        }
        // Technicians cannot process payments - only admin/customer-care can
        return [];
  ...
  const handleStatusClick = async (status: DeviceStatus) => {
    if (status === 'awaiting-parts') {
      setShowPartsModal(true);
      return;
    }
    
    // Check if trying to start repair without parts
    if (status === 'in-repair') {
      const { canStartRepair } = await import('../../../repair/services/repairPartsApi');
      const repairCheck = await canStartRepair(device.id);
      
      if (!repairCheck.canStart) {
        // Show error message instead of allowing status change
        alert(repairCheck.message);
        return;
      }
    }
