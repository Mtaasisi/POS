    ...
    switch (status) {
        ...
        if (role === 'technician' && isAssignedTechnician) {
          return [
            { 
              ...
              description: 'Begin device diagnosis and testing'
            }
          ];
        }
        ...
        if (role === 'technician' && isAssignedTechnician) {
          return [
            { 
              ...
              description: 'Device needs parts before repair can continue'
            },
            { 
              ...
              description: 'Begin the repair process'
            },
            { 
              ...
              description: 'Device cannot be repaired'
            }
          ];
        }
        ...
        if (role === 'technician' && isAssignedTechnician) {
          return [
            { 
              ...
              icon: <Wrench size={18} />,
              color: 'bg-green-600 hover:bg-green-700',
              description: 'Parts have arrived, begin repair'
            },
            { 
              status: 'failed' as DeviceStatus, 
              label: 'Mark as Failed', 
        ...
        if (role === 'technician' && isAssignedTechnician) {
          return [
            { 
              ...
              label: 'Repair Done - Start Testing', 
              icon: <CheckCircle size={18} />,
              color: 'bg-blue-600 hover:bg-blue-700',
              description: 'Repair completed, begin testing'
            },
            { 
              status: 'failed' as DeviceStatus, 
              label: 'Mark as Failed', 
      ...
      case 'reassembled-testing':
        if (role === 'technician' && isAssignedTechnician) {
          return [
            { 
              status: 'repair-complete' as DeviceStatus, 
              label: 'Testing Passed - Complete', 
              icon: <CheckCircle size={18} />,
              color: 'bg-green-600 hover:bg-green-700',
              description: 'All tests passed, repair complete'
            },
            { 
              status: 'in-repair' as DeviceStatus, 
              label: 'Testing Failed - Back to Repair', 
              icon: <Wrench size={18} />,
              color: 'bg-orange-600 hover:bg-orange-700',
              description: 'Issues found, return to repair'
            },
            { 
              status: 'failed' as DeviceStatus, 
              label: 'Mark as Failed', 
        ...
        if (role === 'admin' || role === 'customer-care') {
          return [
            { 
              ...
              label: 'Receive from Technician', 
              icon: <UserCheck size={18} />,
              color: 'bg-blue-600 hover:bg-blue-700',
              description: 'Accept completed device from technician'
            }
          ];
        }
        ...
        if (role === 'technician' && isAssignedTechnician) {
          return [
            { 
              ...
              icon: <UserCheck size={18} />,
              color: 'bg-blue-600 hover:bg-blue-700',
              description: 'Transfer to customer care for pickup'
            }
          ];
        }
        return [];
        ...
        if (role === 'technician' && isAssignedTechnician) {
          return [
            { 
              ...
              icon: <UserCheck size={18} />,
              color: 'bg-blue-600 hover:bg-blue-700',
              description: 'Transfer failed device to customer care'
            }
          ];
        }
        if (role === 'admin' || role === 'customer-care') {
          return [
            { 
              ...
              description: 'Return unrepaired device to customer'
            }
          ];
        }
        return [];
                      ...
                      {isDoneTransition(transition.status) ? (
                        isPaid ? (
                          <div className="space-y-2">
                            <GlassButton
                              ...
                              size="lg"
                              icon={transition.icon}
                              className={`justify-start w-full h-[70px] ${transitions.length === 1 ? 'text-xl' : 'text-base'} ${transition.color || 'bg-green-600 hover:bg-green-700'}`}
                              variant="success"
                            >
                              {transition.label}
                            </GlassButton>
                            {transition.description && (
                              <p className="text-xs text-gray-600 ml-2">{transition.description}</p>
                            )}
                          </div>
                        ) : (
                        ...
                        <div className="space-y-2">
                          <GlassButton
                            ...
                            size="lg"
                            icon={transition.icon}
                            className={`justify-start w-full h-[70px] ${transitions.length === 1 ? 'text-xl' : 'text-base'} ${transition.color || 'bg-blue-600 hover:bg-blue-700'}`}
                            variant="primary"
                          >
                            {transition.label}
                          </GlassButton>
