...
// Temporarily commented out due to TypeScript compilation errors
...
const DeviceRepairDetailModal: React.FC<DeviceRepairDetailModalProps> = ({
  ...
  */
  ...
  const handleDiagnosticChecklistComplete = async (results: any) => {
    ...
    try {
      console.log('🔍 [DiagnosticCompletion] Starting diagnostic completion for device:', device.id);
      ...
      if (success) {
        ...
        console.log('🔍 [DiagnosticCompletion] Refreshing diagnostic data...');
        await loadDiagnosticData(device.id);
        console.log('✅ [DiagnosticCompletion] Diagnostic data refreshed successfully');
      } else {
  ...
  useEffect(() => {
    if (device && isOpen) {
      console.log('🔍 [DeviceRepairDetailModal] Loading data for device:', device.id, device.brand, device.model);
      loadUserNames();
  ...
  const loadRepairParts = async (deviceId: string) => {
    ...
    try {
      ...
      const response = { data: [], error: null }; // Temporary fallback
  ...
  const loadDiagnosticData = async (deviceId: string) => {
    console.log('🔍 [DiagnosticData] Starting to load diagnostic data for device:', deviceId);
    setDiagnosticLoading(true);
    try {
      // Load diagnostic data from both sources
      console.log('🔍 [DiagnosticData] Fetching from devices table and diagnostic_checks table...');
      ...
      console.log('🔍 [DiagnosticData] Raw device diagnostic data:', deviceDiagnostic);
      console.log('🔍 [DiagnosticData] Raw diagnostic checks data:', diagnosticChecks);

      let combinedData = null;

      // Process device diagnostic checklist (JSONB field)
      if (deviceDiagnostic.data?.diagnostic_checklist) {
        console.log('🔍 [DiagnosticData] Found device diagnostic checklist data');
        ...
        console.log('🔍 [DiagnosticData] Parsed device checklist:', deviceChecklist);
        ...
        console.log('🔍 [DiagnosticData] Processed device checklist data:', combinedData);
      } else {
        console.log('🔍 [DiagnosticData] No device diagnostic checklist found');
      }

      // Process individual diagnostic checks
      if (diagnosticChecks.data && diagnosticChecks.data.length > 0) {
        console.log('🔍 [DiagnosticData] Found individual diagnostic checks:', diagnosticChecks.data.length);
        ...
        console.log('🔍 [DiagnosticData] Calculated summary:', summary);

        // If we have device checklist data, merge it, otherwise create new data
        if (combinedData) {
          console.log('🔍 [DiagnosticData] Merging with existing device checklist data');
          combinedData = {
            ...combinedData,
          ...
          console.log('🔍 [DiagnosticData] Creating new data from individual checks');
          combinedData = {
        ...
        console.log('🔍 [DiagnosticData] No individual diagnostic checks found');
      }

      console.log('🔍 [DiagnosticData] Final combined data:', combinedData);
      setDiagnosticData(combinedData);
      
      if (combinedData) {
        console.log('✅ [DiagnosticData] Successfully loaded diagnostic data');
      } else {
        console.log('⚠️ [DiagnosticData] No diagnostic data found for this device');
      }
    } catch (error) {
      ...
      console.log('🔍 [DiagnosticData] Loading completed');
    }
  };
    ...
    try {
      ...
      const results = await Promise.all(
        selectedParts.map(async (part) => {
          ...
          return { data: null, error: 'Function temporarily disabled' }; // Temporary fallback
        })
      );
          ...
          <div className="p-3 sm:p-4">
            {activeTab === 'overview' && (
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-3 sm:gap-4 lg:gap-6">
                ...
                <div className="space-y-3 sm:space-y-4">
                  {/* Actions - Only show for active repairs */}
                  {device.status !== 'done' && device.status !== 'failed' && (
                    ...
                    <div className="space-y-1.5">
                      ...
                      {device && (
                        <div className="relative space-y-3">
                          <div className="space-y-2">
                            ...
                            {/* REMOVED: Payment Warning - Repair payment functionality removed */}
                            ...
                            {device.status === 'repair-complete' && (
                              <button 
                                ...
                                // REMOVED: Payment-based disabled state
                                className={`
                                  flex items-center justify-center gap-2 backdrop-blur-md rounded-lg border transition-all duration-300
                                  py-3 px-6 text-lg font-medium
                                  hover:shadow-lg active:scale-[0.98] focus:ring-2 focus:ring-white/30 hover:border-white/40 backdrop-blur-xl
                      ...
                      {/* REMOVED: Process Payments Button - Repair payment functionality removed */}
                      {/*
                      {device.status === 'repair-complete' && financialInfo && deviceCosts && (
                        // Process Payments button code removed
                      )}
                      */}
                      ...
                      {device.status === 'process-payments' && financialInfo && deviceCosts && (
                        // Record Payment button code removed
                      )}
                      */}
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-6">
                    ...
                    {repairPartsLoading ? (
                      ...
                      <div className="space-y-4">
                        ...
                        {/* Request Parts Button - Always visible when parts exist */}
                        <div className="pt-2 border-t border-gray-200">
                          <button
                            onClick={() => setShowSparePartsSelector(true)}
                            className="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors font-medium text-sm w-full justify-center"
                          >
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => {
                        if (device?.id) {
                          console.log('🔄 [ManualRefresh] Manually refreshing diagnostic data for device:', device.id);
                          loadDiagnosticData(device.id);
                        }
                      }}
                                    ...
                                    <div className="mt-2">
                                      ...
                                      <textarea
                                        ...
                                        onChange={(e) => {
                                          ...
                                          console.log('Admin comment for test:', failedCheck.title, e.target.value);
                                        }}
                                      />
                                    </div>
                                  </div>
                    ...
                    <div className="text-center py-6">
                      ...
                      <div className="space-y-2">
                        ...
                        <button
                          onClick={() => {
                            if (device?.id) {
                              console.log('🔄 [NoDataRefresh] Refreshing diagnostic data for device:', device.id);
                              loadDiagnosticData(device.id);
                            }
                          }}
