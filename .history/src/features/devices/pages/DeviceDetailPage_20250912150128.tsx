...
const DeviceDetailPage: React.FC = () => {
  ...
  const handleRecordPayment = async () => {
    ...
    try {
      ...
      if (paymentMethod && (paymentMethod.includes('mobile_money') || paymentMethod.includes('mpesa') || paymentMethod.includes('airtel'))) {
        const validation = validateMobileMoneyReference(paymentReference, 'Mobile Money');
    ...
    <>
      ...
      >
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              {paymentMethodsLoading ? (
                <div className="w-full py-4 px-6 bg-white/30 backdrop-blur-md border border-white/30 rounded-lg flex items-center justify-center">
          ...
          {/* Reference Number for Mobile Money */}
          {paymentMethod && (paymentMethod.includes('mobile_money') || paymentMethod.includes('mpesa') || paymentMethod.includes('airtel')) && (
            <div>
              <label className="block text-gray-700 mb-3 font-semibold text-base md:text-lg">
                Reference Number *
              </label>
              <input
                type="text"
                ...
                className="w-full py-4 px-6 bg-white/30 backdrop-blur-md border border-white/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-base md:text-lg"
                placeholder={getReferencePlaceholder('Mobile Money')}
                required
              />
              <div className="text-sm text-gray-500 mt-1">
                {getReferenceHelpText('Mobile Money')}
              </div>
            </div>
          )}

          <div className="bg-gray-50 rounded-lg p-4">
            <h4 className="font-semibold text-sm text-gray-800 mb-3">Payment Summary</h4>
