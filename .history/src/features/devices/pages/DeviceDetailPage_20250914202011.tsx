...
const DeviceDetailPage: React.FC = () => {
  ...
  // Get dynamic actions based on device status and user role
  const getDynamicActions = () => {
    const { status, assignedTo } = safeDevice;
    const userRole = currentUser.role;
    const isAssignedTechnician = assignedTo === currentUser.id;
    const actions = [];

    // Status update is always available for relevant users
    if (userRole === 'admin' || userRole === 'customer-care' || (userRole === 'technician' && isAssignedTechnician)) {
      actions.push({
        id: 'update-status',
        label: 'Update Status',
    ...
    // Payment actions
    if ((userRole === 'admin' || userRole === 'customer-care') && (outstanding === null || outstanding > 0)) {
      actions.push({
        id: 'record-payment',
    ...
    // Diagnostic actions based on status
    if (['assigned', 'diagnosis-started'].includes(status) && (userRole === 'technician' && isAssignedTechnician || userRole === 'admin' || userRole === 'customer-care')) {
      actions.push({
        id: 'diagnostic',
        label: 'Diagnostic',
    ...
    // Repair actions based on status
    if (['awaiting-parts', 'in-repair', 'reassembled-testing'].includes(status) && (userRole === 'technician' && isAssignedTechnician || userRole === 'admin' || userRole === 'customer-care')) {
      actions.push({
        id: 'repair-checklist',
    ...
    // Communication actions for customer care
    if (userRole === 'customer-care' && ['returned-to-customer-care', 'done'].includes(status)) {
      actions.push({
        id: 'send-sms',
        label: 'Send SMS',
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-4">
                    ...
                    <div className="space-y-2">
                      {getDynamicActions().length > 0 ? (
                        getDynamicActions().map((action) => (
                          <button
                            key={action.id}
                            onClick={action.onClick}
                            className={`w-full flex items-center justify-center gap-2 px-4 py-3 ${action.color} text-white rounded-lg transition-colors font-medium text-sm`}
                          >
                            {action.icon}
                            {action.label}
                          </button>
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-xs text-gray-400 mt-1">All tasks are complete</p>
                  </div>
                )}
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-medium">{formatCurrency(outstanding !== null ? totalPaid + outstanding : totalPaid)}</span>
                      </div>
                    </div>
                </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100 bg-gray-50 px-6 py-4">
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 px-3 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors font-medium text-xs"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors font-medium text-xs"
            >
            ...
            <button
              onClick={() => setShowAttachmentModal(true)}
              className="flex items-center gap-1 px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors font-medium text-xs"
            >
              <Upload className="w-4 h-4" />
              Files
            </button>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => navigate('/devices')}
              className="px-3 py-2 border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-lg transition-colors text-xs font-medium"
            >
              Close
            </button>
