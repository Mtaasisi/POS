              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200/50 bg-gray-50/50">
                    ...
                    <th className="text-left py-4 px-4 font-medium text-gray-700">Issue & Notes</th>
                    ...
                    <th className="text-left py-4 px-4 font-medium text-gray-700">Timeline</th>
                    <th className="text-right py-4 px-4 font-medium text-gray-700">Costs</th>
                    <th className="text-left py-4 px-4 font-medium text-gray-700">Warranty</th>
                    <th className="text-center py-4 px-4 font-medium text-gray-700">Confirmations</th>
                    <th className="text-center py-4 px-4 font-medium text-gray-700">Purchase</th>
                    ...
                    >
                            ...
                            {device.deviceCondition && Object.keys(device.deviceCondition).length > 0 && (
                              <div className="text-xs text-red-600 mt-1">
                                {Object.entries(device.deviceCondition)
                                  ...
                                  .slice(0, 2)
                                  .join(', ')}
                                {Object.values(device.deviceCondition).filter(Boolean).length > 2 && '...'}
                              </div>
                            )}
                            {device.blacklistCheckResult && (
                              <div className={`text-xs ${
                                device.blacklistCheckResult === 'clean' ? 'text-green-600' : 'text-red-600'
                              }`}>
                            ...
                            if (customer) {
                              return (
                                <div className="space-y-1 mt-1">
                                  {customer.email && (
                                    <div className="text-xs text-gray-500 truncate" title={customer.email}>
                                      {customer.email}
                                    </div>
                                  )}
                                  {customer.city && (
                                  ...
                                  {customer.loyaltyLevel && (
                                    <div className={`text-xs font-medium ${
                                      customer.loyaltyLevel === 'platinum' ? 'text-purple-600' :
                                      customer.loyaltyLevel === 'gold' ? 'text-yellow-600' :
                                      customer.loyaltyLevel === 'silver' ? 'text-gray-600' :
                                      'text-orange-600'
                                    }`}>
                                  ...
                                  {customer.colorTag && (
                                    <div className={`text-xs px-1 py-0.5 rounded ${
                                      customer.colorTag === 'vip' ? 'bg-purple-100 text-purple-700' :
                                      customer.colorTag === 'complainer' ? 'bg-red-100 text-red-700' :
                                      customer.colorTag === 'purchased' ? 'bg-green-100 text-green-700' :
                            ...
                            return (
                              <div className="space-y-1 mt-1">
                                {device.repairCount && device.repairCount > 0 && (
                                  <div className="text-xs text-blue-600">
                                    {device.repairCount} previous repair{device.repairCount > 1 ? 's' : ''}
                                  </div>
                                )}
                              </div>
                            );
                      ...
                      <td className="py-4 px-4 text-right">
                        <div className="space-y-1">
                          ...
                          {device.depositAmount && (
                            <div className="text-sm text-gray-600">
                              Deposit: {new Intl.NumberFormat('en-US', {
                                ...
                                minimumFractionDigits: 0,
                                maximumFractionDigits: 0,
                              }).format(parseFloat(device.depositAmount))}
                            </div>
                          )}
                          {device.deviceCost && (
                            <div className="text-xs text-gray-500">
                          ...
                          {!device.repairCost && !device.depositAmount && !device.deviceCost && (
                      ...
                      <td className="py-4 px-4">
                        <div className="space-y-1">
                          ...
                          {device.warrantyEnd && (
                            <div className="text-xs text-gray-500">
                              Until: {new Date(device.warrantyEnd).toLocaleDateString()}
                            </div>
                          )}
                          {!device.warrantyStatus && !device.warrantyEnd && (
                            <div className="text-sm text-gray-400">No warranty</div>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4">
                        <div className="space-y-1">
                          {device.accessoriesConfirmed && (
                            <div className="text-xs text-green-600">✓ Accessories</div>
                          )}
                          {device.problemConfirmed && (
                            <div className="text-xs text-green-600">✓ Problem</div>
                          )}
                          {device.privacyConfirmed && (
                            <div className="text-xs text-green-600">✓ Privacy</div>
                          )}
                          {!device.accessoriesConfirmed && !device.problemConfirmed && !device.privacyConfirmed && (
                            <div className="text-xs text-gray-400">No confirmations</div>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4">
                        <div className="space-y-1">
                          ...
                          {device.sellerVerificationStatus && (
                            <div className={`text-xs ${
                              device.sellerVerificationStatus === 'verified' ? 'text-green-600' :
                              device.sellerVerificationStatus === 'rejected' ? 'text-red-600' :
                              'text-yellow-600'
                            }`}>
                              {device.sellerVerificationStatus}
                            </div>
                          )}
                          {device.securityCheckStatus && (
                            <div className={`text-xs ${
                              device.securityCheckStatus === 'passed' ? 'text-green-600' :
                              device.securityCheckStatus === 'failed' ? 'text-red-600' :
                              'text-yellow-600'
                            }`}>
                              Security: {device.securityCheckStatus}
                            </div>
                          )}
                          {!device.purchaseType && !device.purchasePrice && !device.sellerVerificationStatus && !device.securityCheckStatus && (
                            <div className="text-xs text-gray-400">No purchase info</div>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4">
                        <div className="space-y-1">
                          {device.adminApprovalStatus && (
                            <div className={`text-xs font-medium ${
                              device.adminApprovalStatus === 'approved' ? 'text-green-600' :
                              device.adminApprovalStatus === 'rejected' ? 'text-red-600' :
                              'text-yellow-600'
                            }`}>
                              {device.adminApprovalStatus}
                            </div>
                          )}
                          {device.adminApprovalDate && (
                            <div className="text-xs text-gray-500">
                              {new Date(device.adminApprovalDate).toLocaleDateString()}
                            </div>
                          )}
                          {device.adminApprovalBy && (
                            <div className="text-xs text-gray-500">
                              By: {device.adminApprovalBy}
                            </div>
                          )}
                          {device.transactionId && (
                            <div className="text-xs text-blue-600">
                              TXN: {device.transactionId.slice(-8)}
                            </div>
                          )}
                          {!device.adminApprovalStatus && !device.adminApprovalDate && !device.adminApprovalBy && !device.transactionId && (
                            <div className="text-xs text-gray-400">No admin info</div>
                          )}
                        </div>
                      </td>
