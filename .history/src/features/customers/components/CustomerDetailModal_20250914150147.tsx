    ...
    try {
      ...
      try {
        ...
        if (!prefsError && prefsData) {
          setCustomerPreferences(prefsData);
        }
      } catch (error) {
      ...
      >
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  {(customer.referralSource || customer.referredBy || referrals.length > 0) && (
                    <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                      <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-6">
                ...
                {customerPreferences && (
                  <div className="pt-6 border-t border-gray-100">
                    <h4 className="font-medium text-gray-700 mb-4">Communication Preferences</h4>
                      ...
                      <tbody>
                        {appointments.map(appointment => (
                          <tr key={appointment.id} className="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td className="p-3">{appointment.technician_name || 'Not assigned'}</td>
                            <td className="p-3">{appointment.duration_minutes} min</td>
                            <td className="p-3 max-w-32 truncate" title={appointment.notes}>
                              {appointment.notes || '-'}
                            </td>
                          </tr>
                        ))}
                      ...
                      <div key={index} className="flex items-start gap-3 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 text-xs text-gray-500">
                            ...
                            {message.cost && <span>Cost: {formatCurrency(message.cost)}</span>}
                          </div>
                        </div>
                      </div>
                    ))}
                      ...
                      <tbody>
                        {returns.map(returnItem => (
                          <tr key={returnItem.id} className="border-b border-gray-100 hover:bg-gray-50">
                            <td className="p-3">{new Date(returnItem.created_at).toLocaleDateString()}</td>
                            <td className="p-3">
                              <div>
                                <div className="font-medium">{returnItem.manual_device_brand} {returnItem.manual_device_model}</div>
                                <div className="text-xs text-gray-500">{returnItem.manual_device_serial}</div>
                              </div>
                            </td>
                            <td className="p-3 max-w-32 truncate" title={returnItem.reason}>
                              {returnItem.reason}
                            </td>
                            <td className="p-3">
                              <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                                returnItem.return_type === 'refund' ? 'bg-green-100 text-green-800' :
                                returnItem.return_type === 'exchange' ? 'bg-blue-100 text-blue-800' :
                            ...
                            <td className="p-3">
                              <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                                ...
                                returnItem.status === 'return-rejected' ? 'bg-red-100 text-red-800' :
                                'bg-yellow-100 text-yellow-800'
                              }`}>
                            ...
                            <td className="p-3">
                              {returnItem.refund_amount ? formatCurrency(returnItem.refund_amount) : '-'}
                            </td>
                          </tr>
                        ))}
                      </tbody>
          ...
          {activeTab === 'referrals' && (
            <div className="space-y-6">
              <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-6">
                ...
                <div>
                  <h4 className="font-medium text-gray-700 mb-4">Customers they referred ({referrals.length})</h4>
        ...
        <div className="flex items-center justify-between pt-6 border-t border-gray-100">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <GlassButton
              onClick={() => {/* TODO: Open WhatsApp chat */}}
            ...
            <GlassButton
              onClick={() => {/* TODO: Open customer preferences modal */}}
