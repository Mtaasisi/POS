      ...
      <div className="p-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              {sparePart.condition && (
                <div className="flex justify-between">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${getConditionColor(sparePart.condition)}`}>
                    {sparePart.condition}
                  </span>
                </div>
              )}
        ...
        {sparePart.description && (
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-600">{sparePart.description}</p>
          </div>
        )}
          ...
          <div className="mb-4">
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              <div className="flex items-center mb-2">
                ...
                <span className="font-medium">{sparePart.supplier.name}</span>
              </div>
              {sparePart.supplier.email && (
                <div className="flex items-center mb-1">
                  <Mail className="h-4 w-4 mr-2 text-gray-600" />
                  <span className="text-sm text-gray-600">{sparePart.supplier.email}</span>
                </div>
              )}
              {sparePart.supplier.phone && (
                <div className="flex items-center">
                  <Phone className="h-4 w-4 mr-2 text-gray-600" />
                  <span className="text-sm text-gray-600">{sparePart.supplier.phone}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {repairPart.notes && (
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3">{repairPart.notes}</p>
          </div>
        )}
        ...
        {showImages && (
          <div className="mb-4">
            ...
            {images.length > 0 ? (
              ...
              <div className="flex items-center justify-center bg-gray-50 rounded-lg border border-gray-200 h-20">
            ...
            <div className="space-y-2">
              {sparePart.variants.map((variant, index) => (
                <div key={variant.id || index} className="bg-gray-50 rounded-lg p-3">
                  ...
                  {variant.sku && (
                    <div className="text-sm text-gray-600 mb-1">SKU: {variant.sku}</div>
                  )}
                    ...
                    <div className="mt-2">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {Object.entries(variant.variant_attributes).map(([key, value]) => (
                          <span key={key} className="bg-white px-2 py-1 rounded text-xs border">
                            {key}: {String(value)}
                          </span>
                        ))}
                      </div>
