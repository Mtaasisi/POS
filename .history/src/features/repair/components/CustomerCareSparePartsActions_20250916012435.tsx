...
import { acceptSpareParts, rejectSpareParts, areAllSparePartsReady } from '../services/repairPartsApi';
...
interface CustomerCareSparePartsActionsProps {
  ...
  currentUser: any;
  deviceId?: string;
  onDeviceStatusUpdate?: (deviceId: string, status: string) => void;
}
  ...
  const handleAcceptParts = async () => {
    ...
    try {
      ...
      if (result.ok && result.data) {
        ...
        if (deviceId && onDeviceStatusUpdate) {
          const updatedParts = repairParts.map(part => {
            if (selectedParts.includes(part.id)) {
              return { ...part, status: 'accepted' as const };
            }
          ...
          // Check if all parts are now accepted or received
          const allPartsProcessed = updatedParts.every(part => 
          ...
          if (allPartsProcessed) {
            ...
            onDeviceStatusUpdate(deviceId, 'parts-arrived');
            toast.success('All parts accepted! Device status updated to "Parts Arrived"');
          }
        }
      } else {
