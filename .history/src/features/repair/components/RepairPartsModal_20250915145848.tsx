...
const RepairPartsModal: React.FC<RepairPartsModalProps> = ({
  ...
  // Mock function to load repair parts - in real app, this would come from database
  const loadRepairParts = async () => {
    // This would typically fetch from a repair_parts table
  ...
  const handleAddPart = () => {
    ...
    const newPart: RepairPart = {
      ...
      total_cost: selectedSparePart.selling_price * quantityNeeded,
      status: 'needed',
      notes: partNotes,
      spare_part: selectedSparePart
    };
  ...
  // Update part status
  const handleUpdatePartStatus = (partId: string, status: RepairPart['status']) => {
    setRepairParts(prev => 
      prev.map(part => 
  ...
  const handleUseSparePart = async (part: RepairPart) => {
    ...
    setLoading(true);
    try {
      const result = await useSparePart({
        spare_part_id: part.spare_part_id,
        quantity: part.quantity_needed,
        reason: `Repair for device: ${device.brand} ${device.model} (${device.serialNumber})`,
        notes: part.notes || `Repair ID: ${device.id}`,
      ...
      if (result.ok) {
        setRepairParts(prev => 
          prev.map(p => 
            p.id === part.id 
              ? { ...p, quantity_used: p.quantity_needed, status: 'used' as const }
              : p
          )
        );
