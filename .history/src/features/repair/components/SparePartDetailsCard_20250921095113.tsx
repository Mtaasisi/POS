    ...
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      <div className="p-2 border-b border-gray-200">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center space-x-1">
            <span className={`px-1.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(repairPart.status)}`}>
      ...
      <div className="p-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-2">
          <div>
            <h4 className="text-xs font-medium text-gray-900 mb-1">Basic Info</h4>
            ...
            <div className="space-y-1 text-xs">
              ...
              {sparePart.condition && (
                <div className="flex justify-between">
                  ...
                  <span className={`px-1 py-0.5 rounded-full text-xs font-medium ${getConditionColor(sparePart.condition)}`}>
                    {sparePart.condition}
                  </span>
                </div>
              )}
        ...
        {sparePart.supplier && (
          <div className="mb-2">
            <h4 className="text-xs font-medium text-gray-900 mb-1">Supplier</h4>
            <div className="bg-gray-50 rounded p-1.5">
              <div className="flex items-center mb-1">
                <User className="h-3 w-3 mr-1 text-gray-600" />
                <span className="font-medium text-xs">{sparePart.supplier.name}</span>
              </div>
        ...
        {repairPart.notes && (
          <div className="mb-2">
            ...
            <p className="text-xs text-gray-600 bg-gray-50 rounded p-1.5">{repairPart.notes}</p>
          </div>
        )}
        ...
        {showImages && (
          <div className="mb-2">
            <div className="flex items-center justify-between mb-1">
              <h4 className="text-xs font-medium text-gray-900">Images</h4>
              {loadingImages && (
            ...
            {images.length > 0 ? (
              <div className="grid grid-cols-3 gap-1">
                ...
                {images.length > 3 && (
                  <div className="flex items-center justify-center bg-gray-100 rounded border border-gray-200 h-12">
                    <span className="text-xs text-gray-600">+{images.length - 3}</span>
                  </div>
                )}
              </div>
            ) : (
              <div className="flex items-center justify-center bg-gray-50 rounded border border-gray-200 h-12">
                <div className="text-center">
                  <ImageIcon className="h-3 w-3 text-gray-400 mx-auto mb-0.5" />
        ...
        {showVariants && sparePart.variants && sparePart.variants.length > 0 && (
          <div className="mb-2">
            <h4 className="text-xs font-medium text-gray-900 mb-1">Variants</h4>
            <div className="space-y-1">
              {sparePart.variants.slice(0, 2).map((variant, index) => (
                <div key={variant.id || index} className="bg-gray-50 rounded p-1.5">
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-medium text-xs">{variant.name}</span>
                    <span className="text-xs text-gray-600">Qty: {variant.quantity}</span>
                  </div>
                  {variant.sku && (
                    <div className="text-xs text-gray-600 mb-0.5">SKU: {variant.sku}</div>
                  )}
                  <div className="flex justify-between text-xs">
                    <span>${variant.selling_price?.toFixed(2) || '0.00'}</span>
                    {isAdmin && (
              ...
              {sparePart.variants.length > 2 && (
                <div className="text-xs text-gray-500 text-center py-1">
                  +{sparePart.variants.length - 2} more variants
                </div>
              )}
            </div>
          </div>
        ...
        {showAllDetails && (
          <div className="border-t border-gray-200 pt-3 mt-3">
            <h4 className="text-sm font-medium text-gray-900 mb-1.5">Additional Information</h4>
