  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      if (needsSparePartData) {
        ...
        // Create a new array with enhanced data instead of modifying the original
        const enhancedData = await Promise.all(data.map(async (part) => {
          if (!part.spare_part && part.spare_part_id) {
            try {
              ...
              if (!sparePartError && sparePartData) {
                ...
                return { ...part, spare_part: sparePartData };
              } else {
