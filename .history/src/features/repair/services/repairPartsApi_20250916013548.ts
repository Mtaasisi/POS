...
// Quick function to get just the spare part names for a device
export const getSparePartNamesForDevice = async (deviceId: string): Promise<{
  data: string[];
  message: string;
  ok: boolean;
}> => {
  try {
    console.log('🔍 [DEBUG] Getting spare part names for device:', deviceId);
    
    const result = await getRepairParts(deviceId);
    if (result.ok && result.data) {
      const partNames = result.data
        .map(part => part.spare_part?.name || 'Unknown Part')
        .filter(name => name !== 'Unknown Part');
      
      console.log('📝 [DEBUG] Spare part names found:', partNames);
      
      return {
        data: partNames,
        message: `Found ${partNames.length} spare part names`,
        ok: true
      };
    } else {
      return {
        data: [],
        message: result.message || 'Failed to get spare part names',
        ok: false
      };
    }
  } catch (error) {
    console.error('Error getting spare part names:', error);
    return {
      data: [],
      message: error instanceof Error ? error.message : 'Failed to get spare part names',
      ok: false
    };
  }
};
...
// Check if repair can be started (has available parts)
export const canStartRepair = async (deviceId: string): Promise<{
  canStart: boolean;
  totalParts: number;
  ...
  try {
    ...
    if (totalParts === 0) {
      return {
        ...
        totalParts: 0,
        availableParts: 0,
        message: 'No spare parts added to this repair',
        reason: 'no_parts'
      };
    }

    if (availableParts === 0) {
      const neededParts = parts.filter(part => part.status === 'needed').length;
      const orderedParts = parts.filter(part => part.status === 'ordered').length;
      const acceptedParts = parts.filter(part => part.status === 'accepted').length;
      
      let reason = 'no_available_parts';
      let message = 'No parts are available for repair';
      
      if (neededParts > 0) {
        message = `${neededParts} parts still needed - request parts first`;
        reason = 'parts_needed';
      } else if (orderedParts > 0) {
        message = `${orderedParts} parts ordered - wait for delivery`;
        reason = 'parts_ordered';
      } else if (acceptedParts > 0) {
        message = `${acceptedParts} parts accepted - wait for delivery`;
        reason = 'parts_accepted';
      }
    ...
    const canStart = availableParts > 0;
    ...
    return {
      ...
      availableParts,
      message: canStart 
        ? `Ready to start repair with ${availableParts}/${totalParts} parts available` 
        : `Cannot start repair - ${availableParts}/${totalParts} parts available`,
      reason: canStart ? 'ready' : 'insufficient_parts'
    };
  } catch (error) {
    console.error('Error checking repair readiness:', error);
    return {
      canStart: false,
      totalParts: 0,
      availableParts: 0,
