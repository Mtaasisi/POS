  ...
  async createPurchaseOrderPayment(data: CreatePurchaseOrderPaymentData): Promise<PurchaseOrderPayment> {
    try {
      ...
      if (paymentAccount.currency !== data.currency) {
        ...
        // For now, we'll allow the payment if the account has sufficient balance
        // In a real system, you'd want to implement proper currency conversion
        // For testing purposes, we'll assume 1 USD = 2500 TZS (approximate rate)
        if (data.currency === 'USD' && paymentAccount.currency === 'TZS') {
          requiredAmount = data.amount * 2500; // Convert USD to TZS
          console.log(`💱 Converting ${data.amount} ${data.currency} to ${requiredAmount} ${paymentAccount.currency}`);
        } else if (data.currency === 'TZS' && paymentAccount.currency === 'USD') {
          requiredAmount = data.amount / 2500; // Convert TZS to USD
          console.log(`💱 Converting ${data.amount} ${data.currency} to ${requiredAmount} ${paymentAccount.currency}`);
        } else {
          // For other currency combinations, we'll allow the payment for now
          console.log(`⚠️ Unsupported currency conversion: ${data.currency} to ${paymentAccount.currency}`);
        }
      }
