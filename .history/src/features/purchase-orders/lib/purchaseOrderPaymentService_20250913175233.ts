...
class PurchaseOrderPaymentService {
  ...
  async createPurchaseOrderPayment(data: CreatePurchaseOrderPaymentData): Promise<PurchaseOrderPayment> {
    try {
      ...
      // Create payment record - handle foreign key constraint issue
      // The constraint is pointing to auth.users, so we need to use a valid auth.users ID
      // or create the payment without the created_by field temporarily
      
      let paymentRecord;
      let paymentError;
      
      // Try to get a valid user ID from auth.users table
      const { data: authUsers, error: authUsersError } = await supabase.auth.getUser();
      let validUserId = data.createdBy;
      
      if (authUsersError || !authUsers?.user) {
        console.warn('⚠️ No authenticated user found, using system user ID');
        validUserId = '00000000-0000-0000-0000-000000000000';
      } else {
        validUserId = authUsers.user.id;
      }
      ...
      paymentRecord = firstAttempt;
      paymentError = firstError;
