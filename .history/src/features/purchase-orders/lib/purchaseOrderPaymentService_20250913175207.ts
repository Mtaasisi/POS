  ...
  async createPurchaseOrderPayment(data: CreatePurchaseOrderPaymentData): Promise<PurchaseOrderPayment> {
    try {
      ...
      if (firstError && firstError.code === '23503') {
        console.warn('⚠️ Foreign key constraint error, trying with system user ID');
        
        // Try with a system user ID that should exist
        ...
        paymentRecord = secondAttempt;
        paymentError = secondError;
      } else {
