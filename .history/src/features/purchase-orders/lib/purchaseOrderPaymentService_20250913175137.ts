...
class PurchaseOrderPaymentService {
  ...
  async createPurchaseOrderPayment(data: CreatePurchaseOrderPaymentData): Promise<PurchaseOrderPayment> {
    try {
      ...
      // Validate that the user exists in auth_users table
      ...
      if (userError || !userExists) {
        console.warn('⚠️ User not found in auth_users table, using system user ID');
        // Use a fallback system user ID or create a system user
        data.createdBy = '00000000-0000-0000-0000-000000000000'; // System user ID
      }
