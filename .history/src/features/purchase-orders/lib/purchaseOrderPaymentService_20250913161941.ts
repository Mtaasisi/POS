...
class PurchaseOrderPaymentService {
  ...
  async createPurchaseOrderPayment(data: CreatePurchaseOrderPaymentData): Promise<PurchaseOrderPayment> {
    try {
      ...
      // Create finance transaction record for tracking
      const { error: transactionError } = await supabase
        ...
        .insert({
          ...
          amount: data.amount,
          currency: data.currency || 'TZS',
          description: `Purchase Order Payment - ${data.paymentMethod}`,
          reference: data.reference,
          notes: data.notes,
          related_id: data.purchaseOrderId,
          related_type: 'purchase_order_payment',
          created_by: data.createdBy
        });

      if (transactionError) {
        console.error('⚠️ Error creating finance transaction:', transactionError);
        // Don't throw error as payment was already recorded
      }
