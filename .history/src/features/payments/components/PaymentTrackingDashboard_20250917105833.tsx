...
const PaymentTrackingDashboard: React.FC<PaymentTrackingDashboardProps> = ({
  ...
  const chartData = useMemo(() => {
    ...
    // Revenue comparison data (last 7 days vs previous 7 days)
    const revenueComparison = dailyData.slice(-7).map((day, index) => ({
      day: day.date,
      current: day.amount,
      previous: dailyData[dailyData.length - 14 + index]?.amount || 0
    }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">Visual representation of payment data and trends</p>
          </div>
          <div className="flex gap-2">
      ...
      {/* Payment Status and Revenue Comparison */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Revenue Comparison */}
        {chartData.revenueComparison.length > 0 && (
          <GlassCard className="p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Revenue Comparison</h3>
                <p className="text-sm text-gray-600">Current vs Previous Week</p>
              </div>
              <TrendingUp className="w-5 h-5 text-purple-600" />
            </div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData.revenueComparison}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                  <XAxis 
                    dataKey="day" 
                    stroke="#6B7280"
                  ...
                  />
                  <Legend />
                  <Bar dataKey="current" fill="#3B82F6" name="Current Week" radius={[2, 2, 0, 0]} />
                  <Bar dataKey="previous" fill="#94A3B8" name="Previous Week" radius={[2, 2, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </GlassCard>
