...
Date: ${new Date(payment.created_at || payment.date).toLocaleDateString()}`;
  ...
  const handleAcceptTransaction = async (payment: any) => {
    try {
      ...
      // Use the new approval function to update status across all tables
      const { data, error } = await supabase.rpc('approve_payment_across_all_tables', {
        payment_id: payment.id,
        source_table: payment.source,
        user_id: currentUser?.id || null
      });

      if (error) {
        console.error('Database approval error:', error);
        throw error;
      }

      if (data && !data.success) {
        console.error('Approval function error:', data.error);
        throw new Error(data.error || 'Failed to approve payment');
      }

      console.log('✅ PaymentTransactions: Payment approved successfully:', data);
      toast.success(`Payment approved successfully${data.updated_count > 1 ? ` (${data.updated_count} related payments updated)` : ''}`);
