...
const PaymentTransactions: React.FC<PaymentTransactionsProps> = ({
  ...
  // Fetch all payment data using the same approach as PaymentTrackingDashboard
  const fetchPaymentData = async () => {
    try {
      ...
      // Handle each result with detailed logging
      if (paymentsData.status === 'fulfilled') {
        setPayments(paymentsData.value);
        console.log(`✅ PaymentTransactions: Fetched ${paymentsData.value.length} payment transactions`);
      } else {
        console.error('❌ PaymentTransactions: Failed to fetch payments:', paymentsData.reason);
      }
      
      if (customerPaymentsData.status === 'fulfilled') {
        setCustomerPayments(customerPaymentsData.value.data || []);
        console.log(`✅ PaymentTransactions: Fetched ${(customerPaymentsData.value.data || []).length} customer payments`);
      } else {
      ...
      if (purchaseOrderPaymentsData.status === 'fulfilled') {
        ...
        console.log(`✅ PaymentTransactions: Fetched ${(purchaseOrderPaymentsData.value.data || []).length} purchase order payments`);
      } else {
      ...
      if (devicePaymentsData.status === 'fulfilled') {
        ...
        console.log(`✅ PaymentTransactions: Fetched ${(devicePaymentsData.value.data || []).length} device payments`);
      } else {
      ...
      if (repairPaymentsData.status === 'fulfilled') {
        ...
        console.log(`✅ PaymentTransactions: Fetched ${(repairPaymentsData.value.data || []).length} repair payments`);
      } else {
      ...
      if (paymentTransactionsData.status === 'fulfilled') {
        ...
        console.log(`✅ PaymentTransactions: Fetched ${(paymentTransactionsData.value.data || []).length} payment transactions`);
      } else {
        console.error('❌ PaymentTransactions: Failed to fetch payment transactions:', paymentTransactionsData.reason);
      }

    } catch (error) {
      console.error('❌ PaymentTransactions: Error fetching payment data:', error);
  ...
  const allPayments = useMemo(() => {
    ...
    console.log(`📊 PaymentTransactions: payments: ${payments.length}, customerPayments: ${customerPayments.length}, purchaseOrderPayments: ${purchaseOrderPayments.length}, devicePayments: ${devicePayments.length}, repairPayments: ${repairPayments.length}, paymentTransactions: ${paymentTransactions.length}`);
    ...
    console.log(`📊 PaymentTransactions: ${uniquePayments.length} unique payments after deduplication`);
