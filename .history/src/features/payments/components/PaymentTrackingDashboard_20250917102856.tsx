    ...
    <div className="space-y-6">
      ...
      {paymentProviders.length > 0 && (
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-2xl font-bold text-gray-900">Payment Providers Performance</h3>
              <p className="text-gray-600">Monitor and analyze payment provider performance metrics</p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => {/* TODO: Navigate to provider management */}}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-900">
                    {paymentProviders.length > 0 
                      ? (paymentProviders.reduce((sum, p) => sum + (p.performance.successRate || 0), 0) / paymentProviders.length).toFixed(1)
                      : '0.0'
                    }%
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-900">
                    {paymentProviders.length > 0 
                      ? (paymentProviders.reduce((sum, p) => sum + (p.performance.averageResponseTime || 0), 0) / paymentProviders.length).toFixed(1)
                      : '0.0'
                    }s
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-900">
                    {paymentProviders.reduce((sum, p) => sum + (p.performance.totalTransactions || 0), 0).toLocaleString()}
                  </p>
                </div>
                <div className="p-3 bg-purple-500 rounded-lg">
                  <CreditCard className="w-6 h-6 text-white" />
            ...
            <GlassCard className="p-6 bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-orange-600">Active Providers</p>
                  <p className="text-2xl font-bold text-orange-900">
                    {paymentProviders.filter(p => p.status === 'active').length}/{paymentProviders.length}
                  </p>
                </div>
                <div className="p-3 bg-orange-500 rounded-lg">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {paymentProviders.map((provider) => {
              const getProviderIcon = (type: string) => {
                switch (type) {
                  case 'mobile_money': return <Smartphone className="w-5 h-5 text-blue-600" />;
                  case 'card': return <CreditCard className="w-5 h-5 text-green-600" />;
                  case 'bank': return <Building className="w-5 h-5 text-purple-600" />;
                  case 'cash': return <DollarSign className="w-5 h-5 text-orange-600" />;
              ...
              return (
                <GlassCard key={provider.id} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                  {/* Header */}
                  ...
                  <div className="space-y-4 mb-4">
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className={`text-sm font-bold ${getPerformanceColor(provider.performance.successRate || 0)}`}>
                          {provider.performance.successRate?.toFixed(1) || '0.0'}%
                        </span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          className={`h-2 rounded-full transition-all duration-300 ${
                            (provider.performance.successRate || 0) >= 95 ? 'bg-green-500' :
                            (provider.performance.successRate || 0) >= 85 ? 'bg-yellow-500' : 'bg-red-500'
                          }`}
                          style={{ width: `${Math.min(provider.performance.successRate || 0, 100)}%` }}
                        ></div>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <span className="font-semibold text-gray-900">
                        {provider.performance.averageResponseTime?.toFixed(1) || '0.0'}s
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <span className="font-semibold text-gray-900">
                        {provider.performance.totalTransactions?.toLocaleString() || '0'}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <span className="font-semibold text-gray-900">
                        {provider.performance.uptime?.toFixed(1) || '0.0'}%
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div>
                        ...
                        <span className="ml-1 font-medium text-gray-700">
                          {provider.configuration?.currency || 'TZS'}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-500">Fees:</span>
                        <span className="ml-1 font-medium text-gray-700">
                          {provider.configuration?.fees?.percentage || 0}%
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 mt-4">
                    <button
                      onClick={() => {/* TODO: Test provider */}}
                      className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors text-xs font-medium"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors text-xs font-medium"
                    >
          ...
          <GlassCard className="p-6">
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm">Performance trend charts will be displayed here</p>
              <p className="text-xs text-gray-400 mt-1">Coming soon in the next update</p>
            </div>
          </GlassCard>
        </div>
      )}
