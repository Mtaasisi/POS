  ...
  const fetchSecurityData = useCallback(async () => {
    ...
    try {
      ...
      if (complianceResult.status === 'fulfilled') {
        ...
        const transformedChecks = complianceResult.value.map((check: any) => ({
          id: check.id,
          name: check.title || check.checkType || 'Compliance Check',
          status: check.status,
          lastChecked: check.details?.checkedAt || check.createdAt,
          nextCheck: check.details?.nextCheck || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
          details: check.description || 'Compliance check details'
        }));
        setComplianceChecks(transformedChecks);
        console.log('✅ Compliance checks loaded:', transformedChecks.length);
      } else {
