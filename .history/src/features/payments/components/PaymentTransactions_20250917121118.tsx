...
const PaymentTransactions: React.FC<PaymentTransactionsProps> = ({
  ...
  const fetchPaymentData = async () => {
    try {
      ...
      const totalData = payments.length + customerPayments.length + purchaseOrderPayments.length + 
                       devicePayments.length + repairPayments.length + paymentTransactions.length;
      
      if (totalData === 0) {
        console.warn('⚠️ PaymentTransactions: No payment data found from any source');
        toast.error('No payment data available. Please check your connection.');
      } else {
        console.log(`✅ PaymentTransactions: Successfully loaded payment data from multiple sources`);
      }
