...
const PaymentTrackingDashboard: React.FC<PaymentTrackingDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'summary' | 'transactions' | 'by-method'>('summary');
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                ...
                <div className="space-y-2">
                  {latestTransactions.map((payment) => {
                    ...
                    return (
                      <div key={payment.id} className={`bg-gradient-to-br ${statusConfig.gradient} border ${statusConfig.border} rounded-xl p-4 shadow-sm hover:shadow-md transition-all duration-200 group`}>
                        <div className="flex items-center justify-between">
