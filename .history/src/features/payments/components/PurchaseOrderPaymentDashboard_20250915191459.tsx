...
const PurchaseOrderPaymentDashboard: React.FC<PurchaseOrderPaymentDashboardProps> = ({
  ...
  const fetchRecentPayments = useCallback(async () => {
    try {
      // Fetch payments without the join first
      ...
      const orderIds = [...new Set(paymentsData.map(payment => payment.purchase_order_id).filter(Boolean))];
      ...
      const enrichedPayments = paymentsData.map(payment => {
        const order = orderMap.get(payment.purchase_order_id);
        const supplier = order?.supplier_id ? supplierMap.get(order.supplier_id) : null;
        
        return {
          ...payment,
          lats_purchase_orders: order ? {
