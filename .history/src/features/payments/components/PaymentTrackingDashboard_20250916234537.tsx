...
const PaymentTrackingDashboard: React.FC<PaymentTrackingDashboardProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Comprehensive Data Sources Status - Minimal Style */}
      <div className="mb-6">
        <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Activity className="w-5 h-5 text-green-600" />
              <h3 className="text-sm font-semibold text-gray-800">Data Sources Status</h3>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div className="text-center p-3 bg-blue-50 rounded-lg">
              <div className="text-lg font-bold text-blue-600">{payments.length}</div>
              <div className="text-xs text-gray-600">Transactions</div>
            </div>
            <div className="text-center p-3 bg-green-50 rounded-lg">
              <div className="text-lg font-bold text-green-600">{paymentProviders.length}</div>
              <div className="text-xs text-gray-600">Providers</div>
            </div>
            <div className="text-center p-3 bg-purple-50 rounded-lg">
              <div className="text-lg font-bold text-purple-600">{financeAccounts.length}</div>
              <div className="text-xs text-gray-600">Accounts</div>
            </div>
            <div className="text-center p-3 bg-orange-50 rounded-lg">
              <div className="text-lg font-bold text-orange-600">{enhancedTransactions.length}</div>
      ...
      {/* Enhanced Metrics Dashboard - Minimal Style */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 rounded-xl p-4">
          ...
          <div className="text-lg font-bold text-emerald-900">{metrics.totalPayments}</div>
        </div>

        <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-4">
          <div className="text-xs font-medium text-blue-700 uppercase tracking-wide mb-1">Total Amount</div>
          <div className="text-lg font-bold text-blue-900">{formatMoney(metrics.totalAmount)}</div>
        </div>

        <div className="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-xl p-4">
          <div className="text-xs font-medium text-green-700 uppercase tracking-wide mb-1">Completed</div>
          <div className="text-lg font-bold text-green-900">{formatMoney(metrics.completedAmount)}</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-xl p-4">
          ...
          <div className="text-lg font-bold text-purple-900">{formatMoney(metrics.totalFees)}</div>
        </div>
      </div>

      {/* Comprehensive Analytics Section - Minimal Style */}
          ...
          {financialAnalytics && (
            <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
              <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
                ...
                <h3 className="text-sm font-semibold text-gray-800">Financial Analytics</h3>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Total Revenue</span>
                  <p className="text-lg font-bold text-green-600">{formatMoney(financialAnalytics.summary.totalRevenue)}</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Total Expenses</span>
                  <p className="text-lg font-bold text-red-600">{formatMoney(financialAnalytics.summary.totalExpenses)}</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Net Profit</span>
                  <p className="text-lg font-bold text-blue-600">{formatMoney(financialAnalytics.summary.netProfit)}</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Revenue Growth</span>
                  <p className="text-sm font-medium text-gray-900">{financialAnalytics.summary.revenueGrowth?.toFixed(1) || '0.0'}%</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Expense Growth</span>
                  <p className="text-sm font-medium text-gray-900">{financialAnalytics.summary.expenseGrowth?.toFixed(1) || '0.0'}%</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Profit Growth</span>
                  <p className="text-sm font-medium text-gray-900">{financialAnalytics.summary.profitGrowth?.toFixed(1) || '0.0'}%</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <p className="text-lg font-bold text-green-600">{paymentInsights.topPaymentMethod}</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Avg Transaction</span>
                  <p className="text-lg font-bold text-blue-600">{formatMoney(paymentInsights.averageTransactionValue)}</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Peak Hour</span>
                  <p className="text-lg font-bold text-orange-600">{paymentInsights.peakHour}</p>
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Failure Rate</span>
                  <p className="text-sm font-medium text-gray-900">{paymentInsights.failureRate?.toFixed(1) || '0.0'}%</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Refund Rate</span>
                  <p className="text-sm font-medium text-gray-900">{paymentInsights.refundRate?.toFixed(1) || '0.0'}%</p>
                </div>
              </div>
            </div>
          )}
      ...
      {/* Payment Providers Performance - Minimal Style */}
      {paymentProviders.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
          <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
            <Settings className="w-5 h-5 text-indigo-600" />
            <h3 className="text-sm font-semibold text-gray-800">Payment Providers Performance</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {paymentProviders.map((provider) => (
              <div key={provider.id} className="bg-gray-50 border border-gray-200 rounded-lg p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-semibold text-gray-900">{provider.name}</h4>
                  <span className={`px-2 py-1 text-xs rounded-full ${
                    provider.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                  }`}>
                    {provider.status}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="space-y-1">
                    <span className="text-gray-500 uppercase tracking-wide">Success Rate</span>
                    <p className="font-medium text-gray-900">{provider.performance.successRate?.toFixed(1) || '0.0'}%</p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-gray-500 uppercase tracking-wide">Response Time</span>
                    <p className="font-medium text-gray-900">{provider.performance.averageResponseTime}s</p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-gray-500 uppercase tracking-wide">Transactions</span>
                    <p className="font-medium text-gray-900">{provider.performance.totalTransactions}</p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-gray-500 uppercase tracking-wide">Uptime</span>
                    <p className="font-medium text-gray-900">{provider.performance.uptime?.toFixed(1) || '0.0'}%</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* Enhanced Filters - Minimal Style */}
      <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
      ...
      {/* Payment Transactions List - Minimal Style */}
      <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
