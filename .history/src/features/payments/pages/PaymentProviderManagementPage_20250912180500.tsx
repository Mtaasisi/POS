...
const PaymentProviderManagementPage: React.FC = () => {
  ...
  const fetchProviders = useCallback(async () => {
    setIsLoading(true);
    try {
      // Use the payment provider service which handles fallback to finance_accounts
      // Use real metrics for better data accuracy
      const serviceProviders = await paymentProviderService.getPaymentProvidersWithRealMetrics();

      // Convert service providers to the expected format for this page
      const providers: PaymentProvider[] = serviceProviders.map(provider => ({
        ...
        testStatus: 'success' as const, // Default to success since we're using existing data
        performance: {
      ...
      // Don't clear existing providers on connection errors
  ...
  const handleDeleteProvider = async (providerId: string, providerName: string) => {
    ...
    try {
      // Use the payment provider service to delete
  ...
  return (
    <PageErrorBoundary pageName="Payment Provider Management" showDetails={true}>
...
export default PaymentProviderManagementPage;
