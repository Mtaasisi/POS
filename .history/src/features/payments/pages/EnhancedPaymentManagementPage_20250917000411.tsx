  ...
  const fetchPaymentData = useCallback(async (forceRefresh = false) => {
    ...
    await withErrorHandling(async () => {
      ...
      try {
        ...
        // Fetch purchase order payments (without nested supplier data to avoid foreign key issues)
        ...
        // Fetch supplier data separately to avoid foreign key issues
        ...
        const transformedPayments: PaymentData[] = [
          ...
          ...(poPayments || []).map(payment => {
            const supplier = suppliersData.find(s => s.id === payment.lats_purchase_orders?.supplier_id);
            return {
              id: payment.id,
              amount: Number(payment.amount),
              status: payment.status as 'completed' | 'pending' | 'failed',
              ...
              date: payment.payment_date,
              customer: supplier?.name || 'Unknown Supplier',
              reference: payment.reference,
