          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            {activeTab === 'tracking' && (
              <PaymentTrackingDashboard
                ...
                onNavigateToTab={(tab) => {
                  setActiveTab(tab as 'tracking' | 'providers' | 'purchase-orders' | 'transactions');
                }}
              />
            )}
            {activeTab === 'providers' && (
            ...
            {activeTab === 'transactions' && (
              <PaymentTransactions
                ...
                onEdit={(payment) => {
                  console.log('Edit transaction:', payment);
                }}
                onCopy={(payment) => {
                  console.log('Copy transaction:', payment);
                }}
                onPrint={(payment) => {
                  console.log('Print transaction:', payment);
                }}
                onShare={(payment) => {
                  console.log('Share transaction:', payment);
                }}
                onDelete={(payment) => {
                  console.log('Delete transaction:', payment);
                }}
                onFlag={(payment) => {
                  console.log('Flag transaction:', payment);
                }}
                onStar={(payment) => {
                  console.log('Star transaction:', payment);
                }}
              />
            )}
          </div>
