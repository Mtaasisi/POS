...
import { Save, ArrowLeft, X, Plus, Check, Layers, FileText } from 'lucide-react';
    ...
    try {
      ...
      if (formData.images && formData.images.length > 0 && updatedProduct) {
        ...
        const imageData = formData.images.map((image, index) => ({
          product_id: (updatedProduct && 'id' in updatedProduct) ? updatedProduct.id : productId!,
        ...
        if (tempImages.length > 0) {
          console.log('Updating temporary image records for product:', (updatedProduct && 'id' in updatedProduct) ? updatedProduct.id : productId);
