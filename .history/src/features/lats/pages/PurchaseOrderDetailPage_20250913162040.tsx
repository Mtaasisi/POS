...
const PurchaseOrderPaymentModal = lazy(() => import('../../purchase-orders/components/PurchaseOrderPaymentModal'));
...
const PurchaseOrderDetailPage: React.FC = () => {
  ...
  const handleApproveOrder = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPurchaseOrder({ ...purchaseOrder, status: 'sent', approvedAt: new Date().toISOString() });
  ...
  const handleReceive = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Purchase order received successfully');
        setPurchaseOrder({ ...purchaseOrder, status: 'received', receivedAt: new Date().toISOString() });
      } else {
        toast.error(response.message || 'Failed to receive purchase order');
      }
    } catch (error) {
  ...
  const handlePartialReceive = async (receivedItems: any[]) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPurchaseOrder({ ...purchaseOrder, items: updatedItems, status: 'partially_received' });
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      ...
      // await sendMessageToSupplier(purchaseOrder.id, message);
    } catch (error) {
  ...
  const handleAddPayment = async (paymentData: any) => {
    ...
    try {
      ...
      // await addPaymentToOrder(purchaseOrder.id, paymentData);
    } catch (error) {
      ...
      >
        ...
        <Suspense fallback={
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
            <div className="bg-white rounded-lg p-6">
              ...
              <p className="text-sm text-gray-600 mt-2">Loading shipping modal...</p>
            </div>
          </div>
        }>
            ...
            <div className="max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              <Suspense fallback={
                <div className="bg-white rounded-lg p-8 text-center">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mx-auto mb-4"></div>
                  <p className="text-sm text-gray-600">Loading shipping tracker...</p>
                </div>
              }>
                <ShippingTracker 
                  shippingInfo={purchaseOrder.shippingInfo}
        ...
        <Suspense fallback={
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
            <div className="bg-white rounded-lg p-6">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mx-auto"></div>
              <p className="text-sm text-gray-600 mt-2">Loading payment modal...</p>
            </div>
          </div>
        }>
          {purchaseOrder && (
