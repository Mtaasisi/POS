...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  const [showQualityCheckModal] = useState(false);
  const [showQualityCheckDetailsModal] = useState(false);
  const [selectedQualityCheckId] = useState<string>('');
  const [showAddToInventoryModal] = useState(false);
  ...
  const [qualityChecks] = useState<any[]>([]);
  const [qualityCheckItems] = useState<any[]>([]);
  const [isLoadingQualityCheckItems] = useState(false);
  ...
  const [showHistoryModal] = useState(false);
  const [currentItem, setCurrentItem] = useState<any>(null);
  const [inventoryStats] = useState<any[]>([]);
  ...
  const handleMarkAsReceived = async () => {
    ...
    try {
      const response = await PurchaseOrderService.markAsReceived(purchaseOrder.id, currentUser?.id || '', 'Marked as received via page UI');
      if (response.success) {
        await loadPurchaseOrder();
  ...
  const handleQualityCheckComplete = async () => {
    ...
    try {
      ...
      if (purchaseOrder.status === 'received') {
        ...
        toast.info('Quality check recorded');
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          {false && false && (
            <div className="space-y-6">
              {(purchaseOrder.status === 'shipped' || purchaseOrder.status === 'received') && purchaseOrder.shippingInfo ? (
                <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-4">
                  <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
