          ...
          <table class="items-table">
            <thead>
              <tr>
                ...
                <th style="width: 10%;">Price (TZS)</th>
                <th style="width: 10%;">Total</th>
                <th style="width: 10%;">Total (TZS)</th>
            ...
            <tbody>
              ${order.items.map(item => {
                ...
                <tr>
                  ...
                  <td style="text-align: center; color: ${pendingQty > 0 ? '#dc3545' : '#28a745'};">${pendingQty}</td>
                  <td style="text-align: right;">${formatCurrency(item.costPrice, order.currency)}</td>
                  <td style="text-align: right; font-weight: bold;">${formatCurrency(item.quantity * item.costPrice, order.currency)}</td>
...
${index + 1}. ${item.product?.name || `Product ${item.productId}`}
   ...
   Ordered: ${item.quantity} | Received: ${receivedQty} | Pending: ${pendingQty}
   Unit Price: ${formatCurrency(item.costPrice, order.currency)}
   Total: ${formatCurrency(item.quantity * item.costPrice, order.currency)}
...
5. This purchase order is subject to our standard terms and conditions.
  ...
  const formatCurrency = (amount: number, currencyCode: string = 'TZS') => {
    ...
    // Add currency symbol/code based on currency
