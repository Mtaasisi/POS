...
const POSPageOptimized: React.FC = () => {
  ...
  const { sales: dbSales, loadSales } = useInventoryStore();
  ...
  const todaysSales = useMemo(() => {
    ...
    console.log('📊 Sales Data Debug:', {
      totalSales: dbSales.length,
      todaySales: todaySales.length,
      todayTotal: total,
      today: today,
      sampleSale: dbSales[0] ? {
        id: dbSales[0].id,
        created_at: dbSales[0].created_at,
        total_amount: dbSales[0].total_amount
      } : 'No sales data'
    });
  ...
  useEffect(() => {
    const checkDailyClosureStatus = async () => {
      try {
        ...
        console.log('❌ Error in checkDailyClosureStatus:', err);
        setIsDailyClosed(false);
        setDailyClosureInfo(null);
      }
    };
