...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  const handleExportExcel = () => {
    try {
      if (purchaseOrder) {
        const workbook = generateExcelContent(purchaseOrder);
...
5. This purchase order is subject to our standard terms and conditions.
  ...
  const generateExcelContent = (order: PurchaseOrder) => {
                          ...
                          {(purchaseOrder.status === 'sent' || purchaseOrder.status === 'confirmed') && (
                            <>
                              ...
                              {console.log('🔍 PurchaseOrderDetailPage: Payment status check:', {
                                ...
                                payment_status: (purchaseOrder as any).payment_status,
                                purchaseOrder: purchaseOrder
                              })}
                              
                              {/* Show payment button if not fully paid */}
                              {(((purchaseOrder as any).payment_status === 'unpaid' || (purchaseOrder as any).payment_status === 'partial') || !(purchaseOrder as any).payment_status) && (
                                <button
                              ...
                              {((purchaseOrder as any).payment_status === 'paid') && (
                                <button
