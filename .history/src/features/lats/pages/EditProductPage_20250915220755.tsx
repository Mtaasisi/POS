...
const EditProductPageContent: React.FC = () => {
  ...
  const loadProductData = async () => {
    ...
    try {
      // First try to get product with images and explicit SKU field
      let { data: product, error } = await supabase!
        .from('lats_products')
        .select('id, name, description, category_id, sku, barcode, is_active, created_at, updated_at, lats_product_variants(*)')
        .eq('id', productId)
        .single();
      ...
      if (error) {
        ...
        const fallbackResult = await supabase!
          .from('lats_products')
          .select('id, name, description, category_id, sku, barcode, is_active, created_at, updated_at')
          .eq('id', productId)
          .single();
      ...
      if (product) {
        // If we got the product but no variants, try to fetch variants separately
        if (!product.lats_product_variants) {
          console.log('🔄 Fetching variants separately...');
        ...
        // Ensure SKU is properly handled
        const productSku = product.sku || '';
        console.log('🔍 Setting SKU in form data:', productSku);
        ...
        // Debug: Check form data after a short delay
        setTimeout(() => {
          console.log('🔍 Form data after timeout:', {
            name: formData.name,
            sku: formData.sku,
            categoryId: formData.categoryId
          });
        ...
        if (productSku) {
          console.log('✅ SKU successfully set:', productSku);
        } else {
          console.warn('⚠️ SKU is empty or null in database');
        }
        ...
        const hadVariantsOriginally = (
          ...
          (productVariants && productVariants.some((variant: any) => 
            variant.name || 
        ...
        if (import.meta.env.MODE === 'development') {
          console.log('🔍 Product variant analysis:', {
            ...
            variantCount: productVariants?.length || 0,
        ...
        // Load variants if they exist (handle both property names)
        const productVariants = product.variants || product.lats_product_variants || [];
        if (productVariants && productVariants.length > 0) {
          const processedVariants = productVariants.map((variant: any) => ({
            name: variant.name || '',
            sku: variant.sku || '',
      ...
      <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
        ...
        <GlassCard className="mb-6">
          <div className="space-y-6">
            ...
            {(() => {
              console.log('🔍 Rendering ProductInformationForm with SKU:', formData.sku);
              return (
