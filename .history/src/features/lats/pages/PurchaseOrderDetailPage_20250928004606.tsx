...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  const updatePurchaseOrderShipping = async (shippingData: any) => {
    ...
    try {
      const response = await PurchaseOrderService.updatePurchaseOrder(purchaseOrder.id, {
        ...purchaseOrder,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
                  ...
                  {inventoryStats.map((stat, index) => (
                    <div key={index} className="bg-white border border-gray-200 rounded-lg p-4 text-center">
                      ...
                      <div className="text-xs text-gray-400 mt-1">
                        {stat.total_value ? `TZS ${stat.total_value.toLocaleString()}` : ''}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {/* Enhanced Inventory Management Modals */}
        <StatusUpdateModal
