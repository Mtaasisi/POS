...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  const handleQualityCheck = async (itemId: string, status: 'passed' | 'failed' | 'attention') => {
    try {
      // Update quality check status for the item
      const response = await PurchaseOrderService.updateItemQualityCheck(itemId, status);
      
      if (response.success) {
        toast.success(`Quality check updated for item`);
  ...
  const handleCompleteQualityCheck = async () => {
    try {
      ...
      const response = await PurchaseOrderService.completeQualityCheck(purchaseOrder.id);
  ...
  const handleSendSMS = async () => {
    ...
    try {
      ...
      const response = await PurchaseOrderService.sendSMS({
  ...
  const handleDuplicateOrder = async () => {
    ...
    try {
      // Create a duplicate of the current order
      const duplicateData = {
        ...purchaseOrder,
        orderNumber: `${purchaseOrder.orderNumber}-COPY`,
        status: 'draft',
      ...
      // Remove the ID to create a new order
      delete duplicateData.id;
      
      const response = await PurchaseOrderService.createPurchaseOrder(duplicateData);
      
      if (response.success) {
        toast.success('Order duplicated successfully');
        // Navigate to the new order or refresh the list
        navigate(`/lats/purchase-orders/${response.data.id}`);
      } else {
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      const response = await PurchaseOrderService.addNote(noteData);
      ...
      if (response.success) {
        ...
        setOrderNotes(prev => [newNoteObj, ...prev]);
        setNewNote('');
  ...
  const handleBulkAction = async (action: string) => {
    ...
    try {
      ...
      switch (action) {
        case 'update_status':
          response = await PurchaseOrderService.bulkUpdateStatus(bulkSelectedItems, 'processing');
          break;
        case 'assign_location':
          response = await PurchaseOrderService.bulkAssignLocation(bulkSelectedItems, 'Warehouse A');
          break;
  ...
  const handleReturnOrder = async (returnData: any) => {
    ...
    try {
      const response = await PurchaseOrderService.returnOrder({
        purchaseOrderId: purchaseOrder.id,
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      setIsSaving(true);
      const response = await PurchaseOrderService.cancelOrder(purchaseOrder.id);
