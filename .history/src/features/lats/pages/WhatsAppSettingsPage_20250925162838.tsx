    ...
    try {
      ...
      try {
        ...
        if (error.message?.includes('ERR_CONNECTION_REFUSED') || 
          ...
          toast('Could not check instance state. Please verify your connection.');
        } else if (error.message?.includes('CORS') || error.message?.includes('Access-Control-Allow-Origin')) {
          setInstanceState({ stateInstance: 'notAuthorized' });
          toast('CORS issue detected. Please start the development proxy server with: npm run dev:proxy');
