...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  const handleCancelOrder = async () => {
    ...
    if (window.confirm('Are you sure you want to cancel this purchase order? This action cannot be undone.')) {
      try {
        const response = await PurchaseOrderService.updateStatus(purchaseOrder.id, 'cancelled');
        if (response.success) {
  ...
  const handleReturnOrder = async () => {
    ...
    if (window.confirm('Are you sure you want to return this purchase order? This will mark the order as returned.')) {
      try {
        const response = await PurchaseOrderService.updateStatus(purchaseOrder.id, 'returned');
        if (response.success) {
