  ...
  const loadPurchaseOrder = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Load items data immediately for overview display
        console.log('🔍 [PurchaseOrderDetailPage] Loading items data for overview...');
        setIsLoadingItems(true);
        try {
          const itemsResponse = await PurchaseOrderService.getPurchaseOrderItemsWithProducts(id);
          if (itemsResponse.success && itemsResponse.data) {
            setPurchaseOrderItems(itemsResponse.data);
            console.log('✅ [PurchaseOrderDetailPage] Items loaded for overview:', itemsResponse.data.length, 'items');
          } else {
            console.error('❌ Failed to load items for overview:', itemsResponse.message);
          }
        } catch (error) {
          console.error('❌ Error loading items for overview:', error);
        } finally {
