...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  const loadPurchaseOrder = useCallback(async () => {
    ...
    try {
      ...
      useEffect(() => {
        console.log('🔍 [PurchaseOrderDetailPage] URL ID parameter:', {
          id,
          type: typeof id,
          length: id?.length,
          isUUID: id ? /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id) : false
        });
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{purchaseOrder.expectedDeliveryDate ? formatDate(purchaseOrder.expectedDeliveryDate) : 'N/A'}</p>
                  </div>
                </div>
              </div>
            </div>
