...
const POcreate: React.FC = () => {
  ...
  // Get edit and duplicate parameters from URL
  const urlParams = new URLSearchParams(window.location.search);
  const editOrderId = urlParams.get('edit');
  const duplicateParam = urlParams.get('duplicate');
  const duplicateData = urlParams.get('data');
  const isEditMode = !!editOrderId;
  const isDuplicateMode = duplicateParam === 'true' && duplicateData;
  
  // Debug URL parameters
  console.log('🔍 DEBUG: URL parameters:', {
    search: window.location.search,
    editOrderId,
    editOrderIdType: typeof editOrderId,
    editOrderIdLength: editOrderId?.length,
    editOrderIdTrimmed: editOrderId?.trim(),
    isEditMode,
    duplicateParam,
    duplicateData: duplicateData ? 'present' : 'missing',
    isDuplicateMode,
  ...
  const loadPurchaseOrderForEdit = useCallback(async (orderId: string) => {
    if (!orderId) {
      console.log('❌ DEBUG: No orderId provided to loadPurchaseOrderForEdit');
      return;
    }
    
    console.log('🔍 DEBUG: Starting loadPurchaseOrderForEdit with orderId:', orderId);
    ...
    try {
      console.log('🔍 DEBUG: About to call getPurchaseOrder with ID:', orderId);
      
      const response = await getPurchaseOrder(orderId);
      
      console.log('🔍 DEBUG: getPurchaseOrder response:', response);
      
      if (response.ok && response.data) {
        const order = response.data;
        console.log('✅ DEBUG: Purchase order loaded for edit:', order);
        ...
        console.error('❌ DEBUG: Failed to load purchase order for edit:', errorMsg);
        setEditError(errorMsg);
        toast.error(errorMsg);
      }
    } catch (error) {
      const errorMsg = 'Error loading purchase order for editing';
      console.error('❌ DEBUG: Exception loading purchase order for edit:', error);
      setEditError(errorMsg);
      toast.error(errorMsg);
    } finally {
  ...
  // Load duplicate order data when duplicate parameter is present
  const loadDuplicateOrderData = useCallback(async (duplicateDataString: string) => {
    try {
      console.log('🔍 DEBUG: Loading duplicate order data...');
      const duplicatedOrder = JSON.parse(decodeURIComponent(duplicateDataString));
      console.log('✅ DEBUG: Duplicate order data parsed:', duplicatedOrder);
      
      // Load supplier data
      if (duplicatedOrder.supplierId) {
        const supplier = suppliers.find(s => s.id === duplicatedOrder.supplierId);
        if (supplier) {
          setSelectedSupplier(supplier);
        }
      }
      
      // Load cart items from duplicated order
      if (duplicatedOrder.items && duplicatedOrder.items.length > 0) {
        const cartItems: PurchaseCartItem[] = duplicatedOrder.items.map((item: any) => ({
          id: `${item.productId}-${item.variantId}-${Date.now()}`,
          productId: item.productId,
          variantId: item.variantId,
          name: item.product?.name || 'Unknown Product',
      ...
      // Set other order details
      if (duplicatedOrder.expectedDelivery) {
        setExpectedDelivery(duplicatedOrder.expectedDelivery);
        setShippingInfo(prev => ({
          ...prev,
          expectedDelivery: duplicatedOrder.expectedDelivery
        }));
      }
      
      if (duplicatedOrder.notes) {
        setPurchaseOrderNotes(duplicatedOrder.notes);
      }
      
      if (duplicatedOrder.paymentTerms) {
        setPaymentTerms(duplicatedOrder.paymentTerms);
      }
      
      if (duplicatedOrder.currency) {
        const currency = SUPPORTED_CURRENCIES.find(c => c.code === duplicatedOrder.currency);
        if (currency) {
          setSelectedCurrency(currency);
        }
      }
      
      if (duplicatedOrder.status) {
        setPurchaseOrderStatus(duplicatedOrder.status);
      }
      
      console.log('✅ DEBUG: Duplicate order data loaded successfully');
      toast.success('Duplicate order data loaded');
      
    } catch (error) {
      console.error('❌ DEBUG: Error loading duplicate order data:', error);
      toast.error('Failed to load duplicate order data');
    }
  }, [suppliers]);
  ...
  useEffect(() => {
    console.log('🔍 DEBUG: Edit mode useEffect triggered:', {
      editOrderId,
      suppliersLength: suppliers.length,
      isEditMode,
      hasEditOrderId: !!editOrderId,
      isDuplicateMode,
      duplicateData: duplicateData ? 'present' : 'missing'
    });
    
    // Don't run edit mode if we're in duplicate mode
    if (isDuplicateMode) {
      console.log('🔍 DEBUG: Skipping edit mode because we are in duplicate mode');
      return;
    }
    
    if (editOrderId && suppliers.length > 0) {
      console.log('🔍 DEBUG: Calling loadPurchaseOrderForEdit with ID:', editOrderId);
      loadPurchaseOrderForEdit(editOrderId);
    } else if (editOrderId && suppliers.length === 0) {
      console.log('🔍 DEBUG: Waiting for suppliers to load before editing...');
    }
  }, [editOrderId, suppliers.length, loadPurchaseOrderForEdit, isDuplicateMode, duplicateData]);

  // Load duplicate order data when duplicate parameter is present
  useEffect(() => {
    console.log('🔍 DEBUG: Duplicate mode useEffect triggered:', {
      isDuplicateMode,
      duplicateData: duplicateData ? 'present' : 'missing',
      suppliersLength: suppliers.length
    });
    
    if (isDuplicateMode && duplicateData && suppliers.length > 0) {
      console.log('🔍 DEBUG: Calling loadDuplicateOrderData...');
      loadDuplicateOrderData(duplicateData);
    } else if (isDuplicateMode && suppliers.length === 0) {
      console.log('🔍 DEBUG: Waiting for suppliers to load before duplicating...');
    }
  }, [isDuplicateMode, duplicateData, suppliers.length, loadDuplicateOrderData]);
  ...
  const handleAddToPurchaseCart = useCallback((product: any, variant?: any, quantity: number = 1) => {
    console.log('🔍 DEBUG: Adding product to cart in edit mode:', {
      isEditMode,
  ...
  const handleClearCart = useCallback(async () => {
    if (purchaseCartItems.length > 0) {
      const confirmed = await confirm('Are you sure you want to clear the purchase cart?');
      if (confirmed) {
        setPurchaseCartItems([]);
      }
    }
  }, [purchaseCartItems.length, confirm]);
  ...
  return (
    <div className="min-h-screen">
                ...
                {isEditMode && isLoadingEditData && (
                  <div className="p-4 bg-purple-50 border border-purple-200 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-purple-800 font-medium">Loading purchase order for editing...</span>
                    </div>
                  </div>
                )}
      ...
      {/* Shipping Configuration Modal - TODO: Implement ShippingConfigurationModal component */}
      {showShippingModal && (
      ...
      {/* Draft Management Modal - TODO: Implement PurchaseOrderDraftModal component */}
      {showDraftModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4">
