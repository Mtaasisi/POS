...
const PurchaseOrdersPage: React.FC = () => {
  ...
  // Debug: Log purchase orders data when it changes
  useEffect(() => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        TSh {filteredOrders.reduce((sum, order) => {
                          const amount = order.totalAmount || 
                            (order.items && order.items.length > 0 ? 
                              order.items.reduce((itemSum, item) => itemSum + (item.quantity * item.costPrice), 0) : 0);
                          return sum + amount;
                        }, 0).toLocaleString()}
                      </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/30 overflow-hidden">
            {/* Table Header */}
            <div className="bg-gradient-to-r from-gray-50/80 to-gray-100/80 backdrop-blur-sm border-b border-gray-200/50 px-6 py-4">
            ...
            <div className="divide-y divide-gray-100/50">
              {filteredOrders.map((order, index) => (
                <div key={order.id} className="group hover:bg-gradient-to-r hover:from-blue-50/30 hover:to-purple-50/30 transition-all duration-300">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-gray-900">
                            {order.totalAmount ? formatCurrency(order.totalAmount, order.currency) : 
                             order.items && order.items.length > 0 ? 
                             formatCurrency(order.items.reduce((sum, item) => sum + (item.quantity * item.costPrice), 0), order.currency) :
                             'TSh 0'}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                    ...
                    <div className="col-span-1">
                      <div className="flex items-center justify-center gap-1">
                        <button
                          ...
                          className="p-2 bg-blue-500 hover:bg-blue-600 text-white rounded-xl transition-all duration-300 transform hover:scale-110 shadow-lg"
                          title="View Details"
                        >
                        ...
                        {order.status === 'draft' && (
                          <>
                            <button
                              ...
                              className="p-2 bg-gray-500 hover:bg-gray-600 text-white rounded-xl transition-all duration-300 transform hover:scale-110 shadow-lg"
                              title="Edit Order"
                            >
                            ...
                            <button
                              ...
                              className="p-2 bg-red-500 hover:bg-red-600 text-white rounded-xl transition-all duration-300 transform hover:scale-110 shadow-lg"
                              title="Delete Order"
                            >
