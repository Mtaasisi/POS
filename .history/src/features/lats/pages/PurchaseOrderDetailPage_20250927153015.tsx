...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  const handleTabChange = useCallback(async (tabName: string, forceRefresh = false) => {
    ...
    if (!loadedTabs.has(tabName) || forceRefresh) {
      ...
      switch (tabName) {
        case 'items':
          // Load items with product details from database
          if (purchaseOrder) {
            try {
              console.log('🔍 [PurchaseOrderDetailPage] Loading items with products for PO:', purchaseOrder.id);
              const itemsData = await PurchaseOrderService.getPurchaseOrderItemsWithProducts(purchaseOrder.id);
              if (itemsData.success) {
                // Update the purchase order items with detailed product information
                const updatedPurchaseOrder = {
                  ...purchaseOrder,
                  items: itemsData.data || []
                };
                setPurchaseOrder(updatedPurchaseOrder);
                console.log('✅ [PurchaseOrderDetailPage] Items loaded with product details:', itemsData.data?.length || 0, 'items');
              } else {
                console.error('❌ Failed to load items with product details:', itemsData.message);
                toast.error('Failed to load items details');
              }
            } catch (error) {
              console.error('❌ Failed to load items with product details:', error);
              toast.error('Failed to load items details');
            }
          }
          break;
