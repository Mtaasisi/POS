...
const InventorySparePartsPage: React.FC = () => {
  ...
  const filteredSpareParts = React.useMemo(() => {
    console.log('🔍 [DEBUG] Filtering spare parts:', {
      totalSpareParts: spareParts.length,
      searchTerm,
      selectedCategory,
      stockFilter,
      sparePartsSample: spareParts.slice(0, 3).map(p => ({ id: p.id, name: p.name, part_number: p.part_number }))
    });
    ...
    const filtered = spareParts.filter(part => {
      ...
      const passesAllFilters = matchesSearch && matchesCategory && matchesStock;
      
      // Debug specific part if it's the one we're looking for
      if (part.name === 'macbook A1237' || part.part_number === '893284') {
        console.log('🔍 [DEBUG] Filtering macbook A1237:', {
    ...
    try {
      ...
      if (editingSparePart) {
        ...
        if (response.ok) {
          ...
          console.log('🔍 [DEBUG] Refreshing spare parts after update...');
          await loadSpareParts();
          console.log('🔍 [DEBUG] Spare parts refreshed after update');
        } else {
        ...
        if (response.ok) {
          ...
          setShowSparePartForm(false);
          console.log('🔍 [DEBUG] Refreshing spare parts after creation...');
          await loadSpareParts();
          console.log('🔍 [DEBUG] Spare parts refreshed after creation');
        } else {
