...
const SalesReportsPage: React.FC = () => {
  ...
  const fetchSales = async () => {
    try {
      ...
      // FIXED QUERY - Simplified to avoid 400 errors
      ...
      if (salesError) {
        console.warn('❌ Complex sales query failed, trying simpler query:', salesError.message);
        
        // Fallback to simpler query without joins
        const { data: simpleSalesData, error: simpleSalesError } = await supabase
          .from('lats_sales')
          .select(`
            id,
            sale_number,
        ...
        if (simpleSalesError) {
          console.error('❌ Simple sales query also failed:', simpleSalesError);
          setError(`Failed to load sales data: ${simpleSalesError.message}`);
          return;
        }

        // Use the simple data without sale items
        const salesData = simpleSalesData;
        console.log(`✅ Loaded ${salesData?.length || 0} total sales from database (without sale items)`);
      } else {
  ...
  const fetchAllSales = async () => {
    try {
      ...
      // Fetch user names for cashier display
      const uniqueUserIds = [...new Set((salesData || []).map(sale => sale.created_by).filter(Boolean))];
      // Filter out invalid UUIDs (like 'care') to prevent 400 errors
      const validUserIds = uniqueUserIds.filter(id => {
        const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
        return uuidRegex.test(id);
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-300 bg-white/80 backdrop-blur-sm transition-all duration-200"
          ...
          <div className="p-6">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  <strong>Sales Loaded:</strong>
                  <div className="ml-2 text-xs">
                    <div>Total Sales: {sales.length}</div>
                    <div>Filtered Sales: {sales.filter(sale => {
                      const saleDate = new Date(sale.created_at);
                      const now = new Date();
                      const startDate = new Date();
                ...
                <div>
                  ...
                  <div className="ml-2 text-xs">
                    <div>With Subtotal: {sales.filter(s => s.subtotal > 0).length}</div>
                    <div>With Discount: {sales.filter(s => s.discount_amount > 0).length}</div>
                    <div>With Tax: {sales.filter(s => s.tax > 0).length}</div>
                  </div>
                </div>
                <div>
                  <strong>Sample Sale Data:</strong>
                  <div className="ml-2 text-xs">
                    {sales.length > 0 && (
                      <>
                        <div>Sale #{sales[0].sale_number}</div>
                        <div>Subtotal: {sales[0].subtotal || 'null'}</div>
                        <div>Discount: {sales[0].discount_amount || 'null'}</div>
                        <div>Total: {sales[0].total_amount || 'null'}</div>
                      </>
                    )}
                  </div>
                </div>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-6 text-sm text-gray-600">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span>{sale.customer_name || (sale.customer_id ? `Customer: ${sale.customer_id.slice(0, 8)}...` : 'Walk-in')}</span>
                          </div>
                          <div className="flex items-center gap-1">
                      ...
                      <div className="text-right">
                        ...
                        {sale.discount_amount > 0 && (
                          <div className="text-xs text-red-600 font-medium">
                            -{formatMoney(sale.discount_amount)} discount
                          </div>
                        )}
