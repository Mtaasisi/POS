...
const POSPageOptimized: React.FC = () => {
  ...
  const loadAllCustomers = async () => {
    try {
      ...
      console.log('🔄 Loading entire customer database for POS search...', new Date().toISOString());
      console.log('🔍 About to call fetchAllCustomersSimple function');
      const result = await fetchAllCustomersSimple();
      console.log('🔍 fetchAllCustomersSimple returned:', result?.length, 'customers');
      if (result && Array.isArray(result)) {
        setAllCustomers(result);
        console.log(`✅ Loaded ${result.length} customers from entire database for POS search`);
        
        // Debug: Show first few customer names
        if (result.length > 0) {
          console.log(`🔍 First 5 customers loaded:`, result.slice(0, 5).map(c => ({
            name: c.name,
            phone: c.phone,
            email: c.email
          })));
        ...
        console.log('⚠️ No customers returned from database');
        setAllCustomers([]);
      }
    } catch (error) {
      console.error('❌ Error loading all customers from database:', error);
      // Fallback to context customers if fetch fails
      setAllCustomers(customers);
      console.log(`⚠️ Using fallback: ${customers.length} customers from context`);
    } finally {
        ...
        const errors = results
          .map((result, index) => {
            if (result.status === 'rejected') {
              const dataTypes = ['products', 'categories', 'suppliers', 'sales', 'customers'];
  ...
  useEffect(() => {
    if (debouncedSearch.trim()) {
      handleCustomerSearch(debouncedSearch);
    } else {
  ...
  // Customer search functionality - searches through entire customer database
  const handleCustomerSearch = async (query: string) => {
    if (query.trim()) {
      console.log(`🔍 Searching entire customer database with query: "${query}"`);
      
      // Always use allCustomers (loaded from entire database) for comprehensive search
      let customersToSearch = allCustomers;
      
      // If allCustomers is empty, try to reload from database
      if (customersToSearch.length === 0) {
        console.log(`⚠️ No customers loaded, fetching from database...`);
        try {
          const result = await fetchAllCustomersSimple();
          if (result && Array.isArray(result)) {
            setAllCustomers(result);
            customersToSearch = result;
            console.log(`✅ Loaded ${result.length} customers from database for search`);
          }
        } catch (error) {
          console.error('❌ Failed to load customers from database:', error);
          // Fallback to context customers
          customersToSearch = customers;
        }
      }
      
      console.log(`📊 Searching through ${customersToSearch.length} customers from entire database`);
      
      // Enhanced search - search in multiple fields
      const filteredCustomers = customersToSearch.filter(customer => {
        ...
        const cityMatch = customer.city?.toLowerCase().includes(searchTerm);
        const tagMatch = customer.customerTag?.toLowerCase().includes(searchTerm);
        const referralMatch = customer.referralSource?.toLowerCase().includes(searchTerm);
        
        // Debug logging for first few customers
        if (customersToSearch.indexOf(customer) < 3) {
          console.log(`🔍 Customer ${customersToSearch.indexOf(customer) + 1}:`, {
            name: customer.name,
            ...
            matches: { nameMatch, phoneMatch, emailMatch, cityMatch, tagMatch, referralMatch }
          });
        }
        
        return nameMatch || phoneMatch || emailMatch || cityMatch || tagMatch || referralMatch;
      });
      
      console.log(`✅ Found ${filteredCustomers.length} matching customers from entire database`);
      
      // If no results, show some sample customer names for debugging
      if (filteredCustomers.length === 0 && customersToSearch.length > 0) {
        console.log(`🔍 Sample customer names for debugging:`, 
          customersToSearch.slice(0, 5).map(c => c.name).filter(Boolean)
        );
        
        // Special debug for "samuel" search
        if (query.toLowerCase().includes('samuel')) {
          console.log(`🔍 Looking for customers with "samuel" in name:`, 
            customersToSearch.filter(c => c.name?.toLowerCase().includes('samuel')).map(c => ({
              name: c.name,
              phone: c.phone,
              email: c.email
            }))
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              <div className="mb-6">
                <div className="relative mb-4">
                  ...
                  <input
                    type="text"
                    placeholder="Search entire customer database by name, phone, email, city, or tags..."
              ...
              {customerSearchQuery.trim() && (
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">
                    ...
                    {loadingAllCustomers && <span className="text-sm text-blue-600 ml-2">Loading entire database...</span>}
                  </h3>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-sm text-gray-600">
                                ...
                                {customer.city && <span className="ml-2">🏙️ {customer.city}</span>}
                                {customer.customerTag && <span className="ml-2">🏷️ {customer.customerTag}</span>}
                              </div>
                            </div>
