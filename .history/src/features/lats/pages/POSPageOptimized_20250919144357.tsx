...
const POSPageOptimized: React.FC = () => {
  ...
  // Customer search functionality - now searches through all customers
  const handleCustomerSearch = (query: string) => {
    if (query.trim()) {
      // Use allCustomers if available, otherwise fallback to context customers
      const customersToSearch = allCustomers.length > 0 ? allCustomers : customers;
      console.log(`🔍 Searching customers with query: "${query}"`);
      console.log(`📊 Searching through ${customersToSearch.length} customers`);
