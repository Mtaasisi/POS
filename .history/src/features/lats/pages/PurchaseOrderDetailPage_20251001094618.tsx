    ...
    try {
      ...
      if (response.ok && response.data) {
        ...
        if (currentUser?.id && response.data?.id) {
          console.log('🔄 Checking if order status needs correction...');
          try {
            ...
            if (fixResult.statusChanged) {
              ...
              toast.success(`Order status updated: ${fixResult.message}`);
              
              // Reload the order to get the updated status
              const { getPurchaseOrder: getPO } = useInventoryStore.getState();
              const updatedResponse = await getPO(trimmedId);
              if (updatedResponse.ok && updatedResponse.data) {
                console.log('✅ Reloaded purchase order with corrected status');
                setPurchaseOrder(updatedResponse.data);
              }
            } else {
              console.log('✅ Order status is correct, no changes needed');
            }
          } catch (statusFixError) {
            console.warn('⚠️ Failed to check/fix order status:', statusFixError);
            // Don't fail the entire operation for status fix issues
          }
        }
      } else {
...
>>>>>>> 2e74e4e8 (Complete LATS CHANCE POS System Backup - January 2025)
          break;
  ...
  const handlePartialReceive = async (receivedItems: any[]) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error('❌ Failed to update received quantities:', result.message);
