...
import { 
  ...
  Clock, CheckSquare, XSquare, Send, CheckCircle, XCircle, CreditCard, Truck
} from 'lucide-react';
...
const PurchaseOrdersPage: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'draft' | 'sent' | 'confirmed' | 'processing' | 'shipping' | 'shipped' | 'partial_received' | 'received' | 'cancelled'>('all');
  ...
  // Show success message when orders are loaded (only on initial load)
  useEffect(() => {
    if (purchaseOrders && purchaseOrders.length > 0 && !isLoading && !lastUpdated) {
      console.log('🔍 [PurchaseOrdersPage] Initial load - Purchase orders data:', purchaseOrders.length, 'orders');
      
      // Show success message for initial load only
      toast.success(`Loaded ${purchaseOrders.length} purchase orders`, {
        duration: 2000,
        position: 'top-right'
      });
    }
  }, [purchaseOrders, isLoading, lastUpdated]);
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'shipping': return 'text-cyan-600 bg-cyan-100';
      case 'shipped': return 'text-teal-600 bg-teal-100';
  ...
  const getSmartActionButtons = (order: any) => {
    ...
    switch (order.status) {
      ...
      case 'received':
        // Step 5: Received - Order completed
        break;
