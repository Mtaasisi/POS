...
${currentUser?.name || 'Purchase Manager'}`;
  ...
  const handleReturnOrder = async () => {
    ...
    if (window.confirm('Are you sure you want to return this purchase order? This will mark the order as returned.')) {
      try {
        const response = await PurchaseOrderService.updateOrderStatus(
          purchaseOrder.id, 
          'returned', 
  ...
  const handleArchiveOrder = async () => {
    ...
    if (window.confirm('Are you sure you want to archive this purchase order? Archived orders are moved to a separate section.')) {
      setIsArchiving(true);
      try {
        const response = await PurchaseOrderService.updateOrderStatus(
          purchaseOrder.id, 
