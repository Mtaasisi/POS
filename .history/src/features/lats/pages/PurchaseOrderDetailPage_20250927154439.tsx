    ...
    if (!loadedTabs.has(tabName) || forceRefresh) {
      ...
      switch (tabName) {
        ...
        case 'history':
          ...
          break;
        case 'items':
          // Load purchase order items from database using migrated function
          if (purchaseOrder) {
            setIsLoadingPurchaseOrderItems(true);
            try {
              console.log('🔍 [PurchaseOrderDetailPage] Loading purchase order items for PO:', purchaseOrder.id);
