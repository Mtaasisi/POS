...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  const handleDuplicateOrder = () => {
    console.log('🔄 DEBUG: handleDuplicateOrder called');
    console.log('🔄 DEBUG: purchaseOrder:', purchaseOrder);
    
    if (!purchaseOrder) {
      console.log('❌ DEBUG: No purchase order to duplicate');
      return;
    }
    
    console.log('🔄 DEBUG: Original purchase order ID:', purchaseOrder.id);
    console.log('🔄 DEBUG: Original purchase order type:', typeof purchaseOrder.id);
    
    // Create a deep copy of the current order with all IDs cleared
    const duplicatedOrder = {
      ...
      totalPaid: 0,
      receivedQuantity: 0,
      // Reset item quantities and clear any item IDs
      items: purchaseOrder.items.map(item => ({
        ...item,
        id: undefined, // Clear item ID
        receivedQuantity: 0,
        status: 'pending'
      })),
      // Clear any other potential ID references
      supplierId: purchaseOrder.supplierId, // Keep supplier ID as it's needed
      // Clear any nested IDs that might exist
      // Note: payments array doesn't exist on PurchaseOrder type, so we skip this
      // Clear any other ID fields
      createdBy: currentUser?.id,
      updatedBy: currentUser?.id
    };
    
    console.log('🔄 DEBUG: Duplicated order created:');
    console.log('🔄 DEBUG: - ID:', duplicatedOrder.id);
    console.log('🔄 DEBUG: - Order Number:', duplicatedOrder.orderNumber);
    console.log('🔄 DEBUG: - Status:', duplicatedOrder.status);
    console.log('🔄 DEBUG: - Items count:', duplicatedOrder.items?.length);
    console.log('🔄 DEBUG: - Supplier ID:', duplicatedOrder.supplierId);
    ...
    const createUrl = `/lats/purchase-orders/create?${queryParams.toString()}`;
    console.log('🔄 DEBUG: Opening URL:', createUrl);
    console.log('🔄 DEBUG: Query params:', Object.fromEntries(queryParams.entries()));
