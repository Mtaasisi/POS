...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  console.log('🔍 [PurchaseOrderDetailPage] Component props and params:', {
    editMode,
    id,
  ...
  const loadPurchaseOrder = useCallback(async () => {
    if (!id) {
      console.error('❌ [PurchaseOrderDetailPage] No ID provided');
      setIsLoadingOrder(false);
      return;
    }
    
    console.log('🔍 [PurchaseOrderDetailPage] Starting to load purchase order:', {
      id,
      type: typeof id,
      length: id.length
    });
    ...
    try {
      ...
      console.log('🔍 [PurchaseOrderDetailPage] Getting store state...');
      const storeState = useInventoryStore.getState();
      console.log('🔍 [PurchaseOrderDetailPage] Store state retrieved:', {
        hasGetPurchaseOrder: typeof storeState.getPurchaseOrder === 'function'
      });
      
      const { getPurchaseOrder: getPO } = storeState;
      console.log('🔍 [PurchaseOrderDetailPage] Calling getPurchaseOrder with ID:', id);
      
      // Test database connectivity first
      console.log('🔍 [PurchaseOrderDetailPage] Testing database connectivity...');
      try {
        const testResponse = await supabase.from('lats_purchase_orders').select('id').limit(1);
        console.log('🔍 [PurchaseOrderDetailPage] Database connectivity test:', {
          data: testResponse.data,
          error: testResponse.error,
          count: testResponse.count
        });
      } catch (testError) {
        console.error('❌ [PurchaseOrderDetailPage] Database connectivity test failed:', testError);
      }
      
      const response = await getPO(id);
      console.log('🔍 [PurchaseOrderDetailPage] getPurchaseOrder response:', {
        ok: response.ok,
      ...
      if (response.ok && response.data) {
        console.log('🔍 [PurchaseOrderDetailPage] Purchase order data received:', {
          id: response.data.id,
        ...
        // Use the items data from the main purchase order response
        console.log('🔍 [PurchaseOrderDetailPage] Using items data from main response...');
        if (response.data.items && response.data.items.length > 0) {
          setPurchaseOrderItems(response.data.items);
          console.log('✅ [PurchaseOrderDetailPage] Items loaded for overview:', response.data.items.length, 'items');
        } else {
          console.log('ℹ️ No items found in main response, setting empty array');
        ...
        const errorMessage = response.message || 'Failed to load purchase order';
      ...
      const errorMessage = `Failed to load purchase order: ${error instanceof Error ? error.message : 'Unknown error'}`;
      setLoadingError(errorMessage);
      toast.error(errorMessage);
      // Don't navigate away immediately, let user see the error
      setTimeout(() => {
      ...
      console.log('🔍 [PurchaseOrderDetailPage] Setting isLoadingOrder to false');
      setIsLoadingOrder(false);
    }
  }, [id, navigate]);
  ...
  useEffect(() => {
    console.log('🔍 PurchaseOrderDetailPage: Component mounted with ID:', id);
    if (id) {
      loadPurchaseOrder();
      
      // Safety timeout to prevent infinite loading
      const timeout = setTimeout(() => {
        console.log('⚠️ [PurchaseOrderDetailPage] Loading timeout reached, setting isLoadingOrder to false');
        setIsLoadingOrder(false);
        setLoadingError('Loading timeout: The request took too long to complete. Please check your connection and try again.');
        toast.error('Loading timeout: Please check your connection and try again');
      }, 15000); // 15 second timeout
      
      return () => clearTimeout(timeout);
    } else {
      console.error('❌ PurchaseOrderDetailPage: No ID provided in URL params');
      setIsLoadingOrder(false);
    }
    ...
    if (!loadedTabs.has(tabName) || forceRefresh) {
      ...
      switch (tabName) {
        ...
        case 'history':
          ...
          break;
        case 'items':
          // Use existing items data and load received items from database
          if (purchaseOrder) {
            setIsLoadingReceivedItems(true);
            try {
              console.log('🔍 [PurchaseOrderDetailPage] Loading received items for PO:', purchaseOrder.id);
              
              // Use existing items data from purchase order
              if (purchaseOrder.items && purchaseOrder.items.length > 0) {
                setPurchaseOrderItems(purchaseOrder.items);
                console.log('✅ [PurchaseOrderDetailPage] Using existing items data:', purchaseOrder.items.length, 'items');
              }
  ...
  const handleExportExcel = async () => {
    try {
      if (purchaseOrder) {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `Purchase_Order_${purchaseOrder.orderNumber}_${new Date().toISOString().split('T')[0]}.xlsx`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
          ...
          <div class="payment-info">
            ...
            <div class="payment-details">
              <div class="payment-column">
                ...
                <p><strong>Payment Due Date:</strong> ${getPaymentDueDate(order.orderDate || order.createdAt, order.paymentTerms)}</p>
              ...
              <div class="payment-column">
                ...
                <p><strong>Payment Method:</strong> ${order.paymentMethod || 'TBD'}</p>
              </div>
            </div>
          </div>
...
5. This purchase order is subject to our standard terms and conditions.
  ...
  if (!purchaseOrder && !isLoadingOrder) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-900 mb-2">
              {loadingError ? 'Loading Error' : 'Purchase Order Not Found'}
            </h2>
            <p className="text-gray-600 mb-4">
              {loadingError || 'The purchase order you\'re looking for doesn\'t exist.'}
            </p>
            {loadingError && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                <p className="text-sm text-red-700">
                  <strong>Error Details:</strong> {loadingError}
                </p>
              </div>
            )}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Main Content */}
      <div className="p-6">
        <div className="bg-white rounded-2xl shadow-xl max-w-7xl mx-auto overflow-hidden">
        {/* Minimal Header */}
              ...
              <div className="mb-8">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 rounded-xl p-5 shadow-sm">
                    ...
                    <div className="text-xl font-bold text-emerald-900">{formatCurrency(purchaseOrder?.totalAmount || 0, purchaseOrder?.currency || 'TZS')}</div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-5 shadow-sm">
                    ...
                    <div className="text-xl font-bold text-blue-900">{purchaseOrderItems?.length || purchaseOrder?.items?.length || 0}</div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-200 rounded-xl p-5 shadow-sm">
                    ...
                    <div className="text-xl font-bold text-orange-900">
                      {purchaseOrderItems?.reduce((sum, item) => sum + (item.quantity || 0), 0) || 
                  ...
                  <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-xl p-5 shadow-sm">
                    ...
                    <div className="text-xl font-bold text-purple-900">
                      {purchaseOrderItems?.reduce((sum, item) => sum + (item.received_quantity || 0), 0) || 
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="pt-3 border-t border-gray-100">
                        <div className="text-center">
                          ...
                          <div className="text-2xl font-bold text-green-600">
                            {formatCurrency(purchaseOrder?.totalAmount || 0, purchaseOrder?.currency || 'TZS')}
                          </div>
            ...
            <div className="space-y-6">
              {isLoadingReceivedItems ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500 mt-2">Items will appear here once they are received and added to inventory</p>
                </div>
              ) : (
                <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-4">
                  <div className="flex items-center justify-between pb-2 border-b border-gray-100">
