...
const POSPageOptimized: React.FC = () => {
  ...
  // Inventory alert preferences from database
  const {
  ...
  const checkLowStock = () => {
    // Use database threshold if available, otherwise fallback to local state
    const threshold = dbLowStockThreshold || lowStockThreshold || 5;
    ...
    if (lowStockProducts.length > 0) {
      ...
      // Check if alerts are enabled and not permanently disabled
      if (areAlertsPermanentlyDisabled || !isLowStockAlertsEnabled) {
        return; // Don't show alerts if disabled
      }

      // Show notification instead of blocking modal if dismissed today
      if (alertsDismissedToday) {
        if (showAlertsAsNotification) {
          setShowAlertsAsNotification(true);
          // Auto-hide notification after configured seconds
          setTimeout(() => {
            setShowAlertsAsNotification(false);
          }, (autoHideNotificationSeconds || 5) * 1000);
        }
      } else if (!alertsDismissed || hasNewAlerts) {
        if (showAlertsAsModal) {
  ...
  const handleCloseInventoryAlerts = async (dismissForToday = false) => {
    setShowInventoryAlerts(false);
    setAlertsDismissed(true);
    
    if (dismissForToday) {
      const success = await dismissAlertsForToday();
      if (success) {
        setAlertsDismissedToday(true);
      }
    }
    
    // Log alert history for analytics
      ...
      for (const alert of inventoryAlerts) {
        await logAlertHistory(
          ...
          alert.productId,
          {
            ...
            dismissedForToday: dismissForToday
          }
        );
      }
    }
  ...
  // Check for dismissed alerts today
  useEffect(() => {
