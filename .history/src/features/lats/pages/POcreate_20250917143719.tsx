...
const POcreate: React.FC = () => {
  ...
  const loadPurchaseOrderForEdit = useCallback(async (orderId: string) => {
    ...
    try {
      ...
      const response = await provider.getPurchaseOrder(orderId);
      ...
      if (response.ok && response.data) {
        ...
        toast.success(`Loaded purchase order ${order.orderNumber} for editing`);
      } else {
        toast.error('Failed to load purchase order for editing');
      }
    } catch (error) {
      console.error('Error loading purchase order for editing:', error);
      toast.error('Error loading purchase order for editing');
    } finally {
      setIsLoadingEditData(false);
    }
  }, [suppliers]);

  // Load purchase order data when in edit mode
  useEffect(() => {
    if (isEditMode && editOrderId && suppliers.length > 0) {
      loadPurchaseOrderForEdit(editOrderId);
    }
  }, [isEditMode, editOrderId, suppliers.length, loadPurchaseOrderForEdit]);
  ...
  const handleCreatePurchaseOrder = useCallback(async () => {
    ...
    try {
      const orderNumber = isEditMode ? editingPurchaseOrder?.orderNumber : generatePONumber();
      
      const purchaseOrderData = {
        ...(isEditMode && { id: editOrderId }), // Include ID for updates
      ...
      if (result.ok) {
        console.log(`✅ DEBUG: Purchase order ${isEditMode ? 'updated' : 'created'} successfully!`);
        console.log(`✅ DEBUG: ${isEditMode ? 'Updated' : 'Created'} purchase order data:`, result.data);
        ...
        if (isEditMode) {
          // In edit mode, navigate back to the purchase order detail page
          navigate(`/lats/purchase-orders/${editOrderId}`);
        } else {
        ...
        console.error(`❌ DEBUG: Purchase order ${isEditMode ? 'update' : 'creation'} failed:`, result);
      ...
      console.error(`❌ DEBUG: Exception during purchase order ${isEditMode ? 'update' : 'creation'}:`, error);
      console.error('❌ DEBUG: Error details:', {
        message: error.message,
        stack: error.stack,
        name: error.name
      });
      toast.error(`Error ${isEditMode ? 'updating' : 'creating'} purchase order. Please try again.`);
    } finally {
      setIsCreatingPO(false);
    }
  }, [selectedSupplier, purchaseCartItems, shippingInfo.expectedDelivery, exchangeRates, purchaseOrderNotes, purchaseOrderStatus, selectedCurrency, createPurchaseOrder, updatePurchaseOrder, navigate, isEditMode, editOrderId, editingPurchaseOrder]);
                  ...
                  <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-blue-800 font-medium">
                        {isLoadingEditData ? 'Loading purchase order for editing...' : 'Loading products...'}
                      </span>
                    </div>
                  </div>
                )}
              ...
              <div className="flex items-center gap-3 mb-6 flex-shrink-0">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {isEditMode 
                      ? `Editing PO ${editingPurchaseOrder?.orderNumber || editOrderId} • ${purchaseCartItems.length} items`
                      : `${purchaseCartItems.length} items to purchase`
                    }
                  </p>
                </div>
                {isEditMode && (
                ...
                <>
                  ...
                  <div className="mt-4 space-y-3 flex-shrink-0">
                    ...
                    <GlassButton
                      ...
                      disabled={!selectedSupplier || purchaseCartItems.length === 0 || isCreatingPO || isLoadingEditData}
                    >
                      {isCreatingPO 
