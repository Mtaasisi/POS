...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  const handleShippingAssigned = async (shippingData: any) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Navigate to shipping details if shipping ID is returned
        if (response.data?.shippingId) {
          console.log('🚚 [PurchaseOrderDetailPage] Navigating to shipping details:', response.data.shippingId);
          navigate(`/lats/shipping/${response.data.shippingId}`);
        }
      } else {
  ...
  const handlePartialReceive = async (receivedItems: any[]) => {
    ...
    try {
      ...
      // Use the PurchaseOrderService to update received quantities
      const success = await PurchaseOrderService.updateReceivedQuantities(
        purchaseOrder.id,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <ShippingAssignmentModal
          ...
          purchaseOrder={purchaseOrder!}
          agents={shippingAgents && shippingAgents.length > 0 ? shippingAgents : [
            // Fallback agents if none are loaded
            {
              id: 'default-dhl',
              name: 'DHL Express',
          ...
          settings={{
            ...
            autoAssignAgents: true // Enable auto-assignment
          }}
