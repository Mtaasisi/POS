...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  }, [purchaseOrder, isEditing, handleSave, handleApproveOrder, handleDuplicateOrder, handlePrintOrder, handleSendWhatsApp, handleSendSMS, refreshPurchaseOrderData]);
        ...
        <div className={`flex items-center justify-between p-6 border-b ${isEditing ? 'border-blue-200 bg-blue-50' : 'border-gray-100'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-full text-sm font-semibold ${getStatusColor(purchaseOrder.status)}`}>
                ...
                {purchaseOrder.paymentStatus && (
                  <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                    purchaseOrder.paymentStatus === 'paid' ? 'bg-green-100 text-green-800' :
                    purchaseOrder.paymentStatus === 'partial' ? 'bg-yellow-100 text-yellow-800' :
                    purchaseOrder.paymentStatus === 'unpaid' ? 'bg-red-100 text-red-800' :
                    'bg-gray-100 text-gray-800'
                  }`}>
                    <CreditCard className="w-3 h-3" />
                    <span className="capitalize">{purchaseOrder.paymentStatus}</span>
                  </div>
                )}
                
                <span className="text-sm text-gray-500">Created {formatDate(purchaseOrder.createdAt)}</span>
              </div>
                {lastRefreshTime && (
