...
const POSPageOptimized: React.FC = () => {
  ...
  const products = useMemo(() => {
    console.log('🔍 [POSPageOptimized] Products transformation:', {
      dbProductsCount: dbProducts.length,
      categoriesCount: categories.length,
      dbProducts: dbProducts.slice(0, 3).map(p => ({ id: p.id, name: p.name, categoryId: p.categoryId }))
    });
    
    if (dbProducts.length === 0) {
      console.log('🔍 [POSPageOptimized] No products loaded from database');
      return [];
    }
    
    // If no categories loaded yet, return products without category info
    if (categories.length === 0) {
      console.log('🔍 [POSPageOptimized] Categories not loaded yet, returning products without category info');
