...
const PurchaseOrderDetailPage: React.FC<PurchaseOrderDetailPageProps> = ({ editMode = false }) => {
  ...
  const [, setQualityChecks] = useState<any[]>([]);
  ...
  const _loadInventoryStats = useCallback(async () => {
    if (!id) return;
    try {
      const response = await PurchaseOrderService.getPurchaseOrderInventoryStats(id);
      if (response.success) {
  ...
  const _handleQualityCheck = async (itemId: string, status: 'passed' | 'failed' | 'attention') => {
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          {false && false && (
            <div className="space-y-6">
              {(purchaseOrder.status === 'shipped' || purchaseOrder.status === 'received') && (purchaseOrder as any).shippingInfo ? (
                <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-4">
                  <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
