...
import { CheckCircle, XCircle, AlertCircle, Package, FileText, PackageCheck } from 'lucide-react';
...
export const QualityCheckSummary: React.FC<QualityCheckSummaryProps> = ({
  ...
  const handleReceiveToInventory = async () => {
    ...
    setIsReceiving(true);
    try {
      // Call the service to receive quality-checked items
      const result = await QualityCheckService.receiveQualityCheckedItems(
        summary.qualityCheckId,
        purchaseOrderId
      );
      
      if (result.success) {
        toast.success(result.message || 'Items received to inventory successfully');
        onItemsReceived?.();
      } else {
        toast.error(result.message || 'Failed to receive items to inventory');
      }
    } catch (error) {
      console.error('Error receiving items:', error);
      toast.error('Failed to receive items to inventory');
    } finally {
    ...
    <div className="bg-white rounded-lg shadow">
      ...
      <div className="p-6">
        ...
        <div className="mt-4 space-y-2">
          {/* Receive to Inventory Button - Show if there are passed items */}
          {summary.passedItems > 0 && (
            <button
              onClick={handleReceiveToInventory}
            ...
            >
              ...
              {isReceiving ? 'Receiving...' : 'Receive to Inventory'}
            </button>
          )}
          ...
          {onViewDetails && (
            <button
              ...
              className="w-full px-4 py-2 border border-blue-500 text-blue-600 rounded-lg hover:bg-blue-50 transition flex items-center justify-center gap-2"
            >
