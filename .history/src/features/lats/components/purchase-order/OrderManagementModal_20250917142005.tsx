...
const OrderCard: React.FC<OrderCardProps> = ({
  ...
  // Get smart action buttons based on status and completion state
  const getSmartActionButtons = (order: EnhancedPurchaseOrder) => {
    ...
    // Status-specific actions (only show if not completed)
    switch (currentStatus) {
      case 'draft':
        actions.push({
          type: 'edit',
      ...
      case 'shipped':
        // Only show receive if not yet received
        if (currentStatus !== 'received') {
          actions.push({
            type: 'receive',
            label: 'Receive',
      ...
      case 'received':
        // Show receipt/invoice if available
        actions.push({
          type: 'receipt',
          label: 'Receipt',
