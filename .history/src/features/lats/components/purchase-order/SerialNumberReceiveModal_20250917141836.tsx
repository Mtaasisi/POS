...
import { PackageCheck, Plus, X, AlertCircle, CheckCircle, List, FileText } from 'lucide-react';
...
const SerialNumberReceiveModal: React.FC<SerialNumberReceiveModalProps> = ({
  ...
  const [bulkMode, setBulkMode] = useState<Map<string, boolean>>(new Map());
  const [bulkSerialText, setBulkSerialText] = useState<Map<string, string>>(new Map());

  // Initialize received items when modal opens
  useEffect(() => {
    if (isOpen) {
      const initialItems = new Map();
      const initialBulkMode = new Map();
      const initialBulkText = new Map();
      ...
      purchaseOrder.items.forEach(item => {
        ...
        initialBulkMode.set(item.id, false);
        initialBulkText.set(item.id, '');
      });
      
      setReceivedItems(initialItems);
      setBulkMode(initialBulkMode);
      setBulkSerialText(initialBulkText);
    }
  ...
  const toggleBulkMode = (itemId: string) => {
    setBulkMode(prev => {
      const newMap = new Map(prev);
      const currentMode = newMap.get(itemId) || false;
      newMap.set(itemId, !currentMode);
      return newMap;
    });
  };

  const updateBulkSerialText = (itemId: string, text: string) => {
    setBulkSerialText(prev => {
      const newMap = new Map(prev);
  ...
  const processBulkSerials = (itemId: string) => {
    const text = bulkSerialText.get(itemId) || '';
    if (!text.trim()) {
      toast.error('Please enter serial numbers');
      return;
    }

    // Parse serial numbers from text (one per line or comma-separated)
    ...
    setReceivedItems(prev => {
      ...
      newMap.set(itemId, {
        ...current,
        serialNumbers: [...current.serialNumbers, ...serialNumbers]
      });
      return newMap;
    });

    // Clear bulk text after processing
    setBulkSerialText(prev => {
      const newMap = new Map(prev);
      newMap.set(itemId, '');
      return newMap;
    ...
    toast.success(`Added ${serialNumbers.length} serial numbers`);
  };
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-medium text-gray-900">TSH {(item.cost_price || 0).toLocaleString()}</p>
                      </div>
                    </div>
                    ...
                    {itemData.quantity > 0 && (
                      <div className="border-t pt-4">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex items-center gap-2">
                            <GlassButton
                              variant="secondary"
                              size="sm"
                              icon={bulkMode.get(item.id) ? <List className="w-3 h-3" /> : <FileText className="w-3 h-3" />}
                              onClick={() => toggleBulkMode(item.id)}
                            >
                              {bulkMode.get(item.id) ? 'Individual' : 'Bulk'}
                            </GlassButton>
                            {!bulkMode.get(item.id) && (
                              <GlassButton
                                variant="secondary"
                                size="sm"
                        ...
                        {bulkMode.get(item.id) && (
                          <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                            <div className="mb-3">
                              ...
                              <textarea
                                value={bulkSerialText.get(item.id) || ''}
                                onChange={(e) => updateBulkSerialText(item.id, e.target.value)}
                                placeholder="SN001, SN002, SN003&#10;or&#10;SN001&#10;SN002&#10;SN003"
                                className="w-full h-24 px-3 py-2 text-sm border border-gray-300 rounded-lg resize-none"
                              />
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-xs text-gray-500">
                                {(bulkSerialText.get(item.id) || '').split(/[\n,]/).filter(s => s.trim()).length} serial numbers detected
                              </span>
                              <GlassButton
                                variant="primary"
                                size="sm"
                                onClick={() => processBulkSerials(item.id)}
                                disabled={!bulkSerialText.get(item.id)?.trim()}
                              >
                                Add Serials
                              </GlassButton>
                            </div>
                        ...
                        {itemData.serialNumbers.length === 0 && !bulkMode.get(item.id) && (
                          <div className="text-center py-4 text-gray-500">
                            <PackageCheck className="w-8 h-8 mx-auto mb-2 opacity-50" />
                            <p className="text-sm">No serial numbers added yet</p>
                            <p className="text-xs">Click "Add Serial" to track individual items or use "Bulk" mode</p>
                          </div>
                        )}
                      </div>
                    )}
