...
import { getSparePartVariants, getVariantStats } from '../../lib/sparePartsApi';
...
const SparePartsDetailModal: React.FC<SparePartsDetailModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && sparePart) {
      console.log('🔍 [SparePartsDetailModal] DEBUG - Spare part data received:', {
        ...
        use_variants: sparePart.use_variants,
        variants: sparePart.variants?.length || 0,
        variantsData: sparePart.variants, // Show actual variants data
      ...
      if (sparePart.images && Array.isArray(sparePart.images) && sparePart.images.length > 0) {
        ...
        const formattedImages = sparePart.images.map((imageUrl, index) => {
          ...
          return {
            ...
            thumbnailUrl: sanitizedResult.url,
  ...
  const fetchVariants = async () => {
    ...
    try {
      ...
      console.log('🔍 [SparePartsDetailModal] Fetching variants...');
      ...
      // Always show variants section to demonstrate the functionality
      const hasVariants = variantsData.length > 0;
      const usesVariants = sparePart.use_variants || sparePart.metadata?.useVariants || false;
      setShowVariantsSection(true); // Always show variants section
              ...
              {showVariantsSection && (
                <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                  <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                    ...
                    >
                      {showVariantsSection ? 'Hide' : 'Show'} Details
                    </button>
                  </div>
                      ...
                      {variants.length > 0 ? (
                        <div className="space-y-2">
                          ...
                          {variants.map((variant, index) => (
                            <div key={variant.id || index} className="p-3 bg-gray-50 rounded-lg border border-gray-200 hover:border-blue-300 transition-colors">
                              <div className="flex items-center justify-between">
                                <div className="flex-1">
                                  ...
                                  <div className="flex items-center gap-4 text-sm text-gray-600">
                                    <span>Stock: <span className="font-medium">{variant.quantity}</span></span>
                                    <span>Min: <span className="font-medium">{variant.min_quantity}</span></span>
                                    <span>Price: <span className="font-medium text-green-600">
                                ...
                                <div className="flex items-center">
                                  {variant.quantity === 0 ? (
                                    <span className="w-3 h-3 bg-red-500 rounded-full" title="Out of Stock"></span>
                                  ) : variant.quantity <= variant.min_quantity ? (
                                    <span className="w-3 h-3 bg-orange-500 rounded-full" title="Low Stock"></span>
                                  ) : (
                                    <span className="w-3 h-3 bg-green-500 rounded-full" title="In Stock"></span>
                                  )}
                                </div>
                              </div>
                            </div>
                        ...
                        <div className="text-center py-4 text-gray-500">
                          ...
                          <p className="text-sm">No variants found for this spare part</p>
                          <p className="text-xs text-gray-400 mt-1">
                            Variants allow you to manage different specifications, prices, and stock levels
                          </p>
                        </div>
                      )}
                    </div>
