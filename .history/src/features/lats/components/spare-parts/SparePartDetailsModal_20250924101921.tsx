                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    {isLoadingLocation ? (
                      ...
                      <div className="text-sm">
                        <p className="font-medium text-gray-900">{locationData.storeLocation.name}</p>
                        {locationData.storeLocation.code && (
                          <p className="text-xs text-gray-600">Code: {locationData.storeLocation.code}</p>
                        )}
                        {locationData.storeLocation.city && (
                          <p className="text-xs text-gray-600">City: {locationData.storeLocation.city}</p>
                        )}
                      </div>
                    ) : (
                      <p className="text-sm text-gray-500">Not assigned to a store location</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    {isLoadingLocation ? (
                      ...
                      <div className="text-sm">
                        <p className="font-medium text-gray-900">{locationData.storageRoom.name}</p>
                        {locationData.storageRoom.code && (
                          <p className="text-xs text-gray-600">Code: {locationData.storageRoom.code}</p>
                        )}
                        {locationData.storageRoom.floor_level && (
                          <p className="text-xs text-gray-600">Floor: {locationData.storageRoom.floor_level}</p>
                        )}
                      </div>
                    ) : (
                      <p className="text-sm text-gray-500">Not assigned to a storage room</p>
                    )}
                  </div>
                  
                  {/* Store Shelf */}
                  <div className="space-y-1">
                    <span className="text-xs text-gray-500 uppercase tracking-wide">Store Shelf</span>
                    {isLoadingLocation ? (
                      <div className="flex items-center gap-2">
                        <div className="w-3 h-3 border border-blue-500 border-t-transparent rounded-full animate-spin"></div>
                        <span className="text-sm text-gray-500">Loading...</span>
                      ...
                      <div className="text-sm">
                        <p className="font-medium text-gray-900">{locationData.storeShelf.name}</p>
                        {locationData.storeShelf.code && (
                          <p className="text-xs text-gray-600">Code: {locationData.storeShelf.code}</p>
                        )}
                        {locationData.storeShelf.section && (
                          <p className="text-xs text-gray-600">Section: {locationData.storeShelf.section}</p>
                        )}
                        {locationData.storeShelf.aisle && (
                          <p className="text-xs text-gray-600">Aisle: {locationData.storeShelf.aisle}</p>
                        )}
                      </div>
                    ) : (
                      <p className="text-sm text-gray-500">Not assigned to a specific shelf</p>
                    )}
                  </div>
                  
                  {/* Fallback Location */}
                  {sparePart.location && (!locationData?.storeLocation && !locationData?.storageRoom && !locationData?.storeShelf) && (
                    <div className="space-y-1">
                      <span className="text-xs text-gray-500 uppercase tracking-wide">Manual Location</span>
                      <p className="text-sm font-medium text-gray-900">{sparePart.location}</p>
                    </div>
                  )}
