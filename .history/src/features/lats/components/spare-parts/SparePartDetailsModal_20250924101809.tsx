...
const SparePartDetailsModal: React.FC<SparePartDetailsModalProps> = ({
  ...
  // Fetch location data for the spare part
  const fetchLocationData = async () => {
    ...
    try {
      ...
      // Fetch store location if available
      if (sparePart.storage_room_id) {
        try {
          console.log('🔍 [SparePartsDetailModal] Fetching storage room:', sparePart.storage_room_id);
          const storageRoom = await storageRoomApi.getById(sparePart.storage_room_id);
          locationInfo.storageRoom = storageRoom;
          
          // If storage room found, fetch the store location
          if (storageRoom?.store_location_id) {
            console.log('🔍 [SparePartsDetailModal] Fetching store location:', storageRoom.store_location_id);
            const storeLocation = await storeLocationApi.getById(storageRoom.store_location_id);
            locationInfo.storeLocation = storeLocation;
          }
        } catch (error) {
          console.error('❌ [SparePartsDetailModal] Error fetching storage room:', error);
        }
      }
      
      // Fetch store shelf if available
      if (sparePart.store_shelf_id) {
        try {
          console.log('🔍 [SparePartsDetailModal] Fetching store shelf:', sparePart.store_shelf_id);
          const storeShelf = await storeShelfApi.getById(sparePart.store_shelf_id);
          locationInfo.storeShelf = storeShelf;
          
          // If shelf found and we don't have store location yet, fetch it
          if (storeShelf?.store_location_id && !locationInfo.storeLocation) {
            console.log('🔍 [SparePartsDetailModal] Fetching store location from shelf:', storeShelf.store_location_id);
            const storeLocation = await storeLocationApi.getById(storeShelf.store_location_id);
            locationInfo.storeLocation = storeLocation;
          }
        } catch (error) {
          console.error('❌ [SparePartsDetailModal] Error fetching store shelf:', error);
        }
      }
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <div className="flex-shrink-0">
                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                    sparePart.quantity > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                  }`}>
                    {sparePart.quantity > 0 ? 'In Stock' : 'Out of Stock'}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="mb-4">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 rounded-lg p-3">
                    ...
                    <div className="text-sm font-bold text-emerald-900">{formatMoney(sparePart.selling_price * sparePart.quantity, safeCurrency).replace(/\.00$/, '').replace(/\.0$/, '')}</div>
                  </div>

                  <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-lg p-3">
                    <div className="text-xs font-medium text-blue-700 uppercase tracking-wide mb-1">Profit</div>
                    <div className="text-sm font-bold text-blue-900">{formatMoney((sparePart.selling_price - sparePart.cost_price) * sparePart.quantity, safeCurrency).replace(/\.00$/, '').replace(/\.0$/, '')}</div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-lg p-3">
                    ...
                    <div className="text-sm font-bold text-purple-900">{formatMoney(sparePart.cost_price * sparePart.quantity, safeCurrency).replace(/\.00$/, '').replace(/\.0$/, '')}</div>
                  </div>
                </div>
              </div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-3 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm font-bold text-green-600">{formatMoney(sparePart.selling_price, safeCurrency).replace(/\.00$/, '').replace(/\.0$/, '')}</p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-xs text-gray-500 uppercase tracking-wide">Cost Price</span>
                        <p className="text-sm font-bold text-red-600">{formatMoney(sparePart.cost_price, safeCurrency).replace(/\.00$/, '').replace(/\.0$/, '')}</p>
                      </div>
                      <div className="space-y-1">
