...
const SparePartDetailsModal: React.FC<SparePartDetailsModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && sparePart?.id) {
      ...
      // Set initial location data immediately to avoid loading delay
      if (sparePart.location) {
        setLocationData({
          storeLocation: null,
          storageRoom: null,
          storeShelf: null,
          fallbackLocation: sparePart.location
        });
      }
      
      // Then fetch detailed location data in background
      fetchLocationData();
      
      // Set a timeout to stop loading after 10 seconds
      const loadingTimeout = setTimeout(() => {
        setIsLoadingLocation(false);
        console.log('⏰ [SparePartsDetailModal] Location loading timeout reached');
      }, 10000);
  ...
  // Fetch location data for the spare part (optimized with parallel calls)
  const fetchLocationData = async () => {
    if (!sparePart?.id) return;
    
    try {
      setIsLoadingLocation(true);
      console.log('🔍 [SparePartsDetailModal] Fetching location data for spare part:', sparePart.id);
      ...
      // Add storage room promise if available
      if (sparePart.storage_room_id) {
        console.log('🔍 [SparePartsDetailModal] Adding storage room promise:', sparePart.storage_room_id);
        promiseMap.storageRoom = storageRoomApi.getById(sparePart.storage_room_id);
        promises.push(promiseMap.storageRoom);
      }
      ...
      if (promises.length > 0) {
        try {
          ...
          // Process storage room result
          if (promiseMap.storageRoom) {
            const storageRoomResult = results[0];
            if (storageRoomResult.status === 'fulfilled' && storageRoomResult.value) {
              locationInfo.storageRoom = storageRoomResult.value;
              console.log('✅ [SparePartsDetailModal] Storage room fetched:', locationInfo.storageRoom.name);
            }
          }
          
          // Process store shelf result
          const shelfIndex = promiseMap.storageRoom ? 1 : 0;
          if (promiseMap.storeShelf) {
            const storeShelfResult = results[shelfIndex];
            if (storeShelfResult.status === 'fulfilled' && storeShelfResult.value) {
              locationInfo.storeShelf = storeShelfResult.value;
              console.log('✅ [SparePartsDetailModal] Store shelf fetched:', locationInfo.storeShelf.name);
            }
          }
          
          // Determine which store location ID to use
          let storeLocationId = null;
          if (locationInfo.storageRoom?.store_location_id) {
            storeLocationId = locationInfo.storageRoom.store_location_id;
          } else if (locationInfo.storeShelf?.store_location_id) {
            storeLocationId = locationInfo.storeShelf.store_location_id;
          }
          
          // Fetch store location if we have an ID
          if (storeLocationId) {
            console.log('🔍 [SparePartsDetailModal] Fetching store location:', storeLocationId);
            try {
              const storeLocation = await Promise.race([
                storeLocationApi.getById(storeLocationId),
                new Promise((_, reject) => 
                  setTimeout(() => reject(new Error('Store location API timeout')), 3000)
                )
              ]);
              locationInfo.storeLocation = storeLocation;
              console.log('✅ [SparePartsDetailModal] Store location fetched:', locationInfo.storeLocation.name);
            } catch (error) {
              console.error('❌ [SparePartsDetailModal] Error fetching store location:', error);
            }
          }
          
        } catch (error) {
          console.error('❌ [SparePartsDetailModal] Error in parallel API calls:', error);
        }
      }
      
      setLocationData(locationInfo);
      
      console.log('✅ [SparePartsDetailModal] Location data fetched successfully:', {
        hasStoreLocation: !!locationInfo.storeLocation,
        hasStorageRoom: !!locationInfo.storageRoom,
        hasStoreShelf: !!locationInfo.storeShelf,
        locationInfo
      });
    } catch (error) {
      console.error('❌ [SparePartsDetailModal] Error fetching location data:', error);
                    ...
                    {isLoadingImages ? (
                      ...
                      <>
                        <div className="aspect-square relative rounded-lg overflow-hidden bg-gray-50 border border-gray-200">
                          <img
                            src={sparePartImages[selectedImageIndex]?.thumbnailUrl || sparePartImages[selectedImageIndex]?.url || sparePartImages[0]?.thumbnailUrl || sparePartImages[0]?.url}
                            alt={sparePart.name}
                        ...
                        {isLoadingLocation ? (
                          ...
                          <div className="text-sm">
                            ...
                            {locationData.storageRoom && (
                              <p className="text-xs text-gray-600">Room: {locationData.storageRoom.code}</p>
                            )}
                            {locationData.storeShelf && (
                              <p className="text-xs text-gray-600">Shelf: {locationData.storeShelf.code}</p>
                            )}
                            {!locationData.storeLocation && !locationData.storageRoom && !locationData.storeShelf && (
                              <p className="text-sm text-gray-500">{locationData.fallbackLocation || sparePart.location || 'Not Set'}</p>
                            )}
                          </div>
                        ) : (
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-3 space-y-3">
                ...
                <div className="space-y-2">
                  {(() => {
                    ...
                    if (pricingData.totalQuantity <= sparePart.min_quantity) {
                    ...
                    if (pricingData.avgCostPrice > 0 && pricingData.avgSellingPrice <= pricingData.avgCostPrice) {
                      recommendations.push({
                        type: 'error',
                        message: 'Average selling price is at or below average cost price - review pricing strategy'
                      });
                    }
          ...
          <div className="flex items-center justify-between pt-6 border-t border-gray-100">
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={pricingData.totalQuantity === 0}
            ...
            <GlassCard className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => setQuantity(Math.min(pricingData.totalQuantity, quantity + 1))}
                      disabled={quantity >= pricingData.totalQuantity}
                      className="w-10 h-10 flex items-center justify-center border-2 border-blue-300 rounded-lg hover:bg-blue-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
                  ...
                  <div className="text-sm text-gray-600 mt-1">
                    Available: {pricingData.totalQuantity} units
                  </div>
                </div>
              ...
              <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                ...
                <GlassButton
                  type="button"
                  onClick={() => {
                    if (quantity > pricingData.totalQuantity) {
                  ...
                  disabled={!reason.trim() || quantity > pricingData.totalQuantity}
