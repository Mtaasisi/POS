...
const SparePartsDetailModal: React.FC<SparePartsDetailModalProps> = ({
  ...
  const fetchSparePartImages = async () => {
    ...
    try {
      ...
      const images = await UnifiedImageService.getProductImages(sparePart.id);
      setSparePartImages(images);
      
      console.log('✅ [SparePartsDetailModal] Images fetched successfully:', {
        count: images.length,
        images: images.map(img => ({ id: img.id, fileName: img.fileName, isPrimary: img.isPrimary }))
      });
    } catch (error) {
