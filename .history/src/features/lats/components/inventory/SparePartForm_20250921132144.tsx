...
import { X, Save, Package, AlertCircle, QrCode, Download, DollarSign, Image as ImageIcon, MapPin, Layers, FileText, Hash, Building, LayoutGrid, Check, Search, Filter, Plus } from 'lucide-react';
...
const SparePartForm: React.FC<SparePartFormProps> = ({ 
  ...
  // Debug form data changes, especially images
  useEffect(() => {
    console.log('🔍 [SparePartForm] Form data changed:', {
      images: formData.images,
      imagesLength: Array.isArray(formData.images) ? formData.images.length : 'not array',
      imagesType: typeof formData.images,
      imagesContent: formData.images
    });
  }, [formData.images]);
  ...
  useEffect(() => {
    if (sparePart) {
      console.log('🔍 [SparePartForm] ===== FORM INITIALIZATION DEBUG START =====');
      console.log('🔍 [SparePartForm] Initializing form with spare part:', {
        id: sparePart.id,
        name: sparePart.name,
        images: sparePart.images,
        imagesType: typeof sparePart.images,
        imagesLength: Array.isArray(sparePart.images) ? sparePart.images.length : 'not array'
      });
      ...
      const initialFormData = {
        ...
        images: [] // Initialize as empty, will be loaded separately
      };
      
      console.log('🔍 [SparePartForm] Initial form data:', initialFormData);
      setFormData(initialFormData);

      // If spare part has a location, try to find the store location
      if (sparePart.location) {
        console.log('🔍 [SparePartForm] Finding location for shelf:', sparePart.location);
        findLocationForShelf(sparePart.location);
      }

      // Load images from database
      console.log('🔍 [SparePartForm] Loading images from database...');
      loadSparePartImages();
      
      console.log('🔍 [SparePartForm] ===== FORM INITIALIZATION DEBUG END =====');
    }
  }, [sparePart]);
  ...
  const loadSparePartImages = async () => {
    ...
    try {
      console.log('🔍 [SparePartForm] ===== IMAGE LOADING DEBUG START =====');
      console.log('🔍 [SparePartForm] Loading images for editing spare part:', sparePart.id);
      console.log('🔍 [SparePartForm] Spare part data:', {
        id: sparePart.id,
        name: sparePart.name,
        images: sparePart.images,
        imagesType: typeof sparePart.images,
      ...
      // Import the getSparePartImages function
      const { getSparePartImages } = await import('../../lib/sparePartsApi');
      console.log('🔍 [SparePartForm] Calling getSparePartImages API...');
      const dbImages = await getSparePartImages(sparePart.id);
      
      console.log('🔍 [SparePartForm] Database images result:', {
        dbImages,
        dbImagesType: typeof dbImages,
        dbImagesLength: Array.isArray(dbImages) ? dbImages.length : 'not array',
        dbImagesContent: dbImages
      });
      
      if (dbImages && dbImages.length > 0) {
        console.log('✅ [SparePartForm] Found database images, converting to form format...');
        // Convert database images to the format expected by SimpleImageUpload (ProductImage format)
        const formattedImages = dbImages.map((dbImage, index) => {
          const formatted = {
            ...
            // Keep additional properties for form data
          ...
          console.log(`🔍 [SparePartForm] Formatted image ${index}:`, formatted);
          return formatted;
        });
        
        console.log('✅ [SparePartForm] Setting form data with formatted images:', formattedImages);
        setFormData(prev => {
          const updated = { ...prev, images: formattedImages };
          console.log('✅ [SparePartForm] Updated form data:', updated);
          return updated;
        });
        console.log('✅ [SparePartForm] Images loaded for editing successfully');
      } else {
        // Fallback to images from spare part record if no database images
        console.log('🔄 [SparePartForm] No database images found, using fallback from spare part record');
        const sparePartImages = sparePart.images || [];
        console.log('🔄 [SparePartForm] Spare part images array:', {
          sparePartImages,
          sparePartImagesType: typeof sparePartImages,
          sparePartImagesLength: Array.isArray(sparePartImages) ? sparePartImages.length : 'not array'
        });
        
        if (sparePartImages.length > 0) {
          console.log('✅ [SparePartForm] Found spare part images, converting to form format...');
          const fallbackImages = sparePartImages.map((url: string, index: number) => {
            ...
            console.log(`🔍 [SparePartForm] Fallback image ${index}:`, fallback);
            return fallback;
          });
          
          console.log('✅ [SparePartForm] Setting form data with fallback images:', fallbackImages);
          setFormData(prev => {
            const updated = { ...prev, images: fallbackImages };
            console.log('✅ [SparePartForm] Updated form data with fallback:', updated);
            return updated;
          });
          console.log('✅ [SparePartForm] Using fallback images successfully');
        } else {
          console.log('⚠️ [SparePartForm] No images found anywhere');
          setFormData(prev => {
            const updated = { ...prev, images: [] };
            console.log('⚠️ [SparePartForm] Updated form data with empty images:', updated);
            return updated;
          });
        }
      }
      
      console.log('🔍 [SparePartForm] ===== IMAGE LOADING DEBUG END =====');
    } catch (error) {
      console.error('❌ [SparePartForm] Failed to load images:', error);
      console.error('❌ [SparePartForm] Error details:', {
        error,
      ...
      setFormData(prev => {
        const updated = { ...prev, images: [] };
        console.log('❌ [SparePartForm] Updated form data with empty images due to error:', updated);
        return updated;
      });
    }
  };
  ...
  const createVariantFromFormData = (): SparePartVariant => {
    return {
      name: 'Variant 1',
      sku: `${formData.partNumber}-V01`,
      ...
      image_url: formData.images && formData.images.length > 0 ? formData.images[0] : undefined
    };
  };
        ...
        <div className="w-full max-w-3xl max-h-[85vh] overflow-y-auto">
          <GlassCard className="p-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Spare Part Variants Section - Removed */}
