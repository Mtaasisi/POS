...
const SparePartAddEditForm: React.FC<SparePartAddEditFormProps> = ({ 
  ...
  const handleUseVariantsToggle = (enabled: boolean) => {
    ...
    if (enabled) {
      ...
      toast('Variants disabled - using main product data');
    }
  };
                ...
                <div>
                    ...
                    <input
                      ...
                      onFocus={async () => {
                        if (deviceInputValue.length >= 2) {
                          const suggestions = await searchDevices(deviceInputValue);
                          setDeviceSuggestions(suggestions);
                          setShowDeviceSuggestions(suggestions.length > 0);
                        }
                      }}
                            ...
                            >
                              ...
                              {isSaved && (
                                <button
                                  ...
                                  title="Remove from saved devices"
                                >
                  ...
                  <div className="mt-2 text-sm text-gray-500">
                    <div className="flex items-center gap-4 flex-wrap">
                      ...
                      <span>💾 Custom devices are saved automatically</span>
                    </div>
                  </div>
                    ...
                    <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        <div className="text-xs font-medium text-green-700 flex items-center gap-1">
                          <span>💾</span>
                          Your saved devices ({savedDevices.length})
                          <span className="text-xs text-green-600 ml-2">
                            ({hybridDeviceStorage.getStorageMode()})
                          </span>
                        </div>
                        <button
                          type="button"
                          onClick={async () => {
                            const recent = savedDevices.slice(-3);
                            for (const device of recent) {
                              await addDevice(device.device_name);
                            }
                          }}
                        ...
                        {savedDevices
                          ...
                          .map((device) => (
                            <button
                              ...
                              type="button"
                              onClick={() => addDevice(device.device_name)}
                              disabled={selectedDevices.includes(device.device_name)}
                              className="px-2 py-1 text-xs bg-green-100 hover:bg-green-200 disabled:bg-gray-50 disabled:text-gray-400 text-green-700 rounded-full border border-green-200 transition-colors"
                            >
                              + {device.device_name}
                              {device.usage_count > 1 && (
                                <span className="ml-1 text-green-600">({device.usage_count}x)</span>
                              )}
                            </button>
                          ))}
                        {savedDevices.length > 5 && (
                          <span className="text-xs text-green-600 px-2 py-1">
                            +{savedDevices.length - 5} more...
                          </span>
                        )}
                      </div>
                    </div>
