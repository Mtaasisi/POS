...
import { createSparePart, updateSparePartWithVariants } from '../../lib/sparePartsApi';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (sparePart) {
        ...
        result = await updateSparePartWithVariants(sparePart.id, sparePartData);
      } else {
        // Create new spare part
        result = await createSparePart(sparePartData);
      }
      
      if (result.ok) {
        toast.success(sparePart ? 'Spare part updated successfully!' : 'Spare part created successfully!');
        await onSave(result.data);
        
        if (!sparePart) {
          setCreatedSparePart(result.data);
