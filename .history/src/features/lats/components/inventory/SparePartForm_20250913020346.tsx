...
import { X, Save, Package, AlertCircle, QrCode, Download, DollarSign, Image as ImageIcon, MapPin, Layers, FileText, Hash, Building, Grid3X3, Check, Search, Filter } from 'lucide-react';
...
const SparePartForm: React.FC<SparePartFormProps> = ({ 
  ...
  // Load categories and suppliers on mount - DISABLED
  // useEffect(() => {
  //   loadSparePartCategories();
  //   loadSuppliers();
  // }, []); // Empty dependency array - these are stable store functions
  ...
  useEffect(() => {
    console.log('🔍 [DEBUG] ===== FORM INITIALIZATION START =====');
    console.log('🔍 [DEBUG] sparePart prop:', sparePart);
    console.log('🔍 [DEBUG] sparePart type:', typeof sparePart);
    
    if (sparePart) {
      console.log('🔍 [DEBUG] Initializing form with spare part data:', sparePart);
      console.log('🔍 [DEBUG] sparePart keys:', Object.keys(sparePart));
      console.log('🔍 [DEBUG] sparePart values:', Object.values(sparePart));
      ...
      const initialFormData = {
        ...
        storageRoomId: sparePart.storageRoomId || '',
        shelfId: sparePart.shelfId || '',
        storeLocationId: sparePart.storageRoomId || '', // For backward compatibility
        location: sparePart.location || '',
        compatibleDevices: sparePart.compatibleDevices || '',
        images: sparePart.images || []
      };
      
      console.log('🔍 [DEBUG] Initial form data to be set:', initialFormData);
      console.log('🔍 [DEBUG] Initial form data keys:', Object.keys(initialFormData));
      
      setFormData(initialFormData);

      // If spare part has a location, try to find the store location
      if (sparePart.location) {
        console.log('🔍 [DEBUG] Finding location for shelf:', sparePart.location);
        findLocationForShelf(sparePart.location);
      }
    } else {
      console.log('🔍 [DEBUG] No sparePart provided, using default form data');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('🔍 [DEBUG] FormData values:', Object.values(formData));
    ...
    try {
      ...
      console.log('🔍 [DEBUG] Prepared sparePartData:', sparePartData);
      console.log('🔍 [DEBUG] sparePartData keys:', Object.keys(sparePartData));
      console.log('🔍 [DEBUG] sparePartData values:', Object.values(sparePartData));
      console.log('🔍 [DEBUG] sparePart?.id:', sparePart?.id);
      console.log('🔍 [DEBUG] currentUser?.id:', currentUser?.id);
      
      console.log('🔍 [DEBUG] Calling onSave with sparePartData...');
      await onSave(sparePartData);
      console.log('✅ [DEBUG] onSave completed successfully');
      
      if (!sparePart) {
        console.log('🔍 [DEBUG] Creating new spare part, setting summary modal');
        setCreatedSparePart(sparePartData);
        setShowSummaryModal(true);
      } else {
        console.log('🔍 [DEBUG] Updating existing spare part');
      }
    } catch (error: any) {
      ...
      console.log('🔍 [DEBUG] Form submission cleanup');
      setIsSubmitting(false);
      console.log('🔍 [DEBUG] ===== FORM SUBMISSION END =====');
    }
  };
