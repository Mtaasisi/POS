...
const SparePartVariantsSection: React.FC<SparePartVariantsSectionProps> = ({
  ...
  const handleVariantImageChange = (index: number, imageFile: File | null) => {
    if (imageFile) {
      // Create a preview URL for the image
      const imageUrl = URL.createObjectURL(imageFile);
      updateVariant(index, 'image_file', imageFile);
      updateVariant(index, 'image_url', imageUrl);
    } else {
      updateVariant(index, 'image_file', null);
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="w-20 h-20 bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden">
                      {variant.image_url ? (
                        ...
                        <div className="relative w-full h-full">
                          <img 
                            src={mainProductImages[0]} 
                            alt={`${variant.name} (using main product image)`}
                            className="w-full h-full object-cover rounded-lg opacity-60"
                          />
                          <div className="absolute inset-0 flex items-center justify-center">
                            <span className="text-xs text-gray-600 bg-white/80 px-1 rounded">Main</span>
                          </div>
                        </div>
                      ) : (
                    ...
                    <div className="flex-1">
                      <SimpleImageUpload
                        ...
                        currentImage={variant.image_file}
                        className="w-full"
                        accept="image/*"
                        maxSize={5 * 1024 * 1024} // 5MB
                      />
                      <div className="text-xs text-gray-500 mt-1">
                        Upload a specific image for this variant. If no image is uploaded, the main product image will be used.
                      </div>
                    </div>
                  </div>
                </div>
