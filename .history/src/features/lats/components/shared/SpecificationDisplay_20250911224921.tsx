...
const SpecificationDisplay: React.FC<SpecificationDisplayProps> = ({
  ...
  // If no valid specifications after filtering, show empty message
            ...
            <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
              ...
              {showCount && (
                <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">
                  {specEntries.length} specifications
                </span>
              )}
            </h3>
          </div>
        ...
        <div className="flex flex-wrap gap-1">
          ...
          {hasMore && (
            <span className="px-2 py-1 bg-gray-100 text-gray-600 rounded-md text-xs font-medium">
              +{specEntries.length - maxDisplay} more
            </span>
          )}
        </div>
      </div>
        ...
        <ul className="space-y-1">
          ...
          {hasMore && (
            <li className="text-xs text-gray-500 text-center">
              +{specEntries.length - maxDisplay} more specifications
            </li>
          )}
        </ul>
      </div>
