      ...
      <div className="bg-white rounded-lg p-4 border border-gray-200">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-sm text-gray-500">
            {product.variants ? product.variants.length : 0} variant{(product.variants?.length || 0) !== 1 ? 's' : ''}
          </span>
        </div>
        ...
        {product.variants && product.variants.length > 0 ? (
            ...
            {product.variants.map((variant, index) => (
              <div key={index} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-3">
                  <div className="space-y-1">
                    <div className="text-xs font-medium text-gray-600">SKU</div>
                    <div className="text-sm font-semibold text-gray-900 font-mono">{variant.sku}</div>
                  </div>
                  <div className="space-y-1">
                    <div className="text-xs font-medium text-gray-600">Cost Price</div>
                    <div className="text-sm font-semibold text-gray-900">TZS {variant.costPrice.toLocaleString()}</div>
                  </div>
                  <div className="space-y-1">
                    <div className="text-xs font-medium text-gray-600">Selling Price</div>
                    <div className="text-sm font-semibold text-gray-900">TZS {variant.sellingPrice.toLocaleString()}</div>
                  </div>
                  <div className="space-y-1">
                    <div className="text-xs font-medium text-gray-600">Stock</div>
                    <div className="text-sm font-semibold text-gray-900">{variant.stockQuantity}</div>
                  </div>
                </div>
                
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-3">
                  <div className="space-y-1">
                    <div className="text-xs font-medium text-gray-600">Min Stock Level</div>
                    <div className="text-sm font-semibold text-gray-900">{variant.minStockLevel}</div>
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-sm font-semibold text-gray-900">
                      TZS {(variant.stockQuantity * variant.sellingPrice).toLocaleString()}
                    </div>
                  </div>
                  <div className="space-y-1">
                    ...
                    <div className={`text-sm font-semibold ${
                      variant.stockQuantity <= variant.minStockLevel 
                        ? 'text-red-600' 
                        : variant.stockQuantity <= variant.minStockLevel * 2 
                          ? 'text-yellow-600' 
                          : 'text-green-600'
                    }`}>
                      {variant.stockQuantity <= variant.minStockLevel 
                ...
                {variant.attributes && Object.keys(variant.attributes).length > 0 && (
                  <div className="mt-4 pt-3 border-t border-gray-200">
                    <div className="text-xs font-medium text-gray-600 mb-2">Variant Specifications</div>
          ...
          <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
            <div className="text-center py-4">
              <div className="text-sm text-gray-500 mb-2">No variants found</div>
              <div className="text-xs text-gray-400">
                This product uses the main product data as a single variant
              </div>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="space-y-1">
        ...
        <div className="bg-white rounded-lg p-3 border border-gray-200">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
            {Object.entries(product.specifications).map(([key, value]) => {
              // Handle both comma and "note" separators
              const values = String(value).split(/[,\n]/).filter(v => v.trim() && !v.trim().toLowerCase().includes('note'));
              return values.map((item, index) => (
                <div key={`${key}-${index}`} className="bg-gray-50 rounded-md p-2 border border-gray-200 hover:bg-gray-100 transition-colors">
                  <div className="text-xs font-medium text-gray-600 mb-1 capitalize">
                    {key.replace(/_/g, ' ')}
                  </div>
                  <div className="text-xs font-semibold text-gray-900 truncate" title={item.trim()}>
                    {item.trim() || 'Not set'}
                  </div>
                </div>
              ));
      ...
      <div className="border-t pt-4">
          ...
          <button
            ...
            onClick={() => {
              const newVariant: ProductVariant = {
                name: `Variant ${(product.variants?.length || 0) + 1}`,
                sku: `${product.sku}-V${((product.variants?.length || 0) + 1).toString().padStart(2, '0')}`,
              ...
              handleChange('variants', [...(product.variants || []), newVariant]);
            }}
        ...
        {product.variants && product.variants.length > 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const updatedVariants = [...(product.variants || [])];
                        updatedVariants[index] = { ...variant, sku: e.target.value };
                        handleChange('variants', updatedVariants);
                      }}
                      className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const updatedVariants = [...(product.variants || [])];
                        updatedVariants[index] = { ...variant, costPrice: parseFloat(e.target.value) || 0 };
                        handleChange('variants', updatedVariants);
                      }}
                      className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const updatedVariants = [...(product.variants || [])];
                        updatedVariants[index] = { ...variant, sellingPrice: parseFloat(e.target.value) || 0 };
                        handleChange('variants', updatedVariants);
                      }}
                      className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const updatedVariants = [...(product.variants || [])];
                        updatedVariants[index] = { ...variant, stockQuantity: parseInt(e.target.value) || 0 };
                        handleChange('variants', updatedVariants);
                      }}
                      className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const updatedVariants = [...(product.variants || [])];
                        updatedVariants[index] = { ...variant, minStockLevel: parseInt(e.target.value) || 0 };
                        handleChange('variants', updatedVariants);
                      }}
                      className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
          ...
          <div className="text-center py-4 text-gray-500">
            <p className="text-sm">No variants added yet. Click "Add Variant" to create one.</p>
          </div>
        )}
      </div>
...
const BulkProductImport: React.FC = () => {
  ...
  const parseCSVFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        // Create default variant from the product data
        ...
        const product: ProductImportData = {
          ...
          useVariants: false // Start with single variant mode
        };
  ...
  const getOrCreateCategory = async (categoryName: string): Promise<string> => {
    try {
      ...
      const { data: existingCategory } = await supabase
        ...
        .select('id')
        .eq('name' as any, categoryName as any)
        .single();

      if (existingCategory && 'id' in existingCategory) {
      ...
      return (newCategory as any).id;
    } catch (error) {
      console.error('Error in getOrCreateCategory:', error);
      // Return a fallback category ID or create a default one
      return '00000000-0000-0000-0000-000000000000'; // This will need to be handled properly
    }
  };
    ...
    try {
      for (const product of importData) {
        try {
          ...
          const categoryId = await getOrCreateCategory(product.category || 'General');
          ...
          const productData = {
            ...
            variants: product.variants && product.variants.length > 0 
              ? product.variants.map(variant => ({
                  ...
                  minQuantity: variant.minStockLevel,
  ...
  const downloadSampleCSV = () => {
    const sampleData = [
      ...
      '"Samsung Galaxy S23","Latest Samsung smartphone with advanced camera","SAM-GS23-001","Electronics",850000,1200000,50,5,"Samsung Electronics","{\"Screen Size\":\"6.1 inches\",\"Storage\":\"128GB\",\"RAM\":\"8GB\",\"Camera\":\"50MP\",\"Battery\":\"3900mAh\"}"',
      '"iPhone 15 Pro","Apple flagship smartphone with titanium design","APP-IP15P-001","Electronics",1200000,1800000,30,3,"Apple Inc","{\"Screen Size\":\"6.1 inches\",\"Storage\":\"256GB\",\"RAM\":\"8GB\",\"Camera\":\"48MP\",\"Battery\":\"3274mAh\"}"',
      '"Dell XPS 13 Laptop","Premium ultrabook for professionals","DELL-XPS13-001","Electronics",1800000,2500000,15,2,"Dell Technologies","{\"Screen Size\":\"13.4 inches\",\"RAM\":\"16GB\",\"Storage\":\"512GB SSD\",\"Processor\":\"Intel i7\",\"Graphics\":\"Intel Iris Xe\"}"',
      '"Nike Air Max 270","Comfortable running shoes","NIKE-AM270-001","Footwear",120000,180000,100,10,"Nike Inc","{\"Size\":\"US 9\",\"Color\":\"Black/White\",\"Material\":\"Mesh/Leather\",\"Weight\":\"320g\"}"',
      '"MacBook Pro 16-inch","Professional laptop for creators","APP-MBP16-001","Electronics",3500000,4500000,8,1,"Apple Inc","{\"Screen Size\":\"16.2 inches\",\"RAM\":\"32GB\",\"Storage\":\"1TB SSD\",\"Processor\":\"Apple M3 Pro\",\"Graphics\":\"Apple M3 Pro GPU\"}"'
    ].join('\n');
  ...
  const saveDraft = () => {
    ...
    const draft = {
      ...
      productCount: importData.length
    };

    const existingDrafts = JSON.parse(localStorage.getItem('bulkImportDrafts') || '[]');
    existingDrafts.push(draft);
    localStorage.setItem('bulkImportDrafts', JSON.stringify(existingDrafts));
  ...
  const getSavedDrafts = () => {
    return JSON.parse(localStorage.getItem('bulkImportDrafts') || '[]');
  };
  ...
  const deleteDraft = (draftId: string) => {
    ...
    localStorage.setItem('bulkImportDrafts', JSON.stringify(updatedDrafts));
            ...
            <>
              ...
              <button
                onClick={() => {
                  const drafts = getSavedDrafts();
                  if (drafts.length > 0) {
                    // Show drafts dropdown or modal
                    setShowDraftModal(true);
                  } else {
                    toast.success('No saved drafts found');
                  }
                }}
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            ...
            {importData.length > 0 && (
              <div className="mb-6 p-4 bg-green-50 rounded-lg border border-green-200">
                <h4 className="text-sm font-semibold text-green-800 mb-2">Save Current Work</h4>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={draftName}
                    ...
                    className="flex-1 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-green-500"
                  />
                  <button
                    onClick={saveDraft}
            ...
            {/* Load Draft Section */}
            <div>
              <h4 className="text-sm font-semibold text-gray-800 mb-3">Load Saved Draft</h4>
              {getSavedDrafts().length > 0 ? (
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {getSavedDrafts().map((draft: any) => (
                    <div key={draft.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-500">
                          {draft.productCount} products • {new Date(draft.createdAt).toLocaleDateString()}
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <button
