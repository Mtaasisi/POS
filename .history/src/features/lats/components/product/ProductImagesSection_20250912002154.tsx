...
const ProductImagesSection: React.FC<ProductImagesSectionProps> = ({
  ...
  // Handle paste from floating button
  const handleGlobalPaste = async () => {
    if (!currentUser?.id || isPasting) return;
    ...
    try {
      ...
      // Upload images using RobustImageService
      const uploadPromises = imageFiles.map(file => 
        RobustImageService.uploadImage(file, tempProductId, currentUser.id)
      );
      ...
      if (successfulUploads.length > 0) {
        ...
        const convertedImages = successfulUploads.map((uploadedImage, index) => ({
          id: uploadedImage.id,
          ...
          isPrimary: index === 0 && images?.length === 0, // First image is primary if no existing images
          is_primary: index === 0 && images?.length === 0,
          uploaded_by: currentUser.id,
          uploadedAt: uploadedImage.uploadedAt,
          created_at: uploadedImage.uploadedAt,
          mimeType: uploadedImage.mimeType
        }));

        // Add to existing images
        setImages(prev => [...(prev || []), ...convertedImages]);
        toast.success(`Successfully pasted ${successfulUploads.length} image(s)`);
      } else {
        toast.error('Failed to upload images from clipboard');
      }
    } catch (error) {
      console.error('Error pasting from clipboard:', error);
      toast.error('Failed to paste images from clipboard');
    } finally {
      setIsPasting(false);
    }
  };
