      ...
      {product.specifications && Object.keys(product.specifications).length > 0 && (
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-gray-500">
              {Object.entries(product.specifications).reduce((total, [key, value]) => {
                const values = String(value).split(',').filter(v => v.trim());
                return total + values.length;
              }, 0)} specification{Object.entries(product.specifications).reduce((total, [key, value]) => {
                const values = String(value).split(',').filter(v => v.trim());
                return total + values.length;
              }, 0) !== 1 ? 's' : ''}
            </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {Object.entries(product.specifications).map(([key, value]) => {
              ...
              return values.map((item, index) => (
                <div key={`${key}-${index}`} className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-200 hover:shadow-md transition-shadow">
