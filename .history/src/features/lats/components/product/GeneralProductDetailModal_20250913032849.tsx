...
import { 
  ...
  Info, Plus, Minus, Save, RotateCcw, ChevronDown, ChevronUp, MoreHorizontal
} from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    <span className="text-xs text-gray-500 uppercase tracking-wide">Stock Value</span>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    <span className="text-xs text-gray-500 uppercase tracking-wide">Avg Stock/Variant</span>
                    <p className="text-sm font-medium text-gray-900">
                      {product.variants && product.variants.length > 0 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
              ...
              {expandedSections.shipping ? (
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-medium text-gray-900">{product.weight || 'Not specified'} {product.weight ? 'kg' : ''}</p>
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-medium text-gray-900 capitalize">{product.shippingClass || 'Standard'}</p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-xs text-gray-500 uppercase tracking-wide">Special Handling</span>
                    <p className="text-sm font-medium text-orange-600">{product.requiresSpecialHandling ? 'Required' : 'Not required'}</p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-xs text-gray-500 uppercase tracking-wide">Shipping Status</span>
                    <p className="text-sm font-medium text-gray-900">{product.shippingStatus || 'Not specified'}</p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-xs text-gray-500 uppercase tracking-wide">Tracking</span>
                    <p className="text-sm font-medium text-gray-900">{product.trackingNumber || 'Not available'}</p>
                  </div>
                </div>
              ) : (
                <div className="text-sm text-gray-600">
                  <span>Weight: {product.weight || 'Not specified'}{product.weight ? 'kg' : ''}</span>
                  <span className="ml-4">Class: {product.shippingClass || 'Standard'}</span>
                  {product.requiresSpecialHandling && <span className="ml-4 text-orange-600">Special handling required</span>}
                </div>
              )}
            </div>
              ...
              {expandedSections.purchaseOrders ? (
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-medium text-gray-900">
                      {product.lastOrderDate ? new Date(product.lastOrderDate).toLocaleDateString() : 'No orders yet'}
                    </p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-xs text-gray-500 uppercase tracking-wide">Last Order Qty</span>
                    <p className="text-sm font-medium text-gray-900">{product.lastOrderQuantity || 'N/A'}</p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-xs text-gray-500 uppercase tracking-wide">Pending Qty</span>
                    <p className="text-sm font-medium text-orange-600">{product.pendingQuantity || 0}</p>
                  </div>
                  <div className="space-y-1">
                    ...
                    <p className={`text-sm font-medium ${
                      ...
                      product.orderStatus === 'cancelled' ? 'text-red-600' : 'text-gray-600'
                    }`}>
                      {product.orderStatus ? product.orderStatus.charAt(0).toUpperCase() + product.orderStatus.slice(1) : 'No orders'}
                    </p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-xs text-gray-500 uppercase tracking-wide">Reorder Level</span>
                    <p className="text-sm font-medium text-blue-600">
                      {currentProduct.variants && currentProduct.variants.length > 0 
                        ? Math.max(...currentProduct.variants.map(v => v.minQuantity || 0))
                        : 0
                      }
                    </p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-xs text-gray-500 uppercase tracking-wide">Stock Alert</span>
                    <p className={`text-sm font-medium ${
                      product.variants && product.variants.some(v => (v.quantity || 0) <= (v.minQuantity || 0))
                        ? 'text-red-600' : 'text-green-600'
                    }`}>
                      {product.variants && product.variants.some(v => (v.quantity || 0) <= (v.minQuantity || 0))
                        ? 'Low stock detected' : 'Stock levels OK'
                      }
                    </p>
                  </div>
                </div>
                ...
                <div className="text-sm text-gray-600">
                  <span>Last Order: {product.lastOrderDate ? new Date(product.lastOrderDate).toLocaleDateString() : 'No orders yet'}</span>
                  <span className="ml-4">Pending: {product.pendingQuantity || 0}</span>
                  {product.variants && product.variants.some(v => (v.quantity || 0) <= (v.minQuantity || 0)) && 
                    <span className="ml-4 text-red-600">Low stock alert</span>
                  }
                </div>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <p className={`text-sm font-medium ${
                    ...
                    product.shippingStatus === 'exception' ? 'text-red-600' : 'text-gray-600'
                  }`}>
                ...
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Tracking</span>
                  <p className="text-sm font-medium text-gray-900 font-mono">{product.trackingNumber || 'Not available'}</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Expected Delivery</span>
                  <p className="text-sm font-medium text-gray-900">
                    {product.expectedDelivery ? new Date(product.expectedDelivery).toLocaleDateString() : 'Not specified'}
                  </p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Agent</span>
                  <p className="text-sm font-medium text-gray-900">{product.shippingAgent || 'Not specified'}</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Shipping Required</span>
                  <p className="text-sm font-medium text-gray-900">{(product as any).requiresShipping !== false ? 'Yes' : 'No'}</p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Digital Product</span>
                  <p className="text-sm font-medium text-gray-900">{(product as any).isDigital ? 'Yes' : 'No'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Avg Price</span>
                  <p className="text-sm font-medium text-gray-900">
                    {product.variants && product.variants.length > 0 
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-medium text-blue-600">
                    {(product as any).isDigital ? 'Digital Product' : 'Physical Product'}
                  </p>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-gray-500 uppercase tracking-wide">Shipping</span>
                  <p className="text-sm font-medium text-gray-900">
                    {(product as any).requiresShipping !== false ? 'Required' : 'Not Required'}
                  </p>
                </div>
                <div className="space-y-1">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
              ...
              {expandedSections.analytics ? (
                ...
                <div className="flex items-center justify-between text-sm text-gray-600">
                  <span>Data completeness: {completeness}%</span>
                  <span>Days in inventory: {daysInStock}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {product.variants && product.variants.length > 0 && (
            <div className="mt-8 bg-white border border-gray-200 rounded-xl p-4 space-y-4">
              <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => toggleSection('completeVariants')}
                  className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 transition-colors"
                >
                  {expandedSections.completeVariants ? (
                    <>
