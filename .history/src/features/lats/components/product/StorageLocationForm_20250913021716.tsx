...
const StorageLocationForm: React.FC<StorageLocationFormProps> = ({
  ...
  const getCurrentShelves = () => {
    ...
    return shelves.sort((a, b) => {
      // First sort by code (case-insensitive)
      const codeComparison = a.code.toLowerCase().localeCompare(b.code.toLowerCase());
      if (codeComparison !== 0) {
        return codeComparison;
      }
      // If codes are the same, sort by ID for consistent ordering
