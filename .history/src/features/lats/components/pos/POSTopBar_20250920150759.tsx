...
import InventoryAlertPreferencesModal from './InventoryAlertPreferencesModal';
...
const POSTopBar: React.FC<POSTopBarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-gray-200 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto justify-between sm:justify-start">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              {/* Inventory Alert Preferences */}
              <button
                onClick={() => setShowAlertPreferences(true)}
                className={`p-2 sm:p-3 rounded-lg transition-all duration-200 shadow-sm touch-target flex items-center justify-center ${
                  areAlertsPermanentlyDisabled 
                    ? 'text-gray-400 bg-gray-100' 
                    : isLowStockAlertsEnabled 
                      ? 'text-orange-600 active:text-orange-800 active:bg-orange-50' 
                      : 'text-gray-500 active:text-gray-700 active:bg-gray-100'
                }`}
                title={areAlertsPermanentlyDisabled ? 'Alerts Disabled' : isLowStockAlertsEnabled ? 'Inventory Alerts Enabled' : 'Inventory Alerts Disabled'}
              >
                {areAlertsPermanentlyDisabled ? (
                  <BellOff size={16} className="sm:hidden" />
                ) : (
                  <Bell size={16} className="sm:hidden" />
                )}
                {areAlertsPermanentlyDisabled ? (
                  <BellOff size={18} className="hidden sm:block" />
                ) : (
      ...
      {/* Inventory Alert Preferences Modal */}
      <InventoryAlertPreferencesModal
        isOpen={showAlertPreferences}
        onClose={() => setShowAlertPreferences(false)}
      />
    </header>
  );
};
