          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex-shrink-0">
              <SimpleImageDisplay
                ...
                size="sm"
                className="w-8 h-8 sm:w-10 sm:h-10 cursor-pointer hover:opacity-90 transition-opacity"
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-1">
                <h3 className={`font-medium truncate text-xs sm:text-sm ${hasNoVariants ? 'text-gray-500' : 'text-gray-900'}`}>{product.name}</h3>
                {getStockStatusBadge()}
              </div>
            ...
            <div className="text-right">
              {showPrices && (
                <div className={`font-semibold text-xs sm:text-sm ${hasNoVariants ? 'text-gray-500' : theme.priceColor}`}>{getPriceDisplay()}</div>
              )}
              {showStockLevels && (
  ...
  return (
    <>
      ...
      >
        ...
        {/* Product Card Header */}
        <div className="p-3 sm:p-4 lg:p-6 cursor-pointer">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-3 lg:gap-4 flex-1 min-w-0">
              {/* Product Icon */}
              {showProductImages && (
                <div className={`relative w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-xl flex items-center justify-center text-lg font-bold ${theme.iconColor}`}>
                  <SimpleImageDisplay
                    images={productImages}
                    productName={product.name}
                    size="lg"
              ...
              {/* Product Info */}
              <div className="flex-1 min-w-0">
                <div className="font-medium text-gray-800 truncate text-sm sm:text-base lg:text-xl leading-tight">
                  {product.name}
                </div>
                <div className="text-lg sm:text-xl lg:text-2xl text-gray-700 mt-1 font-bold">
