...
const ProductInfoModal: React.FC<ProductInfoModalProps> = ({
  ...
  const getPriceRangeDisplay = () => {
    ...
    const prices = product.variants
      .map(v => typeof v.sellingPrice === 'number' ? v.sellingPrice : 0)
      .filter(p => p > 0)
      .sort((a, b) => a - b);
  ...
  const handleGenerateQRCode = () => {
    try {
      ...
      const sellingPrice = typeof currentVariant?.sellingPrice === 'number' ? currentVariant.sellingPrice : 0;
      const qrData = `Product: ${product.name}\nSKU: ${currentVariant?.sku || 'N/A'}\nPrice: ${format.money(sellingPrice)}\nDetails: ${productUrl}`;
  ...
  const handleCopyInfo = () => {
    const sellingPrice = typeof currentVariant?.sellingPrice === 'number' ? currentVariant.sellingPrice : 0;
    const quantity = typeof currentVariant?.quantity === 'number' ? currentVariant.quantity : 0;
      ...
      <GlassCard className="max-w-7xl w-full max-h-[95vh] overflow-hidden">
        <div className="p-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 rounded-xl p-4">
                ...
                <div className="text-lg font-bold text-emerald-900">
                  {format.money(typeof currentVariant?.sellingPrice === 'number' ? currentVariant.sellingPrice : 0)}
                </div>
              </div>
                        ...
                        >
                          ...
                          <div className="flex items-center gap-4 text-base">
                            <span className={`${isSelected ? 'text-blue-700' : 'text-gray-600'}`}>
                              Stock: {typeof variant.quantity === 'number' ? variant.quantity : 0}
                            </span>
                            <span className={`font-semibold text-lg ${isSelected ? 'text-blue-900' : 'text-gray-900'}`}>
                              {format.money(typeof variant.sellingPrice === 'number' ? variant.sellingPrice : 0)}
                            </span>
                          </div>
                        </div>
                      );
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-lg font-bold text-green-600">{format.money(typeof currentVariant.sellingPrice === 'number' ? currentVariant.sellingPrice : 0)}</p>
                    </div>
                    <div className="space-y-1">
                      <span className="text-xs text-gray-500 uppercase tracking-wide">Stock</span>
                      <p className={`text-sm font-medium ${
                        (typeof currentVariant.quantity === 'number' ? currentVariant.quantity : 0) > 10 ? 'text-green-600' : 
                        (typeof currentVariant.quantity === 'number' ? currentVariant.quantity : 0) > 0 ? 'text-orange-600' : 'text-red-600'
                      }`}>
                        {typeof currentVariant.quantity === 'number' ? currentVariant.quantity : 0} units
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex gap-4 mt-8">
            {onAddToCart && currentVariant && (typeof currentVariant.quantity === 'number' ? currentVariant.quantity : 0) > 0 && (
              <button
                onClick={() => {
              ...
              >
                ...
                Add to Cart - {format.money(typeof currentVariant.sellingPrice === 'number' ? currentVariant.sellingPrice : 0)}
              </button>
            )}
            {currentVariant && (typeof currentVariant.quantity === 'number' ? currentVariant.quantity : 0) <= 0 && (
            ...
            <GlassCard className="max-w-md w-full">
              ...
              <div className="text-center space-y-4">
                ...
                <div className="text-sm text-gray-600">
                  ...
                  <p>Price: {format.money(typeof currentVariant?.sellingPrice === 'number' ? currentVariant.sellingPrice : 0)}</p>
                </div>
