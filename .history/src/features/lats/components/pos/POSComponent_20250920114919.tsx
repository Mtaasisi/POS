                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 max-h-96 overflow-y-auto">
                      {products.map((product) => (
                        <div key={product.id} className="border border-gray-200 rounded-lg p-2 bg-white">
                          <h4 className="font-medium text-gray-900 mb-1 text-xs truncate" title={product.name}>{product.name}</h4>
                          <p className="text-xs text-gray-600 mb-2 truncate" title={product.description}>{product.description}</p>
                          {product.variants.map((variant) => (
                            <div key={variant.id} className="mb-1 p-1 bg-gray-50 rounded">
                              <div className="flex items-center justify-between">
                                <div>
                                  <div className="text-xs font-medium truncate" title={variant.name}>{variant.name}</div>
                                  <div className="text-xs text-gray-500 truncate" title={`SKU: ${variant.sku} • Stock: ${variant.quantity}`}>
                                    SKU: {variant.sku} • Stock: {variant.quantity}
                                  </div>
                                </div>
                                <div className="text-right">
                                  <div className="text-xs font-bold text-blue-600 truncate" title={formatMoney(variant.sellingPrice)}>
                                    {formatMoney(variant.sellingPrice)}
                                  </div>
                                  <GlassButton
                                    size="sm"
