...
import { 
  X, Package, Tag, MapPin, AlertCircle, CheckCircle, ShoppingCart, QrCode, Copy, Download,
  Info, Layers, Star, Building, Target, Maximize2, Minimize2
} from 'lucide-react';
...
const ProductInfoModal: React.FC<ProductInfoModalProps> = ({
  ...
  return (
    <div className={`fixed inset-0 z-50 ${isFullscreen ? 'fullscreen-modal' : 'flex items-center justify-center p-4'}`}>
      {/* Backdrop */}
      {!isFullscreen && (
        <div 
      ...
      <div 
        className={`relative bg-white shadow-2xl overflow-hidden ${
          isFullscreen 
            ...
            : 'rounded-2xl max-w-7xl w-full max-h-[95vh]'
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Desktop-Optimized Header */}
        <div className={`flex items-center justify-between border-b border-gray-100 ${isFullscreen ? 'p-4' : 'p-6'}`}>
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center">
              <Package className="w-6 h-6 text-white" />
            </div>
            <div>
              <h2 className={`font-bold text-gray-900 ${isFullscreen ? 'text-2xl' : 'text-xl'}`}>
                {product.name}
              </h2>
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className={`text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-lg transition-colors ${
        ...
        {/* Content */}
        <div className={`overflow-y-auto ${isFullscreen ? 'h-[calc(100vh-120px)] p-6' : 'max-h-[calc(95vh-120px)] p-6'}`}>
          {/* Financial Overview - Desktop Enhanced */}
          <div className="mb-8">
            <div className={`grid gap-6 ${isFullscreen ? 'grid-cols-4' : 'grid-cols-2 lg:grid-cols-4'}`}>
          ...
          {/* Main Content Layout - Desktop Optimized */}
          <div className={`grid gap-8 ${isFullscreen ? 'grid-cols-1 xl:grid-cols-3' : 'grid-cols-1 lg:grid-cols-2'}`}>
            {/* Left Column - Product Image & Basic Info */}
            <div className="space-y-6">
              {/* Product Image - Desktop Enhanced */}
              <div className="space-y-4">
                <div className={`relative rounded-xl overflow-hidden bg-gray-50 border border-gray-200 ${
                  isFullscreen ? 'aspect-[4/3]' : 'aspect-square'
                }`}>
        ...
        <div className={`border-t border-gray-100 ${isFullscreen ? 'p-4' : 'p-6'}`}>
          <div className={`flex items-center gap-3 ${isFullscreen ? 'justify-between' : ''}`}>
            {onAddToCart && currentVariant && currentVariant.quantity > 0 && (
              <button
                ...
                className={`flex items-center justify-center gap-3 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors font-semibold min-h-[56px] ${
                  isFullscreen 
                    ? 'px-8 py-4 text-lg flex-1' 
                    : 'px-6 py-4 text-lg flex-1'
                }`}
              >
            ...
            {currentVariant && currentVariant.quantity <= 0 && (
              <div className={`flex items-center justify-center gap-3 bg-gray-100 text-gray-500 rounded-lg font-semibold min-h-[56px] ${
                isFullscreen ? 'px-8 py-4 text-lg flex-1' : 'px-6 py-4 text-lg flex-1'
              }`}>
            ...
            <button
              onClick={onClose}
              className={`border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-lg transition-colors font-medium min-h-[56px] ${
                isFullscreen ? 'px-8 py-4' : 'px-6 py-4'
              }`}
            >
              Close (Esc)
            </button>
