          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-1">
                <h3 className={`font-medium truncate text-xs ${hasNoVariants ? 'text-gray-500' : 'text-gray-900'}`} title={product.name}>{product.name}</h3>
                {getStockStatusBadge()}
              </div>
              <p className="text-xs text-gray-500 font-mono truncate" title={primaryVariant?.sku || 'N/A'}>{primaryVariant?.sku || 'N/A'}</p>
            ...
            <div className="text-right">
              {showPrices && (
                <div className={`font-semibold text-xs ${hasNoVariants ? 'text-gray-500' : theme.priceColor}`} title={getPriceDisplay()}>{getPriceDisplay()}</div>
              )}
              {showStockLevels && (
                <div className="text-xs text-gray-500 truncate" title={`Stock: ${getTotalStock()}`}>Stock: {getTotalStock()}</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="p-6 cursor-pointer">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4 flex-1 min-w-0">
              ...
              {/* Product Info */}
              <div className="flex-1 min-w-0">
                <div className="font-medium text-gray-800 truncate text-sm leading-tight" title={product.name}>
                  {product.name}
                </div>
                <div className="text-sm text-gray-700 mt-1 font-bold truncate" title={getPriceDisplay()}>
          ...
          {!isDisabled && (
            <div className="mt-3 pt-3 border-t border-gray-100">
              <div className={`text-center text-xs font-medium ${theme.textColor} opacity-70 hover:opacity-100 transition-opacity`}>
                Click to {actionText}
              </div>
            </div>
          )}
