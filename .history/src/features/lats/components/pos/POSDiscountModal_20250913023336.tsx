...
const POSDiscountModal: React.FC<POSDiscountModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 max-w-lg w-full p-8">
        ...
        {/* Preview */}
        {discountValue && (
          <div className="mb-6 p-4 bg-gradient-to-r from-emerald-50 to-blue-50 rounded-xl border border-emerald-200/50">
            <div className="text-sm font-medium text-emerald-700 mb-3">Discount Preview</div>
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Subtotal</span>
                <span className="text-sm font-medium text-gray-900">{formatMoney(currentTotal)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-red-600">Discount</span>
                <span className="text-sm font-medium text-red-600">-{formatMoney(discountAmount)}</span>
              </div>
              <div className="border-t border-gray-200 pt-2">
                <div className="flex justify-between items-center">
                  <span className="text-base font-semibold text-gray-900">Final Total</span>
                  <span className="text-lg font-bold text-emerald-600">{formatMoney(finalTotal)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full p-4 text-2xl font-bold text-center border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/20 bg-gray-50/50 transition-all duration-200"
                readOnly
              />
              <div className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-500 text-lg font-medium">
              ...
              <div className="grid grid-cols-4 gap-2">
                {[5, 10, 15, 20].map((percent) => (
                  <button
                    ...
                    className="py-3 px-4 text-sm font-semibold bg-gradient-to-br from-gray-100 to-gray-200 hover:from-blue-100 hover:to-blue-200 text-gray-700 hover:text-blue-700 rounded-xl border border-gray-200 hover:border-blue-300 transition-all duration-200 hover:shadow-md"
                  >
                    {percent}%
                  </button>
                ))}
