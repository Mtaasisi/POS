      ...
      <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 max-w-4xl w-full p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full p-3 text-xl font-black text-center border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/20 bg-gray-50/50 transition-all duration-200"
                  readOnly
                />
                <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-600 text-base font-bold">
            ...
            {discountType === 'percentage' && (
            <div>
              <label className="block text-sm font-bold text-gray-800 mb-2">QUICK SELECT</label>
                <div className="grid grid-cols-4 gap-2">
                  {[5, 10, 15, 20].map((percent) => (
                    <button
                      key={percent}
                      onClick={() => setDiscountValue(percent.toString())}
                      className="py-2 px-3 text-sm font-bold bg-gradient-to-br from-gray-100 to-gray-200 hover:from-blue-100 hover:to-blue-200 text-gray-700 hover:text-blue-700 rounded-lg border border-gray-200 hover:border-blue-300 transition-all duration-200 hover:shadow-md"
                    >
                      {percent}%
                    </button>
                  ))}
          ...
          <div>
            <label className="block text-sm font-bold text-gray-800 mb-3">NUMBER KEYBOARD</label>
        ...
        <div className="flex gap-3 mt-6 pt-4 border-t border-gray-200">
          <button
            onClick={handleClose}
            className="flex-1 py-3 px-6 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl text-sm font-bold transition-all duration-200 hover:shadow-md"
          >
            Cancel
          </button>
          {hasExistingDiscount && (
