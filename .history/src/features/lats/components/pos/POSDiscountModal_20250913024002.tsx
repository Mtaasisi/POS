      ...
      <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 max-w-4xl w-full p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-gray-600">Add discount to this transaction</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-black text-gray-800 mb-3">DISCOUNT TYPE</label>
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                    <div className="text-center">
                      <div className="text-sm font-black">FIXED AMOUNT</div>
                      <div className="text-xs font-bold text-gray-600 mt-1">TZS off total</div>
                    </div>
                  {discountType === 'fixed' && (
                ...
                >
                    <div className="text-center">
                      ...
                      <div className="text-xs font-bold text-gray-600 mt-1">% off total</div>
                    </div>
                  {discountType === 'percentage' && (
                    <div className="absolute -top-1 -right-1 w-3 h-3 bg-blue-500 rounded-full"></div>
                  )}
            ...
            {/* Preview */}
            {discountValue && (
              <div className="p-3 bg-gradient-to-r from-emerald-50 to-blue-50 rounded-xl border border-emerald-200/50">
                <div className="text-sm font-medium text-emerald-700 mb-2">Discount Preview</div>
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-600">Subtotal</span>
                    <span className="text-xs font-medium text-gray-900">{formatMoney(currentTotal)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-red-600">Discount</span>
                    <span className="text-xs font-medium text-red-600">-{formatMoney(discountAmount)}</span>
                  </div>
                  <div className="border-t border-gray-200 pt-1">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-semibold text-gray-900">Final Total</span>
                      <span className="text-sm font-bold text-emerald-600">{formatMoney(finalTotal)}</span>
                    </div>
                  </div>
                </div>
              </div>
