...
import { Package, ChevronDown, ChevronUp, Tag, Hash, Plus, Minus, Search, AlertCircle, Image, ShoppingCart } from 'lucide-react';
...
import { getSpecificationIcon, getSpecificationTooltip, getShelfDisplay, getShelfIcon, formatSpecificationValue, parseSpecification } from '../../lib/specificationUtils';
...
const VariantProductCard: React.FC<VariantProductCardProps> = ({
  ...
  // Default detailed variant with modal-style UI
  const hasNoVariants = !product.variants || product.variants.length === 0;
  const isDisabled = hasNoVariants || !primaryVariant || (!allowOutOfStockSelection && primaryVariant.quantity <= 0);
  ...
  return (
    <>
      <div 
        className={`relative bg-white border border-gray-200 rounded-xl shadow-sm transition-all duration-300 ${className} ${
          isDisabled ? 'opacity-50 cursor-not-allowed' : `cursor-pointer ${theme.hoverBorder} hover:shadow-md active:scale-95`
        } ${hasNoVariants ? 'border-gray-300 bg-gray-50' : ''}`}
        onClick={handleCardClick}
        title={hasNoVariants ? 'This product has no variants and cannot be added to cart. Please add variants in the inventory management.' : `Click to ${actionText.toLowerCase()}`}
            ...
            {showProductImages && (
              <div className="relative w-16 h-16 rounded-lg overflow-hidden bg-gray-50 flex-shrink-0">
                ...
                {product.variants && product.variants.length > 1 && (
                  <div className="absolute -top-1 -right-1 w-5 h-5 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full border-2 border-white shadow-sm flex items-center justify-center">
        ...
        <div className="p-4 space-y-3">
          ...
          {primaryVariant?.attributes?.specification && (
            <div className="space-y-2">
              <h4 className="text-sm font-medium text-gray-700 uppercase tracking-wide">Specifications</h4>
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(parseSpecification(primaryVariant.attributes.specification)).slice(0, 4).map(([key, value]) => (
                  <div key={key} className="flex justify-between items-center py-1 px-2 bg-gray-50 rounded text-xs">
                    <span className="text-gray-600 capitalize">{key.replace(/_/g, ' ')}</span>
                    <span className="font-medium text-gray-900">{formatSpecificationValue(key, value)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="space-y-1">
              <span className="text-gray-500 uppercase tracking-wide">Product ID</span>
              <p className="font-mono text-gray-900 truncate">{product.id}</p>
            </div>
            <div className="space-y-1">
              <span className="text-gray-500 uppercase tracking-wide">Total Variants</span>
              <p className="font-medium text-gray-900">{product.variants?.length || 0}</p>
            </div>
            <div className="space-y-1">
              <span className="text-gray-500 uppercase tracking-wide">Total Stock</span>
              <p className="font-medium text-gray-900">{getTotalStock()} units</p>
            </div>
            <div className="space-y-1">
              <span className="text-gray-500 uppercase tracking-wide">Stock Status</span>
              <p className={`font-medium ${
                getTotalStock() > 10 ? 'text-green-600' : 
                getTotalStock() > 0 ? 'text-orange-600' : 'text-red-600'
              }`}>
                {getTotalStock() > 10 ? 'In Stock' : getTotalStock() > 0 ? 'Low Stock' : 'Out of Stock'}
              </p>
            </div>
          </div>
        ...
        <div className="p-4 pt-0">
          {!isDisabled ? (
            ...
            >
              ...
              Add to Cart - TSh {getPriceDisplay().replace('$', '').replace('.00', '').replace('.0', '')}
            </button>
          ) : (
            <div className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gray-100 text-gray-500 rounded-lg font-medium">
              <AlertCircle className="w-4 h-4" />
              {hasNoVariants ? 'No Variants Available' : 'Out of Stock'}
            </div>
          )}
        </div>
      </div>
