      ...
      <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 max-w-4xl w-full p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Add discount to this transaction</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="text-xs text-gray-500 mt-1">TZS off total</div>
                  </div>
                  {discountType === 'fixed' && (
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="text-xs text-gray-500 mt-1">% off total</div>
                  </div>
                  {discountType === 'percentage' && (
                    <div className="absolute -top-1 -right-1 w-3 h-3 bg-blue-500 rounded-full"></div>
                  )}
            ...
            {/* Preview */}
            {discountValue && (
              <div className="p-4 bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 rounded-xl border-2 border-red-200 shadow-lg">
                <div className="text-sm font-black text-red-800 mb-3 uppercase tracking-wide">DISCOUNT PREVIEW</div>
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-bold text-gray-800">SUBTOTAL</span>
                    <span className="text-sm font-black text-gray-900">{formatMoney(currentTotal)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-bold text-red-700">DISCOUNT</span>
                    <span className="text-sm font-black text-red-700">-{formatMoney(discountAmount)}</span>
                  </div>
                  <div className="border-t-2 border-red-300 pt-2">
                    <div className="flex justify-between items-center">
                      <span className="text-base font-black text-gray-900">FINAL TOTAL</span>
                      <span className="text-lg font-black text-green-700">{formatMoney(finalTotal)}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {discountType === 'percentage' && (
              <div>
                <label className="block text-sm font-black text-gray-800 mb-2 uppercase tracking-wide">QUICK SELECT</label>
                <div className="grid grid-cols-4 gap-2">
                  {[5, 10, 15, 20].map((percent) => (
                    <button
                      key={percent}
                      onClick={() => setDiscountValue(percent.toString())}
                      className="py-2 px-3 text-sm font-black bg-gradient-to-br from-gray-100 to-gray-200 hover:from-blue-100 hover:to-blue-200 text-gray-800 hover:text-blue-800 rounded-lg border-2 border-gray-300 hover:border-blue-400 transition-all duration-200 hover:shadow-md"
                    >
                      {percent}%
                    </button>
                  ))}
          ...
          <div>
            <label className="block text-sm font-black text-gray-800 mb-3 uppercase tracking-wide">NUMBER KEYBOARD</label>
        ...
        {/* Action Buttons */}
        <div className="flex gap-3 mt-6 pt-4 border-t-2 border-gray-300">
          <button
            onClick={handleClose}
            className="flex-1 py-3 px-6 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-xl text-sm font-black transition-all duration-200 hover:shadow-md"
          >
            CANCEL
          </button>
          {hasExistingDiscount && (
            <button
              ...
              className="flex-1 py-3 px-6 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-xl text-sm font-black transition-all duration-200 hover:shadow-lg shadow-red-500/25"
            >
              CLEAR
            </button>
          )}
          <button
            ...
            className="flex-1 py-3 px-6 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-xl text-sm font-black transition-all duration-200 hover:shadow-lg shadow-blue-500/25 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:shadow-none"
          >
            APPLY DISCOUNT
          </button>
        </div>
