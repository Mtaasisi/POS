...
const CustomerSelectionModal: React.FC<CustomerSelectionModalProps> = ({
  ...
  const loadAllCustomers = async () => {
    try {
      ...
      try {
        const fallbackResult = await searchCustomersFast('', 1, 50);
        if (fallbackResult.ok && fallbackResult.data) {
          setRecentCustomers(fallbackResult.data);
        }
      } catch (fallbackError) {
      ...
      <GlassCard className="w-full max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {!searchQuery && recentCustomers.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">All Customers ({recentCustomers.length})</h3>
              <div className="grid gap-3">
