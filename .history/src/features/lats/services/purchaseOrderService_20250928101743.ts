...
export class PurchaseOrderService {
  ...
  // Helper function to check if all items are fully received
  static async areAllItemsFullyReceived(purchaseOrderId: string): Promise<boolean> {
    try {
      ...
      return items.every(item => item.received_quantity >= item.quantity);
    } catch (error) {
      console.error('Error in areAllItemsFullyReceived:', error);
      return false;
    }
  }

  // Utility function to fix existing orders with incorrect status
  static async fixOrderStatusIfNeeded(purchaseOrderId: string, userId: string): Promise<{
    success: boolean;
    message: string;
    ...
    try {
      ...
      if (orderError || !order) {
        return { success: false, message: 'Order not found', statusChanged: false };
      }

      // Check if all items are fully received
      const allItemsFullyReceived = await this.areAllItemsFullyReceived(purchaseOrderId);
      
      // If order is partial_received but all items are fully received, fix the status
      if (order.status === 'partial_received' && allItemsFullyReceived) {
        // Use direct update to avoid 400 errors
        ...
        // Add simple audit entry
        try {
          await this.addAuditEntry({
            purchaseOrderId,
            action: 'Status auto-corrected to received',
            user: userId,
            details: JSON.stringify({
              message: 'Order status automatically corrected from partial_received to received',
              previous_status: order.status,
              new_status: 'received',
              reason: 'All items fully received'
            })
          });
        } catch (auditError) {
          console.warn('Audit entry failed, but status was updated:', auditError);
        }

        return {
          success: true,
          message: 'Order status corrected from partial_received to received',
          statusChanged: true
        };
      }
  ...
  static async updateInventoryItemStatus(
    itemId: string,
    newStatus: string,
    reason?: string,
    location?: string,
    ...
    try {
      console.log('🔧 [PurchaseOrderService] Updating inventory item status:', {
        itemId,
        newStatus,
        reason,
        location,
      ...
      if (fetchError) {
        console.error('❌ [PurchaseOrderService] Failed to fetch updated item:', fetchError);
        return { success: false, error: 'Item updated but failed to fetch updated data' };
      }

      console.log('✅ [PurchaseOrderService] Item status updated successfully');
      return { success: true, data: updatedItem };
    } catch (error) {
      console.error('❌ [PurchaseOrderService] Unexpected error:', error);
      return { 
        success: false, 
  ...
  static async bulkUpdateInventoryStatus(
    itemIds: string[],
    newStatus: string,
    reason?: string
  ): Promise<{
    ...
    try {
      console.log('🔧 [PurchaseOrderService] Bulk updating inventory items:', {
      ...
      if (error) {
        console.error('❌ [PurchaseOrderService] Bulk update failed:', error);
        return { success: false, error: error.message };
      }

      console.log('✅ [PurchaseOrderService] Bulk update completed:', data);
  ...
  static async getInventoryItemHistory(itemId: string): Promise<{
    success: boolean;
    data?: any[];
    error?: string;
  }> {
    try {
      console.log('🔍 [PurchaseOrderService] Fetching item history:', itemId);
      ...
      if (error) {
        console.error('❌ [PurchaseOrderService] Failed to fetch history:', error);
        return { success: false, error: error.message };
      }

      console.log('✅ [PurchaseOrderService] History fetched:', data?.length || 0, 'entries');
      return { success: true, data: data || [] };
    } catch (error) {
      console.error('❌ [PurchaseOrderService] Unexpected error:', error);
      return { 
  ...
  static async getPurchaseOrderInventoryStats(purchaseOrderId: string): Promise<{
    success: boolean;
    data?: any[];
    error?: string;
  }> {
    try {
      console.log('📊 [PurchaseOrderService] Fetching PO inventory stats:', purchaseOrderId);
      ...
      if (error) {
        console.error('❌ [PurchaseOrderService] Failed to fetch stats:', error);
        return { success: false, error: error.message };
      }

      console.log('✅ [PurchaseOrderService] Stats fetched:', data?.length || 0, 'categories');
      return { success: true, data: data || [] };
    } catch (error) {
      console.error('❌ [PurchaseOrderService] Unexpected error:', error);
      return { 
  ...
  static async exportInventoryToCSV(
    purchaseOrderId: string,
    filters?: {
      status?: string;
      location?: string;
    ...
    try {
      console.log('📤 [PurchaseOrderService] Exporting inventory to CSV:', purchaseOrderId);
      ...
      if (error) {
        console.error('❌ [PurchaseOrderService] Failed to fetch items for export:', error);
        return { success: false, error: error.message };
      }
      ...
      console.log('✅ [PurchaseOrderService] CSV export completed:', items?.length || 0, 'items');
