...
export class PurchaseOrderService {
  ...
  static async getPurchaseOrderItemsWithProducts(purchaseOrderId: string): Promise<{
    ...
    try {
      console.log('🔍 [PurchaseOrderService] Fetching items with products for PO:', purchaseOrderId);
      ...
      if (!functionError && functionData) {
        console.log('✅ [PurchaseOrderService] Used database function, found:', functionData.length, 'items');
        return { success: true, data: functionData };
      }

      console.log('⚠️ [PurchaseOrderService] Database function failed, using fallback query:', functionError?.message);
      
      // Fallback: Use direct query with joins
      const { data, error } = await supabase
        .from('lats_purchase_order_items')
        .select(`
          id,
      ...
      if (error) {
        console.error('❌ [PurchaseOrderService] Error fetching items with products:', error);
        return { success: false, message: 'Failed to fetch items with product details' };
      }

      console.log('✅ [PurchaseOrderService] Fallback query successful, found:', data?.length || 0, 'items');
      return { success: true, data: data || [] };
    } catch (error) {
      console.error('❌ [PurchaseOrderService] Error in getPurchaseOrderItemsWithProducts:', error);
      return { 
        success: false, 
        message: `Unexpected error: ${error instanceof Error ? error.message : 'Unknown error'}` 
      };
  ...
  static async getReceivedItems(purchaseOrderId: string): Promise<{
    ...
    try {
      ...
      // First, get inventory items (items with serial numbers)
      ...
      if (inventoryError) {
        ...
        // Don't return error here, continue with adjustments
        console.log('⚠️ Continuing with inventory adjustments only');
      }

      // Then, get inventory adjustments (items without serial numbers)
      ...
      if (adjustmentsError) {
        ...
        return { success: false, message: 'Failed to fetch inventory adjustments' };
      }
      ...
      const allReceivedItems = [
        ...
        ...(inventoryItems || []).map(item => ({
          ...
          received_quantity: 1, // Each inventory item represents 1 unit
          has_serial: true
        })),
        // Inventory adjustments (without serial numbers)
        ...(adjustments || []).map(adjustment => ({
          ...
          metadata: {
            purchase_order_id: adjustment.purchase_order_id,
      ...
      console.log('🔍 [PurchaseOrderService] Received items query result:', { 
        inventory_items: inventoryItems?.length || 0, 
        adjustments: adjustments?.length || 0,
        total: allReceivedItems.length 
      });
      
      // Debug: Log sample data
      if (adjustments && adjustments.length > 0) {
        console.log('📦 [PurchaseOrderService] Sample adjustment:', adjustments[0]);
      }
      if (inventoryItems && inventoryItems.length > 0) {
        console.log('📦 [PurchaseOrderService] Sample inventory item:', inventoryItems[0]);
      }

      console.log('✅ [PurchaseOrderService] Successfully fetched received items:', allReceivedItems.length, 'items');
      return { success: true, data: allReceivedItems };
    } catch (error) {
