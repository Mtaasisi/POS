...
export class PurchaseOrderService {
  ...
  static async loadShippedItems(purchaseOrderId: string): Promise<any[]> {
    ...
    if (!purchaseOrderId || typeof purchaseOrderId !== 'string' || purchaseOrderId.trim() === '') {
      console.error('❌ Invalid purchase order ID provided to loadShippedItems:', purchaseOrderId);
      throw new Error('Invalid purchase order ID: ID must be a non-empty string');
    }
  ...
  static async getReceivedItems(purchaseOrderId: string): Promise<{
    ...
    try {
      ...
      // Try the RPC function first with retry
      try {
  ...
  static async fixOrderStatusIfNeeded(purchaseOrderId: string, userId: string): Promise<{ success: boolean; message: string; statusChanged?: boolean }> {
    try {
      ...
      // Check if order has received items but status is still draft
      ...
      if (order.status === 'draft' && hasReceivedItems) {
        newStatus = 'received';
        statusChanged = true;
      } else if (order.status === 'received' && hasFullyReceivedItems && 
                 order.items.every((item: any) => item.received_quantity >= item.quantity)) {
        newStatus = 'completed';
        statusChanged = true;
      }
