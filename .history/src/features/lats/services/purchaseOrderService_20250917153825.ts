...
export class PurchaseOrderService {
  ...
  // Get received items (inventory items) for a purchase order
  static async getReceivedItems(purchaseOrderId: string): Promise<{
    ...
    try {
      ...
      console.log('🔍 [PurchaseOrderService] Received items query result:', { data, error });

      if (error) {
        console.error('❌ Error fetching received items:', error);
        return { success: false, message: 'Failed to fetch received items' };
      }

      console.log('✅ [PurchaseOrderService] Successfully fetched received items:', data?.length || 0, 'items');
      return { success: true, data: data || [] };
    } catch (error) {
