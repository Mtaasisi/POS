  ...
  devtools(
    subscribeWithSelector((set, get) => ({
      ...
      isCacheValid: (dataType: keyof InventoryState['dataCache']) => {
        ...
        console.log(`🔍 [DEBUG] Cache validity check for ${dataType}:`, {
          hasData: state.dataCache[dataType] !== null,
          cacheAge,
          cacheDuration: state.CACHE_DURATION,
          isValid
        });
        return isValid;
      },
      ...
      updateCache: (dataType: keyof InventoryState['dataCache'], data: any) => {
        const state = get();
        console.log(`🔍 [DEBUG] Updating cache for ${dataType}:`, { dataLength: data?.length || 0 });
        set({
          dataCache: {
            ...state.dataCache,
            [dataType]: data
      ...
      clearCache: (dataType?: keyof InventoryState['dataCache']) => {
        const state = get();
        console.log(`🔍 [DEBUG] Clearing cache for ${dataType || 'all'}`);
        if (dataType) {
      ...
      loadCategories: async () => {
        ...
        try {
          ...
          console.log('🔄 Creating fallback categories due to database error...');
      ...
      loadProducts: async (filters?: any) => {
        ...
        try {
          ...
          console.log('🔄 Creating fallback sample categories and products...');
      ...
      loadSpareParts: async () => {
        console.log('🔍 [DEBUG] loadSpareParts: Starting to load spare parts...');
        set({ isLoading: true, error: null });
        try {
          const provider = getLatsProvider();
          console.log('🔍 [DEBUG] loadSpareParts: Provider obtained, calling getSpareParts...');
          const response = await provider.getSpareParts();
          console.log('🔍 [DEBUG] loadSpareParts: Response received:', {
            ok: response.ok,
            message: response.message,
          ...
          if (response.ok) {
            ...
            console.log('🔍 [DEBUG] loadSpareParts: Setting spare parts in store:', sparePartsData.length);
            ...
            console.log('✅ [DEBUG] loadSpareParts: Spare parts loaded successfully');
          } else {
          ...
          set({ isLoading: false });
          console.log('🔍 [DEBUG] loadSpareParts: Loading completed');
        }
      },
      ...
      createOrUpdateSparePart: async (sparePart) => {
        ...
        try {
          ...
          if (response.ok) {
            console.log('🔍 [DEBUG] createOrUpdateSparePart: API call successful, clearing cache and reloading...');
            // Clear cache to force fresh data load
            get().clearCache('spareParts');
            console.log('🔍 [DEBUG] createOrUpdateSparePart: Cache cleared, calling loadSpareParts...');
            await get().loadSpareParts();
            console.log('🔍 [DEBUG] createOrUpdateSparePart: loadSpareParts completed');
