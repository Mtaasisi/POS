...
class LatsAnalyticsService {
  ...
  async getComprehensiveAnalytics(): Promise<AnalyticsData> {
    try {
      ...
      const previousCustomers = currentCustomers * 0.9; // Simplified for now
      ...
      const previousOrders = currentOrders * 0.9; // Simplified for now
      ...
      const previousAvgOrderValue = avgOrderValue * 0.95; // Simplified for now
