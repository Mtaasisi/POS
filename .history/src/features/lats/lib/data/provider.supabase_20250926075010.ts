...
class SupabaseDataProvider implements LatsDataProvider {
  ...
  async getSales(): Promise<ApiResponse<Sale[]>> {
    try {
      ...
      if (error) {
        console.warn('Complex sales query failed, trying simpler query:', error.message);
        ...
        if (simpleError) {
          console.error('Simple sales query also failed:', simpleError);
          throw simpleError;
        }

        // Return sales data without the complex joins
        return { ok: true, data: simpleData || [] };
      }
  ...
  async getSale(id: string): Promise<ApiResponse<Sale>> {
    try {
      ...
      if (error) {
        console.warn('Complex sale query failed, trying simpler query:', error.message);
        ...
        if (simpleError) {
          console.error('Simple sale query also failed:', simpleError);
          throw simpleError;
        }

        // Return sale data without the complex joins
        return { ok: true, data: simpleData };
      }
