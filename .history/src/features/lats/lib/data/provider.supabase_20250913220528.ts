      ...
      const transformedData = {
        ...
        shippingInfo: (() => {
          ...
          if (shippingInfo) {
            ...
            // Get the related data that was fetched earlier
            const carrierData = shippingInfo.carrier_id ? await this.getCarrierData(shippingInfo.carrier_id).catch(() => null) : null;
            const agentData = shippingInfo.agent_id ? await this.getAgentData(shippingInfo.agent_id).catch(() => null) : null;
            const managerData = shippingInfo.manager_id ? await this.getManagerData(shippingInfo.manager_id).catch(() => null) : null;
            
            const transformedShippingInfo = {
              id: shippingInfo.id,
              carrier: carrierData?.name || shippingInfo.carrier_name || 'Unknown Carrier',
