  ...
  async getUsageHistory(sparePartId: string, limit: number = 10): Promise<SparePartUsage[]> {
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          device:lats_devices(name, model, serial_number)
        `)
        .eq('spare_part_id', sparePartId)
  ...
  async getRelatedSpareParts(sparePartId: string, limit: number = 5): Promise<SparePart[]> {
    try {
      ...
      const { data, error } = await supabase
        ...
        .neq('id', sparePartId)
        .eq('is_active', true)
        .or(`category_id.eq.${currentSparePart.category_id},supplier_id.eq.${currentSparePart.supplier_id},name.ilike.%${currentSparePart.name.split(' ')[0]}%`)
        .limit(limit);
  ...
  async getSupplierDetails(supplierId: string): Promise<any> {
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          purchase_orders:lats_purchase_orders(id, created_at, status)
        `)
        .eq('id', supplierId)
        .single();
