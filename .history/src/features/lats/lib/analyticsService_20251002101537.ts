    ...
    try {
      ...
      if (products && products.length > 0) {
        ...
        products.forEach(product => {
          const categoryName = 'Electronics'; // Default category since we don't have category data
          const productValue = product.variants?.reduce((sum: number, variant: any) => {
            return sum + ((variant.cost_price || 0) * (variant.quantity || 0));
          }, 0) || 0;
        ...
        topCategories.push(...Array.from(categoryMap.values())
