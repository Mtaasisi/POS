...
const saveSparePartImagesToDatabase = async (sparePartId: string, images: any[], userId?: string): Promise<void> => {
  try {
    ...
    const imageData = images.map((img, index) => ({
      product_id: sparePartId, // Using product_id column for spare parts
    ...
    // Insert images into spare_part_images table
    ...
    if (error) {
      console.error('❌ [DEBUG] Error saving images to database:', error);
      throw error;
    }

    console.log('✅ [DEBUG] Images saved to database successfully:', data);
  } catch (error) {
    console.error('❌ [DEBUG] Failed to save images to database:', error);
    // Don't throw error here to avoid breaking the main spare part creation
  }
};

// Get spare part images from product_images table
export const getSparePartImages = async (sparePartId: string): Promise<any[]> => {
  try {
    ...
    if (error) {
      console.error('❌ [DEBUG] Error getting spare part images:', error);
      return [];
    }

    console.log('✅ [DEBUG] Retrieved spare part images:', data);
    return data || [];
  } catch (error) {
...
export const updateSparePartWithVariants = async (id: string, sparePartData: any): Promise<SparePartResponse> => {
  try {
    ...
    if (sparePartData.images && sparePartData.images.length > 0) {
      console.log('🔍 [DEBUG] Updating images in product_images table...');
      
      // First, delete existing images for this spare part
      ...
      if (deleteError) {
        console.error('❌ [DEBUG] Error deleting existing images:', deleteError);
      }
