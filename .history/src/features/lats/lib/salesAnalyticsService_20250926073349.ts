  ...
  async getSalesAnalytics(period: string = '7d'): Promise<SalesAnalyticsData | null> {
    try {
      ...
      try {
        ...
        const { data: complexSales, error: complexError } = await supabase
          .from('lats_sales')
          .select(`
            id,
            sale_number,
        ...
        if (complexError) {
          console.warn('Complex sales analytics query failed, trying simpler query:', complexError.message);
          ...
          if (simpleError) {
            console.error('Simple sales analytics query also failed:', simpleError);
            sales = [];
            salesError = simpleError;
          } else {
            sales = simpleSales || [];
            salesError = null;
            console.log(`✅ Loaded ${sales.length} sales for analytics (without joins)`);
          }
        } else {
          sales = complexSales || [];
          salesError = null;
          console.log(`✅ Loaded ${sales.length} sales for analytics`);
        }
      } catch (error) {
      ...
      if (salesError) {
        ...
        // Fallback to simpler query without joins
        const { data: simpleSales, error: simpleSalesError } = await supabase
          .from('lats_sales')
          .select(`
            id,
            sale_number,
        ...
        if (simpleSalesError) {
          console.error('Simple sales analytics query also failed:', simpleSalesError);
          return null;
        }

        // Use the simple data without sale items
        const sales = simpleSales;
        console.log(`✅ Loaded ${sales?.length || 0} sales for analytics (without sale items)`);
      } else {
