...
export const usePOSCustomer = () => {
  ...
  const searchCustomers = useCallback((query: string): CustomerSearchResult[] => {
    ...
    customers.forEach(customer => {
      ...
      // Check phone with enhanced conversion logic
      if (customer.phone) {
        // Clean the search term and customer phone for comparison
        ...
        // Enhanced phone number conversion (07123 -> 2557123)
        if (/^0\d{3,}$/.test(cleanSearchTerm)) {
          const convertedNumber = `255${cleanSearchTerm.substring(1)}`;
          if (cleanCustomerPhone.includes(convertedNumber)) {
            relevance += 8;
          }
        }
        
        // Handle reverse conversion (2557123 -> 07123)
        if (/^255\d{3,}$/.test(cleanSearchTerm)) {
          const convertedNumber = `0${cleanSearchTerm.substring(3)}`;
          if (cleanCustomerPhone.includes(convertedNumber)) {
            relevance += 8;
          }
        }
