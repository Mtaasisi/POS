...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    // Check if this error occurred shortly after a reload to prevent infinite loops
    const now = Date.now();
    const { lastReloadTime, reloadCount } = this.state;
    const timeSinceLastReload = lastReloadTime ? now - lastReloadTime : Infinity;
    
    // If error occurred within 5 seconds of last reload, increment reload count
    let newReloadCount = reloadCount;
    if (timeSinceLastReload < 5000) {
      newReloadCount = reloadCount + 1;
      console.warn(`🔄 Error occurred ${timeSinceLastReload}ms after reload. Reload count: ${newReloadCount}`);
    }
    
    this.setState({ 
      errorInfo,
      reloadCount: newReloadCount,
      lastReloadTime: newReloadCount > reloadCount ? now : lastReloadTime
    });
    ...
    console.error('Reload Count:', newReloadCount);
    console.groupEnd();
  ...
  handleRefreshPage = () => {
    ...
    if (reloadCount >= maxReloads) {
      console.error('❌ Maximum reload attempts reached. Redirecting to safe page.');
      // Clear any problematic data and redirect to dashboard
      localStorage.clear();
    ...
    console.log(`🔄 Refreshing page... (attempt ${reloadCount + 1}/${maxReloads})`);
    this.setState({ 
  ...
  render() {
    const { hasError, error, errorInfo, retryCount, reloadCount } = this.state;
        ...
        <div className="min-h-screen bg-gradient-to-br from-red-50 to-orange-50 flex items-center justify-center p-4">
          <GlassCard className="max-w-2xl w-full p-8">
            <div className="text-center space-y-6">
              ...
              <div>
                ...
                {reloadCount >= 3 && (
                  <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded text-sm text-red-800">
                    ⚠️ <strong>Reload Limit Reached:</strong> Multiple reload attempts failed. The system will redirect you to a safe page.
                  </div>
                )}
              </div>
              ...
              {/* Retry Information */}
              {(retryCount > 0 || reloadCount > 0) && (
                <div className="text-sm text-gray-500">
                  {retryCount > 0 && `Retry attempt ${retryCount} of ${maxRetries}`}
                  {retryCount > 0 && reloadCount > 0 && ' • '}
                  {reloadCount > 0 && `Reload attempt ${reloadCount} of 3`}
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                {isReactRefreshError ? (
                  <GlassButton
                    ...
                    disabled={reloadCount >= 3}
                    icon={<RefreshCw className="w-4 h-4" />}
                    className="bg-blue-600 hover:bg-blue-700 text-white"
                  >
                    {reloadCount >= 3 ? 'Max Reloads Reached' : `Refresh Page (${reloadCount}/3)`}
                  </GlassButton>
                ) : (
                  <GlassButton
