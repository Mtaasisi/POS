...
const CustomerCareDashboard: React.FC<CustomerCareDashboardProps> = ({
  ...
  // Auto-refresh dashboard data every 60 seconds (without page reload)
  useEffect(() => {
    const interval = setInterval(() => {
      // Refresh data without reloading the page
      console.log('Auto-refreshing dashboard data...');
      // Trigger a data refresh by calling the fetch functions
      if (currentUser?.id) {
        // Refresh user goal data
        getGoalProgress('new_customers').then(goalProgress => {
          setDailyGoal(goalProgress.goal);
          setProgress(goalProgress.progress);
        }).catch(console.error);
        
        // You can add more data refresh calls here as needed
        // For example: fetchDevices(), fetchDiagnosticRequests(), etc.
      }
    }, 60000); // 60 seconds
          ...
          <StandardButton
            ...
            onClick={() => {
              ...
              if (currentUser?.id) {
                ...
                // Add more refresh calls here as needed
