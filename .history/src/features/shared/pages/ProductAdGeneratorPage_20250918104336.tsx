...
const ProductAdGeneratorPage: React.FC = () => {
  ...
  const handleInputChange = (field: keyof ProductData, value: string) => {
    console.log('🔄 ProductAdGenerator: Updating field:', field, 'with value:', value);
    setProductData(prev => {
      const updated = {
        ...prev,
        [field]: value
      ...
      if ((field === 'name' || field === 'brand') && !prev.image) {
        ...
        console.log('🖼️ ProductAdGenerator: Auto-setting image for:', newName, newBrand);
      }
      
      console.log('✅ ProductAdGenerator: Updated product data:', updated);
      return updated;
    });
  };
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      // Calculate dimensions to fit the template
      const imgWidth = 210;
  ...
  const handlePresetSelect = (preset: typeof productPresets[0]) => {
    console.log('🎯 ProductAdGenerator: Selecting preset:', preset);
    setProductData(preset);
  };

  const handleReset = () => {
    console.log('🔄 ProductAdGenerator: Resetting to initial data');
            ...
            <GlassCard className="p-6">
              ...
              <div className="mb-4 p-3 bg-gray-50 rounded-lg text-xs">
                ...
                <div className="text-gray-600">
                  Name: {productData.name} | Brand: {productData.brand} | Price: {productData.price}
                </div>
                <div className="text-gray-600">
                  Image: {productData.image ? 'Set' : 'Not set'} | Specs: {productData.specifications.length} chars
                </div>
              </div>
