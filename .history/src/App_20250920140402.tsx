...
const RoleProtectedRoute: React.FC<{ 
  ...
  try {
    ...
    // Safely extract and validate the user role
    const userRole = currentUser?.role;
    
    // Debug logging for role validation issues
    if (userRole && typeof userRole !== 'string') {
      console.warn('RoleProtectedRoute: Invalid role type detected:', {
        userRole,
    ...
    // Convert role to string if it's not already a string (fallback for object roles)
    const normalizedRole = userRole ? String(userRole) : null;
    const isValidRole = normalizedRole && safeAllowedRoles.includes(normalizedRole);
