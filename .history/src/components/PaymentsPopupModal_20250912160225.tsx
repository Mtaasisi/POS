...
const PaymentsPopupModal: React.FC<PaymentsPopupModalProps> = ({
  ...
  const getMethodColor = (type: string) => {
    // Neutral state for unselected buttons
                    ...
                    <div key={payment.id} className={`flex items-center justify-between p-3 rounded-lg border ${getMethodColor(methodType)}`}>
                      <div className="flex items-center gap-3">
                        <div className="text-white/90">
                          {getMethodIcon(methodType, payment.paymentMethod)}
                        </div>
                        <div>
                          ...
                          {payment.reference && (
                            <div className="text-white/70 text-xs">Ref: {payment.reference}</div>
                          )}
                        </div>
                      </div>
                      <button
                        onClick={() => removePayment(payment.id)}
                        className="text-white/70 hover:text-white hover:bg-red-500/30 transition-all p-1 rounded"
                        disabled={isProcessing}
                      >
                        <X className="w-4 h-4" />
                      </button>
