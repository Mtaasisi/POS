...
const PaymentMethodIcon: React.FC<PaymentMethodIconProps> = ({
  ...
  const getIconPath = (methodType: string, methodName?: string) => {
    ...
    // Direct type matches
    if (typeLower === 'cash') return '/icons/payment-methods/cash.svg';
    ...
    if (typeLower === 'mobile_money') {
      ...
      if (nameLower.includes('airtel')) return '/icons/payment-methods/beem.svg';
      return '/icons/payment-methods/mpesa.svg'; // default mobile money
    }
    if (typeLower === 'bank' || typeLower === 'bank_transfer') return '/icons/payment-methods/beem.svg';
