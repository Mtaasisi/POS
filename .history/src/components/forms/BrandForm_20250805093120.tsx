...
const BrandForm: React.FC<BrandFormProps> = ({
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    try {
      ...
      const fileName = `brand-logos/${timestamp}-${editingBrand?.name || 'new-brand'}.${fileExtension}`;
      
      // Save the file path for later upload to Hostinger
      setFormData(prev => ({ 
        ...prev, 
        logo_url: `/public/brand-logos/${timestamp}-${editingBrand?.name || 'new-brand'}.${fileExtension}` 
      }));
      
      setLogoFile(file);
      toast.success('Logo uploaded successfully! (Saved locally for Hostinger upload)');
    } catch (error: any) {
