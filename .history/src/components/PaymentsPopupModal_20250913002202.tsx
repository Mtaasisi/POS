...
import { X, DollarSign, CreditCard, Smartphone, Building, CheckCircle, Loader2, Plus, ArrowLeft, Trash2 } from 'lucide-react';
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-3">
                <label className="text-white/90 text-sm font-medium">Input Amount</label>
                <GlassInput
                  type="number"
                  value={isMultiplePaymentMode ? (currentAmount || '') : amount}
                  onChange={(e) => {
                  ...
                  className="bg-white/10 border-white/20 text-white placeholder-white/50 text-lg font-semibold"
                  readOnly={!isMultiplePaymentMode}
                />
                
                {/* Customer Change Display */}
                {!isMultiplePaymentMode && (
                  <div className="bg-green-500/20 border border-green-400/30 rounded-lg p-3">
                    <div className="text-green-300 text-sm font-medium">Customer Change</div>
                    <div className="text-green-100 text-lg font-bold">TSh 0</div>
                  </div>
                )}
                ...
                <div className="flex gap-2 flex-wrap">
                  <button
                    ...
                    className="px-4 py-2 bg-blue-500/20 border border-blue-400/30 rounded-lg text-blue-200 text-sm font-medium hover:bg-blue-500/30 transition-all"
                  >
                    Exact Amount
                  </button>
                  <button
                    onClick={() => handleQuickAmount(10000)}
                    className="px-4 py-2 bg-green-500/20 border border-green-400/30 rounded-lg text-green-200 text-sm font-medium hover:bg-green-500/30 transition-all"
                  >
                    TSh 10,000
                  </button>
                  <button
              ...
              <div className="bg-white/10 backdrop-blur-md rounded-xl border border-white/20 p-4">
                <div className="text-white/70 text-sm font-medium mb-3">Numeric Keypad</div>
                
                {/* Number Keys - 4x3 Grid like the reference */}
                <div className="grid grid-cols-3 gap-3 mb-4">
                  {['1', '2', '3', '4', '5', '6', '7', '8', '9', '.', '0', 'X'].map((key) => (
                    <button
                      key={key}
                      onClick={() => key === 'X' ? handleClear() : handleKeyPress(key)}
                      disabled={isProcessing || (!isMultiplePaymentMode && key !== 'X')}
                      className={`h-14 rounded-lg font-semibold text-lg transition-all duration-200 hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${
                        key === 'X' 
            ...
            <div className="space-y-3">
              <label className="text-white/90 text-sm">
                {isMultiplePaymentMode ? 'Quick Add Payments' : 'Method'}
              </label>
              <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="text-sm flex-1">
                          ...
                          {isMultiplePaymentMode && (
                            <div className="text-xs opacity-70 mt-1">
                              {remainingAmount > 0 ? `+ TSh ${Math.min(currentAmount || remainingAmount, remainingAmount).toLocaleString()}` : 'Complete'}
                            </div>
                          )}
                        </div>
            ...
            {/* Quick Add Instructions - Only show in multiple payment mode */}
            {isMultiplePaymentMode && remainingAmount > 0 && (
              <div className="bg-blue-500/10 border border-blue-400/20 rounded-lg p-3">
                <div className="text-blue-200 text-sm font-medium mb-1">💡 Quick Tip</div>
                <div className="text-blue-100 text-xs">
                  Click any payment method to instantly add a payment for the current amount
                </div>
              </div>
            )}
