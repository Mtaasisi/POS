      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            ...
            {isMultiplePaymentMode && (
              <div className="bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-white/90 text-sm font-medium">Payment Progress</span>
                  <span className="text-white/70 text-sm font-bold">{Math.round((totalPaid / amount) * 100)}%</span>
                </div>
                <div className="w-full bg-white/10 rounded-full h-3 mb-3">
                  <div 
                    className="bg-gradient-to-r from-green-400 to-green-500 rounded-full h-3 transition-all duration-500 shadow-lg"
                    style={{ width: `${Math.min((totalPaid / amount) * 100, 100)}%` }}
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center">
                    <div className="text-green-400 text-lg font-bold">TSh {totalPaid.toLocaleString()}</div>
                    <div className="text-white/60 text-xs">Paid</div>
                  </div>
                  <div className="text-center">
                    <div className="text-orange-400 text-lg font-bold">TSh {remainingAmount.toLocaleString()}</div>
            ...
            {/* Amount Input - Only show in multiple payment mode */}
            {isMultiplePaymentMode && (
              <div className="space-y-3">
                <label className="text-white/90 text-sm">Payment Amount</label>
                <GlassInput
                  type="number"
                  value={currentAmount || ''}
                ...
                <div className="bg-orange-500/20 border border-orange-400/30 rounded-lg p-3">
                  <div className="text-orange-300 text-sm font-medium">Remaining Amount</div>
                  <div className="text-orange-100 text-lg font-bold">TSh {remainingAmount.toLocaleString()}</div>
                </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4">
                {paymentMethods.map((method) => {
                  ...
                  // Define gradient colors for each payment method
                  const getMethodButtonColor = (methodName: string, selected: boolean) => {
                    const baseColors = {
                      'Cash': selected 
                        ? 'bg-gradient-to-r from-green-500/80 to-emerald-500/80 hover:from-green-600/90 hover:to-emerald-600/90 text-white border-white/40 ring-4 ring-green-300/70 shadow-lg shadow-green-500/30' 
                        : 'bg-gradient-to-r from-green-500/60 to-emerald-500/60 hover:from-green-600/80 hover:to-emerald-600/80 text-white border-white/20 hover:border-white/40',
                      'Bank Transfer': selected 
                        ? 'bg-gradient-to-r from-blue-500/80 to-cyan-500/80 hover:from-blue-600/90 hover:to-cyan-600/90 text-white border-white/40 ring-4 ring-blue-300/70 shadow-lg shadow-blue-500/30' 
                        : 'bg-gradient-to-r from-blue-500/60 to-cyan-500/60 hover:from-blue-600/80 hover:to-cyan-600/80 text-white border-white/20 hover:border-white/40',
                      'Airtel Money': selected 
                        ? 'bg-gradient-to-r from-red-500/80 to-pink-500/80 hover:from-red-600/90 hover:to-pink-600/90 text-white border-white/40 ring-4 ring-red-300/70 shadow-lg shadow-red-500/30' 
                        : 'bg-gradient-to-r from-red-500/60 to-pink-500/60 hover:from-red-600/80 hover:to-pink-600/80 text-white border-white/20 hover:border-white/40',
                      'M-Pesa': selected 
                        ? 'bg-gradient-to-r from-yellow-500/80 to-orange-500/80 hover:from-yellow-600/90 hover:to-orange-600/90 text-white border-white/40 ring-4 ring-yellow-300/70 shadow-lg shadow-yellow-500/30' 
                        : 'bg-gradient-to-r from-yellow-500/60 to-orange-500/60 hover:from-yellow-600/80 hover:to-orange-600/80 text-white border-white/20 hover:border-white/40',
                      'Tigo Pesa': selected 
                        ? 'bg-gradient-to-r from-purple-500/80 to-indigo-500/80 hover:from-purple-600/90 hover:to-indigo-600/90 text-white border-white/40 ring-4 ring-purple-300/70 shadow-lg shadow-purple-500/30' 
                        : 'bg-gradient-to-r from-purple-500/60 to-indigo-500/60 hover:from-purple-600/80 hover:to-indigo-600/80 text-white border-white/20 hover:border-white/40',
                      'HaloPesa': selected 
                        ? 'bg-gradient-to-r from-orange-500/80 to-amber-500/80 hover:from-orange-600/90 hover:to-amber-600/90 text-white border-white/40 ring-4 ring-orange-300/70 shadow-lg shadow-orange-500/30' 
                        : 'bg-gradient-to-r from-orange-500/60 to-amber-500/60 hover:from-orange-600/80 hover:to-amber-600/80 text-white border-white/20 hover:border-white/40'
                    };
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 rounded-xl border backdrop-blur-md transition-all duration-300 text-left transform hover:scale-105 active:scale-[0.98] focus:ring-2 focus:ring-white/30 hover:shadow-lg font-medium ${
            ...
            {/* Reference & Notes - Only show in multiple payment mode */}
            {isMultiplePaymentMode && (
            ...
            {/* Add Payment Button - Only show in multiple payment mode */}
            {isMultiplePaymentMode && (
              <GlassButton
                onClick={addPayment}
            ...
            {isMultiplePaymentMode && paymentEntries.length > 0 && (
              <div className="space-y-3">
                ...
                {paymentEntries.map((payment) => {
                  ...
                  return (
                    <div key={payment.id} className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition-all">
                      <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="text-white/50 hover:text-white/80 hover:bg-red-500/20 transition-all p-2 rounded-lg"
                        disabled={isProcessing}
                      >
                        <X className="w-4 h-4" />
                      </button>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <GlassButton
                onClick={isMultiplePaymentMode ? handleSubmit : () => {
                  if (!selectedMethod) {
                    toast.error('Please select a payment method');
                    return;
                  }
                  // For single payment mode, create payment entry and process immediately
                  const method = paymentMethods.find(m => m.id === selectedMethod);
                  const account = getAutoSelectedAccount(selectedMethod);
                ...
                disabled={isMultiplePaymentMode ? (paymentEntries.length === 0 || isProcessing) : (!selectedMethod || isProcessing)}
