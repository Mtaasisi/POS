...
import { createColumnMapping, processFileWithIntelligentDetection, extractValue } from '../lib/columnDetectionUtils';
...
const CustomerUpdateImportModal: React.FC<CustomerUpdateImportModalProps> = ({
  ...
  const [columnMapping] = useState(createColumnMapping());
  ...
  const processCSVFile = async (file: File) => {
    try {
      ...
      const result = await processFileWithIntelligentDetection(
        file,
        columnMapping,
        (message) => toast.success(message)
      );
      
      const { data: rawData, detectedColumns: detected, errors, headers } = result;
      ...
      for (let i = 0; i < rawData.length; i++) {
        ...
        const rawName = extractValue(Object.values(rowData), 'name', detected);
        const rawPhone = extractValue(Object.values(rowData), 'phone', detected);
        const rawWhatsApp = extractValue(Object.values(rowData), 'whatsapp', detected);
        const rawGender = extractValue(Object.values(rowData), 'gender', detected);
        const rawCity = extractValue(Object.values(rowData), 'city', detected);
        const rawNotes = extractValue(Object.values(rowData), 'notes', detected);
        const rawLoyaltyLevel = extractValue(Object.values(rowData), 'loyaltyLevel', detected);
        const rawColorTag = extractValue(Object.values(rowData), 'colorTag', detected);
        const rawReferralSource = extractValue(Object.values(rowData), 'referralSource', detected);
        const rawLocationDescription = extractValue(Object.values(rowData), 'locationDescription', detected);
        const rawNationalId = extractValue(Object.values(rowData), 'nationalId', detected);
        const rawReferredBy = extractValue(Object.values(rowData), 'referredBy', detected);
        const rawTotalSpent = extractValue(Object.values(rowData), 'totalSpent', detected);
        const rawPoints = extractValue(Object.values(rowData), 'points', detected);
        const rawIsActive = extractValue(Object.values(rowData), 'isActive', detected);
        const rawProfileImage = extractValue(Object.values(rowData), 'profileImage', detected);
        const rawBirthday = extractValue(Object.values(rowData), 'birthday', detected);
        const rawEmail = extractValue(Object.values(rowData), 'email', detected);
        const rawId = extractValue(Object.values(rowData), 'id', detected);
        ...
        const customer: ImportedCustomerUpdate = {
          ...
          colorTag: normalizeColorTag(rawColorTag || ''),
          ...
          isActive: rawIsActive?.toLowerCase() === 'yes' || rawIsActive?.toLowerCase() === 'true',
          profileImage: rawProfileImage
        };
        
        // Parse birthday if available
        if (rawBirthday) {
          const { month, day } = parseBirthday(rawBirthday);
        ...
        if (rowErrors.length > 0) {
          validationErrors.push(`Row ${i + 2}: ${rowErrors.join(', ')}`);
        }
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        console.log(`⚠️ ${validationErrors.length} validation errors found`);
      }
                    ...
                    {Object.keys(detectedColumns).length > 0 && (
                      <div className="bg-blue-500/20 border border-blue-500/30 rounded-lg p-3">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          {Object.entries(detectedColumns).slice(0, 8).map(([fieldType, columnIndex]) => (
                            <div key={fieldType} className="flex items-center gap-2 bg-white/10 rounded p-1">
                          ...
                          {Object.keys(detectedColumns).length > 8 && (
                            <div className="text-blue-400 text-xs">
                              +{Object.keys(detectedColumns).length - 8} more
                            </div>
                          )}
                        </div>
