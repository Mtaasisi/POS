...
const PaymentsPopupModal: React.FC<PaymentsPopupModalProps> = ({
  ...
  const handlePayment = async () => {
    if (isMultipleMode) {
      ...
      // Check if any payment entry has no amount set
      const entriesWithoutAmount = paymentEntries.filter(entry => !entry.amount || entry.amount <= 0);
      if (entriesWithoutAmount.length > 0) {
        toast.error('Please set amounts for all payment entries');
        return;
      }
        ...
        <div className="p-4 overflow-y-auto max-h-[calc(85vh-120px)]">
                    ...
                    <div className="mb-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-gray-500">Remaining: TZS {remainingAmount.toLocaleString()}</span>
                          <button
                            onClick={() => setCustomAmount(remainingAmount.toString())}
                      ...
                      <div className="relative">
                        <input
                          ...
                          placeholder="Enter amount or leave empty to set later"
          ...
          {paymentEntries.length > 0 && (
            <div className="mt-8 p-6 bg-green-50 rounded-lg border border-green-200">
              ...
              {/* Show incomplete payments warning */}
              {paymentEntries.some(entry => !entry.amount || entry.amount <= 0) && (
                <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                  <div className="flex items-center gap-2">
                    <AlertCircle className="w-4 h-4 text-orange-600" />
                    <span className="text-sm text-orange-700">
                      Some payments need amounts to be set before processing
                    </span>
                  </div>
                </div>
              )}
