    ...
    <div className="flex flex-col h-full bg-gray-100 rounded-lg overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
        {isLoading ? (
          ...
          [...remarks, ...activityEvents].sort((a, b) => new Date(a.createdAt || a.timestamp).getTime() - new Date(b.createdAt || b.timestamp).getTime()).map((item, index) => {
            const isRemark = 'content' in item;
            const isOwn = isRemark ? isOwnMessage(item) : false;
            const showDate = index === 0 || 
              formatDate(item.createdAt || item.timestamp) !== formatDate(([...remarks, ...activityEvents].sort((a, b) => new Date(a.createdAt || a.timestamp).getTime() - new Date(b.createdAt || b.timestamp).getTime()))[index - 1]?.createdAt || ([...remarks, ...activityEvents].sort((a, b) => new Date(a.createdAt || a.timestamp).getTime() - new Date(b.createdAt || b.timestamp).getTime()))[index - 1]?.timestamp);

            return (
              <div key={isRemark ? item.id : `${item.type}-${item.timestamp}`}>
                {/* Date Separator */}
                {showDate && (
                  ...
                  <div className="flex justify-center my-2">
                    <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 max-w-xs lg:max-w-md">
                      <div className="flex items-center gap-2 mb-1">
                        {item.icon}
                        <span className="text-xs font-semibold text-blue-700">{item.typeLabel}</span>
                      </div>
                      <p className="text-xs text-blue-800">{item.description}</p>
                      <div className="flex items-center justify-end mt-1">
                        <span className="text-xs text-blue-600">{formatTime(item.timestamp)}</span>
                      </div>
                    </div>
                  </div>
                )}
