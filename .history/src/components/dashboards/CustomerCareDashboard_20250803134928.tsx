      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {sortDevices(removeDuplicateDevices(filteredDevices))
              .map((device) => (
        ...
        >
          <div className="space-y-6 max-h-[80vh] overflow-y-auto">
            ...
            <div>
              ...
              <div className="space-y-3">
                {selectedDevice.remarks && selectedDevice.remarks.length > 0 ? (
                  selectedDevice.remarks.map((remark, index) => (
                    <div key={remark.id} className="bg-gray-50 rounded-lg p-3 border">
                      <div className="flex items-start justify-between mb-2">
                        <p className="text-sm font-medium text-gray-900">{remark.content}</p>
                        <span className="text-xs text-gray-500">
                          {new Date(remark.createdAt).toLocaleString()}
                        </span>
                      </div>
                      <p className="text-xs text-gray-600">By: {remark.createdBy}</p>
                    </div>
                  ))
                ) : (
                  <div className="text-center py-4 text-gray-500">
                    <MessageSquare className="h-8 w-8 mx-auto mb-2 text-gray-400" />
                    <p>No remarks or conversations available</p>
                  </div>
                )}
              </div>
            </div>
