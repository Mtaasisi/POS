...
import { Gift, Star, Crown, Users, Award, CreditCard, ShoppingBag, TrendingUp, Search, Loader2, Plus } from 'lucide-react';
...
const LoyaltyProgram: React.FC<LoyaltyProgramProps> = ({ 
  ...
  const fetchCustomers = async () => {
    ...
    try {
      ...
      setError('Failed to load customers. Please run the loyalty tables fix script.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRewardRedeem = async (reward: Reward, customer: Customer) => {
    ...
    try {
      ...
      // Update customer points and rewards redeemed
      const { error: updateError } = await supabase
        .from('loyalty_customers')
        .update({
          points: customer.points - reward.pointsCost,
          rewards_redeemed: customer.rewardsRedeemed + 1,
                  ...
                  {['bronze', 'silver', 'gold', 'platinum'].map((tier) => (
                    <div key={tier} className="text-center">
                      ...
                      <p className="text-2xl font-bold text-gray-700">
                        ...
                        {!loading && !error && filteredCustomers.length === 0 && activeTab === 'customers' && (
                <div className="text-center py-8 text-gray-500">
                  <Users size={48} className="mx-auto mb-4 text-gray-300" />
                  <p>No loyalty customers found</p>
                  <p className="text-sm">Add customers to the loyalty program to see them here</p>
                  <div className="mt-4">
                    <GlassButton
                      variant="outline"
                      onClick={() => {
                        // This would typically open a customer creation modal
                        console.log('Add customer to loyalty program');
                      }}
                    >
                      <Plus size={16} />
                      <span className="ml-2">Add Customer to Loyalty</span>
                    </GlassButton>
                  </div>
                </div>
              )}
