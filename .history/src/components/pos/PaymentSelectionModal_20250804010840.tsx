...
const PaymentSelectionModal: React.FC<PaymentSelectionModalProps> = ({
  ...
  // Calculate the display value for the input
  const displayAmount = amountPaid || totalAmount || 0;

  useEffect(() => {
    if (isOpen) {
      loadPaymentAccounts();
      // Auto-fill amount paid with total amount
      console.log('Modal opened, total amount:', totalAmount);
      if (totalAmount > 0) {
  ...
  // Additional effect to handle when totalAmount becomes available
  useEffect(() => {
    if (isOpen && totalAmount > 0 && amountPaid === 0) {
      console.log('Total amount became available, setting amount paid to:', totalAmount);
      setAmountPaid(totalAmount);
    }
  }, [totalAmount, isOpen, amountPaid]);
      ...
      <GlassCard className="max-w-2xl w-full mx-4">
        ...
        <div className="mb-6">
          ...
          <div className="text-xs text-gray-500 mb-2">
            Debug: amountPaid={amountPaid}, totalAmount={totalAmount}, displayAmount={displayAmount}
          </div>
            ...
            {amountPaid < totalAmount && (
              <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p className="text-sm text-yellow-800">
                  ⚠️ Partial payment: ₦{(totalAmount - amountPaid).toFixed(2)} remaining
                </p>
              </div>
            )}
            {amountPaid > totalAmount && (
              <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-sm text-green-800">
                  💰 Change due: ₦{(amountPaid - totalAmount).toFixed(2)}
                </p>
              </div>
            )}
          </div>
