...
const LocationSelector: React.FC<LocationSelectorProps> = ({ 
  ...
  console.log('🔧 LocationSelector render - isOpen:', isOpen, 'currentLocation:', currentLocation);
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        ...
        // Add mock stats for now (replace with real API calls)
        const locationsWithStats = data?.map(location => ({
          ...location,
          salesToday: Math.floor(Math.random() * 200000) + 50000,
          customersToday: Math.floor(Math.random() * 20) + 5,
          inventoryCount: Math.floor(Math.random() * 500) + 200
        })) || [];
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="bg-gray-50 rounded p-2">
                    ...
                    <p className="font-semibold text-sm">Tsh{location.salesToday.toLocaleString()}</p>
                  </div>
                  <div className="bg-gray-50 rounded p-2">
