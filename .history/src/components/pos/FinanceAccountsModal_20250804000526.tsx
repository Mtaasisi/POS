...
const FinanceAccountsModal: React.FC<FinanceAccountsModalProps> = ({ 
  ...
  const availableIcons = [
    { name: 'Building', icon: Building, color: '#059669' },
    { name: 'DollarSign', icon: DollarSign, color: '#10B981' },
    { name: 'Smartphone', icon: Smartphone, color: '#DC2626' },
    { name: 'CreditCard', icon: CreditCard, color: '#3B82F6' },
    { name: 'PiggyBank', icon: PiggyBank, color: '#F59E0B' },
    { name: 'TrendingUp', icon: TrendingUp, color: '#10B981' },
    { name: 'Wallet', icon: Wallet, color: '#6B7280' },
    { name: 'Star', icon: Star, color: '#F59E0B' },
    { name: 'Bank', icon: Building, color: '#059669' },
    { name: 'Cash', icon: DollarSign, color: '#10B981' },
    { name: 'Mobile', icon: Smartphone, color: '#DC2626' },
    { name: 'Card', icon: CreditCard, color: '#3B82F6' },
    { name: 'Savings', icon: PiggyBank, color: '#F59E0B' },
  ...
  const handleAddAccount = async () => {
    ...
    try {
      const newAccount = await financeAccountService.createFinanceAccount({
        ...accountForm,
  ...
  const handleUpdateAccount = async () => {
    ...
    try {
      const updatedAccount = await financeAccountService.updateFinanceAccount(editingAccount.id, {
        ...accountForm,
        payment_icon: accountForm.payment_icon || financeAccountService.getIconForAccountType(accountForm.type),
        payment_color: accountForm.payment_color || financeAccountService.getColorForAccountType(accountForm.type),
        payment_description: accountForm.payment_description || `${accountForm.type} account`
  ...
  const getAccountTypeIcon = (type: FinanceAccount['type']) => {
  ...
  // Render custom icon for payment methods
  const renderCustomIcon = (iconName: string, color: string = '#3B82F6', size: string = "w-6 h-6") => {
    const iconSize = size;
    const baseClasses = "rounded-full p-2";
    ...
    if (!iconData) {
      // Fallback to default icon based on type
      return getAccountTypeIcon(accountForm.type);
    }
              ...
              <GlassCard key={account.id} className="p-6 hover:shadow-lg transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    {account.payment_icon && account.is_payment_method 
                      ? renderCustomIcon(account.payment_icon, account.payment_color)
                ...
                {accountForm.is_payment_method && (
                  <>
                    <div>
                      ...
                      <div className="grid grid-cols-5 gap-2">
                        {availableIcons.map((iconData) => {
                          const IconComponent = iconData.icon;
                          const isSelected = accountForm.payment_icon === iconData.name;
                          return (
                            <button
                              key={iconData.name}
                              type="button"
                              onClick={() => setAccountForm(prev => ({ 
                                ...prev, 
                                payment_icon: iconData.name,
                                payment_color: iconData.color 
                              }))}
                            ...
                            >
                              ...
                              <span className="text-xs mt-1 block text-center">{iconData.name}</span>
                            </button>
                          );
                        })}
                      </div>
                    ...
                    <div>
                      ...
                      <input
                        type="color"
                        value={accountForm.payment_color}
                        onChange={(e) => setAccountForm(prev => ({ ...prev, payment_color: e.target.value }))}
