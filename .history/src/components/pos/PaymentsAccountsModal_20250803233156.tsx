...
const PaymentsAccountsModal: React.FC<PaymentsAccountsModalProps> = ({ 
  ...
  const loadPaymentMethods = async () => {
    try {
      ...
      console.log('🔗 Loading payment method account links...');
      const methodsWithAccounts = await paymentMethodService.getPaymentMethodsWithAccounts();
      console.log('📊 Methods with accounts:', methodsWithAccounts.length);
      
      const allLinks: PaymentMethodAccount[] = [];
      methodsWithAccounts.forEach(method => {
        console.log(`🔗 Processing method: ${method.name} (${method.id})`);
        if (method.accounts) {
          console.log(`  📎 Found ${method.accounts.length} linked accounts for ${method.name}`);
          // Add the payment_method_id to each account link
          const linksWithMethodId = method.accounts.map(account => ({
            ...account,
            payment_method_id: method.id
          }));
          allLinks.push(...linksWithMethodId);
        } else {
          console.log(`  ❌ No linked accounts for ${method.name}`);
        }
      });
