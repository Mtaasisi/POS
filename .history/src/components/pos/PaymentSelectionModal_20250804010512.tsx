...
const PaymentSelectionModal: React.FC<PaymentSelectionModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      console.log('Setting amount paid to:', totalAmount);
      setAmountPaid(totalAmount);
    }
  }, [isOpen, totalAmount]);

  // Also set amount when totalAmount changes
  useEffect(() => {
    if (isOpen && totalAmount > 0) {
      console.log('Total amount changed, setting amount paid to:', totalAmount);
      setAmountPaid(totalAmount);
    }
  }, [totalAmount, isOpen]);

  // Immediate effect when modal opens
  useEffect(() => {
    if (isOpen) {
          ...
          <div className="bg-gray-50 rounded-xl p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="number"
                  value={amountPaid || totalAmount || 0}
