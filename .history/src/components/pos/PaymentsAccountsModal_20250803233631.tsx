...
const PaymentsAccountsModal: React.FC<PaymentsAccountsModalProps> = ({ 
  ...
  const loadPaymentMethods = async () => {
    try {
      ...
      console.log('📋 Payment methods loaded:', methods.length, methods.map(m => ({ id: m.id, name: m.name })));
      setPaymentMethods(methods);
      
      // Load payment methods with finance accounts
      console.log('🔗 Loading payment methods with finance accounts...');
      const methodsWithFinanceAccounts = await paymentMethodService.getPaymentMethodsWithFinanceAccounts();
      console.log('📊 Methods with finance accounts:', methodsWithFinanceAccounts.length);
      
      // Extract all payment method account links
      const allLinks: PaymentMethodAccount[] = [];
      methodsWithFinanceAccounts.forEach(method => {
        console.log(`🔗 Processing method: ${method.name} (${method.id})`);
        if (method.finance_accounts) {
          console.log(`  📎 Found ${method.finance_accounts.length} linked finance accounts for ${method.name}`);
          // Create payment method account links from finance accounts
          const linksFromFinanceAccounts = method.finance_accounts.map(financeAccount => ({
            id: financeAccount.payment_method_account_id,
            payment_method_id: method.id,
            account_id: financeAccount.id,
            is_default: financeAccount.is_default,
            created_at: new Date().toISOString() // We don't have this from the finance account, so using current time
          }));
          allLinks.push(...linksFromFinanceAccounts);
        } else {
          console.log(`  ❌ No linked finance accounts for ${method.name}`);
        }
      });
      
      console.log('📋 Total payment method account links:', allLinks.length);
      console.log('🔗 Links details:', allLinks.map(link => ({ 
  ...
  const handleLinkPaymentMethod = async (paymentMethodId: string, accountId: string, isDefault: boolean = false) => {
    try {
      console.log('🔗 Attempting to link payment method:', { paymentMethodId, accountId, isDefault });
      
      const success = await paymentMethodService.linkPaymentMethodToAccount(paymentMethodId, accountId, isDefault);
      console.log('✅ Link result:', success);
      
      if (success) {
        console.log('🔄 Reloading payment methods after successful link...');
        await loadPaymentMethods(); // Reload the links
        toast.success('Payment method linked successfully');
      } else {
        console.error('❌ Failed to link payment method');
  ...
  const getLinkedPaymentMethods = (accountId: string) => {
    console.log('🔍 Getting linked payment methods for account:', accountId);
    console.log('📋 Available payment method accounts:', paymentMethodAccounts.length);
    console.log('🔗 Payment method accounts:', paymentMethodAccounts.map(link => ({ 
      payment_method_id: link.payment_method_id, 
      account_id: link.account_id, 
      is_default: link.is_default 
    })));
    
    const linkedMethods = paymentMethodAccounts.filter(link => link.account_id === accountId);
    console.log('✅ Found linked methods for account:', linkedMethods.length, linkedMethods.map(link => ({ 
      payment_method_id: link.payment_method_id, 
      is_default: link.is_default 
    })));
