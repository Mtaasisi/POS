...
interface SeparatedProductSearchProps {
  value: string;
...
const SeparatedProductSearch: React.FC<SeparatedProductSearchProps> = ({
  value,
  onChange,
  onSearch,
  products,
  ...
  useEffect(() => {
    if (value.length >= 2) {
      onSearch(value);
      setShowResults(true);
    } else if (value.length === 0 && (products.length > 0 || parts.length > 0)) {
      // Show initial products when search is empty but we have results
      setShowResults(true);
    } else {
      setShowResults(false);
    }
  }, [value, onSearch, products.length, parts.length]);
  ...
  const renderProductSection = (productList: Product[], title: string, icon: React.ReactNode, bgColor: string) => {
    ...
    return (
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm text-gray-500">({productList.length})</span>
        </div>
        ...
        <div className="space-y-3">
          {productList.map((product) => (
            <div key={product.id} className="space-y-2">
              {/* Product Header */}
              <div className={`flex items-center justify-between p-3 ${bgColor} rounded-xl border border-blue-200/50`}>
    ...
    <div className="relative z-20">
      ...
      <div className="relative">
        <input
          ...
          placeholder="Search products and parts by name, SKU, or barcode..."
      ...
      {/* Search Results */}
      {showResults && (products.length > 0 || parts.length > 0) && (
        <>
            ...
            <div className="p-4">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="font-semibold text-gray-900">
                  {value.length === 0 ? 'Available Items' : 'Search Results'}
                </span>
                <span className="text-sm text-gray-500">({products.length + parts.length} total)</span>
              </div>
      ...
      {/* No Results */}
      {showResults && value.length >= 2 && products.length === 0 && parts.length === 0 && (
...
export default SeparatedProductSearch; 