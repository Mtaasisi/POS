...
const PaymentsAccountsModal: React.FC<PaymentsAccountsModalProps> = ({ 
  ...
  const loadPaymentMethods = async () => {
    try {
      ...
      // Load payment method account links directly from the database
      console.log('🔗 Loading payment method account links directly...');
      ...
      if (linksError) {
        console.error('❌ Error loading payment method accounts:', linksError);
        return;
      }

      console.log('📋 Direct payment method account links loaded:', linksData?.length || 0);
      console.log('🔗 Direct links details:', linksData?.map(link => ({ 
        payment_method_id: link.payment_method_id, 
        account_id: link.account_id, 
        is_default: link.is_default 
      })) || []);
      
      setPaymentMethodAccounts(linksData || []);
    } catch (error) {
