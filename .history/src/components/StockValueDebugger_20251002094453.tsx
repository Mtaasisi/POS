...
const StockValueDebugger: React.FC = () => {
  ...
  const debugStockValues = async () => {
    try {
      ...
      if (!products) {
        throw new Error('No products data returned from database');
      }

      // Get all variants separately
      ...
      const productsWithVariants = products.map(product => ({
        ...product,
        lats_product_variants: variants?.filter(variant => variant.product_id === product.id) || []
      }));

      console.log(`📦 Found ${productsWithVariants.length} active products`);
      ...
      productsWithVariants.forEach((product: any, index: number) => {
