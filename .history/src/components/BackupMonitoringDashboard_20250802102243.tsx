  ...
  const handleManualBackup = async () => {
    try {
      ...
      if (result && result.success) {
        ...
        const errorMessage = result?.error || 'Unknown backup error';
        setError(`Backup failed: ${errorMessage}`);
        setTestResult({ success: false, message: `Backup failed: ${errorMessage}` });
      }
  ...
  const handleTestConnection = async () => {
    try {
      ...
      if (result.success) {
        ...
        setTestResult({ 
          success: false, 
          message: `❌ Backup connection test failed:\n\n${result.error}\n\nPlease check your internet connection and try again.` 
        });
      }
    } catch (error) {
