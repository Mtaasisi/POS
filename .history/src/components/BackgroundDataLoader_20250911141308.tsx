...
// Component that safely uses the customers context
const BackgroundDataLoaderContent: React.FC = () => {
...
const BackgroundDataLoader: React.FC = () => {
  // Check if we're inside a CustomersProvider by trying to access the context
  try {
    // This will throw if not inside CustomersProvider
    const testContext = useCustomers();
    return <BackgroundDataLoaderContent />;
  } catch (error) {
    // Not inside CustomersProvider yet, return null
    return null;
  }
};
