...
const PaymentMethodIcon: React.FC<PaymentMethodIconProps> = ({
  ...
  // Debug logging
  console.log('PaymentMethodIcon Debug:', { type, name, methodType: type?.toLowerCase(), methodName: name?.toLowerCase() });
  ...
  const getIconPath = (methodType: string, methodName?: string) => {
    ...
    console.log('getIconPath called with:', { methodType, methodName, typeLower, nameLower });

    // Direct type matches
    if (typeLower === 'cash') {
      console.log('Cash type matched, returning cash-logo.png');
      return '/icons/payment-methods/cash-logo.png';
    }
  ...
  console.log('PaymentMethodIcon iconPath:', iconPath);
