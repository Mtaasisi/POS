...
const PaymentsPopupModal: React.FC<PaymentsPopupModalProps> = ({
  ...
  const processSinglePayment = async (methodId: string) => {
    if (!methodId) {
    ...
    const payment: PaymentEntry = {
      ...
      amount: amount, // Use full amount for single payment
    ...
    try {
      await onPaymentComplete([payment], amount);
  ...
  const addPayment = () => {
    ...
    if (isMultiplePaymentMode) {
      ...
      // Single payment mode - process immediately
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                {!isMultiplePaymentMode && (
                  <span className="text-white/60 text-xs">Click to process payment</span>
                )}
              </div>
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        // In single payment mode, process immediately
                        if (!isMultiplePaymentMode) {
                          processSinglePayment(method.id);
                        }
                      }}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-sm">
                          <div className="font-semibold">
                            {method.name}
                            {isProcessing && selectedMethod === method.id && (
                              <span className="ml-2 text-xs opacity-70">Processing...</span>
                            )}
                          </div>
                          {autoAccount && (
            ...
            <div className="flex gap-3 pt-2">
              ...
              <GlassButton
                ...
                disabled={isMultiplePaymentMode ? (paymentEntries.length === 0 || isProcessing) : (isProcessing)}
