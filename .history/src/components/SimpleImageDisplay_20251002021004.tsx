...
export const SimpleImageDisplay: React.FC<SimpleImageDisplayProps> = ({
  ...
  // Try to use the original URLs first, only sanitize if absolutely necessary
  ...
  console.log('🔍 SimpleImageDisplay Debug:', {
    productName,
    ...
    sanitizedResult: sanitizedResult ? {
      isSanitized: sanitizedResult.isSanitized,
  ...
  if (sanitizedResult?.isSanitized && sanitizedResult.method === 'fallback') {
    console.warn('🖼️ SimpleImageDisplay: Image URL was sanitized to fallback:', {
      productName,
      originalUrl: primaryImageUrl?.substring(0, 100) + '...',
      sanitizedUrl: sanitizedImageUrl?.substring(0, 50) + '...',
      method: sanitizedResult.method,
      originalLength: sanitizedResult.originalLength
    });
  }
  
  // For thumbnails, try original first, only sanitize if necessary
