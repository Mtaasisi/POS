...
const PaymentsPopupModal: React.FC<PaymentsPopupModalProps> = ({
    ...
    setPaymentEntries(prev => prev.map((entry, i) => {
      if (i === index) {
        ...
        if (field === 'amount' && typeof value === 'number') {
          ...
          if (method && value > method.balance) {
            toast.error(`Amount exceeds ${method.name} balance (${method.currency} ${method.balance.toLocaleString()})`);
  ...
  const handlePayment = async () => {
    if (isMultipleMode) {
      ...
      // Check if any amount exceeds method balance
      const exceededBalance = paymentEntries.some(entry => {
        const method = paymentMethods.find(m => m.id === entry.methodId);
        return method && entry.amount > method.balance;
      });
      if (exceededBalance) {
        toast.error('One or more amounts exceed the payment method balance');
        return;
      }
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-xs text-gray-500">
                                  {method.currency} {method.balance.toLocaleString()}
                                </p>
                                {isAlreadyAdded && (
                          ...
                          <div className="mb-4">
                            ...
                            <div className="relative">
                              ...
                              <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                                <span className="text-sm text-gray-500 font-medium">TZS</span>
                              </div>
                            </div>
                            ...
                            <div className="flex gap-2 mt-2">
                              ...
                              >
                                Use Full ({remainingAmount.toLocaleString()})
                              </button>
                              <button
                                onClick={() => {
                                  const currentRemaining = amount - (totalPaid - entry.amount);
