  ...
  async deductInventory(variantId: string, quantity: number): Promise<void> {
    try {
      ...
      const { data: currentStock, error: fetchError } = await supabase
        .from('product_variants')
        .select('quantity_in_stock, available_quantity')
        .eq('id', variantId)
        .single();
      ...
      if (currentStock.available_quantity < quantity) {
        throw new Error(`Insufficient stock. Available: ${currentStock.available_quantity}, Requested: ${quantity}`);
      }
      ...
      const { error: updateError } = await supabase
        .from('product_variants')
        .update({ 
          quantity_in_stock: currentStock.quantity_in_stock - quantity,
