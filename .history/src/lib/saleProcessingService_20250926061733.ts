...
class SaleProcessingService {
  ...
  async processSale(saleData: Omit<SaleData, 'id' | 'saleNumber' | 'createdAt'>): Promise<ProcessSaleResult> {
    try {
      ...
      // Validate customer information - if no customer ID, create a default walk-in customer
      let finalCustomerId = saleData.customerId;
      
      if (!finalCustomerId) {
        // Create or find a default walk-in customer
        const { data: walkInCustomer, error: customerError } = await supabase
          .from('customers')
          .select('id')
          .eq('name', 'Walk-in Customer')
          .eq('phone', '+255000000000')
          .single();
          
        if (customerError && customerError.code !== 'PGRST116') {
          console.error('❌ Error checking walk-in customer:', customerError);
          return { success: false, error: 'Failed to process walk-in customer' };
        }
        ...
        if (!walkInCustomer) {
          ...
          if (createError) {
            ...
            return { success: false, error: 'Failed to create walk-in customer' };
          }
    ...
    try {
      ...
      const { data: sale, error: saleError } = await supabase
        ...
        .insert([{
          ...
          customer_id: saleData.customerId, // This appears to be required based on the constraint
