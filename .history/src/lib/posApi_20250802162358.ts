...
export async function searchProductsForPOS(query: string): Promise<{ products: Product[], parts: Product[] }> {
  try {
    ...
    try {
      ...
      // Store spare parts separately
      const sparePartsArray = [...sparePartProducts];
    ...
    // Separate products and parts
    const products = allProducts.filter(product => !product.product_code?.includes('SPARE'));
    const parts = allProducts.filter(product => product.product_code?.includes('SPARE') || product.category_id === 'spare_parts');
