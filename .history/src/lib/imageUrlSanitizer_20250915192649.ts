...
export class ImageUrlSanitizer {
  ...
  /**
   * Check if URL path contains base64 data (common cause of 431 errors)
   */
  private static hasBase64InUrlPath(url: string): boolean {
    try {
      const urlObj = new URL(url, window.location.origin);
      ...
      // Check for base64-like patterns in the path
      const base64Pattern = /[A-Za-z0-9+/]{100,}={0,2}/;
      return base64Pattern.test(pathname);
    } catch {
      // If URL parsing fails, check the entire string
      const base64Pattern = /[A-Za-z0-9+/]{100,}={0,2}/;
      return base64Pattern.test(url);
    }
  }
