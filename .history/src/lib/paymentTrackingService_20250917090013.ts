...
class PaymentTrackingService {
  ...
  private async _fetchPaymentTransactionsInternal(
    ...
    try {
      ...
      // Fetch Purchase Order payments with safe query
      try {
        ...
        if (!poPaymentsError && poPayments) {
          ...
          const transformedPOPayments = poPayments.map((payment: any) => ({
            ...
            customerName: payment.lats_purchase_orders?.lats_suppliers?.name || 'Unknown Supplier',
            customerPhone: payment.lats_purchase_orders?.lats_suppliers?.phone,
            customerEmail: payment.lats_purchase_orders?.lats_suppliers?.email,
            amount: payment.amount || 0,
            ...
            supplierName: payment.lats_purchase_orders?.lats_suppliers?.name,
            supplierPhone: payment.lats_purchase_orders?.lats_suppliers?.phone,
            supplierEmail: payment.lats_purchase_orders?.lats_suppliers?.email,
  ...
  private mapPOPaymentStatus(status: string): 'completed' | 'pending' | 'failed' {
    if (status === 'completed') return 'completed';
    if (status === 'pending') return 'pending';
