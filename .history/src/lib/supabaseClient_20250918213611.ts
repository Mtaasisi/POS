  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (error.message?.includes('QUIC_PROTOCOL_ERROR')) {
        console.warn(`🔄 QUIC Protocol Error detected, retrying attempt ${attempt + 1}/${maxRetries + 1}:`, error.message);
        console.warn('   This may be due to network instability or browser network stack issues');
      } else {
        console.warn(`🔄 Network error detected, retrying attempt ${attempt + 1}/${maxRetries + 1}:`, error.message);
      }
