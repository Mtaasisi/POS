...
class FinancialService {
  ...
  async getPOSSales(): Promise<PaymentData[]> {
    try {
      ...
      if (error) {
        console.warn('Complex financial sales query failed, trying simpler query:', error.message);
        ...
        if (simpleError) {
          console.log('Simple financial sales query also failed:', simpleError);
          return [];
        }

        console.log(`✅ Loaded ${simpleData?.length || 0} financial sales (without joins)`);
        return simpleData || [];
      }
