...
class POSService {
  ...
  async getSalesByDateRange(
    ...
    try {
      ...
      if (error) {
        console.warn('Complex POS service sales query failed, trying simpler query:', error.message);
        ...
        if (simpleError) {
          console.error('❌ Simple POS service sales query also failed:', simpleError);
          return { success: false, error: simpleError.message };
        }

        console.log(`✅ Loaded ${simpleSales?.length || 0} POS service sales (without joins)`);
