...
export const getInventoryCategories = async (): Promise<InventoryCategory[]> => {
  try {
    // For now, return empty array since inventory_categories table doesn't exist
    console.log('Inventory categories not available yet');
    return [];
...
export const createInventoryCategory = async (categoryData: Omit<InventoryCategory, 'id' | 'created_at' | 'updated_at'>): Promise<InventoryCategory> => {
  try {
    ...
    console.log('Creating inventory category not available yet');
...
export const getSuppliers = async (): Promise<Supplier[]> => {
  try {
    // For now, return empty array since suppliers table doesn't exist
    console.log('Suppliers not available yet');
    return [];
...
export const createSupplier = async (supplierData: Omit<Supplier, 'id' | 'created_at' | 'updated_at'>): Promise<Supplier> => {
  try {
    ...
    console.log('Creating supplier not available yet');
...
export const getSparePartCategories = async (): Promise<string[]> => {
  ...
  const categories = [...new Set(data?.map(part => part.category) || [])];
...
export const getSparePartBrands = async (): Promise<string[]> => {
  ...
  const brands = [...new Set(data?.map(part => part.brand).filter(Boolean) || [])];
  return brands.sort();
};