...
export async function fetchCustomersPaginated(page: number = 1, pageSize: number = 50) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('customers')
      .select(`
        id, name, email, phone, whatsapp, address, city, country, birth_month, birth_day, gender, notes, is_active, created_at, updated_at, total_returns, profile_image, last_purchase_date, total_purchases, birthday, whatsapp_opt_out, loyalty_level, color_tag, referred_by, total_spent, points, last_visit, created_by, referral_source, initial_notes, referrals, customer_tag, joined_date
      `, { count: 'exact' })
...
export async function loadCustomerDetails(customerId: string) {
  try {
    ...
    const { data, error } = await supabase
      .from('customers')
      .select(`
        id, name, email, phone, whatsapp, address, city, country, birth_month, birth_day, gender, notes, is_active, created_at, updated_at
      `)
      .eq('id', customerId)
      .single();
