...
class SaleProcessingService {
  ...
  private async saveSaleToDatabase(saleData: SaleData): Promise<{ success: boolean; saleId?: string; sale?: SaleData; error?: string }> {
    try {
      ...
      if (saleError) {
        console.error('❌ Error creating sale with minimal data:', saleError);
        console.error('❌ Minimal sale insert data that failed:', JSON.stringify(minimalSaleData, null, 2));
        ...
        if (fallbackError) {
          ...
          return { success: false, error: `Failed to create sale: ${saleError.message}. Fallback also failed: ${fallbackError.message}` };
        }

        console.log('✅ Fallback insert succeeded');
        // Use the fallback sale data
        const sale = fallbackSale;
      }
