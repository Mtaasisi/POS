...
import { mapStatusForDatabase, isNewStatus } from './statusMapping';
  ...
  if (navigator.onLine) {
    try {
      ...
      if (error) {
        ...
        // Check if this is a constraint violation for the new status
        if (error.message && error.message.includes('parts-arrived')) {
          console.warn('Database constraint does not include parts-arrived status yet. This is expected if migration has not been applied.');
        }
        ...
        if (fallbackError) {
          ...
          // Check if this is a constraint violation for the new status
          if (fallbackError.message && fallbackError.message.includes('parts-arrived')) {
            console.warn('Database constraint does not include parts-arrived status yet. This is expected if migration has not been applied.');
...
export async function updateDeviceInDb(deviceId: string, updates: Partial<Device>) {
    ...
    if (validUpdateFields.includes(key) && updates[key as keyof Device] !== undefined) {
      ...
      if (key === 'status') {
        if (typeof value === 'string' && validStatusValues.includes(value)) {
          // Check if this is a new status that needs mapping
          if (isNewStatus(value)) {
            const mappedStatus = mapStatusForDatabase(value);
            filteredUpdates[key] = mappedStatus;
            console.log('[updateDeviceInDb] ✅ Mapped new status:', value, '→', mappedStatus);
          } else {
  ...
  if (error) {
    ...
    // Check if this is a constraint violation for the new status
    if (error.message && (error.message.includes('parts-arrived') || error.message.includes('process-payments'))) {
      console.warn('[updateDeviceInDb] Database constraint does not include new status yet. This is expected if migration has not been applied.');
      console.warn('[updateDeviceInDb] Please run the migration: apply-parts-arrived-migration.sql');
