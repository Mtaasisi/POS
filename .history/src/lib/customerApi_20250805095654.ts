...
export async function fetchAllCustomers() {
  if (navigator.onLine) {
    try {
      ...
      // Fetch POS sales for all customers with all related data
      let posSales = null;
      let posSalesError = null;
      
      try {
        // First try with full related data - fixed table references
        ...
        console.warn('⚠️ Full POS sales query failed, trying basic query:', error);
        ...
        posSales = data;
        posSalesError = basicError;
      }
      
      // If basic query also fails, try to check if table exists
      if (posSalesError) {
        console.warn('⚠️ Basic POS sales query also failed:', posSalesError);
        console.log('🔍 Checking if sales_orders table exists...');
        ...
        if (countError) {
          console.warn('❌ sales_orders table does not exist or is not accessible:', countError);
          posSales = [];
          posSalesError = null;
        } else {
          console.log(`📊 sales_orders table exists with ${count} records`);
        }
      }
      ...
      const customersWithData = customers.map(customer => {
        ...
        const transformedPOSSales = customerPOSSales.map((sale: any) => ({
          ...
          orderItems: sale.sales_order_items?.map((item: any) => ({
            id: item.id,
...
export async function fetchCustomerById(customerId: string) {
  if (navigator.onLine) {
    try {
      ...
      // Fetch POS sales for this customer with all related data
      let posSales = null;
      let posSalesError = null;
      
      try {
        // First try with full related data
        const { data, error } = await supabase
          .from('sales_orders')
          .select(`
            *,
        ...
        console.warn('⚠️ Full POS sales query failed for customer, trying basic query:', error);
