    ...
    try {
      ...
      const devicesWithData = (data || []).map(device => {
        ...
        return {
          ...
          deviceImages: device.device_images,
          accessoriesConfirmed: device.accessories_confirmed,
          problemConfirmed: device.problem_confirmed,
          privacyConfirmed: device.privacy_confirmed,
          // Purchase workflow fields
          purchaseType: device.purchase_type,
          purchasePrice: device.purchase_price,
          sellerVerificationStatus: device.seller_verification_status,
          securityCheckStatus: device.security_check_status,
          blacklistCheckResult: device.blacklist_check_result,
          // Admin approval fields
          adminApprovalStatus: device.admin_approval_status,
          adminApprovalNotes: device.admin_approval_notes,
          adminApprovalDate: device.admin_approval_date,
          adminApprovalBy: device.admin_approval_by,
          // Pickup and transaction fields
          pickupScheduledDate: device.pickup_scheduled_date,
          transactionId: device.transaction_id,
          // Checklist fields
          diagnosticChecklist: device.diagnostic_checklist,
          repairChecklist: device.repair_checklist,
          remarks: transformedRemarks,
          transitions: transformedTransitions,
          ratings: transformedRatings,
          createdAt: device.created_at,
