...
class SaleProcessingService {
  ...
  async processSale(saleData: Omit<SaleData, 'id' | 'saleNumber' | 'createdAt'>): Promise<ProcessSaleResult> {
    try {
      ...
      if (authError || !user) {
        console.error('❌ Authentication failed:', authError?.message || 'User not authenticated');
        ...
        if (refreshError || !session?.user) {
          console.error('❌ Session refresh failed:', refreshError?.message || 'No valid session');
          return { 
            success: false, 
            error: 'Authentication required. Please log in to process sales.' 
          };
        }
  ...
  private async saveSaleToDatabase(saleData: SaleData): Promise<{ success: boolean; saleId?: string; sale?: SaleData; error?: string }> {
    try {
      ...
      // Check authentication first - this is critical for 401 errors
