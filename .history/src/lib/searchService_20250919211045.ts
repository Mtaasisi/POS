...
export class SearchService {
  ...
  async searchCustomers(filters: SearchFilters, terms: string[]): Promise<SearchResult[]> {
    try {
      ...
      if (filters.phone) {
        const phoneFilter = filters.phone.replace(/[\s\-\(\)]/g, '');
        results = results.filter(customer => {
          ...
          // Direct match
          if (cleanCustomerPhone.includes(phoneFilter)) {
            return true;
          }
          
          // Enhanced phone number conversion (07123 -> 2557123)
          if (/^0\d{3,}$/.test(phoneFilter)) {
            const convertedNumber = `255${phoneFilter.substring(1)}`;
          ...
          if (/^255\d{3,}$/.test(phoneFilter)) {
            const convertedNumber = `0${phoneFilter.substring(3)}`;
            if (cleanCustomerPhone.includes(convertedNumber)) {
              return true;
            }
          }
