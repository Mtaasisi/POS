...
export async function addCustomerToDb(customer: Omit<Customer, 'promoHistory' | 'payments' | 'devices'>) {
  ...
  // Automatically add customer to loyalty system
  try {
    ...
    if (loyaltyError) {
      console.warn('Could not add customer to loyalty system:', loyaltyError);
      // Don't throw error here as customer was created successfully
    } else {
      console.log('✅ Customer automatically added to loyalty system');
    }
  } catch (loyaltyError) {
    console.warn('Could not add customer to loyalty system:', loyaltyError);
    // Don't throw error here as customer was created successfully
  ...
  try {
    ...
    await supabase.from('customer_notes').insert({
      ...
      content: `Added 10 initial loyalty points for new customer registration. Customer automatically enrolled in loyalty program.`,
...
export async function addCustomerToLoyalty(customerId: string) {
  try {
    ...
    if (existingLoyalty) {
      console.log('Customer already in loyalty system');
      return { success: true, message: 'Customer already enrolled in loyalty program' };
    }
    ...
    const { error: loyaltyError } = await supabase
      ...
      .insert([{
        customer_id: customerId,
        points: customer.points || 10, // Use existing points or give 10 initial points
        tier: 'bronze',
    ...
    try {
      ...
      console.warn('Could not add enrollment note:', noteError);
    }

    return { success: true, message: 'Customer successfully enrolled in loyalty program' };
  } catch (error) {
...
export async function enrollAllExistingCustomers() {
  try {
    ...
    if (customersToEnroll.length === 0) {
      return { success: true, message: 'All customers are already enrolled in loyalty program' };
    }
    ...
    return { 
      success: true, 
      message: `Successfully enrolled ${customersToEnroll.length} customers in loyalty program`,
      enrolledCount: customersToEnroll.length
    };
    
  } catch (error) {
    console.error('Error enrolling existing customers:', error);
    return { success: false, message: 'Failed to enroll existing customers in loyalty program' };
  }
} 