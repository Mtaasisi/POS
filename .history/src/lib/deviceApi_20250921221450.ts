    ...
    try {
      ...
      const devicesWithData = (data || []).map(device => {
        ...
        return {
          ...
          // Cost fields
          repairCost: null, // Column doesn't exist in database
          depositAmount: null, // Column doesn't exist in database
          deviceCost: device.device_cost,
          repairPrice: null, // Column doesn't exist in database
...
export async function addDeviceToDb(device: Device) {
  // Map camelCase fields to snake_case for DB - only include fields that exist in schema
  const dbDevice = {
    id: device.id,
