      ...
      try {
        ...
        if (!posSalesError && posSales) {
          ...
          const transformedPOSSales = posSales.map((sale: any) => ({
            ...
            method: this.parsePaymentMethod(sale.payment_method)?.type === 'multiple' ? 'Multiple' : this.mapPaymentMethod(this.parsePaymentMethod(sale.payment_method)),
            paymentMethod: this.parsePaymentMethod(sale.payment_method)?.type === 'multiple' ? 'Multiple' : this.mapPaymentMethod(this.parsePaymentMethod(sale.payment_method)),
  ...
  private parsePaymentMethod(paymentMethod: any): any {
    ...
    if (typeof paymentMethod === 'string') {
      try {
        ...
        console.warn('Failed to parse payment method JSON:', error);
