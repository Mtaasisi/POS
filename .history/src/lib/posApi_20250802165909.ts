...
export async function getProductPrice(
  ...
  try {
    ...
    // For spare parts, both productId and variantId are the same (spare part ID)
    // Check if this is a spare part by trying to query the spare_parts table
    try {
      ...
      console.warn('Error fetching spare part price:', sparePartError);
      // Continue to try product variants
    }
    
    // If we get here, the spare part doesn't exist or there was an error
    // Return a default price for spare parts that don't exist
    if (productId === variantId) {
      // This was likely a spare part that doesn't exist anymore
      console.warn(`Spare part ${productId} not found, using default price`);
      return customerType === 'wholesale' ? 45 : 50;
    }
    
    // If not a spare part, try to get price from product variants
    // This would be for regular products from the products table
    ...
    if (variantError) {
      console.error('Error fetching product variant price:', variantError);
    ...
    // Return a default price on any error
    return customerType === 'wholesale' ? 45 : 50;
  }
}
