...
export const exportCustomerDataAsSQL = async (options: ExportOptions = {}): Promise<Blob> => {
  try {
    let query = supabase.from('customers').select('id, name, email, phone, whatsapp, address, city, country, birth_month, birth_day, gender, notes, is_active, created_at, updated_at');
...
export const exportAllDataAsSQL = async (options: ExportOptions = {}): Promise<Blob> => {
  try {
    const [customers, devices, payments] = await Promise.all([
      supabase.from('customers').select('id, name, email, phone, whatsapp, address, city, country, birth_month, birth_day, gender, notes, is_active, created_at, updated_at, total_returns, profile_image, last_purchase_date, total_purchases, birthday, whatsapp_opt_out, loyalty_level, color_tag, referred_by, total_spent, points, last_visit, created_by, referral_source, initial_notes, referrals, customer_tag, joined_date').limit(50000), // Fetch up to 50,000 customers instead of default 1000
