...
export async function searchCustomers(query: string, page: number = 1, pageSize: number = 50) {
  try {
    ...
    // Add special phone number search for partial matches (last digits)
    // If query is numeric and 3+ digits, also search for phone numbers ending with those digits
    if (/^\d{3,}$/.test(query)) {
  ...
  try {
    ...
    const { data, error, count } = await supabase
      ...
      .select(`
        ...
        updated_at
      `, { count: 'exact' })
      .or(`name.ilike.%${query}%,phone.ilike.%${query}%,email.ilike.%${query}%,whatsapp.ilike.%${query}%,city.ilike.%${query}%,referral_source.ilike.%${query}%,customer_tag.ilike.%${query}%,initial_notes.ilike.%${query}%,id.ilike.%${query}%${/^\d{3,}$/.test(query) ? `,phone.like.%${query},whatsapp.like.%${query}` : ''}`)
      .range(offset, offset + pageSize - 1)
      .order('created_at', { ascending: false });
