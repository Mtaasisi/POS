          ...
          const { data: pageData, error: pageError } = await withTimeout(
            retryRequest(async () => {
              const result = await checkSupabase()
                .from('customers')
                .select(`
                  id, name, email, phone, gender, city, birth_month, birth_day, total_returns, profile_image, created_at, updated_at, last_purchase_date, total_purchases, birthday, whatsapp, whatsapp_opt_out, notes, is_active
                `)
                .range(from, to)
          ...
          if (pageData) {
            ...
            const processedCustomers = pageData.map(customer => {
              ...
              const mappedCustomer = {
                ...
                city: customer.city,
                joinedDate: customer.created_at, // Use created_at as joined_date
                loyaltyLevel: 'bronze', // Default value
                colorTag: normalizeColorTag('new'), // Default value
                referredBy: null, // Default value
                totalSpent: 0.00, // Default value
                points: 0, // Default value
                lastVisit: null, // Default value
                isActive: customer.is_active,
                referralSource: null, // Default value
