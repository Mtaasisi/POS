  ...
  try {
    ...
    const { data, error, count } = await supabase
      ...
      .select(`
        ...
        updated_at
      `, { count: 'exact' })
      .or(`name.ilike.%${query}%,phone.ilike.%${query}%,email.ilike.%${query}%,whatsapp.ilike.%${query}%,city.ilike.%${query}%,referral_source.ilike.%${query}%,customer_tag.ilike.%${query}%,initial_notes.ilike.%${query}%,id.ilike.%${query}%`)
      .range(offset, offset + pageSize - 1)
      .order('created_at', { ascending: false });
