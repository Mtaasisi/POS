...
export async function searchCustomers(query: string, page: number = 1, pageSize: number = 50) {
  try {
    ...
    // Create comprehensive search conditions (excluding UUID fields)
    ...
    if (/^\d{3,}$/.test(query)) {
      // Search for phone numbers ending with the query
      searchConditions.push({ phone: { like: `%${query}` } });
      searchConditions.push({ whatsapp: { like: `%${query}` } });
      
      // Search for phone numbers starting with the query
      searchConditions.push({ phone: { like: `${query}%` } });
      searchConditions.push({ whatsapp: { like: `${query}%` } });
      
      // Search for phone numbers containing the query (for partial matches)
      searchConditions.push({ phone: { like: `%${query}%` } });
      searchConditions.push({ whatsapp: { like: `%${query}%` } });
    }
    ...
    if (/^(\+255|255|0)?\d{9}$/.test(cleanQuery)) {
      ...
      phoneVariations.forEach(phone => {
        ...
        searchConditions.push({ phone: { like: `%${phone}%` } });
        searchConditions.push({ whatsapp: { like: `%${phone}%` } });
      });
    }
  ...
  try {
    ...
    const { data, error, count } = await supabase
      ...
      .select(`
        ...
        updated_at
      `, { count: 'exact' })
      .or(`name.ilike.%${query}%,phone.ilike.%${query}%,email.ilike.%${query}%,whatsapp.ilike.%${query}%,city.ilike.%${query}%,referral_source.ilike.%${query}%,customer_tag.ilike.%${query}%,initial_notes.ilike.%${query}%${/^\d{3,}$/.test(query) ? `,phone.like.%${query},whatsapp.like.%${query}` : ''}`)
      .range(offset, offset + pageSize - 1)
      .order('created_at', { ascending: false });
