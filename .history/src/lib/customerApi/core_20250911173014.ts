...
export async function checkCustomerExists(phone: string): Promise<Customer | null> {
  try {
...
export async function addCustomerToDb(customer: Omit<Customer, 'promoHistory' | 'payments' | 'devices'>) {
  try {
    ...
    if (customer.phone) {
      const existingCustomer = await checkCustomerExists(customer.phone);
      if (existingCustomer) {
        throw new Error(`A customer with phone number ${customer.phone} already exists. Customer: ${existingCustomer.name}`);
      }
    }
    ...
    if (error) {
      ...
      if (error?.code === '23505' || error?.message?.includes('duplicate key') || error?.message?.includes('unique constraint')) {
        throw new Error('A customer with this phone number already exists. Please use a different phone number or search for the existing customer.');
