...
export async function fetchAllAppointments() {
  try {
    ...
    // First try with joins to get customer and technician names
    ...
    if (error) {
      console.warn('⚠️ Join query failed, trying without joins:', error);
      ...
      if (simpleError) {
        console.error('❌ Error fetching appointments:', simpleError);
        throw simpleError;
      }
