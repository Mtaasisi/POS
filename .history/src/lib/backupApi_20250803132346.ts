 ...
 */
const readBackupFilesFromFS = (): BackupFile[] => {
  try {
    ...
    const files = fs.readdirSync(backupsDir)
      ...
      .map(file => {
        ...
        const timestamp = timestampMatch ? 
          timestampMatch[1].replace(/T/, ' ').replace(/-\d{3}Z/, '').replace(/(\d{2})-(\d{2})-(\d{2})/, '$1:$2:$3') : 
        ...
        return {
          ...
          size: `${sizeMB} MB`,
          timestamp,
          records: 1240, // Default value, could be read from file content
...
export const getBackupFiles = async (): Promise<BackupFile[]> => {
  try {
    ...
    const fsFiles = readBackupFilesFromFS();
    if (fsFiles.length > 0) {
      return fsFiles;
    }
