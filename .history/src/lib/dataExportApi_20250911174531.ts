...
export const exportAllDataAsSQL = async (options: ExportOptions = {}): Promise<Blob> => {
  try {
    const [customers, devices, payments] = await Promise.all([
      supabase.from('customers').select('id, name, email, phone, whatsapp, address, city, country, birth_month, birth_day, gender, notes, is_active, created_at, updated_at').limit(50000), // Fetch up to 50,000 customers instead of default 1000
