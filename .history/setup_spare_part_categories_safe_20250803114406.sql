-- Safe Setup Spare Part Categories Table
...
-- Add category_id column to spare_parts table (nullable initially)
...
-- Check if all spare parts have been updated
-- If not, set remaining ones to 'Other' category
UPDATE spare_parts 
SET category_id = (
  SELECT id FROM spare_part_categories 
  WHERE name = 'Other'
...
-- Now make category_id NOT NULL after all records are updated
ALTER TABLE spare_parts ALTER COLUMN category_id SET NOT NULL;

-- Optional: Drop the old category column for backward compatibility
-- Uncomment the line below if you want to remove the old category column
