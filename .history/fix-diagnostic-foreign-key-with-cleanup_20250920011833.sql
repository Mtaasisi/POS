-- Fix diagnostic_checks table foreign key constraint with data cleanup
-- This script handles orphaned records before adding the foreign key

-- First, let's check what orphaned records exist
SELECT 
    dc.id,
...
-- Clean up orphaned records in diagnostic_checks
-- These are records that reference non-existent diagnostic_devices
DELETE FROM diagnostic_checks 
WHERE diagnostic_device_id NOT IN (
    SELECT id FROM diagnostic_devices
);
...
-- Add the foreign key constraint now that orphaned records are cleaned up
DO $$
BEGIN
...
SELECT 
    'diagnostic_checks' as table_name,
    COUNT(*) as total_records,
    COUNT(CASE WHEN dd.id IS NOT NULL THEN 1 END) as valid_records,
    COUNT(CASE WHEN dd.id IS NULL THEN 1 END) as orphaned_records
FROM diagnostic_checks dc
LEFT JOIN diagnostic_devices dd ON dc.diagnostic_device_id = dd.id;
