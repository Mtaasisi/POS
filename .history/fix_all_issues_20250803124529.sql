...
-- Insert some default diagnostic templates (only if they don't exist)
INSERT INTO diagnostic_templates (device_type, checklist_items) 
SELECT 'Laptop', '[
...
WHERE NOT EXISTS (SELECT 1 FROM diagnostic_templates WHERE device_type = 'Laptop');
...
WHERE NOT EXISTS (SELECT 1 FROM diagnostic_templates WHERE device_type = 'Phone');
...
WHERE NOT EXISTS (SELECT 1 FROM diagnostic_templates WHERE device_type = 'Tablet');
...
-- Insert default SMS templates (only if they don't exist)
INSERT INTO communication_templates (title, content, module, variables) 
SELECT 'Device Received', 'Dear {customer_name}, we have received your {device_brand} {device_model} for repair. Ticket: {ticket_number}. We will contact you once the repair is complete.', 'sms', ARRAY['customer_name', 'device_brand', 'device_model', 'ticket_number']
WHERE NOT EXISTS (SELECT 1 FROM communication_templates WHERE title = 'Device Received');

INSERT INTO communication_templates (title, content, module, variables) 
SELECT 'Device Ready', 'Dear {customer_name}, your {device_brand} {device_model} is ready for collection. Ticket: {ticket_number}. Please visit our shop to collect your device.', 'sms', ARRAY['customer_name', 'device_brand', 'device_model', 'ticket_number']
WHERE NOT EXISTS (SELECT 1 FROM communication_templates WHERE title = 'Device Ready');

INSERT INTO communication_templates (title, content, module, variables) 
SELECT 'Payment Reminder', 'Dear {customer_name}, please complete payment for your {device_brand} {device_model} repair. Amount: {amount}. Ticket: {ticket_number}', 'sms', ARRAY['customer_name', 'device_brand', 'device_model', 'amount', 'ticket_number']
WHERE NOT EXISTS (SELECT 1 FROM communication_templates WHERE title = 'Payment Reminder');
