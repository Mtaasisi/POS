...
make_executable() {
    if [ -f "backup-to-github.mjs" ] && [ ! -x "backup-to-github.mjs" ]; then
...
main() {
    ...
    case "$command" in
        "backup"|"b")
            ...
            if [ -n "$message" ]; then
                node "$PROJECT_DIR/backup-to-github.mjs" backup "$message"
            else
                node "$PROJECT_DIR/backup-to-github.mjs" backup
            fi
            ;;
        "restore"|"r")
            echo "🔄 Restoring from GitHub..."
            node "$PROJECT_DIR/backup-to-github.mjs" restore
            ;;
        "status"|"s")
            echo "📊 Checking backup status..."
            node "$PROJECT_DIR/backup-to-github.mjs" status
            ;;
        "setup")
            echo "📋 GitHub backup setup..."
            node "$PROJECT_DIR/backup-to-github.mjs" setup
            ;;
        "help"|"-h"|"--help")
            show_usage
            ;;
