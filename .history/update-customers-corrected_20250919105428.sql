...
-- Show duplicate phone numbers that will be merged
SELECT 
    'Duplicate phone numbers that will be merged:' as info,
    phone,
    COUNT(*) as duplicate_count,
    STRING_AGG(name, ', ') as all_names,
    STRING_AGG(location, ', ') as all_locations
FROM temp_customer_data
GROUP BY phone
HAVING COUNT(*) > 1
...
-- Show customers from CSV that don't exist in database (potential new customers)
SELECT 
    'CSV customers not found in database (potential new customers):' as info,
    t.name,
    t.phone,
    t.location
FROM temp_customer_data t
LEFT JOIN customers c ON c.phone = t.phone
WHERE c.id IS NULL AND t.rn = 1
ORDER BY t.name
LIMIT 10;
