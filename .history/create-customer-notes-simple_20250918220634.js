// Simple browser script to create customer_notes table
...
async function createCustomerNotesTable() {
  ...
  try {
    ...
    // Try to execute the SQL using the SQL editor approach
    ...
    // This will fail if the table doesn't exist, which will help us confirm the issue
    console.log('🧪 Testing if customer_notes table exists...');
...
GRANT ALL ON customer_notes TO authenticated;
      ...
      console.log('✅ Customer notes table exists (foreign key constraint working as expected)');
      return true;
    } else if (testError) {
...
createCustomerNotesTable().then(success => {
  if (success) {
    ...
    console.log('💥 Customer notes table needs to be created manually.');
  }
});
