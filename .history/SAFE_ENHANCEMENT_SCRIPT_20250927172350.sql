-- SAFE ENHANCEMENT SCRIPT FOR INVENTORY MANAGEMENT
-- This script is safe to run multiple times without errors
-- Run this SQL directly in your Supabase SQL Editor

-- Step 1: Create audit trail table for inventory item changes (safe creation)
DO $$
BEGIN
    -- Create table if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'inventory_item_audit') THEN
...
-- Step 2: Create indexes for audit table (safe creation)
...
-- Step 3: Create RLS policies for audit table (safe creation)
DO $$
BEGIN
    -- Drop policy if it exists, then create it
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'inventory_item_audit' AND policyname = 'Users can view inventory item audit') THEN
        DROP POLICY "Users can view inventory item audit" ON inventory_item_audit;
    END IF;
...
-- Step 4: Ensure inventory_items table has all required columns
DO $$
BEGIN
...
-- Step 5: Create or replace all functions (safe creation)
...
-- Step 6: Grant permissions (safe granting)
DO $$
BEGIN
...
-- Step 7: Create indexes for better performance (safe creation)
...
-- Step 8: Create enhanced view (safe creation)
...
-- Grant access to the view (safe granting)
DO $$
BEGIN
