...
-- 5. Create test data if needed (with proper conflict handling)
-- First, add unique constraint to name column if it doesn't exist
ALTER TABLE auth_users ADD CONSTRAINT IF NOT EXISTS auth_users_name_unique UNIQUE (name);

-- Then insert test data with conflict handling
