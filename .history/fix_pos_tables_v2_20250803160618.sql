-- Fix POS Tables v2 - Create tables first, then add references
-- Run this script to fix the database structure

-- 1. Create locations table FIRST (before any references)
...
-- 3. Now add location_id to sales_orders table if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- 4. Add location_id to product_variants table if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 5. Add missing columns to sales_orders if they don't exist
DO $$ 
BEGIN
...
-- 6. Add missing columns to sales_order_items if they don't exist
DO $$ 
BEGIN
