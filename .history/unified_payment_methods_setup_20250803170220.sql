...
ALTER TABLE payment_methods_accounts ENABLE ROW LEVEL SECURITY;

-- 12. Create RLS policies
CREATE POLICY "Users can view payment methods" ON payment_methods 
FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Users can view payment method accounts" ON payment_methods_accounts 
FOR SELECT USING (auth.role() = 'authenticated');
...
CREATE INDEX IF NOT EXISTS idx_payment_method_accounts_method_id ON payment_methods_accounts(payment_method_id);
CREATE INDEX IF NOT EXISTS idx_payment_method_accounts_account_id ON payment_methods_accounts(account_id);
...
COMMENT ON TABLE payment_methods_accounts IS 'Mapping between payment methods and finance accounts';
