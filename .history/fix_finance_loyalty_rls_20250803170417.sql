-- Fix RLS Policies for Finance and Loyalty Tables
...
-- Grant permissions for loyalty_rewards if it exists
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'loyalty_rewards') THEN
...
-- Success message
SELECT 'Finance and Loyalty tables RLS policies updated successfully!' as status; 