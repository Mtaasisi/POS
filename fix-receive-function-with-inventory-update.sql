-- Fix the complete_purchase_order_receive function to properly update inventory
-- This addresses the issue where received items are not connected to inventory
...
BEGIN
    ...
    LOOP
        ...
        -- Update product variant quantity (THIS IS THE KEY FIX)
...
DO $$
BEGIN
    RAISE NOTICE 'Receive function updated with inventory quantity updates';
    RAISE NOTICE 'Now properly updates product variant quantities and creates stock movements';
    RAISE NOTICE 'Received items will be connected to inventory system';
END $$;
