...
-- 2. SALE DETAILS WITH CUSTOMER INFO (WORKS IMMEDIATELY)
-- This query gets all sales with customer details - no specific ID needed
SELECT 
    s.id,
    s.sale_number,
    s.customer_id,
...
-- 3. SALE ITEMS WITH PRODUCT AND VARIANT DETAILS (WORKS IMMEDIATELY)
-- This query gets all sale items with product details - no specific ID needed
SELECT 
    si.id,
    si.sale_id,
    si.product_id,
