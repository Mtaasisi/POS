...
export async function uploadProductImage(
  ...
  try {
    ...
    console.log('📸 Uploaded images:', {
      ...
      originalUrl,
      thumbnailUrl,
      originalSize: optimizedFile.size,
...
export async function deleteProductImage(imageId: string, imageUrl: string, thumbnailUrl?: string): Promise<void> {
  try {
    // Extract file paths from URLs
    const originalUrlParts = imageUrl.split('/');
    const originalFilePath = originalUrlParts.slice(originalUrlParts.indexOf('product-images') + 1).join('/');
    
    const filesToDelete = [originalFilePath];
    
    // If thumbnail URL exists, extract and add to deletion list
    if (thumbnailUrl) {
      const thumbnailUrlParts = thumbnailUrl.split('/');
      const thumbnailFilePath = thumbnailUrlParts.slice(thumbnailUrlParts.indexOf('product-images') + 1).join('/');
      filesToDelete.push(thumbnailFilePath);
    }
