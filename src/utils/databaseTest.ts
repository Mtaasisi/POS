...
export const testDatabaseConnection = async () => {
  try {
    ...
    // Test 3: Try to fetch a specific sale (the one that's failing) - using safe query
    const failingSaleId = '36487185-0673-4e03-83c2-26eba8d9fef7';
    
    const specificSaleResult = await safeQuery(
      () => supabase
        .from('lats_sales')
        ...
        .limit(1)
        .maybeSingle(),
      () => SupabaseErrorHandler.createLatsSalesFallbackQuery(supabase, failingSaleId)
    );

    if (specificSaleResult.error) {
      console.error(`❌ Specific sale ${failingSaleId} not found:`, specificSaleResult.error);
    } else {
      console.log('✅ Specific sale found:', specificSaleResult.data);
    }

    return { 
      success: true, 
      salesCount: salesCount?.length || 0,
      specificSaleFound: !specificSaleResult.error,
      specificSaleError: specificSaleResult.error?.message
    };
  } catch (error) {
