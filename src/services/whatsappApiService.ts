...
export class WhatsAppApiService {
  ...
  static async checkInstanceState(instanceId: string, apiToken: string, host?: string): Promise<ConnectionStatusResponse> {
    try {
      const response = await greenApiService.getStateInstance(instanceId, apiToken, host);
      return {
  ...
  static async checkInstanceStateAlternative(instanceId: string, apiToken: string, host?: string): Promise<ConnectionStatusResponse> {
    try {
      ...
      const alternativeHost = host || 'https://api.green-api.com';
      const response = await greenApiService.getStateInstance(instanceId, apiToken, alternativeHost);
      return {
        stateInstance: response.stateInstance
      };
    } catch (error: any) {
  ...
  static async generateQRCode(instanceId: string, apiToken: string, host?: string): Promise<string | null> {
    try {
      const qrCode = await greenApiService.getQRCodeHTTP(instanceId, apiToken, host);
      return qrCode;
    } catch (error: any) {
  ...
  static async generateQRCodeWebSocket(instanceId: string, apiToken: string, host?: string): Promise<WebSocket | null> {
    try {
      const ws = await greenApiService.scanQRCodeWebSocket(instanceId, apiToken, host);
      return ws;
    } catch (error: any) {
   ...
   */
  static async setProfilePicture(instanceId: string, apiToken: string, file: File, host?: string): Promise<any> {
    try {
      const response = await greenApiService.setProfilePicture(instanceId, apiToken, file, host);
      return response;
    } catch (error: any) {
