...
class DashboardService {
  ...
  private async getAppointmentStats() {
    try {
      ...
      const upcomingCount = appointments?.filter(a => a.appointment_date > today).length || 0;
  ...
  private async getInventoryStats() {
    try {
      const { data: products, error } = await supabase
        ...
        .select('id, quantity, minimum_stock_level, cost_price');

      if (error) throw error;

      const lowStock = products?.filter(p => p.quantity <= (p.minimum_stock_level || 0)).length || 0;
  ...
  private async getFinancialStats() {
    try {
      ...
      const todayRevenue = payments?.filter(p => 
        p.payment_date?.startsWith(today) && p.payment_status === 'completed'
      ).reduce((sum, p) => sum + p.amount, 0) || 0;

      const weeklyRevenue = payments?.filter(p => 
        new Date(p.payment_date) >= weekAgo && p.payment_status === 'completed'
      ).reduce((sum, p) => sum + p.amount, 0) || 0;
      ...
      const completed = payments?.filter(p => p.payment_status === 'completed').length || 0;
  ...
  async getRecentNotifications(userId: string, limit: number = 5): Promise<NotificationSummary[]> {
    try {
      const { data: notifications, error } = await supabase
        .from('notifications')
        .select('id, title, message, type, priority, created_at, category')
        .eq('user_id', userId)
        .eq('status', 'unread')
    ...
    try {
      const { data: products, error } = await supabase
        ...
        .select(`
          ...
          product:lats_products(name, category)
        `)
        .lte('quantity', supabase.sql`minimum_stock_level`)
      ...
      return products?.map(p => {
        ...
        const minStock = p.minimum_stock_level || 0;
        ...
        return {
          id: p.id,
          productName: (p.product as any)?.name || 'Unknown Product',
          currentStock,
          minimumStock: minStock,
          category: (p.product as any)?.category || 'Unknown',
          alertLevel
        };
      }) || [];
    } catch (error) {
  ...
  private async getDeviceActivities(limit: number): Promise<RecentActivity[]> {
    try {
      const { data: devices, error } = await supabase
        .from('devices')
        .select('id, customer_name, device_model, status, updated_at')
      ...
      return devices?.map(d => ({
        ...
        description: `${d.device_model} - ${d.customer_name}`,
        time: d.updated_at,
        status: d.status
      })) || [];
    } catch (error) {
  ...
  private async getPaymentActivities(limit: number): Promise<RecentActivity[]> {
    try {
      const { data: payments, error } = await supabase
        .from('customer_payments')
        .select('id, amount, payment_method, payment_date, customer_id')
  ...
  private async getAppointmentActivities(limit: number): Promise<RecentActivity[]> {
    try {
      const { data: appointments, error } = await supabase
        .from('appointments')
        .select('id, customer_name, service_type, status, updated_at')
        .order('updated_at', { ascending: false })
        .limit(limit);
      ...
      return appointments?.map(a => ({
        ...
        description: `${a.service_type} - ${a.customer_name}`,
        time: a.updated_at,
        status: a.status
      })) || [];
    } catch (error) {
  ...
  async getFinancialSummary(): Promise<FinancialSummary> {
    try {
      ...
      const { data: payments, error } = await supabase
        .from('customer_payments')
        .select('amount, payment_method, payment_date, payment_status')
        .gte('payment_date', monthAgo.toISOString());
