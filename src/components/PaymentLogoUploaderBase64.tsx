...
interface PaymentLogoUploaderBase64Props {
  currentIcon?: string;
...
const PaymentLogoUploaderBase64: React.FC<PaymentLogoUploaderBase64Props> = ({
  currentIcon,
  onIconChange,
  paymentMethodName,
  className = ''
  ...
  // Helper function to check if it's a base64 data URL
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 1 * 1024 * 1024) {
      toast.error('Image size must be less than 1MB for base64 encoding');
      return;
    }
  ...
  const handleUrlInput = (url: string) => {
    ...
    // Validate URL format
    if (!isUrl(url) && !url.startsWith('/') && !isBase64DataUrl(url)) {
      toast.error('Please enter a valid URL, path, or base64 data URL');
      return;
    }
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-500">
            Upload an image, enter a URL, or use emoji for the payment method logo
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <input
            type="text"
            placeholder="https://example.com/logo.png, /icons/logo.svg, or emoji"
            value={displayIcon && !isEmoji(displayIcon) && !isBase64DataUrl(displayIcon) ? displayIcon : ''}
        ...
        <div className="flex items-start space-x-2">
          ...
          <div className="text-sm text-blue-800">
            ...
            <ul className="text-xs space-y-1 ml-2">
              <li>• Images: PNG, JPG, JPEG, GIF (max 1MB - stored as base64)</li>
...
export default PaymentLogoUploaderBase64;
