    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const customer: ImportedCustomer = {
          ...
          colorTag: normalizeColorTag(values[headers.indexOf('color tag')] || values[headers.indexOf('tag')] || 'new'),
          ...
          // Additional fields
          locationDescription: values[headers.indexOf('location description')] || values[headers.indexOf('address')] || values[headers.indexOf('home address')] || values[headers.indexOf('business address')] || '',
          nationalId: values[headers.indexOf('national id')] || values[headers.indexOf('id number')] || '',
          referredBy: values[headers.indexOf('referred by')] || values[headers.indexOf('referrer')] || '',
          totalSpent: parseFloat(values[headers.indexOf('total spent')] || values[headers.indexOf('spent')] || '0') || 0,
          points: parseInt(values[headers.indexOf('points')] || '0') || 0,
          isActive: values[headers.indexOf('is active')] !== 'false', // Default to true
          profileImage: values[headers.indexOf('profile image')] || values[headers.indexOf('image')] || ''
        };
