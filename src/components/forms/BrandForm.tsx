...
const BrandForm: React.FC<BrandFormProps> = ({
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    // Validate file size (max 2MB for hosting)
    ...
    try {
      ...
      if (fileUploadService.isDevelopment()) {
        // In development: use base64 for immediate preview
        const base64Data = await fileUploadService.fileToBase64(file);
        ...
        const filePath = fileUploadService.generateBrandLogoPath(brandName, fileExtension);
        ...
        if (uploadResult.success && uploadResult.url) {
          ...
          toast.success('Logo uploaded successfully! (Hosted)');
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Clean up old logo if it was replaced and we're in production
      if (!fileUploadService.isDevelopment() && editingBrand && originalLogoUrl && formData.logo_url !== originalLogoUrl) {
        // Extract path from original URL for deletion
        const urlParts = originalLogoUrl.split('/');
        const fileName = urlParts[urlParts.length - 1];
        if (fileName) {
          const oldPath = `brand-logos/${fileName}`;
          await fileUploadService.deleteFile(oldPath);
          console.log('🗑️ Cleaned up old logo:', oldPath);
        }
      }
