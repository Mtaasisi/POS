...
const BrandForm: React.FC<BrandFormProps> = ({
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    // Validate file size (max 5MB for hosting)
    ...
    try {
      console.log('🔍 Logo upload started:', {
        ...
        fileSize: file.size,
        fileType: file.type,
        isDevelopment: localBrandStorage.isDevelopment()
      });
      ...
      if (localBrandStorage.isDevelopment()) {
        ...
        // In production: upload to local hosting storage
    ...
    try {
      ...
      if (!localBrandStorage.isDevelopment() && editingBrand && originalLogoUrl && formData.logo_url !== originalLogoUrl) {
        ...
        if (fileName) {
          ...
          await localBrandStorage.deleteFile(oldPath);
