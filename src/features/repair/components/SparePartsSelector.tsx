...
const SparePartsSelector: React.FC<SparePartsSelectorProps> = ({
  ...
  const handleAddPart = (sparePart: SparePart) => {
    ...
    if (sparePart.quantity <= 0) {
      toast.error(`${sparePart.name} is out of stock.`);
      return;
    }
    ...
    if (existingPart) {
      ...
      if (existingPart.quantity + 1 > sparePart.quantity) {
        toast.error(`Cannot add more. Only ${sparePart.quantity} available in stock.`);
        return;
      }
  ...
  const handleUpdateQuantity = (sparePartId: string, newQuantity: number) => {
    ...
    if (newQuantity > sparePart.quantity) {
      toast.error(`Cannot select ${newQuantity}. Only ${sparePart.quantity} available in stock.`);
      return;
    }
