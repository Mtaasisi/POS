        ...
        <div className={`${getSizeClasses()} relative group`}>
          {loading ? (
            ...
            <div className="relative w-full h-full">
              ...
              />
              {/* Show if using fallback image */}
              {variantImages.length === 0 && variant.images?.length === 0 && productImages.length > 0 && (
                <div className="absolute bottom-0 left-0 right-0 bg-blue-500 text-white text-xs px-1 py-0.5 rounded-b-lg text-center">
                  Product Image
                </div>
              )}
              {editable && (
