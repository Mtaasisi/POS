...
import { 
  Settings, Users, Building, Truck, Plus, Edit, Trash2, Save, X, 
  CheckCircle, XCircle, Mail, Phone, ExternalLink, UserPlus, 
  Shield, Bell, Clock, DollarSign, FileText
} from 'lucide-react';
import { 
  ShippingSettings, ShippingAgent, ShippingManager, ShippingCarrier,
  ShippingAgentFormData, ShippingManagerFormData, ShippingCarrierFormData
...
interface ShippingSettingsManagerProps {
  ...
  onCarrierCreate: (carrier: ShippingCarrierFormData) => void;
  onCarrierUpdate: (id: string, carrier: Partial<ShippingCarrierFormData>) => void;
  onCarrierDelete: (id: string) => void;
}
...
const ShippingSettingsManager: React.FC<ShippingSettingsManagerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'settings' | 'agents' | 'managers' | 'carriers'>('settings');
  ...
  const [carrierForm, setCarrierForm] = useState<ShippingCarrierFormData>({
    name: '', code: '', trackingUrl: '', supportedServices: [], contactInfo: {}, isActive: true
  });
  ...
  const resetForms = () => {
    ...
    setCarrierForm({ name: '', code: '', trackingUrl: '', supportedServices: [], contactInfo: {}, isActive: true });
    setShowAddForm(false);
    setEditingId(null);
  };

  const handleAddAgent = () => {
    if (!agentForm.name.trim() || !agentForm.email.trim() || !agentForm.phone.trim()) {
  ...
  const handleAddCarrier = () => {
    if (!carrierForm.name.trim() || !carrierForm.code.trim() || !carrierForm.trackingUrl.trim()) {
      toast.error('Please fill in all required fields');
      return;
    }
    onCarrierCreate(carrierForm);
    resetForms();
    toast.success('Carrier added successfully');
  };

  const startEdit = (type: 'agent' | 'manager' | 'carrier', id: string) => {
    setEditingId(id);
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              type="number"
              value={settings.maxShippingCost}
          ...
          <div>
            ...
            <select
              value={settings.defaultCarrierId}
              onChange={(e) => handleSettingsChange('defaultCarrierId', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
        ...
        <div className="flex flex-wrap gap-3">
          {['email', 'sms', 'whatsapp'].map(channel => (
            <label key={channel} className="flex items-center gap-2 bg-gray-50 rounded-lg px-4 py-3">
              <input
                type="checkbox"
                checked={settings.notificationChannels.includes(channel as any)}
                onChange={(e) => {
                  const channels = settings.notificationChannels.filter(c => c !== channel);
                  if (e.target.checked) {
        ...
        {managers.map(manager => (
          <div key={manager.id} className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-lg transition-all">
            ...
            <div className="space-y-2 text-sm text-gray-600 mb-4">
              ...
              <div className="flex items-center gap-2">
                <Users size={14} />
                <span>{manager.agents.length} agents</span>
              </div>
            </div>
  ...
  const renderCarriersTab = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Shipping Carriers</h3>
          <p className="text-gray-600">Manage available shipping carriers and services</p>
        </div>
        <GlassButton
          onClick={() => {
      ...
      {showAddForm && activeTab === 'carriers' && (
        <div className="bg-green-50 rounded-xl p-6 border border-green-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <input
              type="text"
              placeholder="Carrier Code (e.g., DHL)"
            ...
            <input
              type="text"
              placeholder="Tracking URL (use {tracking_number})"
              value={carrierForm.trackingUrl}
              onChange={(e) => setCarrierForm({...carrierForm, trackingUrl: e.target.value})}
              className="col-span-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
            />
            <input
              type="text"
              placeholder="Contact Phone"
              value={carrierForm.contactInfo?.phone || ''}
              onChange={(e) => setCarrierForm({
                ...carrierForm, 
                contactInfo: {...carrierForm.contactInfo, phone: e.target.value}
              })}
              className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
            />
            <input
              ...
              value={carrierForm.contactInfo?.email || ''}
              onChange={(e) => setCarrierForm({
                ...carrierForm, 
                contactInfo: {...carrierForm.contactInfo, email: e.target.value}
              })}
              className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
            />
          </div>
          ...
          <div className="flex gap-3">
            <GlassButton
              onClick={editingId ? () => {
                onCarrierUpdate(editingId, carrierForm);
                resetForms();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {carriers.map(carrier => (
          <div key={carrier.id} className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-lg transition-all">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{carrier.supportedServices.join(', ')}</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                {carrier.isActive ? (
            ...
            <div className="space-y-2 text-sm text-gray-600 mb-4">
              {carrier.contactInfo?.phone && (
                <div className="flex items-center gap-2">
                  <Phone size={14} />
                  <span>{carrier.contactInfo.phone}</span>
                </div>
              )}
              {carrier.contactInfo?.email && (
                <div className="flex items-center gap-2">
                  <Mail size={14} />
                  <span>{carrier.contactInfo.email}</span>
                </div>
              )}
              {carrier.contactInfo?.website && (
                <div className="flex items-center gap-2">
                  <ExternalLink size={14} />
                  <a href={carrier.contactInfo.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-700">
                    Website
                  </a>
                </div>
              )}
        ...
        {carriers.length === 0 && (
          <div className="col-span-full text-center py-8 text-gray-500">
            ...
            <p className="text-sm">Add shipping carriers to enable tracking</p>
          </div>
        )}
      </div>
    </div>
