...
import { ArrowLeft, Building2, Grid3X3, Plus, Search, Trash2, Copy, Download } from "lucide-react";
    ...
    const run = async () => {
      ...
      try {
        ...
        const { data: roomData, error: roomErr } = await supabase
          .from<StorageRoom>("lats_storage_rooms")
          .select("*")
          .eq("id", roomId)
          .single();
        ...
        try {
          ...
          const { data: locData } = await supabase
            .from<StoreLocation>("lats_store_locations")
            .select("*")
        ...
        const { data: shelfData, error: shelfErr } = await supabase
          .from<StoreShelf>("lats_store_shelves")
          .select("*")
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="text-xl font-semibold text-gray-900">
                {room.name} <span className="text-gray-400">•</span> {room.code}
              </div>
