...
class SparePartsRelationshipService {
  ...
  async getUsageHistory(sparePartId: string, limit: number = 10): Promise<SparePartUsage[]> {
    try {
      // Since device_id is not a foreign key, we'll fetch usage data without the join
      // and then fetch device data separately if needed
      const { data, error } = await supabase
      ...
      // Get unique device IDs from the usage data
      const deviceIds = [...new Set(data.map(item => item.device_id).filter(Boolean))];
      
      // Fetch device information separately if we have device IDs
      let deviceData: any[] = [];
      ...
      const deviceMap = new Map(deviceData.map(device => [device.id, device]));
  ...
  async getSupplierDetails(supplierId: string): Promise<any> {
    try {
      // First fetch supplier data without the join
      ...
      // Fetch purchase orders separately
      ...
      if (ordersError) {
        console.warn('⚠️ Could not fetch purchase orders for supplier:', ordersError.message);
      }

      // Calculate supplier statistics
      const totalOrders = purchaseOrders?.length || 0;
      const lastOrderDate = purchaseOrders && purchaseOrders.length > 0 
