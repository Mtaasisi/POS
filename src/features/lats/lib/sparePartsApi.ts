...
// Utility function to validate stock availability
export const validateStockAvailability = async (sparePartId: string, quantityNeeded: number): Promise<{
  isValid: boolean;
  ...
  try {
    ...
    if (error) {
      return {
        ...
        availableStock: 0,
        partName: 'Unknown',
        error: `Failed to fetch spare part: ${error.message}`
      };
    }
    ...
    if (!sparePart) {
      return {
        ...
        availableStock: 0,
        partName: 'Unknown',
        error: `Spare part not found: ${sparePartId}`
      };
    }

    if (sparePart.quantity < quantityNeeded) {
      return {
        isValid: false,
        availableStock: sparePart.quantity,
        partName: sparePart.name,
        error: `Insufficient stock for ${sparePart.name}. Available: ${sparePart.quantity}, Needed: ${quantityNeeded}`
      };
    }
...
export const createRepairParts = async (dataArray: CreateRepairPartData[]): Promise<RepairPartsResponse> => {
  try {
    ...
    const successfulStockUpdates: Array<{spare_part_id: string, original_quantity: number}> = [];
    const successfulUsageRecords: string[] = [];
    let hasErrors = false;
    const errors: string[] = [];
    ...
    for (const data of dataArray) {
      ...
      if (fetchCurrentError) {
        ...
        errors.push(`Failed to fetch stock for part ${data.spare_part_id}`);
        hasErrors = true;
        continue;
      }
      ...
      if (stockError) {
        ...
        errors.push(`Failed to update stock for part ${data.spare_part_id}`);
        hasErrors = true;
        continue;
      }

      // Track successful stock update for potential rollback
      successfulStockUpdates.push({
        spare_part_id: data.spare_part_id,
        original_quantity: currentPart.quantity
      });
      ...
      if (usageError) {
        ...
        errors.push(`Failed to record usage for part ${data.spare_part_id}`);
        hasErrors = true;
        continue;
      }

      successfulUsageRecords.push(data.spare_part_id);
    }

    // If there were errors, attempt rollback for successful operations
    if (hasErrors) {
      console.warn('Some operations failed, attempting rollback...');
      
      // Rollback successful stock updates
      for (const update of successfulStockUpdates) {
        const { error: rollbackError } = await supabase
          ...
          .eq('id', update.spare_part_id);

        if (rollbackError) {
          console.error('Failed to rollback stock for part:', update.spare_part_id, rollbackError);
        }
      }

      // Rollback successful usage records
      for (const sparePartId of successfulUsageRecords) {
        const { error: rollbackError } = await supabase
          .from('lats_spare_part_usage')
          .delete()
          .eq('spare_part_id', sparePartId)
          .eq('device_id', dataArray[0]?.device_id)
          .eq('reason', `Repair attachment - Device: ${dataArray[0]?.device_id}`);

        if (rollbackError) {
          console.error('Failed to rollback usage record for part:', sparePartId, rollbackError);
        }
      }

      // Delete the repair parts that were created
      ...
      if (repairPartIds.length > 0) {
        ...
        if (deleteError) {
          console.error('Failed to rollback repair parts:', deleteError);
        }
      }

      throw new Error(`Operation failed: ${errors.join(', ')}. All changes have been rolled back.`);
    }

    // Emit event to refresh inventory data
    if (typeof window !== 'undefined' && window.dispatchEvent) {
      window.dispatchEvent(new CustomEvent('lats:spare-parts-updated', {
        detail: { action: 'stock-deducted', partsCount: repairParts?.length || 0 }
      }));
    }
...
export const createRepairPart = async (data: CreateRepairPartData): Promise<RepairPartResponse> => {
  try {
    ...
    const { error: stockError } = await supabase
      .from('lats_spare_parts')
      .update({
        quantity: sparePart.quantity - data.quantity_needed,
        updated_at: new Date().toISOString()
      })
      .eq('id', data.spare_part_id);
    ...
    if (typeof window !== 'undefined' && window.dispatchEvent) {
      window.dispatchEvent(new CustomEvent('lats:spare-parts-updated', {
        detail: { action: 'stock-deducted', partsCount: 1 }
      }));
    }

    return {
      data: repairPart,
      message: 'Repair part created and stock deducted successfully',
      ok: true
    };
  } catch (error) {
