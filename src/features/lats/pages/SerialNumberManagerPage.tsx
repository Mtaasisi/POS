...
const SerialNumberManagerPage: React.FC = () => {
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      if (existingItem) {
        toast.error('Serial number already exists for this product');
        return;
      }
  ...
  const handleBulkAdd = async () => {
    ...
    try {
      ...
      if (checkError) {
        console.error('Error checking serial numbers:', checkError);
        toast.error('Failed to validate serial numbers');
        return;
      }

      if (existingItems && existingItems.length > 0) {
        const duplicates = existingItems.map(item => item.serial_number).join(', ');
        toast.error(`Serial numbers already exist: ${duplicates}`);
        return;
      }
