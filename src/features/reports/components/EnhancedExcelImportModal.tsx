...
const EnhancedExcelImportModal: React.FC<EnhancedExcelImportModalProps> = ({
  ...
  const [columnMapping] = useState<ColumnMapping>({
    ...
    notes: ['notes', 'comments', 'remarks', 'description', 'details', 'additional info', 'extra info', 'customer notes', 'client notes', 'additional notes', 'extra notes', 'remarks notes', 'description notes'],
  ...
  const processExcelFile = async (file: File) => {
    // Prevent processing if modal is not open
    if (!isOpen) return;
    ...
    try {
      ...
      const mappingReport = Object.entries(detectedColumns).map(([field, index]) => {
        const originalHeader = headers[index] || 'Unknown';
        return `${field}: "${originalHeader}" (column ${index + 1})`;
      }).join(', ');
  ...
  const handleBulkFix = () => {
    const fixedData = editableData.map(customer => {
      const fixed = { ...customer };
      
      // Fix missing names
      if (!fixed.name?.trim()) {
        fixed.name = 'Unknown Customer';
      }
      
      // Fix phone numbers
      if (!fixed.phone?.trim()) {
        fixed.phone = '0000000000';
      } else if (!/^[+]?[0-9\s\-()]{8,}$/.test(fixed.phone)) {
        // Try to clean up phone number
        const cleaned = fixed.phone.replace(/[^\d+]/g, '');
        fixed.phone = cleaned.length >= 8 ? cleaned : '0000000000';
      }
      
      // Fix gender
      if (fixed.gender && !['male', 'female', 'other'].includes(fixed.gender)) {
        fixed.gender = 'other';
      }
      
      // Fix loyalty level
      if (fixed.loyaltyLevel && !['bronze', 'silver', 'gold', 'platinum'].includes(fixed.loyaltyLevel)) {
        fixed.loyaltyLevel = 'bronze';
      }
      
      // Fix birth day
      if (fixed.birthDay) {
        const day = parseInt(fixed.birthDay);
        if (isNaN(day) || day < 1 || day > 31) {
          fixed.birthDay = '';
        }
      }
    ...
    fixedData.forEach((customer, index) => {
      const rowErrors = validateCustomerData(customer, index + 2);
      newErrors.push(...rowErrors);
    });
    
    setValidationErrors(newErrors);
    
    const fixedCount = editableData.length - newErrors.length;
    toast.success(`Auto-fixed ${editableData.length - newErrors.length} rows. ${newErrors.length} errors remain.`);
  };
  ...
  const handleClose = () => {
    ...
    setDetectedColumns({}); // Clear detected columns
    setFileHeaders([]); // Clear file headers
  ...
  const downloadTemplate = () => {
    // Create a comprehensive template with all supported columns and proper formatting
    const template = `Name,Phone Number,WhatsApp Number,Gender,City,Notes,Loyalty Level,Color Tag,Birth Month,Birth Day,Referral Source,Location Description,National ID,Referred By,Total Spent,Points,Is Active,Email,Profile Image
...
Alice Brown,+255745678901,+255745678901,female,Dodoma,Social media customer,platinum,purchased,9,10,instagram,321 Social Street Dodoma,4567890123456789,John Doe,150000,300,true,alice.brown@email.com,`;
    ...
    toast.success('Template downloaded! Use this format for best results. Gender: male/female/other, Loyalty: bronze/silver/gold/platinum, Color Tag: new/vip/complainer/purchased');
  };
          ...
          <div className="space-y-6">
            <div className="text-center">
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Import customers from Excel/CSV file with intelligent column detection. 
                  The system automatically detects column positions regardless of their order!
                </p>
              </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <ul className="text-sm text-blue-700 space-y-1">
                <li>• <strong>Intelligent Column Detection</strong> - Automatically detects column positions regardless of order</li>
            ...
            {Object.keys(detectedColumns).length > 0 && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 text-sm">
                  {Object.entries(detectedColumns).map(([fieldType, columnIndex]) => (
                    <div key={fieldType} className="flex items-center gap-2 bg-white rounded-lg p-2 border border-blue-100">
                ...
                <div className="mt-3 p-2 bg-blue-100 rounded text-sm text-blue-800">
                  <strong>Smart Detection:</strong> The system automatically detected {Object.keys(detectedColumns).length} columns from your Excel file. 
                  You can change column positions in your file and the system will still work correctly!
                </div>
              </div>
            )}
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div className="text-center">
                      ...
                      <div className="text-red-700">Total Errors</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-yellow-600">{validationErrors.filter(e => e.includes('Name is required')).length}</div>
                      <div className="text-yellow-700">Missing Names</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-orange-600">{validationErrors.filter(e => e.includes('Phone')).length}</div>
                      <div className="text-orange-700">Phone Issues</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">{importedData.length - validationErrors.filter(e => e.includes('Row')).length}</div>
                      <div className="text-green-700">Valid Rows</div>
                    </div>
                  </div>
                  <div className="mt-3 p-3 bg-blue-100 rounded text-sm text-blue-800">
                    <strong>Quick Fix Options:</strong>
                    <ul className="mt-2 space-y-1">
                      <li>• <button onClick={() => setShowValidationDetails(!showValidationDetails)} className="text-blue-600 hover:underline">
                        {showValidationDetails ? 'Hide' : 'Show'} detailed error list
                      </button></li>
                      <li>• <button onClick={handleBulkFix} className="text-blue-600 hover:underline">
                        Auto-fix common issues
                      </button></li>
                      <li>• <button onClick={downloadTemplate} className="text-blue-600 hover:underline">
                        Download template for correct format
                      </button></li>
                      <li>• <button onClick={() => setShowFormatHelp(!showFormatHelp)} className="text-blue-600 hover:underline">
                        {showFormatHelp ? 'Hide' : 'Show'} format guide
                      </button></li>
                      <li>• Rows with validation errors will be automatically skipped during import</li>
                      <li>• You can edit individual rows in the table below to fix errors</li>
                    </ul>
                  </div>
                </div>
                ...
                {showValidationDetails && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <h4 className="font-medium text-yellow-800 mb-2 flex items-center gap-2">
                      <AlertCircle className="w-4 h-4" />
                      Detailed Validation Errors ({validationErrors.length} rows will be skipped):
                    </h4>
                    <div className="max-h-64 overflow-y-auto">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {validationErrors.map((error: string, index: number) => (
                          <div key={index} className="py-1 px-2 bg-yellow-100 rounded text-sm text-yellow-800 border border-yellow-200">
                            • {error}
                          </div>
                        ))}
                      </div>
            ...
            {showFormatHelp && (
              <div className="mb-4 bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    <h5 className="font-medium text-green-700 mb-2">Required Fields:</h5>
                    <ul className="space-y-1 text-green-600">
                      ...
                      <li>• <strong>City:</strong> Customer's city/location</li>
                    </ul>
                  </div>
                  <div>
                    <h5 className="font-medium text-green-700 mb-2">Optional Fields:</h5>
                    <ul className="space-y-1 text-green-600">
                      <li>• <strong>WhatsApp:</strong> WhatsApp number</li>
                      <li>• <strong>Notes:</strong> Customer notes/comments</li>
                      <li>• <strong>Loyalty Level:</strong> bronze, silver, gold, platinum</li>
                      <li>• <strong>Color Tag:</strong> new, vip, complainer, purchased</li>
                      <li>• <strong>Birth Month/Day:</strong> Numbers (1-12 for month, 1-31 for day)</li>
                      <li>• <strong>Referral Source:</strong> How customer found you</li>
                    </ul>
                  </div>
                </div>
                <div className="mt-3 p-2 bg-green-100 rounded text-sm text-green-800">
                  <strong>Tips:</strong> Use the template for best results. Phone numbers should include country code (+255 for Tanzania). 
                  Gender and loyalty level are case-sensitive.
                </div>
              </div>
            )}
            ...
            {Object.keys(detectedColumns).length > 0 && (
              <div className="mb-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 text-sm">
                  {Object.entries(detectedColumns).map(([field, index]) => {
                    const originalHeader = fileHeaders[index] || 'Unknown';
                    return (
                      <div key={field} className="flex items-center gap-2 p-2 bg-blue-100 rounded">
                        <span className="w-3 h-3 bg-blue-500 rounded-full"></span>
                        <span className="text-blue-700 font-medium">{field}:</span>
                        <span className="text-blue-600">"{originalHeader}"</span>
                      </div>
                    );
                  })}
                </div>
                <div className="mt-3 p-2 bg-blue-100 rounded text-sm text-blue-800">
                  <strong>Note:</strong> The system automatically mapped your Excel columns to the correct fields. 
                  If any mapping looks incorrect, please use the template format.
                </div>
              </div>
            )}
