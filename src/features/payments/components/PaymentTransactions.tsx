...
import { 
  ...
  LayoutGrid, List, TestTube, Filter as FilterIcon, Copy, Edit, 
...
Date: ${new Date(payment.created_at || payment.date).toLocaleDateString()}`;
  ...
  const handleApprovePayment = async (payment: any) => {
    try {
      ...
      let sourceTable = payment.source;
      if (!sourceTable) {
        // Try to determine source based on payment properties
        if (payment.purchase_order_id) {
          sourceTable = 'purchase_order_payments';
        } else if (payment.device_id || payment.customer_id) {
          sourceTable = 'customer_payments';
        } else if (payment.order_id || payment.provider) {
          sourceTable = 'payment_transactions';
        } else {
          throw new Error('Cannot determine payment source table');
        }
      }
      
      console.log(`🔄 PaymentTransactions: Approving payment ${payment.id} from source: ${sourceTable}`);
      console.log('🔍 PaymentTransactions: Payment object structure:', {
        id: payment.id,
      ...
      const mainUpdatePromise = supabase
        ...
        .update({
          status: 'approved',
          updated_at: new Date().toISOString(),
          ...(sourceTable === 'customer_payments' && currentUser?.id && {
            // Only add updated_by if currentUser.id is valid
      ...
      // Note: Removed automatic approval of related payments
      // Each payment should be approved individually by the user

      // If this is a purchase order payment, check if we should update the purchase order status
      if (sourceTable === 'purchase_order_payments' && payment.purchase_order_id) {
