...
const PaymentAutomation: React.FC = () => {
  ...
  // Fetch automation data from database
  const fetchAutomationData = useCallback(async () => {
    setIsLoading(true);
    try {
      console.log('🔄 Fetching automation rules and workflows from database...');
      
      // Fetch automation rules and workflows from database
      const [rulesResult, workflowsResult] = await Promise.allSettled([
        paymentAutomationService.getAutomationRules(),
        paymentAutomationService.getAutomationWorkflows()
      ]);

      // Handle automation rules
      if (rulesResult.status === 'fulfilled') {
        setAutomationRules(rulesResult.value);
        console.log('✅ Automation rules loaded:', rulesResult.value.length);
      } else {
        console.warn('Failed to fetch automation rules:', rulesResult.reason);
        // Keep existing rules if fetch fails
      }

      // Handle automation workflows
      if (workflowsResult.status === 'fulfilled') {
        setAutomationWorkflows(workflowsResult.value);
        console.log('✅ Automation workflows loaded:', workflowsResult.value.length);
      } else {
        console.warn('Failed to fetch automation workflows:', workflowsResult.reason);
        // Keep existing workflows if fetch fails
      }
  ...
  const handleSaveRule = async () => {
    try {
      if (editingRule) {
        ...
        const result = await paymentAutomationService.updateAutomationRule(editingRule.id, ruleFormData);
        if (result) {
        ...
        const newRule = await paymentAutomationService.createAutomationRule(ruleFormData);
        if (newRule) {
  ...
  const handleDeleteRule = async (ruleId: string) => {
    if (window.confirm('Are you sure you want to delete this rule?')) {
      try {
        const result = await paymentAutomationService.deleteAutomationRule(ruleId);
        if (result) {
  ...
  const handleDeleteWorkflow = async (workflowId: string) => {
    if (window.confirm('Are you sure you want to delete this workflow?')) {
      try {
        const result = await paymentAutomationService.deleteAutomationWorkflow(workflowId);
        if (result) {
