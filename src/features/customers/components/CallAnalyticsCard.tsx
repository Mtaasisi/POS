...
const CallAnalyticsCard: React.FC<CallAnalyticsCardProps> = ({ customer }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      ...
      {/* Summary View (Always Visible) - Matching Financial Overview Style */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-4">
          <div className="text-xs font-medium text-blue-700 uppercase tracking-wide mb-1">Total Calls</div>
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-xl p-4">
          <div className="text-xs font-medium text-green-700 uppercase tracking-wide mb-1">Duration</div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-xl p-4">
          <div className="text-xs font-medium text-purple-700 uppercase tracking-wide mb-1">Call Span</div>
          <div className="text-lg font-bold text-purple-900">
            {customer.firstCallDate && customer.lastCallDate ? 
              Math.ceil((new Date(customer.lastCallDate).getTime() - new Date(customer.firstCallDate).getTime()) / (1000 * 60 * 60 * 24)) : 0} days
          </div>
          <div className="text-xs text-purple-600 mt-1">
            {customer.totalCalls > 0 ? Math.round(customer.totalCalls / Math.max(1, Math.ceil((Date.now() - new Date(customer.firstCallDate || customer.createdAt || Date.now()).getTime()) / (1000 * 60 * 60 * 24 * 30)))) : 0}/month
          </div>
        </div>

        <div className="bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-200 rounded-xl p-4">
          <div className="text-xs font-medium text-orange-700 uppercase tracking-wide mb-1">Loyalty Level</div>
          <div className="text-lg font-bold text-orange-900">{customer.callLoyaltyLevel || 'Basic'}</div>
          <div className="text-xs text-orange-600 mt-1">
            {customer.missedCalls || 0} missed calls
          </div>
        </div>

        <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 border border-indigo-200 rounded-xl p-4">
          <div className="text-xs font-medium text-indigo-700 uppercase tracking-wide mb-1">Last Call</div>
          <div className="text-lg font-bold text-indigo-900">
            {customer.lastCallDate ? new Date(customer.lastCallDate).toLocaleDateString() : 'Unknown'}
          </div>
          <div className="text-xs text-indigo-600 mt-1">
            {customer.firstCallDate ? new Date(customer.firstCallDate).toLocaleDateString() : 'Unknown'} first
          </div>
        </div>
      </div>
