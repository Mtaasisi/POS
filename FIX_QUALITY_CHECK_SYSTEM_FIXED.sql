...
INSERT INTO quality_check_templates (name, description, category) 
SELECT 'Electronics Quality Check', 'Standard quality check for electronic devices', 'electronics'
WHERE NOT EXISTS (SELECT 1 FROM quality_check_templates WHERE name = 'Electronics Quality Check');

-- Insert default accessories template
INSERT INTO quality_check_templates (name, description, category) 
SELECT 'Accessories Quality Check', 'Standard quality check for accessories', 'accessories'
WHERE NOT EXISTS (SELECT 1 FROM quality_check_templates WHERE name = 'Accessories Quality Check');

-- Insert default general template
INSERT INTO quality_check_templates (name, description, category) 
SELECT 'General Quality Check', 'Standard quality check for general items', 'general'
WHERE NOT EXISTS (SELECT 1 FROM quality_check_templates WHERE name = 'General Quality Check');
...
BEGIN
    ...
    IF v_electronics_template_id IS NOT NULL THEN
        ...
        SELECT v_electronics_template_id, 'Physical Condition', 'Check for scratches, dents, or damage', true, 1
        WHERE NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_electronics_template_id AND name = 'Physical Condition');
        
        INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) 
        SELECT v_electronics_template_id, 'Functionality Test', 'Test basic functionality and features', true, 2
        WHERE NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_electronics_template_id AND name = 'Functionality Test');
        
        INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) 
        SELECT v_electronics_template_id, 'Accessories Check', 'Verify all accessories are included', true, 3
        WHERE NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_electronics_template_id AND name = 'Accessories Check');
        
        INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) 
        SELECT v_electronics_template_id, 'Packaging Condition', 'Check packaging for damage', false, 4
        WHERE NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_electronics_template_id AND name = 'Packaging Condition');
    END IF;
    
    -- Insert accessories criteria
    IF v_accessories_template_id IS NOT NULL THEN
        INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) 
        SELECT v_accessories_template_id, 'Physical Condition', 'Check for damage or defects', true, 1
        WHERE NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_accessories_template_id AND name = 'Physical Condition');
        
        INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) 
        SELECT v_accessories_template_id, 'Compatibility Check', 'Verify compatibility with intended device', true, 2
        WHERE NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_accessories_template_id AND name = 'Compatibility Check');
        
        INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) 
        SELECT v_accessories_template_id, 'Packaging Check', 'Check packaging condition', false, 3
        WHERE NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_accessories_template_id AND name = 'Packaging Check');
    END IF;
    
    -- Insert general criteria
    IF v_general_template_id IS NOT NULL THEN
        INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) 
        SELECT v_general_template_id, 'Physical Condition', 'Check for damage or defects', true, 1
        WHERE NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_general_template_id AND name = 'Physical Condition');
        
        INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) 
        SELECT v_general_template_id, 'Quantity Verification', 'Verify correct quantity received', true, 2
        WHERE NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_general_template_id AND name = 'Quantity Verification');
        
        INSERT INTO quality_check_criteria (template_id, name, description, is_required, sort_order) 
        SELECT v_general_template_id, 'Packaging Check', 'Check packaging condition', false, 3
        WHERE NOT EXISTS (SELECT 1 FROM quality_check_criteria WHERE template_id = v_general_template_id AND name = 'Packaging Check');
    END IF;
END $$;
